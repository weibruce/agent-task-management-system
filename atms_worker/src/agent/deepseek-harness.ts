 ...
 * process and exposes Atms DAG tools through a turn-local MCP bridge.
 */
...
const DEFAULT_DSH_MAX_TOKENS = 4096;
const DEFAULT_DSH_CONTEXT_WINDOW = 200_000;
const DSH_FORK_COMMIT = "f0fd50751713ab7e1b7cb1f13310a70c106d81b1";
const MCP_TOOL_PREFIX = "mcp__atms__";
const DEFAULT_SYSTEM_PROMPT = "You are a Atms DAG worker. Complete the assigned task and call the provided handoff tool exactly once.";
...
function parseMaxTokens(value: number | string | undefined): number {
  ...
  if (!Number.isSafeInteger(parsed) || parsed <= 0) {
    throw new Error("ATMS_DSH_MAX_TOKENS must be a positive integer");
  }
  return parsed;
}
...
function defaultCordisConfigPath(): string {
  return fileURLToPath(new URL("../../dsh/atms.cordis.yml", import.meta.url));
}

function normalizeBaseUrl(value: string): string {
  let url = value.trim().replace(/\/+$/, "").replace(/\/chat\/completions$/i, "");
  // When the Manager Agent runs on the host (host_shell placement), the LLM is
  // reachable via 127.0.0.1. The Docker-internal alias host.docker.internal
  // does not resolve on the host, so rewrite it to avoid connection errors.
  if (process.env.ATMS_MANAGER_AGENT_RUNTIME_PLACEMENT === "host_shell"
...
function projectedSystemPrompt(context: AgentRunContext): string {
  const skills = context.skillProjection?.definitions?.map((skill) => [
    `Atms Skill: ${skill.name ?? skill.id}`,
...
function notificationEvents(
  ...
  switch (event.type) {
    ...
    case "turn/end": {
      console.error(`[DSH] turn_end_reason reason=${JSON.stringify(event.data.reason)}`);
      const error = turnEndError(event.data.reason);
      if (error) console.error(`[DSH] turn_end_error mapped=${JSON.stringify(error)}`);
      return {
...
export class DeepSeekHarnessAdapter implements AgentClient {
  ...
  constructor(options: DeepSeekHarnessAdapterOptions = {}) {
    this.runtimeBin = resolve(
      options.runtimeBin
        ?? process.env.ATMS_DSH_RUNTIME_BIN?.trim()
        ?? DEFAULT_DSH_RUNTIME_BIN,
    );
    this.cordisConfigPath = resolve(
      options.cordisConfigPath
        ?? process.env.ATMS_DSH_CORDIS_CONFIG?.trim()
        ?? defaultCordisConfigPath(),
    );
    this.maxTokens = parseMaxTokens(
      options.maxTokens ?? process.env.ATMS_DSH_MAX_TOKENS?.trim(),
    );
  }
  ...
  async *run(
    ...
    const runtimeRoot = mkdtempSync(join(tmpdir(), "atms-dsh-"));
    const sessionId = context.sessionId?.trim() || `atms-${randomUUID()}`;
    ...
    try {
      ...
      const childEnv = {
        ...
        ATMS_DSH_API_KEY: context.apiKey,
        ATMS_DSH_PROVIDERS_JSON: providerProfile.providersJson,
        ATMS_DSH_MCP_SCRIPT: bridge.scriptPath,
        ATMS_MCP_BRIDGE_URL: bridge.url,
        ATMS_MCP_BRIDGE_TOKEN: bridge.token,
      };
      ...
      console.error(`[DSH] turn_start session=${sessionId} tools=${bridgeTools.length} model=${context.model}`);
      const t0 = Date.now();
      await harness.start();
      console.error(`[DSH] harness_start_ms=${Date.now() - t0} session=${sessionId}`);
      ...
      const runTask = (async (): Promise<void> => {
        ...
        console.error(`[DSH] prompt_sent_ms=${Date.now() - t1} session=${sessionId} messageId=${messageId}`);
        bindController();
        try {
          ...
          while (true) {
            ...
            eventCount++;
            if (eventCount % 10 === 1) {
              console.error(`[DSH] event_${eventCount} session=${sessionId} elapsed_ms=${Date.now() - t2}`);
            }
            if (!receivedPrompt) {
          ...
          console.error(`[DSH] session_idle events=${eventCount} total_ms=${Date.now() - t2} session=${sessionId}`);
        } finally {
          subscription.close();
        }
      })().then(() => { console.error(`[DSH] runTask_resolved session=${sessionId}`); queue.end(); }, (error: unknown) => { console.error(`[DSH] runTask_rejected session=${sessionId} error=${error instanceof Error ? error.message : String(error)}`); queue.fail(error); });

      for await (const event of queue) {
        console.error(`[DSH] yield_event type=${event.type} session=${sessionId}`);
        yield event;
      }
      console.error(`[DSH] queue_drained session=${sessionId}`);
      await runTask;
      console.error(`[DSH] done_event_yield session=${sessionId}`);
      yield {
        type: "done",
        usage: aggregateUsage,
      ...
      console.error(`[DSH] closing harness+bridge session=${sessionId} total_ms=${Date.now() - startedAt}`);
      await harness?.close().catch(() => undefined);
      await bridge?.close().catch(() => undefined);
      rmSync(runtimeRoot, { recursive: true, force: true });
      console.error(`[DSH] cleanup_done session=${sessionId}`);
    }
  }
}
...
async function createToolBridge(tools: DagToolDefinition[], root: string): Promise<ToolBridge> {
  ...
  const scriptPath = join(root, "atms-tools-mcp-server.mjs");
...
const BRIDGE_URL = process.env.ATMS_MCP_BRIDGE_URL;
const BRIDGE_TOKEN = process.env.ATMS_MCP_BRIDGE_TOKEN;
  ...
  try {
    if (request.method === "initialize") {
      result(id, {
        ...
        serverInfo: { name: "atms-tools", version: ${JSON.stringify(WORKER_RUNTIME_VERSION)} }
      });
