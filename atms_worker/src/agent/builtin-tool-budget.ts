    ...
    if (calls >= maxCalls) {
      return {
        ...
        hookSpecificOutput: {
          ...
          permissionDecisionReason: `Built-in tool budget exhausted (${maxCalls}/${maxCalls}). Stop inspecting and call an allowed Atms DAG handoff tool now; DAG tools remain available.`,
        },
      };
    }
    calls += 1;
