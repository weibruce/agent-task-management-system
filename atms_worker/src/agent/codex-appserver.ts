 ...
 *
 * Mirrors the Codex AppServer client contract used by Atms workers.
 * @version 0.1.0
 */
...
const CLIENT_NAME = "atms_codex_appserver";
const CLIENT_TITLE = "Atms Codex AppServer Adapter";
...
async function startCodexProviderRelay(baseUrl: string, providerApiKey: string): Promise<CodexProviderRelay> {
  ...
  const relayApiKey = `atms-dispatch-${randomBytes(32).toString("hex")}`;
...
function guardedSpawnCommand(
  ...
  configuredGuard = process.env.ATMS_CODEX_PROC_GUARD,
  platform = process.platform,
): { bin: string; args: string[]; env: Record<string, string | undefined> } {
  const spawnEnv = { ...env };
  if (!env.ATMS_CODEX_API_KEY || platform !== "linux") return { bin, args, env: spawnEnv };
