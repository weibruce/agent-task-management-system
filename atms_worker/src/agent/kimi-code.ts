 ...
 * in-process Atms DAG tool handlers.
 * @version 0.1.0
 */
...
import {
  ...
  formatAtmsPromptHandoff,
  formatAtmsPromptToolCall,
  parseAtmsPromptHandoff,
  parseAtmsPromptToolCalls,
  stripAtmsPromptMarkers,
} from "atms-protocol";
...
function writeProjectedSkill(root: string, definition: AgentSkillDefinition): void {
  ...
  const description = (definition.description || definition.name || `Atms Skill ${definition.id}`)
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 240);
  ...
  writeFileSync(join(dir, "SKILL.md"), [
    ...
    `description: ${JSON.stringify(description || `Atms Skill ${definition.id}`)}`,
    "type: prompt",
    `whenToUse: ${JSON.stringify(description || `When this Atms task requires ${definition.id}`)}`,
    "disableModelInvocation: false",
    "---",
    "",
    `Atms Skill id: ${definition.id}`,
    "",
...
function mergedKimiSdkSkillsDir(directories: readonly string[], kimiHome: string): string | undefined {
  ...
  const merged = join(kimiHome, "atms-skills-merged");
...
function resolveKimiAgentTransport(options?: KimiCodeAdapterOptions): KimiAgentTransport {
  const explicit = normalizeKimiAgentTransport(options?.transport ?? process.env.ATMS_KIMI_AGENT_TRANSPORT);
...
export class KimiCodeAdapter implements AgentClient {
      ...
      if (this.shouldUsePromptToolBridge(tools)) {
        yield {
          ...
          data: {
            ...
            canonical_protocol: ATMS_PROMPT_TOOL_CALL_PROTOCOL,
      ...
      try {
        ...
        if (mcpBridge) {
          yield {
            ...
            data: { server: "atms-tools", tool_names: tools.map((tool) => tool.name) },
          };
        }
  ...
  private async *runSdkMode(
    ...
    const session = this.createSdkSession({
      ...
      clientInfo: { name: "atms-worker", version: WORKER_RUNTIME_VERSION },
    });
  ...
  private async *runPromptMode(
    ...
    try {
      ...
      const toolMarkers = parseAtmsPromptToolCalls(accumulatedText);
      ...
      const handoff = parseAtmsPromptHandoff(accumulatedText);
      ...
      if (toolMap.has("handoff") && !handoffEmitted) {
        yield {
          ...
          data: { expected_marker: `<${ATMS_PROMPT_HANDOFF_PROTOCOL}>{...}</${ATMS_PROMPT_HANDOFF_PROTOCOL}>` },
        };
      }
      if (this.shouldUsePromptToolBridge(tools, forceToolBridge) && !toolMarkerEmitted) {
        yield {
          ...
          data: { expected_marker: `<${ATMS_PROMPT_TOOL_CALL_PROTOCOL}>{...}</${ATMS_PROMPT_TOOL_CALL_PROTOCOL}>` },
        };
      }
      if (!authoritativeFinishCompleted && fallbackVisibleText) {
  ...
  private shouldUsePromptToolBridge(tools: DagToolDefinition[], force = false): boolean {
    ...
    return (force || process.env.ATMS_KIMI_PROMPT_TOOL_BRIDGE === "1") && names.has("create_and_run") && names.has("finish");
  }
  ...
  private buildPromptModePrompt(
    ...
    if (this.shouldUsePromptToolBridge(tools, forceToolBridge)) {
      blocks.push(
        "",
        "Atms tool execution protocol:",
        "Emit the structured Atms Tool call marker before any user-facing prose.",
        "When the user asks to inspect, start, supervise, or change real Atms state, output exactly one marker per required tool call.",
        ...
        ...(reconcileK3ManagerFinish
          ? [
              ...
              "Do not put any guessed, placeholder, or fabricated run ID in that finish text; Atms appends the authoritative run ID returned by create_and_run.",
            ]
          : []),
    ...
    if (toolMap.has("handoff")) {
      blocks.push(
        "",
        "Atms prompt-mode handoff fallback:",
  ...
  private async createToolMcpBridge(
    ...
    const scriptPath = join(kimiHome, "atms-tools-mcp-server.mjs");
    writeFileSync(
      scriptPath,
    ...
    return {
      servers: [{
        ...
        args: [scriptPath],
        env: [
          { name: "ATMS_MCP_BRIDGE_URL", value: baseUrl },
          { name: "ATMS_MCP_BRIDGE_TOKEN", value: token },
        ],
      }],
...
function visiblePromptModeText(text: string, hasMarker: boolean): string {
  if (!hasMarker) return stripAtmsPromptMarkers(text);
  const closingTags = [
    `</${ATMS_PROMPT_TOOL_CALL_PROTOCOL}>`,
    `</${ATMS_PROMPT_HANDOFF_PROTOCOL}>`,
  ];
  ...
  return stripAtmsPromptMarkers(text.slice(boundary));
}
...
async function callTool(name, args, toolCallId) {
  if (!BRIDGE_URL || !BRIDGE_TOKEN) {
    return { content: "Atms MCP bridge is not configured", is_error: true };
  }
  ...
  if (!response.ok) {
    return {
      content: typeof json.error === "string" ? json.error : "Atms MCP bridge request failed",
      is_error: true
    };
  }
  return json;
