...
export function createAgentClient(backend?: string): AgentClient {
  ...
  if (raw === "codex") {
    console.warn("[atms_worker] codex backend is deprecated, use codex_appserver instead");
  }
  const key = normalizeManagerAgentRuntimeAgentType(raw) ?? DEFAULT_MANAGER_AGENT_RUNTIME_AGENT_TYPE;
  if (key === "manager-agent-smoke" && process.env.ATMS_MANAGER_AGENT_SMOKE === "1") {
    return new ManagerAgentSmokeClient();
  }
  const factory = REGISTRY[key];
  if (!factory) {
    const available = process.env.ATMS_MANAGER_AGENT_SMOKE === "1"
