/**
 * Agent client types for atms_worker.
 *
 * Shared agent client interface for Atms workers.
 * @version 0.1.0
 */

import type { AgentBuiltinToolName, AgentToolDefinition, DagWorkspaceAccess } from "atms-protocol";
...
export interface AgentRunContext {
  systemPrompt?: string;
  /**
   * `append` preserves the Claude Code preset and adds Atms instructions.
  ...
  /** Hard per-turn budget for built-in tools; Atms DAG tools are not counted or denied. */
   ...
   * `dontAsk` with every exposed built-in and Atms MCP tool pre-approved.
   */
