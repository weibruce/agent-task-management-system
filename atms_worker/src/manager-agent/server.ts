...
function stablePluginModelCallIdentifiers(input: {
  ...
  hash.update("atms.plugin.model-tool-call.v1\0");
...
function managerAgentTurnTimeoutMs(): number {
  const raw = Number(process.env.MANAGER_AGENT_TURN_TIMEOUT_MS ?? "1800000");
...
function managerRequestHeaders(
  ...
  headers.delete(ATMS_MANAGER_TURN_HEADER);
  ...
  ) headers.set(ATMS_MANAGER_TURN_HEADER, credential);
...
}, responseMode: "chat" | "voice", pluginContext?: AtmsPluginTurnContextV1, pluginToolTurnToken?: string, canvasContext?: GenerativeUiCanvasContextV1, managerSkills?: ManagerAgentPromptSkill[]): DagToolDefinition[] {
  if (pluginContext && (
...
function pluginContextDigest(context: AtmsPluginTurnContextV1): string {
  const hash = createHash("sha256");
  const analysis = analyzeGenerativeUiJsonValue(atmsPluginTurnContextDigestInput(context), {
...
function normalizeBackend(agentType: string | undefined): string {
  if (process.env.ATMS_MANAGER_AGENT_SMOKE === "1" && process.env.AGENT_BACKEND === "manager-agent-smoke") {
...
function managerAgentSkillProjection(
  ...
  const home = process.env.ATMS_HOME?.trim();
  if (home) {
  ...
  const definitions = (skills ?? [])
    ...
    .map((skill) => ({
      ...
      description: skill.description || "Selected Atms plugin Skill",
...
async function handleChat(
  ...
  if (pluginContext) {
    const validation = validateAtmsPluginTurnContext(pluginContext);
...
export function startManagerAgentServer(port = Number(process.env.MANAGER_AGENT_PORT || "9001")): http.Server {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === "GET" && url.pathname === "/health") {
      json(res, 200, {
        ...
        worker_id: process.env.WORKER_ID || process.env.ATMS_WORKER_ID || null,
        project_id: process.env.PROJECT_ID || null,
        fingerprint: process.env.ATMS_MANAGER_AGENT_FINGERPRINT || null,
  ...
  server.listen(port, "0.0.0.0", () => {
    console.log(`[atms_worker] manager-agent server listening on ${port}`);
  });
  return server;
}
