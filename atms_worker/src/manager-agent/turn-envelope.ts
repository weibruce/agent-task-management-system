...
import {
  ATMS_MANAGER_TURN_ENVELOPE_REQUIRED_ENV,
  ATMS_MANAGER_TURN_KEY_ID_ENV,
  ATMS_MANAGER_TURN_PUBLIC_KEY_ENV,
...
function requiredByEnvironment(env: NodeJS.ProcessEnv): boolean {
  const value = env[ATMS_MANAGER_TURN_ENVELOPE_REQUIRED_ENV];
  return env.MANAGER_AGENT_MODE === "1"
    || value === "1"
    || value?.toLowerCase() === "true"
    || Boolean(env[ATMS_MANAGER_TURN_PUBLIC_KEY_ENV]);
}
...
function expectedPlacement(env: NodeJS.ProcessEnv): ManagerAgentTurnRuntimePlacementV1 {
  const placement = env.ATMS_MANAGER_AGENT_RUNTIME_PLACEMENT;
...
function publicKeyFromEnvironment(env: NodeJS.ProcessEnv): KeyObject {
  const encoded = nonEmpty(env[ATMS_MANAGER_TURN_PUBLIC_KEY_ENV]);
...
export class ManagerAgentTurnEnvelopeVerifier {
  ...
  constructor(env: NodeJS.ProcessEnv = process.env) {
    ...
    this.#keyId = nonEmpty(env[ATMS_MANAGER_TURN_KEY_ID_ENV]);
    this.#workerId = nonEmpty(env.ATMS_WORKER_ID) ?? nonEmpty(env.WORKER_ID);
