/**
 * atms_worker entry point — reads env vars, connects to Manager WS,
...
const WORKER_ID = process.env.ATMS_WORKER_ID ?? `worker-${process.pid}`;
const DEFAULT_MANAGER_PORT = process.env.ATMS_MANAGER_PORT?.trim() || "19191";
const DEFAULT_MANAGER_WS_BASE = (
  process.env.ATMS_MANAGER_WS_URL?.trim() || `ws://localhost:${DEFAULT_MANAGER_PORT}`
).replace(/\/+$/, "");
...
const TOKEN = process.env.ATMS_WORKER_TOKEN ?? "";
const ALLOW_INSECURE_REMOTE_WS = process.env.ATMS_ALLOW_INSECURE_REMOTE_WS === "1";
const CONFIGURED_CAPABILITIES = (process.env.ATMS_WORKER_CAPABILITIES ?? "")
  .split(",")
...
const configuredLiveSteerQueueSize = Number(process.env.ATMS_LIVE_STEER_QUEUE_MAX ?? 32);
...
client.on("connected", () => {
  console.log(`[atms_worker] connected to ${MANAGER_WS_URL}`);
});

client.on("disconnected", () => {
  pendingCredentialBrokerCalls.close("Manager connection disconnected before credential broker completion");
  console.log("[atms_worker] disconnected, will reconnect...");
});

client.on("error", (err) => {
  console.error("[atms_worker] ws error:", err);
});
...
client.on("task", async (msg) => {
  ...
  const runId = envelope
    ...
    : String(data.run_id ?? process.env.ATMS_RUN_ID ?? "");
  ...
  try {
    ...
    console.error(`[atms_worker] ${message}`);
  ...
  if (!task) {
    console.error("[atms_worker] received task with empty body");
    return;
  }

  console.log(
    `[atms_worker] task received: run=${runId} node=${dagConfig.node_id} backend=${backend} agent_type=${agentType} provider=${provider || "<unset>"} model=${model || "<unset>"}`,
  );
  ...
  try {
    promptResult = await runPrompt(job, {
      ...
      auditDir: process.env.ATMS_AUDIT_DIR,
    ...
    console.error("[atms_worker] prompt runner error:", err);
  } finally {
    ...
    if (closeResult.driverError) {
      console.error("[atms_worker] agent turn driver close error:", closeResult.driverError);
    }
...
client.on("dag_actor_command", (msg) => {
  ...
  void routing
    .then((result) => {
      if (!result.handled) {
        console.log("[atms_worker] dag_actor_command ignored:", result.reason ?? "unrecognized message");
      }
    })
    .catch((err) => {
      console.error("[atms_worker] dag_actor_command routing error:", err);
    })
    .finally(() => {
...
client.on("inject", (msg) => {
  ...
  console.log("[atms_worker] inject received:", JSON.stringify(data));
  if (
    mode === "interrupt" &&
    activePrompt &&
...
client.on("dag_inbox", (msg) => {
  ...
  console.log("[atms_worker] dag_inbox dropped:", JSON.stringify(data));
});

// ── Graceful shutdown ────────────────────────────────────────

async function shutdown() {
  console.log("[atms_worker] shutting down...");
  const prompt = activePrompt;
  if (prompt) {
...
console.log(`[atms_worker] starting: worker_id=${WORKER_ID} capabilities=${CAPABILITIES.join(",") || "-"}`);
client.connect();
}
