...
export async function runPrompt(
  ...
  const effectiveTask = job.actorCheckpoint
    ? [
        "## Atms portable actor checkpoint",
  ...
  // ATMS_DETERMINISTIC_PROMPT_DELAY_MS is set, sleep before processing
  // the task. This gives a harness a chance to inject while the run is
  // still active. Default off.
  const delayEnv = process.env.ATMS_DETERMINISTIC_PROMPT_DELAY_MS;
    ...
    : correctionOnly
      ? [
          ...
          correctionRepairsWorkspaceEvidence
            ? "You may inspect and rewrite only the declared .atms workspace evidence JSON, compute its SHA-256, reuse durable broker receipts, and then call handoff exactly once. Do not modify source files, rerun tests, or repeat external side effects."
    ...
    for await (const event of agent.run(effectiveTask, dagTools, context)) {
      switch (event.type) {
        ...
        case "debug": {
          ...
          console.log(
            `[atms_worker] ATMS_AGENT_DEBUG ${JSON.stringify({
              run_id: job.runId,
              node_id: job.dagConfig.node_id,
              source: event.source,
              message: debugMessage,
