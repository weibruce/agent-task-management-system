...
export type WorkerSkillVisualViewRegistry = ReadonlyMap<string, AtmsA2uiSurfaceV1>;
...
export function createWorkerSkillVisualViewRegistry(
  ...
  const registry = new Map<string, AtmsA2uiSurfaceV1>();
  if (!context) return registry;

  const localViews = new Map<string, AtmsA2uiSurfaceV1[]>();
...
function renderSkillContext(
  ...
  const sections = [
    "## Atms digest-pinned Worker Skill Context",
  ...
  return {
    ...
    skillProjection: {
      ...
      definitions: context.skills.map((skill) => ({
        id: skill.id,
        name: skill.id,
        description: "Pinned instructions for this assigned Atms DAG Worker task",
