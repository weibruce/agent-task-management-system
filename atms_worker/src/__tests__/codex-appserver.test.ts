...
describe("CodexAppServerAdapter", () => {
  ...
  it("keeps the provider key in a loopback relay and gives Codex only a dispatch-scoped key", async () => {
    ...
    try {
      ...
      expect(relay.apiKey).toMatch(/^atms-dispatch-[0-9a-f]{64}$/);
  ...
  it("spawns Responses Codex with the relay URL and scoped key instead of the provider key", async () => {
    ...
    const previousGuard = process.env.ATMS_CODEX_PROC_GUARD;
    process.env.ATMS_CODEX_PROC_GUARD = "/test/proc-guard.so";
    try {
      ...
      expect(spawnEnv.ATMS_CODEX_API_KEY).toMatch(/^atms-dispatch-[0-9a-f]{64}$/);
      expect(spawnEnv.ATMS_CODEX_API_KEY).not.toBe(providerKey);
      ...
      if (previousGuard === undefined) delete process.env.ATMS_CODEX_PROC_GUARD;
      else process.env.ATMS_CODEX_PROC_GUARD = previousGuard;
    }
  }, 15000);

  it("preloads the configured non-dumpable guard into provider-backed Linux Codex", async () => {
    const guard = "/usr/local/lib/atms-codex-secret-guard.so";
    ...
    expect(_guardedCodexSpawnCommandForTest(
      "/app/codex",
      ["app-server"],
      { ATMS_CODEX_API_KEY: "provider-secret" },
      guard,
      "linux",
    )).toEqual({
      bin: "/app/codex",
      ...
      env: {
        ATMS_CODEX_API_KEY: "provider-secret",
        LD_PRELOAD: "/usr/local/lib/atms-codex-secret-guard.so",
      },
    });
  ...
  it("sends correct initialize parameters", async () => {
    ...
    expect(reqs[0].params).toMatchObject({
      clientInfo: {
        name: "atms_codex_appserver",
        title: "Atms Codex AppServer Adapter",
