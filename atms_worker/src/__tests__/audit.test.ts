...
describe("audit writers", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "atms-audit-"));
  });
  ...
  describe("Claude SDK trace writer", () => {
    it("stores complete redacted SDK records outside the Manager transcript", async () => {
      ...
      expect(records[0]).toMatchObject({
        schema_version: "atms.claude-sdk-trace/v1",
        run_id: "run-trace",
        node_id: "node-a",
        record_type: "query_start",
        prompt: "use ***",
  ...
  describe("ATMS_HOME defaults", () => {
    it("stores audit records under ATMS_HOME when baseDir is omitted", async () => {
