...
describe("Worker runtime version identity", () => {
  ...
  it("prefers explicit Manager build arguments and trims their runtime values", () => {
    expect(resolveWorkerRuntimeIdentity({
      ATMS_WORKER_VERSION: " 9.8.7-test.1 ",
      ATMS_WORKER_PROTOCOL_VERSION: " 7.6.5 ",
      ATMS_WORKER_SOURCE_FINGERPRINT: " sha256:test ",
      ATMS_WORKER_IMAGE_REVISION: " revision-test ",
    })).toEqual({
  ...
  it("keeps Docker version arguments optional and passes them into the runtime environment", () => {
    const dockerfile = readFileSync(new URL("../../Dockerfile", import.meta.url), "utf8")
      .replace(/\r\n/g, "\n");
    expect(dockerfile).toContain("ARG ATMS_WORKER_VERSION\n");
    expect(dockerfile).toContain("ARG ATMS_WORKER_PROTOCOL_VERSION\n");
    expect(dockerfile).not.toMatch(/ARG ATMS_WORKER_(?:PROTOCOL_)?VERSION=/);
    expect(dockerfile).toContain('ATMS_WORKER_VERSION="${ATMS_WORKER_VERSION}"');
    expect(dockerfile).toContain('ATMS_WORKER_PROTOCOL_VERSION="${ATMS_WORKER_PROTOCOL_VERSION}"');
  });

  it("keeps Worker build source overrides optional, build-only Docker arguments", () => {
    const dockerfile = readFileSync(new URL("../../Dockerfile", import.meta.url), "utf8")
      .replace(/\r\n/g, "\n");
    expect(dockerfile.match(/^ARG ATMS_WORKER_BUILD_APT_MIRROR$/gm)).toHaveLength(3);
    expect(dockerfile.match(/^ARG ATMS_WORKER_BUILD_APT_SECURITY_MIRROR$/gm)).toHaveLength(3);
    expect(dockerfile.match(/^ARG NPM_CONFIG_REGISTRY$/gm)).toHaveLength(2);
    expect(dockerfile.match(/^ARG ATMS_DSH_FORK_REPOSITORY=https:\/\/github\.com\/xiaotianfotos\/deepseek-harness\.git$/gm))
      .toHaveLength(1);
