...
async function exerciseMcpBridge() {
  const script = process.env.ATMS_DSH_MCP_SCRIPT;
  const bridgeUrl = process.env.ATMS_MCP_BRIDGE_URL;
  const bridgeToken = process.env.ATMS_MCP_BRIDGE_TOKEN;
  if (!script || !bridgeUrl || !bridgeToken) throw new Error("DSH MCP bridge environment is incomplete");

  const child = spawn(process.execPath, [script], {
    env: {
      ATMS_MCP_BRIDGE_URL: bridgeUrl,
      ATMS_MCP_BRIDGE_TOKEN: bridgeToken,
    },
...
lines.on("line", async (line) => {
  ...
  if (request.method === "initialize") {
    if (process.env.DSH_FAKE_RECORD_FILE) {
      const providerProfiles = JSON.parse(process.env.ATMS_DSH_PROVIDERS_JSON ?? "{}");
      ...
      appendFileSync(process.env.DSH_FAKE_RECORD_FILE, `${JSON.stringify({
        ...
        apiKeyPresent: Boolean(process.env.ATMS_DSH_API_KEY),
        managerToken: process.env.ATMS_WORKER_TOKEN,
      })}\n`);
    }
