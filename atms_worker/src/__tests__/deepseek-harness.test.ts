...
function tempRoot(): string {
  const root = mkdtempSync(join(tmpdir(), "atms-dsh-test-"));
  tempRoots.push(root);
  return root;
}
...
describe("DeepSeekHarnessAdapter", () => {
  ...
  it("pins the compatible fork revision and uses the capability-aware pi-ai adapter", () => {
    ...
    const composition = readFileSync(new URL("../../dsh/atms.cordis.yml", import.meta.url), "utf8");
    const dockerCommit = /^ARG ATMS_DSH_FORK_COMMIT=([a-f0-9]{40})$/m.exec(dockerfile)?.[1];

    expect(dockerCommit).toBe(_deepSeekHarnessForkCommitForTest);
    expect(composition).toContain("@deepseek-ai/dsh-llm-pi-ai");
    expect(composition).toContain("ATMS_DSH_PROVIDERS_JSON");
  ...
  it("maps DSH streaming, tool, usage, and completion events without leaking the Manager token", async () => {
    ...
    const customConfig = join(root, "fork-atms.cordis.yml");
    vi.stubEnv("ATMS_WORKER_TOKEN", "manager-secret");
    vi.stubEnv("ATMS_DSH_CORDIS_CONFIG", customConfig);
    ...
    const events = await collect(adapter, context({
      environmentVariables: {
        ...
        ATMS_WORKER_TOKEN: "context-manager-secret",
      },
    }));
    ...
    expect(recorded.dshHome).toEqual(expect.stringMatching(/atms-dsh-/));
  ...
  it("runs the generated MCP proxy through the authenticated loopback tool bridge", async () => {
    ...
    expect(exercised.mcp.initialized.serverInfo.name).toBe("atms-tools");
    ...
    expect(exercised.mcp.composite).toEqual({
      content: [{ type: "text", text: "Unknown tool: mcp__atms__handoff" }],
      isError: true,
    });
  ...
  it("projects only the explicitly allowed Atms-managed read tools into DSH MCP", async () => {
