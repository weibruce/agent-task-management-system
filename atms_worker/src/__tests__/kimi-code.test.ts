...
describe("KimiCodeAdapter", () => {
  ...
  describe("buildKimiEnv", () => {
    it("sets KIMI_CODE_HOME and KIMI_MODEL_* from context", () => {
      const env = adapter.buildKimiEnv(
        {
          ...
          environmentVariables: {
            ...
            ATMS_WORKER_TOKEN: "reserved-turn-value",
          },
        },
        "/tmp/kimi-home-123",
      );
      ...
      expect(env.ATMS_WORKER_TOKEN).toBeUndefined();
    });
  ...
  describe("checkReadiness", () => {
    it("yields actionable error when kimi binary missing", async () => {
      const mockedAdapter = new KimiCodeAdapter("/tmp/atms-missing-kimi-binary");
    ...
    itOnPosix("does not expose the Manager admin token to the real readiness subprocess", async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "atms-kimi-readiness-env-"));
      const kimiBin = join(tempDir, "kimi");
      writeFileSync(kimiBin, `#!/usr/bin/env node
if (process.argv.includes("--version")) {
  console.log(process.env.ATMS_MANAGER_ADMIN_TOKEN ? "TOKEN_LEAKED" : "TOKEN_ABSENT");
  process.exit(0);
}
process.exit(2);
`, "utf-8");
      chmodSync(kimiBin, 0o755);
      vi.stubEnv("ATMS_MANAGER_ADMIN_TOKEN", "readiness-secret-0123456789abcdef");
      try {
        await expect(new KimiCodeAdapter(kimiBin).checkReadiness()).resolves.toEqual({
          ready: true,
          version: "TOKEN_ABSENT",
        });
        expect(process.env.ATMS_MANAGER_ADMIN_TOKEN).toBe("readiness-secret-0123456789abcdef");
      } finally {
    ...
    itOnWindows("runs npm kimi.cmd shims through node and the package main", async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "atms-kimi-cmd-shim-"));
...
`, "utf-8");
  ...
  describe("run (integration-style with mocked spawn)", () => {
    ...
    it("yields readiness error when kimi binary is not found", async () => {
      const missingAdapter = new KimiCodeAdapter("/tmp/atms-missing-kimi-binary-run-test");
    ...
    it("supports Kimi Agent SDK sessions with external Atms tools", async () => {
      const captured: {
      ...
      for await (const event of sdkAdapter.run("say hello", [echoTool], {
        ...
        skillProjection: {
          ...
          definitions: [{
            id: "atms-interaction",
            description: "Keep voice turns concise and operate the Atms canvas",
    ...
    it("falls back to prompt mode and emits handoff when ACP requires OAuth auth", async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "atms-kimi-acp-auth-fallback-"));
      const kimiBin = join(tempDir, "kimi");
      writeFileSync(kimiBin, `#!/usr/bin/env node
...
`, "utf-8");
    ...
    it("uses native SDK tools for Manager Agent after ACP authentication fallback", async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "atms-kimi-manager-sdk-fallback-"));
      const kimiBin = join(tempDir, "kimi");
      writeFileSync(kimiBin, `#!/usr/bin/env node
const readline = require("node:readline");
if (process.argv.includes("--version")) {
...
`, "utf-8");
    ...
    it("reconciles K3 Manager completion with the authoritative nested run ID", async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "atms-kimi-manager-tool-bridge-"));
      const kimiBin = join(tempDir, "kimi");
...
if (process.argv.includes("--prompt")) {
  ...
  console.log(JSON.stringify({
    role: "assistant",
    content: "不要显示的内部前言。\\n<atms_tool_call>" + createMarker + "</atms_tool_call>\\n<atms_tool_call>" + finishMarker + "</atms_tool_call>\\n已启动。"
  }));
  process.exit(0);
}
console.error("unexpected args " + process.argv.join(" "));
...
`, "utf-8");
      ...
      const previousPromptToolBridge = process.env.ATMS_KIMI_PROMPT_TOOL_BRIDGE;
      ...
      process.env.ATMS_KIMI_PROMPT_TOOL_BRIDGE = "1";
      try {
        ...
        const upsertGeneratedViewTool: DagToolDefinition = {
          name: "upsert_generated_view",
          description: "Create or replace a Atms A2UI Block",
          input_schema: {
            type: "object",
            properties: {
              id: { type: "string" },
        ...
        for await (const event of managerAdapter.run(
          ...
          {
            ...ctx,
            systemPrompt: "You are the Atms Manager Agent. Never invent run IDs.",
            provider: "kimi",
            model: "k3",
            baseUrl: "https://api.kimi.com/coding/v1",
        ...
        expect(prompt).not.toContain("You are the Atms Manager Agent");
        expect(agentInstructions).toBe("You are the Atms Manager Agent. Never invent run IDs.\n");
        expect(args).toContain("--skills-dir");
        expect(prompt).toContain("Atms tool execution protocol");
        ...
        expect(prompt).toContain("<atms_tool_call>");
        expect(events).toContainEqual(expect.objectContaining({
          type: "debug",
          ...
          data: expect.objectContaining({
            ...
            canonical_protocol: "atms_tool_call",
          }),
        }));
        ...
        if (previousPromptToolBridge === undefined) delete process.env.ATMS_KIMI_PROMPT_TOOL_BRIDGE;
        else process.env.ATMS_KIMI_PROMPT_TOOL_BRIDGE = previousPromptToolBridge;
        rmSync(tempDir, { recursive: true, force: true });
      }
    });
    ...
    it("finishes a K3 Manager run at most once and preserves failure-path summaries", async () => {
      ...
      try {
        for (const testCase of cases) {
          const tempDir = mkdtempSync(join(tmpdir(), `atms-kimi-${testCase.id}-`));
          const kimiBin = join(tempDir, "kimi");
          writeFileSync(kimiBin, `#!/usr/bin/env node
if (process.argv.includes("--version")) {
  console.log("kimi-code fixture 0.0.0");
  ...
  const markers = markerOrder.map((marker) => {
    const payload = marker === "create"
      ...
      : marker === "create-distinct"
        ...
        : finishMarker;
    return "<atms_tool_call>" + payload + "</atms_tool_call>";
  }).join("");
  console.log(JSON.stringify({
    role: "assistant",
...
`, "utf-8");
    ...
    it("registers Manager Agent tools through a local MCP bridge for ACP sessions", async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "atms-kimi-acp-mcp-"));
      const kimiBin = join(tempDir, "kimi");
...
`, "utf-8");
      ...
      try {
        ...
        for await (const event of mcpAdapter.run("Run the Manager Agent live smoke.", [createAndRunTool, finishTool], {
          ...ctx,
          systemPrompt: "You are the Atms Manager Agent. Use registered tools directly.",
          provider: "kimi",
          model: "k3",
          baseUrl: "https://api.kimi.com/coding/v1",
          skillProjection: { mode: "explicit", definitions: [] },
        ...
        expect(readFileSync(agentsPath, "utf-8"))
          .toBe("You are the Atms Manager Agent. Use registered tools directly.\n");
        expect(readFileSync(promptPath, "utf-8")).toBe("Run the Manager Agent live smoke.");
        expect(mcpServers[0].name).toBe("atms-tools");
        expect(mcpServers[0].command).toBe(process.execPath);
        expect(mcpServers[0].args[0]).toContain("atms-tools-mcp-server.mjs");
        expect(mcpServers[0].env.map((entry) => entry.name)).toEqual([
          "ATMS_MCP_BRIDGE_URL",
          "ATMS_MCP_BRIDGE_TOKEN",
        ]);
