...
function temporaryRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workspace-deps-"));
  roots.push(root);
  return root;
}
...
function fixture(): { workspace: string; repository: string; cache: string } {
  ...
  for (const packageName of PACKAGES) {
    ...
    const linkedCache = packageName === "atms_protocol" || packageName === "atms_worker";
...
describe("trusted workspace dependency projection", () => {
  it("projects matching immutable caches and binds local packages to the current worktree", () => {
    ...
    expect(fs.realpathSync(path.join(repository, "atms_manager", "node_modules", "typescript")))
      .toBe(fs.realpathSync(path.join(cache, "atms_manager", "node_modules", "typescript")));
    expect(fs.realpathSync(path.join(repository, "atms_manager", "node_modules", "atms-protocol")))
      .toBe(fs.realpathSync(path.join(repository, "atms_protocol")));
    expect(fs.realpathSync(path.join(repository, "atms_manager", "node_modules", "atms-plugin-sdk")))
      .toBe(fs.realpathSync(path.join(repository, "atms_plugin_sdk")));
    expect(fs.realpathSync(path.join(repository, "agent-ui", "node_modules", "atms-protocol")))
      .toBe(fs.realpathSync(path.join(repository, "atms_protocol")));
    expect(fs.realpathSync(path.join(repository, "atms_node", "node_modules", "atms-protocol")))
      .toBe(fs.realpathSync(path.join(repository, "atms_protocol")));
    expect(fs.realpathSync(path.join(repository, "atms_cli", "node_modules", "atms-protocol")))
      .toBe(fs.realpathSync(path.join(repository, "atms_protocol")));
    expect(fs.realpathSync(path.join(repository, "atms_cli", "node_modules", "atms-plugin-sdk")))
      .toBe(fs.realpathSync(path.join(repository, "atms_plugin_sdk")));
  });
  ...
  it("fails closed on changed metadata and an untrusted existing dependency tree", () => {
    const { workspace, repository, cache } = fixture();
    fs.writeFileSync(
      path.join(repository, "atms_manager", "package-lock.json"),
      `${JSON.stringify({ name: "atms_manager", lockfileVersion: 3, changed: true })}\n`,
    );
    fs.mkdirSync(path.join(repository, "atms_worker", "node_modules"));
    fs.writeFileSync(path.join(repository, "atms_worker", "node_modules", "owned.txt"), "keep\n");
    ...
    expect(fs.existsSync(path.join(repository, "atms_manager", "node_modules"))).toBe(false);
    expect(fs.readFileSync(path.join(repository, "atms_worker", "node_modules", "owned.txt"), "utf8"))
      .toBe("keep\n");
  });
  ...
  it("does nothing for a read-only dispatch", () => {
    ...
    expect(fs.existsSync(path.join(repository, "atms_protocol", "node_modules"))).toBe(false);
  });

  it("ignores release-only package versions while retaining the dependency fence", () => {
    const { workspace, repository, cache } = fixture();
    writePackage(path.join(repository, "atms_protocol"), "atms_protocol", "2");
    ...
    expect(result.projected).toContain("atms_protocol");
  });
});
