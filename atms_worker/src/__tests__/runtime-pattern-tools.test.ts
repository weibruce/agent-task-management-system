...
describe("runtime pattern worker tools", () => {
  ...
  it("detects protected and out-of-scope workspace mutations", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workspace-policy-"));
    try {
  ...
  it("ignores runtime-owned audit traces in workspace policy snapshots", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workspace-runtime-audit-"));
    try {
      fs.mkdirSync(path.join(root, "src"));
      fs.writeFileSync(path.join(root, "src", "code.ts"), "unchanged");
      const policy = { writable_paths: ["src"], readonly_paths: [] };
      const before = snapshotWorkspace(root, policy);
      fs.mkdirSync(path.join(root, ".atms-runtime", "audit"), { recursive: true });
      fs.writeFileSync(
        path.join(root, ".atms-runtime", "audit", "claude-sdk.jsonl"),
  ...
  it("rejects policy roots that enter reserved metadata directories", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workspace-reserved-"));
    try {
      ...
      expect(() => snapshotWorkspace(root, { writable_paths: ["repo/.atms-runtime"] }))
        .toThrow(/reserved segment \.atms-runtime/);
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("does not hide nested model-created Git metadata from snapshots", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workspace-nested-git-"));
    try {
  ...
  it("ignores only the declared repository root Git metadata", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workspace-root-git-"));
    try {
      fs.mkdirSync(path.join(root, "repo", ".git"), { recursive: true });
  ...
  it("ignores Git metadata directly beneath a declared writable checkout", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workspace-writable-git-"));
    try {
      fs.mkdirSync(path.join(root, "repo", ".git"), { recursive: true });
  ...
  it("ignores Manager-declared concurrent paths without hiding other unauthorized mutations", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workspace-concurrent-"));
    try {
  ...
  it("excludes only the exact Manager review projection and still detects sibling evidence writes", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-review-projection-"));
    try {
  ...
  it("ignores shared Manager Git writes during parallel fanout without hiding nested Git metadata", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workspace-parallel-git-"));
    try {
  ...
  it("rejects workspace symlinks that escape the policy root", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workspace-symlink-"));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workspace-outside-"));
    try {
  ...
  it("withholds handoff when an agent mutates a readonly artifact", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-policy-run-"));
  ...
  it("preserves an exact large and deep handoff after workspace verification", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-policy-exact-handoff-"));
    process.env.WORKSPACE = root;
    fs.mkdirSync(path.join(root, "src"));
    const exact = {
      long: "x".repeat(5000),
