...
describe("host Manager Agent REST authentication", () => {
  it("keeps the Manager credential out of shells and Agent backend environments", () => {
    const source = {
      ATMS_MANAGER_ADMIN_TOKEN: TOKEN,
      ATMS_PLUGIN_CAPABILITY_SECRET: "capability-secret",
      ATMS_WORKER_TOKEN: "worker-control-plane-token",
      ATMS_DAG_MUTATION_TOKEN: "dag-mutation-token",
    ...
    expect(source.ATMS_MANAGER_ADMIN_TOKEN).toBe(TOKEN);
    expect(source.ATMS_PLUGIN_CAPABILITY_SECRET).toBe("capability-secret");
    expect(source.ATMS_WORKER_TOKEN).toBe("worker-control-plane-token");
    expect(source.ATMS_DAG_MUTATION_TOKEN).toBe("dag-mutation-token");
  });
  ...
  it("adds only the scoped turn credential to /api mutations and redacts Manager errors", async () => {
    ...
    const server = http.createServer((req, res) => {
      observed.push({
        ...
        turn: req.headers[ATMS_MANAGER_TURN_HEADER] as string | undefined,
      });
      req.resume();
    ...
    try {
      ...
      await expect(_withManagerTurnEnvelopeForTest(envelope, () => _requestManagerForTest("/write", {
        ...
        headers: {
          ...
          [ATMS_MANAGER_TURN_HEADER]: "caller-controlled",
        },
        body: "{}",
  ...
  it("requires a valid scoped Manager signature and consumes each turn once", () => {
    ...
    const env: NodeJS.ProcessEnv = {
      ATMS_MANAGER_TURN_PUBLIC_KEY: (publicKey.export({ format: "der", type: "spki" }) as Buffer).toString("base64url"),
      ATMS_MANAGER_TURN_KEY_ID: "manager-test-key",
      ATMS_MANAGER_TURN_ENVELOPE_REQUIRED: "1",
      ATMS_MANAGER_AGENT_RUNTIME_PLACEMENT: "host_shell",
      ATMS_WORKER_ID: "worker-test",
    ...
    function signEnvelope(
      ...
      const claims = {
        ...
        issuer: "atms-manager" as const,
        audience: "atms-manager-agent-worker" as const,
  ...
  it("rejects unsigned /chat requests before invoking an Agent", async () => {
    ...
    vi.stubEnv("ATMS_MANAGER_TURN_PUBLIC_KEY", (publicKey.export({ format: "der", type: "spki" }) as Buffer).toString("base64url"));
    vi.stubEnv("ATMS_MANAGER_TURN_KEY_ID", "manager-route-key");
    vi.stubEnv("ATMS_MANAGER_TURN_ENVELOPE_REQUIRED", "1");
    vi.stubEnv("ATMS_MANAGER_AGENT_RUNTIME_PLACEMENT", "host_shell");
    vi.stubEnv("ATMS_WORKER_ID", "worker-route");
...
function headerEnvelope(): ManagerAgentTurnEnvelopeV1 {
  ...
  return {
    claims: {
      ...
      issuer: "atms-manager",
      audience: "atms-manager-agent-worker",
