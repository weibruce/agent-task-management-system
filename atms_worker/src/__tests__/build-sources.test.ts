...
describe("Worker deb822 source override CLI", () => {
  function makeSourcesFile(content: string): string {
    const dir = mkdtempSync(join(tmpdir(), "atms-worker-apt-sources-"));
    tempDirs.push(dir);
...
describe("Worker build source environment-name CLI mode", () => {
  const NPM_REGISTRY_ENV = "ATMS_WORKER_BUILD_NPM_REGISTRY";
...
describe("Worker Dockerfile source wiring", () => {
  ...
  it("wires the APT override helper into every stage before apt-get update", () => {
    ...
    for (const stage of stages) {
      ...
      expect(body).toMatch(/^ARG ATMS_WORKER_BUILD_APT_MIRROR$/m);
      expect(body).toMatch(/^ARG ATMS_WORKER_BUILD_APT_SECURITY_MIRROR$/m);
      ...
      const helperRunIndex = stage.lines
        .findIndex((line) => line === "RUN node /opt/atms/scripts/configure-apt-sources.mjs");
  ...
  it("keeps the APT override arguments optional with no vendor default", () => {
    expect(dockerfile).not.toMatch(/ARG ATMS_WORKER_BUILD_APT_MIRROR=/);
    expect(dockerfile).not.toMatch(/ARG ATMS_WORKER_BUILD_APT_SECURITY_MIRROR=/);
  });
  ...
  it("mounts one stable npm cache for every npm install without weakening lockfiles", () => {
    ...
    const cacheMount = "--mount=type=cache,id=atms-worker-npm-node22-v1,target=/root/.npm,sharing=locked";
  ...
  it("keeps npm command metadata out of committed image layers", () => {
    ...
    for (const instruction of npmRuns) {
      expect(instruction.text).toContain(
        "--mount=type=cache,id=atms-worker-npm-node22-v1,target=/root/.npm,sharing=locked",
      );
    }
  });
  ...
  it("allows the pinned DSH fork and Corepack bootstrap to use validated mirrors", () => {
    ...
    expect(dockerfile).toContain('git remote add origin "${ATMS_DSH_FORK_REPOSITORY}"');
    ...
    for (const instruction of corepackRuns) {
      ...
      expect(instruction.text).toContain(
        "--mount=type=cache,id=atms-worker-corepack-v1,target=/root/.cache/node/corepack,sharing=locked",
      );
      expect(instruction.text).toContain(
...
describe("Worker source fingerprint participation", () => {
  it("includes the APT sources helper in the Manager fingerprint inputs", () => {
    const dagEnvironmentSource = readFileSync(
      join(repoRoot, "atms_manager", "src", "server", "dag-environment.ts"),
      "utf8",
    );
    expect(dagEnvironmentSource).toContain("\"atms_worker/scripts/configure-apt-sources.mjs\"");
    const inputsMatch = /const SOURCE_INPUTS = \[(.*?)\] as const;/s.exec(dagEnvironmentSource);
    expect(inputsMatch).not.toBeNull();
    expect(inputsMatch?.[1]).toContain("atms_worker/scripts/configure-apt-sources.mjs");
  });
