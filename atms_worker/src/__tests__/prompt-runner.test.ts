...
describe("prompt runner", () => {
  ...
  it("permits only evidence-file repair before the corrected handoff", async () => {
    ...
    expect(observedContext?.systemPrompt).toContain(
      "inspect and rewrite only the declared .atms workspace evidence JSON",
    );
    ...
    expect(policySnapshot?.data?.writable_paths).toEqual(["repo/.atms"]);
  });
  ...
  it("filters Atms DAG tools through the node allowlist", async () => {
    let observedTools: string[] = [];
    const mockAgent: AgentClient = {
      run(_prompt, tools) {
        observedTools = tools.map((tool) => tool.name);
  ...
  it("streams agent debug events without sending them as content", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("ATMS_AGENT_DEBUG"));
  ...
  it("streams redacted tool inputs and result previews", async () => {
    ...
    const sent: string[] = [];
    const auditDir = mkdtempSync(join(tmpdir(), "atms-worker-redacted-audit-"));
    await runPrompt(
      {
        task: "test",
        sender: "test",
  ...
  it("redacts task, text, errors, WS events, audit files, and session files", async () => {
    ...
    const tmpHome = mkdtempSync(join(tmpdir(), "atms-worker-all-path-redaction-"));
  ...
  it("persists per-node session transcripts without plaintext credentials", async () => {
    const oldHome = process.env.ATMS_HOME;
    const tmpHome = mkdtempSync(join(tmpdir(), "atms-worker-session-store-"));
    process.env.ATMS_HOME = tmpHome;
    try {
      const mockAgent: AgentClient = {
        run() {
  ...
  it("records checkpoint resume metadata in the forked session transcript", async () => {
    const oldHome = process.env.ATMS_HOME;
    const tmpHome = mkdtempSync(join(tmpdir(), "atms-worker-checkpoint-transcript-"));
    process.env.ATMS_HOME = tmpHome;
    try {
      const mockAgent: AgentClient = {
        run() {
  ...
  it.each(["checkpoint-claude-adapter", "checkpoint-codex-adapter"])(
    ...
    async (backend) => {
      ...
      expect(observedPrompt).toContain("Atms portable actor checkpoint");
