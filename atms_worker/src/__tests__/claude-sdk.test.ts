...
describe("ClaudeSdkAdapter", () => {
  ...
  it("supports explicit custom prompt replacement and isolates Claude SDK settings", async () => {
    ...
    for await (const event of adapter.run("test", [], {
      ...ctx,
      systemPrompt: "Atms DAG contract",
      systemPromptMode: "replace",
    })) {
      events.push(event);
    }

    expect(captured[0].systemPrompt).toBe("Atms DAG contract");
  ...
  it("appends Atms instructions to the Claude Code system prompt preset by default", async () => {
    const captured: Record<string, unknown>[] = [];
    vi.doMock("@anthropic-ai/claude-agent-sdk", () => ({
      async *query(params: { options?: Record<string, unknown> }) {
        captured.push(params.options ?? {});
    ...
    for await (const event of adapter.run("test", [], {
      ...ctx,
      systemPrompt: "Atms Manager contract",
    })) {
      events.push(event);
    }
  ...
  it("projects only Atms Skills into an isolated native Claude Skill directory", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-claude-skills-test-"));
  ...
  it("keeps native Claude Skills and resumes the same Manager Agent session", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-claude-manager-session-"));
    const home = path.join(root, "home");
  ...
  it("does not pass Atms DAG session identifiers to Claude Code", async () => {
    const captured: Record<string, unknown>[] = [];
    vi.doMock("@anthropic-ai/claude-agent-sdk", () => ({
      async *query(params: { options?: Record<string, unknown> }) {
        captured.push(params.options ?? {});
  ...
  it("passes Anthropic auth through SDK env without exposing the secret in telemetry", async () => {
    ...
    expect((captured[0].env as Record<string, string>).ATMS_WORKER_TOKEN).toBeUndefined();
  ...
  it("uses dontAsk for a host Manager Agent while pre-approving Atms MCP tools", async () => {
    const captured: Record<string, unknown>[] = [];
    vi.doMock("@anthropic-ai/claude-agent-sdk", () => ({
      async *query(params: { options?: Record<string, unknown> }) {
        captured.push(params.options ?? {});
    ...
    const managerTool: DagToolDefinition = {
      ...
      description: "List Atms orchestrations",
  ...
  it("wires a fail-closed read hook when DAG workspace access is declared", async () => {
    ...
    const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "atms-claude-read-hook-"));
