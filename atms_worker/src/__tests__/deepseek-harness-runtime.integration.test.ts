...
const runtimeConfigured = Boolean(process.env.ATMS_DSH_RUNTIME_BIN?.trim());
...
describe.skipIf(!runtimeConfigured)("DeepSeek Harness packaged runtime", () => {
  it("runs the real JSON-RPC runtime through model SSE and MCP handoff", async () => {
    ...
    const handoff: DagToolDefinition = {
      name: "handoff",
      description: "Return the completed work to Atms",
      input_schema: {
        type: "object",
        properties: {
          port: { type: "string" },
    ...
    expect(exposedToolNames).toEqual(["mcp__atms__handoff"]);
    expect(JSON.stringify(provider.requests[0]?.body)).toContain("mcp__atms__handoff");
