...
function fixture() {
  const workspace = mkdtempSync(join(tmpdir(), "atms-dsh-read-tools-"));
...
describe("DeepSeek Harness Atms-managed read tools", () => {
  ...
  it("allows a declared root to be created after tool initialization and still resolves it safely", async () => {
    ...
    const escapedRoot = mkdtempSync(join(tmpdir(), "atms-dsh-read-tools-outside-"));
