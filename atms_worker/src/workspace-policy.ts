...
// Runtime-owned metadata lives in .atms-runtime on the host-backed
...
const RESERVED_POLICY_SEGMENTS = new Set([".git", ".atms-runtime", "node_modules"]);
...
export function snapshotWorkspace(root: string, policy: DagWorkspaceAccess): WorkspaceSnapshot {
  ...
  const visit = (absolute: string, relative: string): void => {
    ...
    if (stat.isDirectory()) {
      if (gitMetadataRoots.has(relative) || relative === ".atms-runtime" || path.posix.basename(relative) === "node_modules") return;
