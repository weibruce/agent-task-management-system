...
import type { DagWorkspaceAccess } from "atms-protocol";

const DEFAULT_CACHE_ROOT = "/opt/atms-dependency-cache";
...
const LOCAL_DEPENDENCIES: Partial<Record<(typeof TRUSTED_PACKAGES)[number], Record<string, string>>> = {
  "agent-ui": {
    "atms-protocol": "atms_protocol",
  },
  atms_plugin_sdk: {
    "atms-protocol": "atms_protocol",
  },
  atms_manager: {
    "atms-plugin-sdk": "atms_plugin_sdk",
    "atms-protocol": "atms_protocol",
  },
  atms_worker: {
    "atms-protocol": "atms_protocol",
...
function projectionMarkerMatches(targetModules: string, packageName: string, fingerprint: string): boolean {
  ...
  const marker = path.join(targetModules, ".atms-dependency-projection.json");
...
function populateFacade(
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(targetModules, ".atms-dependency-projection.json"),
...
export function materializeTrustedWorkspaceDependencies(
  workspace: string,
  access: DagWorkspaceAccess | undefined,
  cacheRoot = process.env.ATMS_WORKSPACE_DEPENDENCY_CACHE_ROOT?.trim() || DEFAULT_CACHE_ROOT,
