/**
 * Atms Worker image build helper.
 *
 ...
 *
 *   ATMS_WORKER_BUILD_APT_MIRROR          -> stanzas with non-security suites
 *   ATMS_WORKER_BUILD_APT_SECURITY_MIRROR -> stanzas with security suites
 *
...
export const APT_MAIN_MIRROR_ENV = "ATMS_WORKER_BUILD_APT_MIRROR";
export const APT_SECURITY_MIRROR_ENV = "ATMS_WORKER_BUILD_APT_SECURITY_MIRROR";
...
export function runCli(options = {}) {
  ...
  if (argv[0] === PRINT_ENV_FLAG) {
    ...
    try {
      ...
      fail(`Atms Worker build source override failed: ${error.message}`);
      return 1;
    }
  ...
  try {
    ...
    fail(
      `Atms Worker APT source override failed: unable to read deb822 sources at ${sourcesPath}. `
  ...
  try {
    ...
    fail(`Atms Worker APT source override failed: ${error.message}`);
    return 1;
  }
