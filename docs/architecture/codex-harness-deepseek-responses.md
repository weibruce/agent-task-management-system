...
| Manager Agent | Atms setting with `responses_base_url` | Responses | host |
| DAG actor | Atms setting with `responses_base_url` | Responses | container |
...
- Atms default reasoning effort: `high`

`deepseek-v4-pro` is recorded as known but currently unsupported for Responses,
so Atms rejects it before starting Codex. This follows DeepSeek's official
...
0.145.0. Atms obtains the current native Codex base instructions from
...
key and relay URL through `ATMS_CODEX_API_KEY`. The relay replaces that
...
`atms-codex-secret-guard.so` into the Worker and dynamically linked Codex
...
mode, or provider-side truncation; the Atms harness does not depend on
those fields.
...
- all three Worker containers initialized `atms_codex_appserver/0.145.0`;
...
- the nested checkout was selected as `cwd=/workspace/repository` while
  `/workspace` remained the Atms audit boundary;
...
verifies Codex 0.145.0, and the Worker maps the declared Atms workspace
...
| --- | --- | --- | --- |
| Shared Atms Manager prompt | yes | yes | n/a |
| Preserve native harness prompt and append Atms instructions | yes | yes (`developerInstructions`) | yes (`developerInstructions`) |
| Native Atms tools | in-process MCP | app-server dynamic tools | app-server dynamic tools |
| Manager tool schema parity | yes | yes, parity-tested | n/a |
| Native Skill bodies, references, and assets | yes | yes for Atms/plugin roots | yes, explicit turn projection |
...
| Workspace read-only mode | hooks plus SDK permissions | Codex sandbox | Atms outer policy + Codex `read-only` |
...
roots" switch. Atms must not claim strict fixed-projection parity until
that is enforceable.
...
2. Map Atms's per-root mixed workspace policy and, where a workflow asserts
