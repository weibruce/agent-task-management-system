...
branches, or push. Atms's Worker boundary first rejects changes outside the
...
`.atms/test-reports/` directory. Writing after `commit_workspace` keeps the
...
surface, not a fictional translation to Claude tool names. The outer Atms
...
boundaries, Atms's post-turn workspace-policy verification, and the
...
the checked-out Atms dependency metadata matches the metadata baked
...
cache and whose Atms-local entries point at the current isolated worktree.
The cache covers `atms_protocol`, `atms_plugin_sdk`,
`atms_manager`, `atms_worker`, `atms_node`, and `atms_cli`;
...
dependency trees without Atms's matching projection marker all fail
...
- Whether reviewer comments should be posted during the pilot. The safer
  default is to keep review evidence inside Atms and post only the final
