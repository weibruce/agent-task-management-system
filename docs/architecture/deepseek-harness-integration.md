...
Atms uses the owner-maintained
[`xiaotianfotos/deepseek-harness`](https://github.com/xiaotianfotos/deepseek-harness)
fork. The integration branch is `codex/atms-sdk-v0.1.2`. The fork carries
SDK steering and cancellation requests plus a Node-only deploy mode; the
Atms composition is maintained in this repository as a patch over the
...
`deepseek_harness` is an independent Worker backend. For each Atms turn it:
...
3. exposes that turn's Atms DAG tools and, when the DAG explicitly requests
   them, Atms-managed `Read`, `Grep`, `Glob`, and `LS` implementations
   through a temporary stdio MCP proxy, under DSH names such as
   `mcp__atms__handoff`;
4. maps DSH text, reasoning, tool, usage, and turn events back to Atms; and
...
tool transcript. Operators can set `ATMS_DSH_MAX_TOKENS` to another
positive integer when a model requires a tighter limit.

The Atms Cordis composition mounts DSH's configurable `llm-pi-ai` adapter.
For each turn the Worker builds a provider route from the selected Atms
...
a key in the same map. Atms has no global DSH effort list and supplies no
...
Atms Worker event loop. Claude, Codex, and Kimi adapters keep their existing
...
Atms Cordis patch in the image. The SDK launches that module with
`--profile sdk-minimal --patch <Atms patch>`, so ordinary DAG containers
...
```bash
git clone --depth 1 --branch codex/atms-sdk-v0.1.2 \
...
Point the Worker at the fork's `dsh` runtime module and Atms patch. Values
must be absolute paths.

```bash
export ATMS_DSH_RUNTIME_BIN=/absolute/path/deepseek-harness/python/sdk-runtime/src/deepseek_harness_runtime/runtime/node/node_modules/@deepseek-ai/dsh/lib/bin.js
export ATMS_DSH_CORDIS_CONFIG=/absolute/path/atms/atms_worker/dsh/atms.cordis.yml
```

Select `deepseek_harness` (aliases `dsh`, `deepseek`, and `deepseek-harness`
are accepted) and an active Atms model setting whose protocol is
...
the prompt, then binds Atms's turn controller as soon as the prompt receipt
...
Atms binds these requests to its turn controller, so live steering and
...
- DSH accepts exact `allowed_builtin_tools` only for `Read`, `Grep`, `Glob`, and
  ...
  four names are Atms-managed MCP implementations, not DSH-native
  filesystem plugins.
- A model setting must declare `reasoning_effort_map` before a DAG can select a
  ...
  default. This is intentional: Atms does not infer one provider's
...
- DSH-managed read/search tools enforce a call budget only when the workflow
  explicitly sets `max_builtin_tool_calls`. Atms does not impose a DSH
...
- DSH is not yet a complete Claude Code replacement in Atms: persistent
...
`mcp__atms__handoff`, DSH invokes the Atms handler with structured
...
([Actions](https://github.com/xiaotianfotos/atms/actions/runs/32023994801),
Atms run `d40c93aa-d9ff-4b57-a97c-e1c98c8d3e14`) reached the stable
...
runtime profile
([Actions](https://github.com/xiaotianfotos/atms/actions/runs/32030182632),
Atms run `1ee15bc9-db13-47ca-8d71-c55dda6a9dea`) completed successfully in
