...
labeled `atms-deploy`; PR Review and Auto Fix use separate runners.
The live DAG validation runner is retired.

Each deployment runner keeps its machine-specific paths and public address in
`~/.config/atms/production.env` with mode `0600`. The tracked workflow does
not contain hostnames, private addresses, account names, or mount paths. The
runner environment file defines:

- `ATMS_DEPLOY_RUNNER_ROOT` for the dedicated Actions runner;
- `ATMS_PRODUCTION_ROOT` for immutable releases;
- `ATMS_PRODUCTION_HOME` for persistent runtime data;
- `ATMS_PRODUCTION_RESOURCES` for optional external Skills;
- `ATMS_PRODUCTION_PUBLIC_HOST` for the required LAN-facing host or IP;
- optional `ATMS_PRODUCTION_UI_URL` when the default
  `https://<public-host>:19192` address needs overriding;
- optional `ATMS_PRODUCTION_UI_PORT` and
  `ATMS_PRODUCTION_UI_HTTP_PORT` overrides (defaults: `19192` and
  `19193`);
- optional Manager URL/host/port overrides when the default endpoint would
  ...
  `ATMS_PRODUCTION_ALLOW_INSECURE_REMOTE_WS=1` explicitly for an isolated
...
- user service: `atms-production.service`;
- runner service: `atms-deploy-runner.service` using only the
  `atms-deploy` custom label;
...
Machine-local `~/.config/atms/automation.env` (mode `0600`) supplies the
...
units for the `atms-pr-review` and `atms-auto-fix` runners. The standard
...
optional `ATMS_WORKER_BUILD_APT_MIRROR`,
`ATMS_WORKER_BUILD_APT_SECURITY_MIRROR`, and
`ATMS_WORKER_BUILD_NPM_REGISTRY` values are validated before Docker
...
`ATMS_PRODUCTION_HOME`; database, settings, sessions, certificates, and
