...
Atms 不需要发明新的文本语言，但需要把当前隐式存在的 DAG 语言正式化：
...
当前 Atms 已经有一套可工作的 YAML 语言，但它是由代码行为隐式定义的：

- [`parseDAGYaml()`](../atms_manager/src/orchestration/yaml-loader.ts) 直接把
  YAML 转成 `ParsedDAG`；
- [`graph.ts`](../atms_manager/src/orchestration/graph.ts) 同时承担源码结构、
  规范图和部分运行时接口的类型职责；
...
interface CanonicalWorkflowIR {
  ...
  source_api_version: "atms.ai/v1" | "legacy/v0";
...
- 未来可提供 `atms dag migrate` 生成供人工 review 的 v1 YAML；
...
`spec.triggers` 中的 `overlap` 和 `max_concurrency` 不是各自独立的局部开关。
Manager 会把同一 Workflow 的所有已启用 trigger 合并成一条最严格规则：任一 trigger
使用 `skip`，整个 Workflow 在已有活动运行时都拒绝新运行；并发上限取所有 trigger
声明值的最小值。这条规则同时覆盖 interval、event 和手动启动，不能通过换一个入口绕过。

创建 `dag_runs` 记录前，Manager 先在 SQLite 中原子预留名额，创建成功后再由活动运行
记录接管。没有已启用 trigger 的旧 Workflow 保持原有的无限制并行行为。当前只支持
`skip` / `allow`，没有持久化 queue 或 replace-running 语义；被拒绝时返回
`overlap_policy` 或 `max_concurrency`。手动 HTTP 启动返回 `409 Conflict`，并在
`data.reason` 中提供结构化原因；trigger 入口则在对应 delivery 项中返回跳过原因。
