...
Atms currently uses YAML as both the authoring format and the persisted
...
YAML is a serialization format, not the domain model. Atms already has an
...
limited to deterministic JSON Schema relationships that Atms can explain;
...
- a future `atms dag migrate` command may produce reviewed v1 source, but it is
...
CLI entry points are `atms dag schema` and `atms dag validate <file>`. Manager Agent
