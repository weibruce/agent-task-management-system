...
`assets/orchestrations/pr-review.yaml.template` is Atms's provider-neutral,
...
2. The three stable reviewer slots (historically labeled Qwen, Kimi, and GLM)
   ...
   Worker pre-tool hook or uses the Atms-managed DSH MCP implementations;
...
`ATMS_PR_REVIEW_DIVERSITY_POLICY` controls a configuration-time preflight:
...
- Atms run id and replayable event history
...
`atms dag run-template ... --wait`, downloads the declared JSON artifact,
...
the exact Atms run id and report identity. Any request-changes finding is
...
`atms-pr-review` label. Live catalog validation continues to use the
`atms-live` label and may start a current-commit transient runtime because it
is explicitly validating that commit's Manager/Worker protocol. Auto Fix uses a
third Actions runner labeled `atms-auto-fix`. PR Review and Auto Fix may run
...
- `ATMS_STABLE_ROOT`: immutable auto-deployed release root whose `current`
  symlink identifies the active stable runtime;
- `ATMS_STABLE_HOME`: persistent Home used by that Manager;
- `ATMS_STABLE_MANAGER_URL`: host-local Manager URL. It must be loopback or
  the Docker bridge gateway, never a LAN Manager endpoint;
- `ATMS_PR_REVIEW_PRIMARY_MODEL`: exact setting id, display name, or model
  name selected from the stable Manager database. It drives the first review;
- `ATMS_PR_REVIEW_ARBITER_MODEL`: a distinct active setting selected from
  the same database and drives the second review;
- `ATMS_PR_REVIEW_THIRD_MODEL`: a third distinct active setting that drives
...
`ATMS_GITHUB_API_BASE_URL`, so credential-free-accessible GitHub Enterprise
