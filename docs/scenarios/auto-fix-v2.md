...
- Qwen3.8-Max coding roles explicitly select
  ...
  Atms still verifies that the turn changed only declared paths.
- On Linux Docker Nodes, Manager automatically requests the fixed Codex Worker
  ...
  Docker mounts, cgroups, the disposable-container lifecycle, and Atms's
...
- The Worker image contains an immutable dependency cache for Agent UI and the
  Atms protocol, plugin SDK, Manager, Worker, Node, and CLI packages. Before
  ...
  Atms dependencies link back to that dispatch's worktree. A metadata
...
- Read-only reviewer containers keep `/workspace` read-only and receive one
  nested writable mount only at `/workspace/.atms-runtime` for trusted
...
```bash
printf '%s' "$GITHUB_AUTOFIX_TOKEN" | atms credential set github-autofix \
...
```bash
printf '%s' "$GITHUB_AUTOFIX_APP_JSON" | atms credential set github-autofix \
...
```json
{
  ...
  "require_draft": true,
  "writable_paths": ["atms_manager/src", "atms_manager/tests"]
}
```
...
```json
{
  ...
  "local_tests": [
    {
      "id": "manager-focused",
      "command": "npm --prefix atms_manager exec vitest run tests/example.test.ts",
      "timeout_seconds": 900
    }
  ],
  "shared": {
...
report below their own `.atms/test-reports/` directory after the broker has
...
```bash
export ATMS_AUTO_FIX_V2_IMPLEMENTATION_MODEL='<Qwen3.8-Max setting id>'
export ATMS_AUTO_FIX_V2_REVIEW_MODEL='<GLM-5.2 setting id>'
...
starts, Worker removes every Atms control-plane token/secret from the child
...
```bash
npm --prefix atms_protocol run build
npm --prefix atms_manager exec vitest run \
