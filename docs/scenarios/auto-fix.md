...
runner labeled `atms-auto-fix` submits the run to the auto-deployed stable
...
- `ATMS_STABLE_ROOT`, `ATMS_STABLE_HOME`, and
  `ATMS_STABLE_MANAGER_URL`;
- `ATMS_AUTO_FIX_IMPLEMENTATION_MODEL` for investigation, implementation,
  and revision;
- `ATMS_AUTO_FIX_REVIEW_MODEL` for correctness and regression review;
- `ATMS_AUTO_FIX_ARBITRATION_MODEL` for adversarial review, arbitration,
  and publication;
- `ATMS_AUTO_FIX_RUNNER_ROOT` for the dedicated Actions runner.
