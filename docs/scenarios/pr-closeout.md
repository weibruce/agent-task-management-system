...
repository owner still performs the merge outside Atms.
...
```bash
atms dag run-template pr-closeout --wait \
  --input '{
...
```bash
atms dag artifact <run-id> pr-closeout.json --output pr-closeout.json
```
...
```bash
atms dag run-template pr-closeout \
  --input '{
    "repo":"xiaotianfotos/atms",
    "pr":26,
    "phase":"merge",
...
persisted Atms handoffs. Merge candidacy requires a completed, current-head
...
```bash
atms dag approvals
atms dag decide <run-id> merge_approval \
  --decision approved \
  --actor human:owner \
  --proposal-hash <hash>
...
Local evidence is an operator assertion with an immutable SHA. Atms run
