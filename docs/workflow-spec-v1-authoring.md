...
WorkflowSpec is Atms's versioned DAG language. YAML and JSON are equivalent
...
```bash
atms dag schema > /tmp/atms-workflow-schema.json
atms --json dag validate ./workflow.yaml
```
...
atms dag artifact <run-id> evidence.tar.gz --output evidence.tar.gz
```
...
```bash
atms --json dag validate ./workflow.yaml
atms dag sync ./workflow.yaml
atms profile sync ./runtime.profile.yaml --workflow verified-task
atms run --workflow verified-task --profile local-main --prompt "Inspect the release"
```
...
Unversioned Atms YAML remains accepted as `legacy/v0`. Its aliases, inline
