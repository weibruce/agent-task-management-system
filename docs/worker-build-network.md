...
Atms builds exactly one canonical Worker image from
`atms_worker/Dockerfile`. By default the build uses the Debian
...
| Variable | Effect |
| --- | --- |
| `ATMS_WORKER_BUILD_APT_MIRROR` | Replaces the Debian main repository URL in non-security deb822 stanzas. |
| `ATMS_WORKER_BUILD_APT_SECURITY_MIRROR` | Replaces the Debian security repository URL in security deb822 stanzas. |
| `ATMS_WORKER_BUILD_NPM_REGISTRY` | Maps to the Docker `NPM_CONFIG_REGISTRY` build argument observed by every npm/pnpm operation and to `COREPACK_NPM_REGISTRY` while Corepack resolves pnpm. It is build-only and is never persisted as a runtime environment variable. |
| `ATMS_WORKER_BUILD_DSH_GIT_REMOTE` | Maps to the Docker `ATMS_DSH_FORK_REPOSITORY` build argument used to fetch the immutable DSH fork commit, allowing an internal read-only Git mirror instead of GitHub. |
...
- the Manager Worker image build (`DagEnvironmentController.runBuild`),
  resolved by `atms_manager/src/server/worker-build-network.ts`;
...
`atms_worker/scripts/configure-apt-sources.mjs --print-env NAME`, so a
...
`atms-worker-npm-node22-v1` cache at `/root/.npm`. The pinned DSH build
...
offline mode or permit stale/missing dependencies. Atms does not add a
...
`--build-arg NAME` entries only when non-empty. Atms never declares,
...
No source or proxy URL appears in status, task artifacts, or Atms-authored
build log messages.
...
- Atms does not choose or ship mirror vendors. Concrete fnOS mirror
