...
Atms stores execution credentials in the Manager database as AES-256-GCM
...
```bash
printf '%s' "$API_KEY" | atms credential set build-api \
  --type api_key --name "Build API"

jq -n --arg app_id "$LARK_APP_ID" --arg app_secret "$LARK_APP_SECRET" \
  '{app_id:$app_id,app_secret:$app_secret}' \
  | atms credential set lark-bot --type bot --name "Lark Bot" --json-stdin
...
jq -n --rawfile private_key ~/.ssh/id_ed25519 \
  ...
  | atms credential set deploy-ssh --type ssh_key --name "Deploy SSH" --json-stdin

atms credential list
atms credential audit deploy-ssh
atms credential revoke deploy-ssh
```
...
- JavaScript strings cannot be reliably zeroed. Atms minimizes plaintext
