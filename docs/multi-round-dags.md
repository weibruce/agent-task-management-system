...
This is strict `atms.ai/v1`: it uses `kind`, `config`, and explicit
...
`atms run --prompt` value into the default `prompt` input of a ready source node;
...
```bash
atms --json dag validate ./multi-round.yaml
atms dag sync ./multi-round.yaml
atms profile sync ./runtime.profile.yaml --workflow multi-round-research
atms run --workflow multi-round-research --profile local-main \
...
`atms dag watch` and continuous `atms dag supervise` stop polling and return
...
atms dag rounds "$RUN_ID"
atms dag commands "$RUN_ID"
```

Resume one actor with stable command identities for retry safety:

```bash
atms dag send-command "$RUN_ID" \
...
```bash
atms dag rounds "$RUN_ID"
atms dag commands "$RUN_ID" --round round-0002
```
...
atms dag complete "$RUN_ID" --expected-round "$CURRENT_ROUND"
```

Completion is valid only at the current waiting boundary. A premature request
or stale round ID returns a conflict. To abort instead, `atms stop "$RUN_ID"`
...
```bash
curl -sS -X POST "$ATMS_MANAGER_URL/api/runs/$RUN_ID/commands" \
  -H 'Content-Type: application/json' \
  -H "X-Atms-Dag-Token: $ATMS_DAG_MUTATION_TOKEN" \
  -d '{
...
```bash
curl -sS -X POST "$ATMS_MANAGER_URL/api/runs/$RUN_ID/complete" \
  -H 'Content-Type: application/json' \
  -H "X-Atms-Dag-Token: $ATMS_DAG_MUTATION_TOKEN" \
  -d '{"expected_round_id":"round-0002"}'
```

When `ATMS_DAG_MUTATION_TOKEN` is configured, all listed mutation requests
must send it as `X-Atms-Dag-Token`, including loopback requests; the `atms`
