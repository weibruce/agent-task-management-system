...
Atms keeps successful, failed, and cancelled run workspaces for seven days by default. The policy applies only to per-run directories under `$ATMS_HOME/workspace`; run metadata, events, evidence, sessions, and the reserved `default` workspace are not deleted.

Cleanup runs on the Manager host. The current local Manager and Node topology shares the same `ATMS_HOME`, so those run workspaces are covered. A remote Node that uses an independent data root requires its own node-side retention policy and is outside this cleaner's scope.
...
```text
$ATMS_HOME/manager/workspace-retention.json
```

Before that file exists, deployments may seed the initial values with `ATMS_WORKSPACE_CLEANUP_ENABLED`, `ATMS_WORKSPACE_RETENTION_SUCCESS_DAYS`, and `ATMS_WORKSPACE_RETENTION_FAILURE_DAYS`. Saving in the UI creates the persistent file. The scheduler interval defaults to six hours and can be set with `ATMS_WORKSPACE_CLEANUP_INTERVAL_MS`.
...
Mutation requests require a local connection or a valid `X-Atms-Dag-Token` when `ATMS_DAG_MUTATION_TOKEN` is configured.
