...
Atms's pattern library separates reusable orchestration knowledge into four
...
3. **Skill guidance**: selection, adaptation, composition, and validation rules
   in `skills/atms-dag-patterns/SKILL.md`.
...
must be present in `ATMS_DAG_COMMAND_ALLOWLIST`; timeout, output capture,
...
`command_field` additionally requires `ATMS_DAG_ALLOW_DYNAMIC_COMMANDS=true`.
...
loopback clients, or with `ATMS_DAG_APPROVAL_TOKEN` when Manager is remote.
...
`ATMS_DAG_MUTATION_TOKEN`, including run lifecycle operations, workflow and
...
Workspace policy paths cannot enter `.git`, `.atms-runtime`, or
`node_modules`. Declare the repository or project root instead. In particular,
do not add `.atms-runtime` to `writable_paths`: Node already overlays that
...
allowlists for backend-provided shell/file tools and Atms DAG tools.
...
Atms MCP handlers confined by `workspace_access`; it rejects shell or
...
through its Anthropic-compatible endpoint and the Atms `claude-sdk`
...
run-scoped `${ATMS_HOME}/workspace/<run-id>` directory. This enables
...
```bash
atms dag artifacts <run-id>
atms dag artifact <run-id> diagnosis.json --output diagnosis.json
```
...
`ATMS_DAG_COMMAND_ALLOWLIST` (for example, `node,git`). No dynamic command
...
```bash
atms patterns list
atms --json patterns show quorum
```
...
still comes from Atms database settings and profiles.
...
The Atms Manager Agent receives all Skill metadata discovered under
`${ATMS_HOME}/skills` on every turn. For pattern work it loads
`atms-dag-patterns` through `read_skill`, then uses
...
```bash
atms dag approvals
atms dag decide <run-id> <node-id> --decision approved --actor <id> --proposal-hash <sha256>
atms dag triggers
atms dag trigger-event <event> --idempotency-key <key> --payload '<json>'
atms dag state-get <namespace> <key>
atms dag state-set <namespace> <key> '<json>' --expected-version <n>
```
...
Atms adapts the article's orchestration ideas into generic DAG primitives
