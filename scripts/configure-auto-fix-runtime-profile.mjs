...
export function selectAutoFixRuntimeSetting(settings, selector, role) {
  ...
  if (!wanted) throw new Error(`ATMS_AUTO_FIX_${role.toUpperCase()}_MODEL is required`);
...
async function request(managerUrl, pathname, init) {
  ...
  const response = await fetch(`${managerUrl}${pathname}`, {
    ...
    headers: {
      ...(init?.headers ?? {}),
      ...(mutation && process.env.ATMS_DAG_MUTATION_TOKEN
...
export async function configureAutoFixRuntimeProfile({
  managerUrl = process.env.ATMS_MANAGER_URL ?? "http://127.0.0.1:19191",
  profileId = process.env.ATMS_AUTO_FIX_PROFILE_ID ?? "auto-fix-mixed",
  implementationSelector = process.env.ATMS_AUTO_FIX_IMPLEMENTATION_MODEL,
  reviewSelector = process.env.ATMS_AUTO_FIX_REVIEW_MODEL,
  arbitrationSelector = process.env.ATMS_AUTO_FIX_ARBITRATION_MODEL,
} = {}) {
