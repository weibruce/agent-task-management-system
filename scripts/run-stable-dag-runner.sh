...
TASK="${ATMS_STABLE_TASK:-}"
case "$TASK" in
  pr-review)
    INPUT="${ATMS_PR_REVIEW_INPUT:-}"
    INPUT_FILE="${ATMS_PR_REVIEW_INPUT_FILE:-}"
    ARTIFACT_DIR="${ATMS_PR_REVIEW_ARTIFACT_DIR:-${GITHUB_WORKSPACE:-$PWD}/artifacts/pr-review}"
    ...
    TIMEOUT_SECONDS="${ATMS_PR_REVIEW_TIMEOUT_SECONDS:-4200}"
    PROFILE_SCRIPT="$ATMS_STABLE_RELEASE/scripts/configure-pr-review-runtime-profile.mjs"
    MARKDOWN_SCRIPT="$ATMS_STABLE_RELEASE/scripts/render-pr-review-markdown.mjs"
    ARTIFACT_NAMES=(pr-review.json)
    ;;
  auto-fix)
    INPUT="${ATMS_AUTO_FIX_INPUT:-}"
    INPUT_FILE="${ATMS_AUTO_FIX_INPUT_FILE:-}"
    ARTIFACT_DIR="${ATMS_AUTO_FIX_ARTIFACT_DIR:-${GITHUB_WORKSPACE:-$PWD}/artifacts/auto-fix}"
    TIMEOUT_SECONDS="${ATMS_AUTO_FIX_TIMEOUT_SECONDS:-10800}"
    PROFILE_SCRIPT="$ATMS_STABLE_RELEASE/scripts/configure-auto-fix-runtime-profile.mjs"
    CHECKPOINT_SCRIPT="$ATMS_STABLE_RELEASE/scripts/auto-fix-checkpoint.mjs"
    ARTIFACT_NAMES=(auto-fix.json auto-fix.patch auto-fix.md)
    ;;
  *)
    echo "ATMS_STABLE_TASK must be pr-review or auto-fix." >&2
    exit 1
    ;;
esac
...
PROFILE_ID="$("$ATMS_STABLE_NODE" "$PROFILE_SCRIPT")"
...
if [ -n "$INPUT_FILE" ]; then
  ...
  if [ "$TASK" = "auto-fix" ]; then
    "$ATMS_STABLE_NODE" "$CHECKPOINT_SCRIPT" hydrate "$INPUT_FILE"
  fi
  INPUT="$(<"$INPUT_FILE")"
fi
if [ -z "$INPUT" ]; then
...
RUN_ID="${ATMS_STABLE_RUN_ID:-$(
  "$ATMS_STABLE_NODE" -e 'process.stdout.write(require("node:crypto").randomUUID())'
)}"
...
collect_pr_review_execution_evidence() {
  ...
  "$ATMS_STABLE_NODE" "$ATMS_STABLE_RELEASE/scripts/pr-review-execution-evidence.mjs" \
...
render_pr_review_markdown() {
  ...
  "$ATMS_STABLE_NODE" "$MARKDOWN_SCRIPT" \
    "$COMMAND_PATH" \
...
if [ "$cli_status" -ne 0 ]; then
  ...
  printf '%s\n' "$ATMS_STABLE_REVISION" >"$ARTIFACT_DIR/manager-revision.txt"
  if [ "$TASK" = "auto-fix" ]; then
    for _attempt in 1 2 3 4 5; do
      checkpoint_result="$("$ATMS_STABLE_NODE" "$CHECKPOINT_SCRIPT" record "$INPUT_FILE" "$RUN_ID" 2>/dev/null || true)"
...
if [ "$RUN_STATUS" != "completed" ]; then
  ...
  render_pr_review_markdown
  if [ "$TASK" = "auto-fix" ]; then
    "$ATMS_STABLE_NODE" "$CHECKPOINT_SCRIPT" record "$INPUT_FILE" "$RUN_ID" >"$ARTIFACT_DIR/checkpoint.json" || true
  fi
  printf '%s\n' "$RUN_ID" >"$ARTIFACT_DIR/run-id.txt"
  printf '%s\n' "$ATMS_STABLE_REVISION" >"$ARTIFACT_DIR/manager-revision.txt"
...
case "$TASK" in
  pr-review)
    "$ATMS_STABLE_NODE" "$ATMS_STABLE_RELEASE/scripts/validate-pr-review-artifacts.mjs" \
      "$COMMAND_PATH" \
      "$ARTIFACT_DIR/pr-review.json" \
      "$ARTIFACT_DIR/pr-review.md"
    ;;
  auto-fix)
    "$ATMS_STABLE_NODE" "$ATMS_STABLE_RELEASE/scripts/validate-auto-fix-artifacts.mjs" \
      "$COMMAND_PATH" \
      "$ARTIFACT_DIR/auto-fix.json" \
      "$ARTIFACT_DIR/auto-fix.patch" \
      "$ARTIFACT_DIR/auto-fix.md"
    "$ATMS_STABLE_NODE" "$CHECKPOINT_SCRIPT" record "$INPUT_FILE" "$RUN_ID" >"$ARTIFACT_DIR/checkpoint.json"
    ;;
esac
