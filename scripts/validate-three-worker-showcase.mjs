...
const DEFAULT_HOME = process.env.ATMS_HOME || path.join(os.homedir(), ".atms");
...
function usage() {
  return [
    ...
    "  --setting-id ID                  Required for all/prepare; env ATMS_PATTERN_SETTING_ID",
    "  --expected-model ID               Optional exact real-model assertion",
    "  --base-url URL                   Default: ATMS_MANAGER_URL or http://127.0.0.1:19191",
    ...
    "  --asset PATH                     External Workflow asset; env ATMS_SHOWCASE_ASSET",
    "  --prompt-file PATH               UTF-8 mission; env ATMS_SHOWCASE_PROMPT",
    ...
    "  --help",
    "",
    "Mutation authentication is read only from ATMS_DAG_MUTATION_TOKEN and",
    "ATMS_MANAGER_ADMIN_TOKEN. Tokens are never accepted as CLI arguments.",
  ].join("\n");
}
...
const baseUrl = (cli["base-url"] ?? process.env.ATMS_MANAGER_URL ?? "http://127.0.0.1:19191")
  .replace(/\/+$/, "");
const requestedSettingId = cli["setting-id"] ?? process.env.ATMS_PATTERN_SETTING_ID ?? "";
const expectedModel = cli["expected-model"] ?? process.env.ATMS_PATTERN_EXPECTED_MODEL ?? "";
...
const assetPath = (cli.asset || process.env.ATMS_SHOWCASE_ASSET)
  ? path.resolve(cli.asset ?? process.env.ATMS_SHOWCASE_ASSET)
  : "";
...
const showcasePrompt = promptFilePath
  ...
  : (process.env.ATMS_SHOWCASE_PROMPT ?? "").trim();
const requiredSurfaceTerms = Array.from(new Set(
  String(cli["required-surface-terms"] ?? process.env.ATMS_SHOWCASE_REQUIRED_TERMS ?? "")
    .split(",")
...
const statePath = path.resolve(
  cli["state-file"]
    ?? process.env.ATMS_THREE_WORKER_STATE_FILE
    ?? path.join(DEFAULT_VALIDATION_DIR, "three-worker-showcase-state.json"),
);
const outputPath = path.resolve(
  cli.output
    ?? process.env.ATMS_THREE_WORKER_REPORT_FILE
...
const restartEvidencePath = cli["restart-evidence"]
  ...
  : process.env.ATMS_SHOWCASE_RESTART_EVIDENCE
    ? path.resolve(process.env.ATMS_SHOWCASE_RESTART_EVIDENCE)
    : "";
const stallTimeoutMs = parseIntegerOption(
  cli["stall-timeout-ms"],
  "--stall-timeout-ms",
  process.env.ATMS_THREE_WORKER_STALL_TIMEOUT_MS ?? "1200000",
  0,
);
const pollIntervalMs = parseIntegerOption(
  cli["poll-interval-ms"],
  "--poll-interval-ms",
  process.env.ATMS_THREE_WORKER_POLL_INTERVAL_MS ?? "2000",
  250,
  30000,
);
const mutationToken = process.env.ATMS_DAG_MUTATION_TOKEN ?? "";
const managerAdminToken = process.env.ATMS_MANAGER_ADMIN_TOKEN ?? "";
...
if (requiredSurfaceTerms.length === 0) {
  throw new Error("Missing --required-surface-terms or ATMS_SHOWCASE_REQUIRED_TERMS.");
}
...
if ((phase === "all" || phase === "prepare") && !requestedSettingId) {
  throw new Error("Missing --setting-id or ATMS_PATTERN_SETTING_ID.");
}
if (phase === "all" || phase === "prepare") {
  if (!assetPath || !fs.statSync(assetPath, { throwIfNoEntry: false })?.isFile()) {
    throw new Error("Missing external --asset or ATMS_SHOWCASE_ASSET Workflow file.");
  }
  if (!showcasePrompt) {
    throw new Error("Missing --prompt-file or ATMS_SHOWCASE_PROMPT mission.");
  }
...
async function request(pathname, init = {}) {
  ...
  if (method !== "GET") {
    if (mutationToken) headers.set("X-Atms-Dag-Token", mutationToken);
