...
test('the live acceptance remains explicit rather than a Manager or built-in Skill intent route', () => {
  const forbiddenProductSources = [
    'skills/atms-dag-ops/SKILL.md',
    'atms_protocol/src/manager-agent-tools.ts',
    'atms_manager/src/server/host-codex-manager-agent.ts',
  ]
...
test('GitHub Actions does not dispatch retired real-model DAG validation', () => {
  ...
  for (const filename of fs.readdirSync(workflowDir)) {
    ...
    assert.doesNotMatch(workflow, /atms-live|validate:dag-patterns|validate:three-worker-showcase|ATMS_PATTERN_MODEL_BASE_URL/, filename)
  }
})
...
test('the live adapter requires an external asset and uses the public idle TTL', () => {
  ...
  assert.match(runner, /ATMS_SHOWCASE_ASSET must name an external Workflow file/)
  assert.match(runner, /ATMS_SHOWCASE_PROMPT is required/)
  assert.match(runner, /ATMS_SHOWCASE_REQUIRED_TERMS is required/)
  assert.match(runner, /--required-surface-terms/)
  assert.match(runner, /ATMS_DAG_WORKER_IDLE_TTL_MS/)
