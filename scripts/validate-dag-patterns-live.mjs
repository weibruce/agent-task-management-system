...
const baseUrl = option("--base-url", process.env.ATMS_MANAGER_URL ?? "http://127.0.0.1:19191").replace(/\/+$/, "");
const settingId = option("--setting-id", process.env.ATMS_PATTERN_SETTING_ID);
const expectedModel = option("--expected-model", process.env.ATMS_PATTERN_EXPECTED_MODEL ?? "");
const profileId = option("--profile-id", "live-pattern-validation");
const workflowSuffix = option("--workflow-suffix", process.env.ATMS_LIVE_WORKFLOW_SUFFIX ?? "");
const stallTimeoutMs = Number(option(
  "--stall-timeout-ms",
  process.env.ATMS_LIVE_PATTERN_STALL_TIMEOUT_MS ?? "1200000",
));
const maxAttempts = Number(option("--max-attempts", process.env.ATMS_LIVE_PATTERN_ATTEMPTS ?? "2"));
const outputPath = option("--output", "");
const requestedPatterns = repeatedOption("--pattern");
const approvalToken = process.env.ATMS_DAG_APPROVAL_TOKEN ?? "";
const mutationToken = process.env.ATMS_DAG_MUTATION_TOKEN ?? approvalToken;
const managerAdminToken = process.env.ATMS_MANAGER_ADMIN_TOKEN ?? "";
const agentType = process.env.ATMS_PATTERN_AGENT_TYPE ?? "claude-sdk";

if (!settingId) {
  console.error("Missing --setting-id or ATMS_PATTERN_SETTING_ID.");
  process.exit(2);
}
  ...
  const response = await fetch(`${baseUrl}${pathname}`, {
    ...
    headers: {
      ...
      ...(mutationToken && init?.method && init.method !== "GET"
        ? { "X-Atms-Dag-Token": mutationToken }
        : {}),
    },
  });
...
async function prepareRuntimeState(patternId) {
  ...
  if (patternId === "ratchet" && process.env.ATMS_HOME) {
    fs.rmSync(path.join(process.env.ATMS_HOME, "live-ratchet.metric"), { force: true });
  }
}
...
async function decidePendingApproval(runId) {
  ...
  if (approvalToken) headers["x-atms-approval-token"] = approvalToken;
