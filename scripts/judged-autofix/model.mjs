...
export async function api(plan, route, body, raw = false) {
  const r = await fetch(plan.manager_url.replace(/\/$/, '') + route, {
    ...
    headers: { 'content-type': 'application/json', 'x-atms-dag-token': process.env.ATMS_DAG_MUTATION_TOKEN ?? '' },
...
export function workflow(id, kind) {
  ...
  return { api_version: 'atms.ai/v1', kind: 'Workflow', metadata: { id, name: id }, spec: {
