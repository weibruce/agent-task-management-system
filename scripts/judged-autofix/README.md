...
This Linux host controller unfolds repair iterations into fresh Atms DAG runs.
...
host, a clean `codex/` branch, and a Atms Manager supporting idempotent run
...
`ATMS_DAG_MUTATION_TOKEN` on the supervising host. The local model does not
