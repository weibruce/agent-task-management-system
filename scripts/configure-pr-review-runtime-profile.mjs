...
export function selectRuntimeSetting(settings, selector, role, agentType = "claude-sdk") {
  ...
  if (!wanted) throw new Error(`ATMS_PR_REVIEW_${role.toUpperCase()}_MODEL is required for stable review`);
...
async function request(managerUrl, pathname, init) {
  ...
  const response = await fetch(`${managerUrl}${pathname}`, {
    ...
    headers: {
      ...(init?.headers ?? {}),
      ...(process.env.ATMS_DAG_MUTATION_TOKEN && isMutation
        ? { "x-atms-dag-token": process.env.ATMS_DAG_MUTATION_TOKEN }
        : {}),
    },
  });
...
export async function configurePrReviewRuntimeProfile({
  managerUrl = process.env.ATMS_MANAGER_URL ?? "http://127.0.0.1:29191",
  profileId = process.env.ATMS_PR_REVIEW_PROFILE_ID,
  workflowId = process.env.ATMS_PR_REVIEW_WORKFLOW_ID ?? "pr-review",
  agentType = process.env.ATMS_PR_REVIEW_AGENT_TYPE ?? "claude-sdk",
  modelSelector = process.env.ATMS_PR_REVIEW_MODEL,
  primarySelector = process.env.ATMS_PR_REVIEW_PRIMARY_MODEL,
  arbiterSelector = process.env.ATMS_PR_REVIEW_ARBITER_MODEL,
  thirdSelector = process.env.ATMS_PR_REVIEW_THIRD_MODEL,
  reasoningEffort = process.env.ATMS_PR_REVIEW_REASONING_EFFORT,
  diversityPolicy = process.env.ATMS_PR_REVIEW_DIVERSITY_POLICY ?? "executions",
} = {}) {
