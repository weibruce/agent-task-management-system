...
VALIDATOR_IMAGE="${ATMS_AUTO_FIX_VALIDATOR_IMAGE:-mcr.microsoft.com/playwright:v1.57.0-noble@sha256:3bed4b1a12f2338642f3d8cba28e291deef3c66bd4a964bbeb3e57bbff511dbd}"
RUNNER_TEMP_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}"
VALIDATION_ROOT="$(mktemp -d "$RUNNER_TEMP_ROOT/atms-auto-fix-validation.XXXXXX")"
VALIDATION_CHECKOUT="$VALIDATION_ROOT/source"
NODE_BIN="${ATMS_NODE_BIN:-node}"
...
docker run --rm \
  ...
  --pids-limit 1024 \
  --memory "${ATMS_AUTO_FIX_MEMORY_LIMIT:-8g}" \
  --cpus "${ATMS_AUTO_FIX_CPU_LIMIT:-8}" \
