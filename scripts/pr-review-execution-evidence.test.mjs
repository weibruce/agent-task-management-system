...
test('collector uses the established Manager header without following credential-bearing redirects',async()=>{
 const {collectPrReviewExecutionEvidence}=await mod();let auth;const old=process.env.ATMS_DAG_MUTATION_TOKEN;process.env.ATMS_DAG_MUTATION_TOKEN='test-audit-token';const server=http.createServer((req,res)=>{auth=req.headers;res.setHeader('content-type','application/json');res.end(JSON.stringify({success:true,data:{messages:[]}}));});await new Promise(r=>server.listen(0,'127.0.0.1',r));const dir=fs.mkdtempSync(path.join(os.tmpdir(),'review-audit-auth-'));
 try{await collectPrReviewExecutionEvidence({managerUrl:`http://127.0.0.1:${server.address().port}/`,runId,outputPath:path.join(dir,'evidence.json')});assert.equal(auth['x-atms-dag-token'],'test-audit-token');assert.equal(auth.authorization,undefined);}
 finally{if(old===undefined)delete process.env.ATMS_DAG_MUTATION_TOKEN;else process.env.ATMS_DAG_MUTATION_TOKEN=old;await new Promise(r=>server.close(r));fs.rmSync(dir,{recursive:true,force:true});}
});
...
test('stable runner collects sanitized execution evidence on both successful quorum and command failure',async()=>{
 ...
 try{
  ...
  fs.writeFileSync(path.join(scripts,'lib/stable-automation-runtime.sh'),`initialize_stable_automation_runtime() { ATMS_STABLE_RELEASE="$ATMS_TEST_RELEASE"; ATMS_STABLE_NODE="$ATMS_TEST_NODE"; ATMS_STABLE_REVISION=test; }
stable_hr() {
 if [ "$1" = "--json" ]; then
  if [ "$ATMS_TEST_MODE" = "failure" ]; then return 1; fi
  printf '%s\\n' '{"run_id":"${runId}","status":"completed"}'; return 0
 fi
 if [ "$1" = "dag" ] && [ "$2" = "artifact" ]; then
  cp "$ATMS_TEST_PUBLICATION" "$6"; return 0
 fi
 return 0
}
`);
  ...
  for(const mode of ['success','failure']){
   ...
   const env={...process.env,ATMS_TEST_RELEASE:dir,ATMS_TEST_NODE:process.execPath,ATMS_TEST_MODE:mode,ATMS_TEST_PUBLICATION:publication,ATMS_STABLE_TASK:'pr-review',ATMS_STABLE_RUN_ID:runId,ATMS_PR_REVIEW_INPUT:'{}',ATMS_PR_REVIEW_INPUT_FILE:'',ATMS_PR_REVIEW_ARTIFACT_DIR:artifacts,ATMS_MANAGER_URL:`http://127.0.0.1:${server.address().port}`};
