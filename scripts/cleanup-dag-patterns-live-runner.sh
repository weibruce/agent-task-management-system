...
if [ -n "${ATMS_LIVE_HOME_BASE:-}" ] && [ -z "${ATMS_RUNNER_BASE:-}" ]; then
  echo "ATMS_RUNNER_BASE is required when ATMS_LIVE_HOME_BASE is configured." >&2
  exit 1
fi
...
container_ids_for_scope() {
  ...
  if [ "$slot" != "__legacy_unscoped__" ]; then
    docker ps -aq --filter "label=org.atms.live_slot=$slot"
    return
  fi
  while IFS= read -r id; do
    ...
    if ! label="$(
      docker container inspect \
        --format '{{with index .Config.Labels "org.atms.live_slot"}}{{.}}{{end}}' \
        "$id" 2>/dev/null
    )"; then
      continue
    fi
    [ -z "$label" ] && printf '%s\n' "$id"
  done < <(docker ps -aq --filter "label=org.atms.live_run")
}

image_ids_for_scope() {
  local slot="$1" id label
  if [ "$slot" != "__legacy_unscoped__" ]; then
    docker images --filter "label=org.atms.live_slot=$slot" --format "{{.ID}}" | sort -u
    return
  fi
  while IFS= read -r id; do
    [ -n "$id" ] || continue
    ...
    [ -z "$label" ] && printf '%s\n' "$id"
  done < <(docker images --filter "label=org.atms.live_run" --format "{{.ID}}" | sort -u)
}
...
if [ -n "${ATMS_LIVE_SLOT:-}" ]; then
  LIVE_SLOT="$(sanitize_slot "$ATMS_LIVE_SLOT")"
  case "$LIVE_SLOT" in
    ""|.|..) exit 1 ;;
  esac
  HOME_BASE="$HOME_ROOT/slots/$LIVE_SLOT"
  LOCK_FILE="$RUNNER_BASE/slots/$LIVE_SLOT/dag-patterns-live.lock"
  if [ "${ATMS_CLEANUP_LOCK_HELD:-0}" = "1" ]; then
