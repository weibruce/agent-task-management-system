...
for (const code of [75, 1]) test(`stable runner preserves unknown execution only for CLI exit ${code}`, { skip: process.platform === "win32" }, t => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "atms-stable-observation-"));
  ...
  for (const dir of ["runtime", "atms_cli/dist", "scripts"]) fs.mkdirSync(path.join(release, dir), { recursive: true });
  ...
  fs.writeFileSync(path.join(release, "atms_cli/dist/cli.js"), `
  ...
  const result = spawnSync("bash", [path.join(root, "scripts/run-stable-dag-runner.sh")], {
    ...
    env: { ...process.env, PATH: bin + path.delimiter + process.env.PATH, ATMS_STABLE_ROOT: stable, ATMS_STABLE_HOME: home,
      ATMS_STABLE_MANAGER_URL: "http://127.0.0.1:19991", ATMS_STABLE_TASK: "pr-review",
      ATMS_STABLE_RUN_ID: "fixed-run", ATMS_PR_REVIEW_INPUT: "{}",
      ATMS_PR_REVIEW_INPUT_FILE: "", ATMS_PR_REVIEW_ARTIFACT_DIR: artifacts },
  });
...
test("stable runner uses the deployed release and never starts a transient Manager", () => {
  ...
  assert.match(bootstrap, /ATMS_STABLE_RELEASE=.*readlink -f.*current/);
  assert.match(bootstrap, /dag-mutation\.token/);
  assert.match(runner, /\$ATMS_STABLE_RELEASE\/scripts\/configure-/);
  assert.match(runner, /ATMS_PR_REVIEW_TIMEOUT_SECONDS:-4200/);
...
test("PR closeout reuses the stable release and keeps GitHub review evidence readable", () => {
  ...
  assert.match(workflow, /runs-on: \[self-hosted, Linux, X64, atms-pr-review\]/);
  assert.match(workflow, /GH_TOKEN: \$\{\{ github\.token \}\}/);
  assert.match(workflow, /GITHUB_TOKEN: \$\{\{ github\.token \}\}/);
  assert.match(workflow, /readlink -f "\$ATMS_STABLE_ROOT\/current"/);
  ...
  assert.match(workflow, /artifact_dir="\$RUNNER_TEMP\/atms-pr-closeout-/);
  assert.match(workflow, /ATMS_PR_CLOSEOUT_ARTIFACT_DIR=\$artifact_dir.*\$GITHUB_ENV/);
  assert.doesNotMatch(jobHeader, /\$\{\{ runner\.temp \}\}/);
  assert.doesNotMatch(workflow, /actions\/checkout|actions\/setup-node|npm (?:ci|run)|secrets\.ATMS/);
});
...
test("Auto Fix keeps model selection local and publishes only a human-gated Draft PR", () => {
  ...
  assert.match(workflow, /runs-on: \[self-hosted, Linux, X64, atms-auto-fix\]/);
  ...
  assert.doesNotMatch(workflow, /ATMS_AUTO_FIX_(?:IMPLEMENTATION|REVIEW|ARBITRATION)_MODEL/);
  ...
  assert.match(stableRunner, /ATMS_AUTO_FIX_TIMEOUT_SECONDS:-10800/);
