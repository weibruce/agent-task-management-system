...
initialize_production_tokens() {
  ...
  ATMS_NODE_TOKEN="$(load_or_create_production_token "$node_bin" "$secret_dir/node-registration.token" "Node registration")" || return 1
  ATMS_WORKER_TOKEN="$(load_or_create_production_token "$node_bin" "$secret_dir/worker-registration.token" "Worker registration")" || return 1
  ATMS_DAG_MUTATION_TOKEN="$(load_or_create_production_token "$node_bin" "$secret_dir/dag-mutation.token" "DAG mutation")" || return 1
  export ATMS_NODE_TOKEN ATMS_WORKER_TOKEN ATMS_DAG_MUTATION_TOKEN
}
