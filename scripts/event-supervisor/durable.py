...
def unit_text(record_path, record):
    return ('[Unit]\nDescription=Atms durable event supervisor\n[Service]\nType=exec\n'
...
def write_unit(path, content):
    fd, tmp = tempfile.mkstemp(prefix='.atms-', dir=path.parent)
    try:
...
def register(spec_path, home, unit_dir):
    ...
    if record_path.exists():
        ...
        record = {'version': 1, 'key': key, 'unit': 'atms-event-' + key + '.service',
...
def main():
    ...
    default_home = Path(os.environ.get('ATMS_HOME', str(Path.home() / '.atms'))) / 'event-supervision'
