...
export function selectAutoFixV2Setting(settings, selector, role) {
  ...
  if (!wanted) throw new Error(`ATMS_AUTO_FIX_V2_${role.toUpperCase()}_MODEL is required`);
...
async function request(managerUrl, pathname, init) {
  const response = await fetch(`${managerUrl}${pathname}`, {
    ...
    headers: {
      ...(init?.headers ?? {}),
      ...(init?.method && init.method !== "GET" && process.env.ATMS_DAG_MUTATION_TOKEN
...
export async function configureAutoFixV2RuntimeProfile({
  ...
  profileId = process.env.ATMS_AUTO_FIX_V2_PROFILE_ID ?? "auto-fix-v2-mixed",
  implementationSelector = process.env.ATMS_AUTO_FIX_V2_IMPLEMENTATION_MODEL,
  reviewSelector = process.env.ATMS_AUTO_FIX_V2_REVIEW_MODEL,
} = {}) {
