...
const port = Number(process.env.ATMS_MANAGER_PORT || "19391");
if (!Number.isSafeInteger(port) || port < 1 || port > 65_535) {
  throw new Error("ATMS_MANAGER_PORT must be a valid TCP port");
}
if (!process.env.ATMS_HOME) {
  throw new Error("ATMS_HOME must point to an isolated QA directory");
}
...
const [
  ...
  import("../atms_manager/dist/server/http.js"),
  import("../atms_manager/dist/server/browser-renderer-tools-websocket.js"),
  import("../atms_manager/dist/server/host-codex-manager-agent.js"),
  import("../atms_manager/dist/persistence/store.js"),
  import("../atms_manager/dist/persistence/db.js"),
]);
...
async function invokeManagerTool(command) {
  ...
  const tools = createManagerTools({
    ...
    workspace: process.env.ATMS_HOME,
