...
test("deploys an owner-dispatched revision on the isolated deploy runner", () => {
  ...
  assert.match(workflow, /runs-on: \[self-hosted, Linux, X64, atms-deploy\]/);
  assert.match(
    workflow,
    /ATMS_WORKER_BUILD_APT_MIRROR: \$\{\{ vars\.ATMS_WORKER_BUILD_APT_MIRROR \}\}/,
  );
  assert.match(
    workflow,
    /ATMS_WORKER_BUILD_APT_SECURITY_MIRROR: \$\{\{ vars\.ATMS_WORKER_BUILD_APT_SECURITY_MIRROR \}\}/,
  );
  assert.match(
    workflow,
    /ATMS_WORKER_BUILD_NPM_REGISTRY: \$\{\{ vars\.ATMS_WORKER_BUILD_NPM_REGISTRY \}\}/,
  );
...
test("production deployment is atomic, health checked, and rollback capable", () => {
  ...
  assert.match(deploy, /atms-worker:production-/);
  assert.match(deploy, /org\.atms\.worker\.source_fingerprint=\$WORKER_SOURCE_FINGERPRINT/);
  assert.match(deploy, /org\.atms\.worker\.protocol_version=\$WORKER_CONTRACT_VERSION/);
  assert.match(deploy, /org\.opencontainers\.image\.version=\$WORKER_VERSION/);
  assert.match(deploy, /ATMS_WORKER_SOURCE_FINGERPRINT=\$WORKER_SOURCE_FINGERPRINT/);
  assert.match(deploy, /ATMS_WORKER_PROTOCOL_VERSION=\$WORKER_CONTRACT_VERSION/);
  assert.match(deploy, /ATMS_WORKER_VERSION=\$WORKER_VERSION/);
  assert.match(deploy, /ATMS_WORKER_IMAGE_REVISION=\$REVISION/);
  assert.match(deploy, /ATMS_PRODUCTION_HEALTH_ATTEMPTS:-60/);
  assert.match(deploy, /ATMS_PRODUCTION_HEALTH_ATTEMPTS must be an integer from 1 through 300/);
  ...
  assert.match(
    deploy,
    /ExecStart=\/bin\/bash -c 'release="\\\$ATMS_PRODUCTION_ROOT\/current"; until \[ -x "\\\$release\/scripts\/run-production-service\.sh" \]; do sleep 10; done; cd "\\\$release"; exec "\\\$release\/scripts\/run-production-service\.sh"'/,
  );
  ...
  assert.match(deploy, /ATMS_PRODUCTION_MANAGER_HOST=\$MANAGER_HOST/);
  ...
  assert.match(deploy, /ATMS_PRODUCTION_MANAGER_PORT=\$MANAGER_PORT/);
  assert.match(deploy, /ATMS_PRODUCTION_ALLOW_INSECURE_REMOTE_WS:-0/);
  assert.match(deploy, /Environment=ATMS_ALLOW_INSECURE_REMOTE_WS=\$ALLOW_INSECURE_REMOTE_WS/);
  assert.match(deploy, /ATMS_PRODUCTION_DAG_COMMAND_ALLOWLIST:-node/);
  assert.match(deploy, /Environment=ATMS_PRODUCTION_DAG_COMMAND_ALLOWLIST=\$DAG_COMMAND_ALLOWLIST/);
  assert.match(deploy, /ATMS_PRODUCTION_UI_HOST=\$UI_HOST/);
  assert.match(deploy, /ATMS_PRODUCTION_UI_PORT=\$UI_PORT/);
  assert.match(deploy, /ATMS_PRODUCTION_UI_HTTP_PORT=\$UI_HTTP_PORT/);
  assert.match(deploy, /must be a LAN-accessible host or address/);
  assert.match(deploy, /ATMS_PRODUCTION_UI_PORT:-19192/);
  assert.match(deploy, /ATMS_HOME="\$\(realpath "\$ATMS_HOME"\)"/);
  assert.match(deploy, /ATMS_CODEX_BIN/);
  ...
  assert.match(service, /ATMS_PRODUCTION_UI_PORT:-19192/);
  ...
  assert.match(service, /ATMS_MANAGER_HOST="\$MANAGER_HOST"/);
  assert.match(service, /ATMS_PRODUCTION_MANAGER_PUBLIC_URL:-\$MANAGER_URL/);
  assert.match(service, /ATMS_PRODUCTION_ALLOW_INSECURE_REMOTE_WS/);
  assert.match(service, /export ATMS_ALLOW_INSECURE_REMOTE_WS="\$ALLOW_INSECURE_REMOTE_WS"/);
  assert.match(service, /ATMS_PRODUCTION_DAG_COMMAND_ALLOWLIST:-node/);
  assert.match(service, /export ATMS_DAG_COMMAND_ALLOWLIST="\$DAG_COMMAND_ALLOWLIST"/);
  ...
  assert.match(service, /export ATMS_NODE_TOKEN/);
  assert.match(service, /export ATMS_WORKER_TOKEN/);
  assert.match(service, /export ATMS_DAG_MUTATION_TOKEN/);
  assert.doesNotMatch(service, /export ATMS_CONTROL_PLANE_TOKEN/);
  assert.match(service, /Production UI must bind all interfaces/);
  assert.match(service, /ATMS_UI_SERVE_STATIC=1/);
  assert.match(service, /RELEASE_ROOT="\$\(readlink -f "\$CURRENT"\)"/);
  assert.match(service, /export PATH="\$RELEASE_ROOT\/runtime:\$PATH"/);
  assert.match(service, /ATMS_REPO_ROOT="\$RELEASE_ROOT"/);
  ...
  assert.doesNotMatch(service, /atms-worker:latest/);
});

test("production tokens are distinct, persistent, private, and fail closed", { skip: process.platform === "win32" }, () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "atms-production-tokens-"));
...
test("production deployment preserves database compatibility across success and rollback", { skip: process.platform === "win32" }, () => {
  ...
  const runDeployment = (healthExit, {
    ...
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "atms-production-deploy-"));
    ...
    const unitPath = path.join(fakeHome, ".config", "systemd", "user", "atms-production.service");
    const previousRelease = path.join(productionRoot, "releases", "previous");
    const databasePath = path.join(home, "manager", "atms.db");
    ...
    fs.mkdirSync(path.join(previousRelease, "atms_manager", "dist", "persistence"), { recursive: true });
    fs.writeFileSync(path.join(previousRelease, "atms_manager", "package.json"), '{"type":"module"}\n');
    fs.writeFileSync(
      path.join(previousRelease, "atms_manager", "dist", "persistence", "db.js"),
    ...
    write("atms_manager/dist/index.js", "// manager\n");
    write("atms_manager/package.json", '{"type":"module"}\n');
    write(
      "atms_manager/dist/server/dag-environment.js",
      `export function dagWorkerSourceFingerprint() { return "${workerFingerprint}"; }\n`,
    );
    write("atms_node/dist/cli.js", "// node\n");
    write("atms_cli/dist/cli.js", 'throw new Error("Deployment must not start DAG validation");\n');
    write("atms_protocol/package.json", '{"type":"module"}\n');
    write("atms_protocol/dist/index.js", 'export const WORKER_CONTRACT_VERSION = "1";\n');
    write("atms_worker/package.json", '{"version":"0.1.0"}\n');
    write("agent-ui/dist/index.html", "<!doctype html>\n");
    write("atms_worker/Dockerfile", "FROM scratch\n");
    ...
    write("atms_worker/scripts/configure-apt-sources.mjs", fs.readFileSync(path.join(repoRoot, "atms_worker/scripts/configure-apt-sources.mjs"), "utf8"), 0o755);
...
if [ "$2" = restart ] && [ ! -e "$CAPTURE_SYSTEMCTL.rollout-written" ] && [ "$(cat "$ATMS_PRODUCTION_ROOT/current/REVISION")" = "$ATMS_DEPLOY_REVISION" ]; then
  touch "$CAPTURE_SYSTEMCTL.rollout-written"
  printf 'rollout-write\n' >> "$ATMS_PRODUCTION_HOME/manager/atms.db"
  printf 'rollout-wal-write\n' >> "$ATMS_PRODUCTION_HOME/manager/atms.db-wal"
  printf 'rollout-shm\n' > "$ATMS_PRODUCTION_HOME/manager/atms.db-shm"
fi
exit 0
`);
...
`);
    ...
    const result = spawnSync("bash", [deployScript], {
      ...
      env: {
        ...
        ATMS_PRODUCTION_ROOT: productionRoot,
        ATMS_PRODUCTION_HOME: home,
        ATMS_PRODUCTION_RESOURCES: resources,
        ATMS_DEPLOY_REVISION: revision,
        ATMS_PRODUCTION_PUBLIC_HOST: "production.test",
        ATMS_PRODUCTION_ALLOW_INSECURE_REMOTE_WS: "1",
        ATMS_PRODUCTION_HEALTH_ATTEMPTS: "1",
        ATMS_CODEX_BIN: path.join(fakeBin, "codex"),
  ...
  try {
    ...
    assert.match(systemctlLog, /--user stop atms-production\.service/);
    assert.match(systemctlLog, /--user start atms-production\.service/);
    const lockEntries = fs.readdirSync(path.join(interrupted.productionRoot, "locks"));
    assert.equal(lockEntries.some((name) => name.startsWith("atms.db.pre-")), false);
  } finally {
  ...
  try {
    ...
    assert.match(dockerRemovals, new RegExp(`atms-worker:production-${passed.unreferencedRevision.slice(0, 12)}`));
    assert.doesNotMatch(dockerRemovals, new RegExp(`atms-worker:production-${previousRevision.slice(0, 12)}`));
    const dockerBuildArgs = fs.readFileSync(passed.dockerBuildArgsPath, "utf8");
    assert.match(dockerBuildArgs, new RegExp(`org\\.atms\\.worker\\.source_fingerprint=${workerFingerprint}`));
    assert.match(dockerBuildArgs, /org\.atms\.worker\.protocol_version=1/);
    ...
    assert.match(dockerBuildArgs, new RegExp(`ATMS_WORKER_SOURCE_FINGERPRINT=${workerFingerprint}`));
    assert.match(dockerBuildArgs, /ATMS_WORKER_PROTOCOL_VERSION=1/);
    assert.match(dockerBuildArgs, /ATMS_WORKER_VERSION=0\.1\.0/);
    assert.match(dockerBuildArgs, new RegExp(`ATMS_WORKER_IMAGE_REVISION=${revision}`));
    assert.doesNotMatch(dockerBuildArgs, /ATMS_WORKER_BUILD_APT/);
    assert.doesNotMatch(dockerBuildArgs, /NPM_CONFIG_REGISTRY/);
    assert.doesNotMatch(dockerBuildArgs, /ATMS_DSH_FORK_REPOSITORY/);
  ...
  const customSources = runDeployment(0, {
    extraEnv: {
      ATMS_WORKER_BUILD_APT_MIRROR: "https://deb.fn.example/debian/",
      ATMS_WORKER_BUILD_APT_SECURITY_MIRROR: "https://deb.fn.example/debian-security",
      ATMS_WORKER_BUILD_NPM_REGISTRY: "https://npm.fn.example",
      ATMS_WORKER_BUILD_DSH_GIT_REMOTE: "https://git.fn.example/deepseek-harness.git",
    },
  });
  try {
    ...
    assert.match(customBuildArgs, /--build-arg\nATMS_DSH_FORK_REPOSITORY=https:\/\/git\.fn\.example\/deepseek-harness\.git\n/);
    assert.match(customBuildArgs, new RegExp(`ATMS_WORKER_SOURCE_FINGERPRINT=${workerFingerprint}`));
  } finally {
  ...
  const invalidSource = runDeployment(0, {
    extraEnv: { ATMS_WORKER_BUILD_NPM_REGISTRY: "ftp://npm.fn.example" },
  });
  try {
    assert.notEqual(invalidSource.result.status, 0);
    assert.match(invalidSource.result.stderr, /ATMS_WORKER_BUILD_NPM_REGISTRY/);
...
test("tracked deployment configuration contains no machine-local identity", () => {
  const files = [
    ...
    "ops/systemd/atms-deploy-runner.service",
...
test("worker build network contract is shared by production and live entry points", () => {
  ...
  assert.match(deploy, /atms_worker_build_network_args/);
  assert.match(deploy, /ATMS_WORKER_BUILD_NETWORK_ARGS/);
  ...
  assert.match(helper, /ATMS_DSH_FORK_REPOSITORY=/);
...
test("worker build network helper validates sources and forwards proxy names only", { skip: process.platform === "win32" }, () => {
  ...
  const runHelper = (extraEnv = {}) => spawnSync("bash", [
    ...
    [
      "set -euo pipefail",
      'source "$1"',
      "atms_worker_build_network_args || exit 1",
      'if [ ${#ATMS_WORKER_BUILD_NETWORK_ARGS[@]} -gt 0 ]; then printf "%s\\n" "${ATMS_WORKER_BUILD_NETWORK_ARGS[@]}"; fi',
    ].join("\n"),
  ...
  const whitespaceOnly = runHelper({ ATMS_WORKER_BUILD_APT_MIRROR: " \t " });
  ...
  const custom = runHelper({
    ATMS_WORKER_BUILD_APT_MIRROR: "HTTPS://DEB.example.com:8443/debian/",
    ATMS_WORKER_BUILD_APT_SECURITY_MIRROR: "  https://deb.example.com/debian-security  ",
    ATMS_WORKER_BUILD_NPM_REGISTRY: "https://npm.example.com/",
    ATMS_WORKER_BUILD_DSH_GIT_REMOTE: "https://git.example.com/deepseek-harness.git/",
  ...
  assert.deepEqual(custom.stdout.trim().split("\n"), [
    ...
    "--build-arg",
    "ATMS_DSH_FORK_REPOSITORY=https://git.example.com/deepseek-harness.git",
    "--build-arg",
    "HTTP_PROXY",
    "--build-arg",
    "no_proxy",
  ...
  const argvCaptureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "atms-build-network-argv-"));
...
`,
  ...
  try {
    const shimmed = runHelper({
      NODE_BIN: nodeShim,
      ARGV_LOG: argvLog,
      ATMS_WORKER_BUILD_APT_MIRROR: "https://deb.example.com/debian/",
      ATMS_WORKER_BUILD_NPM_REGISTRY: "https://npm.example.com/",
      ATMS_WORKER_BUILD_DSH_GIT_REMOTE: "https://git.example.com/deepseek-harness.git/",
