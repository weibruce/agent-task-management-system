...
test("authenticates profile sync with the isolated DAG mutation token", async () => {
  const previousToken = process.env.ATMS_DAG_MUTATION_TOKEN;
  const previousAdminToken = process.env.ATMS_MANAGER_ADMIN_TOKEN;
  process.env.ATMS_DAG_MUTATION_TOKEN = "test-mutation-token";
  process.env.ATMS_MANAGER_ADMIN_TOKEN = "unused-admin-token";
  let syncHeaders;
  ...
  try {
    ...
    assert.equal(syncHeaders?.["x-atms-dag-token"], "test-mutation-token");
    assert.equal(syncHeaders?.authorization, undefined);
  } finally {
    await new Promise((resolve, reject) => server.close((error) => error ? reject(error) : resolve()));
    if (previousToken === undefined) delete process.env.ATMS_DAG_MUTATION_TOKEN;
    else process.env.ATMS_DAG_MUTATION_TOKEN = previousToken;
...
test("formal PR Review submits to the durable stable Manager", () => {
  ...
  assert.match(workflow, /atms-pr-review/);
  assert.match(
    workflow,
    /ATMS_PR_REVIEW_PROFILE_ID: \$\{\{ inputs\.profile_id \|\| \(inputs\.agent_type == 'deepseek_harness' && 'pr-review-dsh' \|\| 'pr-review-mixed'\) \}\}/,
  );
  assert.doesNotMatch(workflow, /inputs\.profile_id \|\| 'pr-review-mixed'/);
  assert.match(
    workflow,
    /ATMS_PR_REVIEW_AGENT_TYPE: \$\{\{ inputs\.agent_type \|\| 'claude-sdk' \}\}/,
  );
  assert.match(
    workflow,
    /ATMS_PR_REVIEW_REASONING_EFFORT: \$\{\{ inputs\.reasoning_effort \}\}/,
  );
  assert.doesNotMatch(workflow, /install:all|build:packages|run-pr-review-live-runner/);
  assert.doesNotMatch(workflow, /vars\.ATMS_PR_REVIEW_(?:HOME_TEMPLATE|PRIMARY_MODEL|ARBITER_MODEL|THIRD_MODEL)/);
});
