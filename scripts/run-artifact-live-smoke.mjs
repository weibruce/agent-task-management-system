...
async function runCli(args, env) {
  return await new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [path.join(repoRoot, "atms_cli", "dist", "cli.js"), ...args], {
      cwd: repoRoot,
      env: { ...process.env, ...env },
      stdio: ["ignore", "pipe", "pipe"],
    });
...
const home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-artifact-live-"));
let manager;
let node;
try {
  ...
  const managerEnv = {
    ...
    ATMS_MANAGER_PORT: String(port),
  };
  manager = startProcess(process.execPath, [path.join(repoRoot, "atms_manager", "dist", "index.js")], managerEnv);
  await waitFor("Manager health", async () => {
    const response = await fetch(`${baseUrl}/health`);
    return response.ok;
  });

  node = startProcess(process.execPath, [path.join(repoRoot, "atms_node", "dist", "cli.js")], {
    ATMS_HOME: home,
    ATMS_MANAGER_WS_URL: `ws://127.0.0.1:${port}`,
    ATMS_PROJECT_ID: "p1",
    ATMS_NODE_ID: "artifact-smoke-node",
    ATMS_NODE_PROVIDER: "mock",
  });
  ...
  if (process.env.ATMS_KEEP_ARTIFACT_SMOKE_HOME === "1") {
    console.error(`kept smoke ATMS_HOME at ${home}`);
  } else {
