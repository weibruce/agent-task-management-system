...
PRODUCTION_ROOT="${ATMS_PRODUCTION_ROOT:-$HOME/.local/share/atms-production}"
ATMS_HOME="${ATMS_PRODUCTION_HOME:-$HOME/.local/share/atms-production-data}"
RESOURCE_ROOT="${ATMS_PRODUCTION_RESOURCES:-$HOME/.local/share/atms-resources}"
REVISION="${ATMS_DEPLOY_REVISION:-}"
SERVICE_NAME="atms-production.service"
UNIT_PATH="$HOME/.config/systemd/user/$SERVICE_NAME"
MANAGER_PORT="${ATMS_PRODUCTION_MANAGER_PORT:-39191}"
DOCKER_BRIDGE_GATEWAY="$(docker network inspect bridge --format '{{(index .IPAM.Config 0).Gateway}}' 2>/dev/null || true)"
MANAGER_HOST="${ATMS_PRODUCTION_MANAGER_HOST:-$DOCKER_BRIDGE_GATEWAY}"
case "$MANAGER_HOST" in *:*) MANAGER_URL_HOST="[$MANAGER_HOST]" ;; *) MANAGER_URL_HOST="$MANAGER_HOST" ;; esac
MANAGER_URL="${ATMS_PRODUCTION_MANAGER_URL:-http://$MANAGER_URL_HOST:$MANAGER_PORT}"
ALLOW_INSECURE_REMOTE_WS="${ATMS_PRODUCTION_ALLOW_INSECURE_REMOTE_WS:-0}"
DAG_COMMAND_ALLOWLIST="${ATMS_PRODUCTION_DAG_COMMAND_ALLOWLIST:-node}"
UI_HOST="${ATMS_PRODUCTION_UI_HOST:-0.0.0.0}"
UI_PORT="${ATMS_PRODUCTION_UI_PORT:-19192}"
UI_HTTP_PORT="${ATMS_PRODUCTION_UI_HTTP_PORT:-19193}"
HEALTH_ATTEMPTS="${ATMS_PRODUCTION_HEALTH_ATTEMPTS:-60}"
PUBLIC_HOST="${ATMS_PRODUCTION_PUBLIC_HOST:-}"
UI_URL="${ATMS_PRODUCTION_UI_URL:-}"

case "$PRODUCTION_ROOT" in /*) ;; *) echo "ATMS_PRODUCTION_ROOT must be absolute." >&2; exit 1 ;; esac
case "$ATMS_HOME" in /*) ;; *) echo "ATMS_PRODUCTION_HOME must be absolute." >&2; exit 1 ;; esac
case "$RESOURCE_ROOT" in /*) ;; *) echo "ATMS_PRODUCTION_RESOURCES must be absolute." >&2; exit 1 ;; esac
...
case "$ALLOW_INSECURE_REMOTE_WS" in 0|1) ;; *) echo "ATMS_PRODUCTION_ALLOW_INSECURE_REMOTE_WS must be 0 or 1." >&2; exit 1 ;; esac
if [ "$DAG_COMMAND_ALLOWLIST" != "node" ]; then
  echo "ATMS_PRODUCTION_DAG_COMMAND_ALLOWLIST must be exactly node." >&2
  exit 1
fi
case "$MANAGER_HOST" in
  localhost|127.*|::1|\[::1\]|0.0.0.0|::|\[::\])
    echo "ATMS_PRODUCTION_MANAGER_HOST must be the Docker bridge gateway; loopback and wildcard binds are not supported." >&2
    exit 1
    ;;
esac
...
if [ "$UI_HOST" != "0.0.0.0" ] && [ "$UI_HOST" != "::" ]; then
  echo "ATMS_PRODUCTION_UI_HOST must bind all interfaces (0.0.0.0 or ::)." >&2
  exit 1
fi
case "$PUBLIC_HOST" in
  ""|localhost|127.*|::1|\[::1\])
    echo "ATMS_PRODUCTION_PUBLIC_HOST must be a LAN-accessible host or address." >&2
    exit 1
    ;;
esac
...
if [[ ! "$HEALTH_ATTEMPTS" =~ ^[0-9]+$ ]] || [ "$HEALTH_ATTEMPTS" -lt 1 ] || [ "$HEALTH_ATTEMPTS" -gt 300 ]; then
  echo "ATMS_PRODUCTION_HEALTH_ATTEMPTS must be an integer from 1 through 300." >&2
  exit 1
fi
if [ -z "$UI_URL" ]; then
  if [[ "$PUBLIC_HOST" == *:* ]]; then
...
case "$UI_URL" in
  https://localhost:*|https://localhost/*|https://127.*|https://\[::1\]*)
    echo "ATMS_PRODUCTION_UI_URL must use the LAN-facing HTTPS endpoint." >&2
    exit 1
    ;;
  https://*) ;;
  *)
    echo "ATMS_PRODUCTION_UI_URL must use HTTPS." >&2
    exit 1
    ;;
esac
if [[ ! "$REVISION" =~ ^[0-9a-f]{40}$ ]]; then
  echo "ATMS_DEPLOY_REVISION must be an exact 40-character commit SHA." >&2
  exit 1
fi
if [ ! -f "$SOURCE_ROOT/atms_manager/dist/index.js" ] \
  || [ ! -f "$SOURCE_ROOT/atms_node/dist/cli.js" ] \
  || [ ! -f "$SOURCE_ROOT/atms_cli/dist/cli.js" ] \
...
if [ -d "$SOURCE_ROOT/.git" ]; then
  ...
  if [ "$SOURCE_REVISION" != "$REVISION" ]; then
    echo "Checked-out revision does not match ATMS_DEPLOY_REVISION." >&2
    exit 1
  fi
fi
...
mkdir -p "$PRODUCTION_ROOT/releases" "$PRODUCTION_ROOT/locks" "$ATMS_HOME" "$(dirname "$UNIT_PATH")"
ATMS_HOME="$(realpath "$ATMS_HOME")"
...
if [ -n "$CODEX_BIN" ]; then
  if [ ! -f "$CODEX_BIN" ] || [ ! -x "$CODEX_BIN" ]; then
    echo "ATMS_CODEX_BIN is not an executable file: $CODEX_BIN" >&2
    exit 1
  fi
  CODEX_BIN="$(realpath "$CODEX_BIN")"
  if [[ ! "$CODEX_BIN" =~ ^/[A-Za-z0-9._/@+=:-]+$ ]]; then
    echo "ATMS_CODEX_BIN must resolve to a safe absolute path." >&2
    exit 1
  fi
  CURRENT_UID="$(id -u)"
  for trusted_path in "$CODEX_BIN" "$(dirname "$CODEX_BIN")"; do
    ...
    if [ "$trusted_owner" != "0" ] && [ "$trusted_owner" != "$CURRENT_UID" ]; then
      echo "ATMS_CODEX_BIN must be owned by the service user or root." >&2
      exit 1
    fi
    if (( (8#$trusted_mode & 8#022) != 0 )); then
      echo "ATMS_CODEX_BIN and its parent directory must not be group/world writable." >&2
      exit 1
    fi
  done
  CODEX_UNIT_ENV="Environment=ATMS_CODEX_BIN=$PRODUCTION_ROOT/current/runtime/codex"
fi
...
WORKER_IMAGE="atms-worker:production-$SHORT_REVISION"
...
docker build \
  --label "org.atms.production_revision=$REVISION" \
  --label "org.atms.worker.source_fingerprint=$WORKER_SOURCE_FINGERPRINT" \
  --label "org.atms.worker.protocol_version=$WORKER_CONTRACT_VERSION" \
  ...
  --build-arg "ATMS_WORKER_SOURCE_FINGERPRINT=$WORKER_SOURCE_FINGERPRINT" \
  --build-arg "ATMS_WORKER_PROTOCOL_VERSION=$WORKER_CONTRACT_VERSION" \
  --build-arg "ATMS_WORKER_VERSION=$WORKER_VERSION" \
  --build-arg "ATMS_WORKER_IMAGE_REVISION=$REVISION" \
  ${ATMS_WORKER_BUILD_NETWORK_ARGS[@]+"${ATMS_WORKER_BUILD_NETWORK_ARGS[@]}"} \
  -t "$WORKER_IMAGE" \
  -f "$SOURCE_ROOT/atms_worker/Dockerfile" \
  "$SOURCE_ROOT"
...
[Unit]
Description=Atms persistent production service
...
WorkingDirectory=%h
Environment=ATMS_PRODUCTION_ROOT=$PRODUCTION_ROOT
Environment=ATMS_HOME=$ATMS_HOME
Environment=ATMS_PRODUCTION_RESOURCES=$RESOURCE_ROOT
Environment=ATMS_PRODUCTION_MANAGER_URL=$MANAGER_URL
Environment=ATMS_PRODUCTION_MANAGER_HOST=$MANAGER_HOST
Environment=ATMS_PRODUCTION_MANAGER_PORT=$MANAGER_PORT
Environment=ATMS_PRODUCTION_MANAGER_PUBLIC_URL=${ATMS_PRODUCTION_MANAGER_PUBLIC_URL:-$MANAGER_URL}
Environment=ATMS_ALLOW_INSECURE_REMOTE_WS=$ALLOW_INSECURE_REMOTE_WS
Environment=ATMS_PRODUCTION_DAG_COMMAND_ALLOWLIST=$DAG_COMMAND_ALLOWLIST
Environment=ATMS_PRODUCTION_UI_URL=$UI_URL
Environment=ATMS_PRODUCTION_UI_HOST=$UI_HOST
Environment=ATMS_PRODUCTION_UI_PORT=$UI_PORT
Environment=ATMS_PRODUCTION_UI_HTTP_PORT=$UI_HTTP_PORT
Environment=ATMS_PRODUCTION_PUBLIC_HOST=$PUBLIC_HOST
...
DATABASE_PATH="$ATMS_HOME/manager/atms.db"
DATABASE_BACKUP="$PRODUCTION_ROOT/locks/atms.db.pre-$SHORT_REVISION-$$"
...
if [ "$DATABASE_EXISTED" = "0" ]; then
  ...
  && [ -f "$PREVIOUS_RELEASE/atms_manager/dist/persistence/db.js" ]; then
  mkdir -p "$PROBE_HOME/manager"
  cp -p "$DATABASE_BACKUP" "$PROBE_HOME/manager/atms.db"
  if [ -f "$DATABASE_WAL_BACKUP" ]; then
    cp -p "$DATABASE_WAL_BACKUP" "$PROBE_HOME/manager/atms.db-wal"
  fi
  if ATMS_HOME="$PROBE_HOME" "$PREVIOUS_RELEASE/runtime/node" --input-type=module -e '
    import { pathToFileURL } from "node:url";
    const databaseModule = await import(pathToFileURL(process.argv[1]).href);
    databaseModule.getDb();
    databaseModule.closeDb();
  ' "$PREVIOUS_RELEASE/atms_manager/dist/persistence/db.js" >/dev/null 2>&1; then
    PREVIOUS_DATABASE_COMPATIBLE=1
  fi
fi
rm -rf "$PROBE_HOME"
...
for old_release in "${OLD_RELEASES[@]}"; do
  ...
  if [[ "$old_revision" =~ ^[0-9a-f]{40}$ ]] \
    ...
    docker image rm "atms-worker:production-${old_revision:0:12}" >/dev/null 2>&1 || true
  fi
done

echo "Atms production deployed: $REVISION"
echo "Atms production URL: $UI_URL"
