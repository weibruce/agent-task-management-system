...
test("syncs the private profile after resolving three stable settings", async () => {
  ...
  const oldToken = process.env.ATMS_DAG_MUTATION_TOKEN;
  process.env.ATMS_DAG_MUTATION_TOKEN = "test-token";
  try {
    ...
    assert.equal(calls[1].init.headers["x-atms-dag-token"], "test-token");
    const body = JSON.parse(calls[1].init.body);
    assert.equal(body.workflow_id, "auto-fix");
    assert.match(body.yaml_text, /profile_id: "auto-fix-mixed"/);
  } finally {
    globalThis.fetch = originalFetch;
    if (oldToken === undefined) delete process.env.ATMS_DAG_MUTATION_TOKEN;
    else process.env.ATMS_DAG_MUTATION_TOKEN = oldToken;
  }
});
