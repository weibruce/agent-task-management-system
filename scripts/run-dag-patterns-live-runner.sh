...
LIVE_TASK="${ATMS_LIVE_TASK:-patterns}"
RUNNER_BASE="${ATMS_RUNNER_BASE:-$HOME/.atms-runners}"
HOME_ROOT="${ATMS_LIVE_HOME_BASE:-$RUNNER_BASE/atms_home}"
ARTIFACT_ROOT="${ATMS_LIVE_ARTIFACTS:-$RUNNER_BASE/artifacts}"
PREFERRED_MANAGER_PORT="${ATMS_MANAGER_PORT:-}"
MODEL_BASE_URL="${ATMS_PATTERN_MODEL_BASE_URL:-}"
MODEL_NAME="${ATMS_PATTERN_MODEL:-qwen3.6}"
MODEL_MAC="${ATMS_PATTERN_MODEL_MAC:-}"
WAKE_MODEL="${ATMS_WAKE_MODEL:-1}"
MODEL_PROTOCOL="${ATMS_PATTERN_MODEL_PROTOCOL:-anthropic_compatible}"
AGENT_TYPE="${ATMS_PATTERN_AGENT_TYPE:-claude-sdk}"
HOME_TEMPLATE="${ATMS_LIVE_HOME_TEMPLATE:-}"
SHOWCASE_ASSET="${ATMS_SHOWCASE_ASSET:-}"
SHOWCASE_PROMPT="${ATMS_SHOWCASE_PROMPT:-}"
SHOWCASE_REQUIRED_TERMS="${ATMS_SHOWCASE_REQUIRED_TERMS:-}"
RUN_KEY="${ATMS_LIVE_RUN_KEY:-${GITHUB_RUN_ID:-manual}-${GITHUB_RUN_ATTEMPT:-1}}"
RUN_KEY="$(printf '%s' "$RUN_KEY" | tr -c 'A-Za-z0-9_.-' '-')"
LIVE_SLOT_INPUT="${ATMS_LIVE_SLOT:-}"
LIVE_SLOT="$(printf '%s' "${LIVE_SLOT_INPUT:-legacy}" | tr -c 'A-Za-z0-9_.-' '-')"
LIVE_RUN_LABEL="org.atms.live_run"
if [ -n "$LIVE_SLOT_INPUT" ]; then
  ...
  LIVE_RUN_LABEL="org.atms.live_run_v2"
fi

case "$LIVE_SLOT" in
  ""|.|..)
    echo "ATMS_LIVE_SLOT must contain a safe runner slot name." >&2
    exit 1
    ;;
esac

export ATMS_LIVE_SLOT="$LIVE_SLOT"
...
case "$LIVE_TASK" in
  ...
  *)
    echo "Unsupported ATMS_LIVE_TASK: $LIVE_TASK" >&2
    exit 1
    ;;
esac

if [ -n "$HOME_TEMPLATE" ] && [ "$LIVE_TASK" != "pr-review" ]; then
  echo "ATMS_LIVE_HOME_TEMPLATE is currently supported only for pr-review." >&2
  exit 1
fi
if [ -z "$HOME_TEMPLATE" ] && [ -z "$MODEL_BASE_URL" ]; then
  echo "ATMS_PATTERN_MODEL_BASE_URL is required for isolated live DAG execution." >&2
  exit 1
fi
if [ -n "$HOME_TEMPLATE" ]; then
  if [[ "$HOME_TEMPLATE" != /* ]] || [ ! -f "$HOME_TEMPLATE/manager/atms.db" ] \
    || [ ! -f "$HOME_TEMPLATE/manager/secrets/master.key" ]; then
    echo "ATMS_LIVE_HOME_TEMPLATE must be an absolute Seed Home containing Manager DB and master key." >&2
    exit 1
  fi
  if [ -n "$(find "$HOME_TEMPLATE" -type l -print -quit 2>/dev/null)" ]; then
    echo "ATMS_LIVE_HOME_TEMPLATE must not contain symbolic links." >&2
    exit 1
  fi
fi
if [ "$LIVE_TASK" = "three-worker-showcase" ]; then
  if [ -z "$SHOWCASE_ASSET" ] || [ ! -f "$SHOWCASE_ASSET" ]; then
    echo "ATMS_SHOWCASE_ASSET must name an external Workflow file." >&2
    exit 1
  fi
  if [ -z "$SHOWCASE_PROMPT" ]; then
    echo "ATMS_SHOWCASE_PROMPT is required for three-Worker acceptance." >&2
    exit 1
  fi
  if [ -z "$SHOWCASE_REQUIRED_TERMS" ]; then
    echo "ATMS_SHOWCASE_REQUIRED_TERMS is required for semantic acceptance." >&2
    exit 1
  fi
fi

export ATMS_HOME="$HOME_BASE/run-$RUN_KEY"
unset ATMS_ASSET_DIR
export ATMS_PATTERN_MODEL="$MODEL_NAME"
export ATMS_PATTERN_MODEL_PROTOCOL="$MODEL_PROTOCOL"
export ATMS_PATTERN_AGENT_TYPE="$AGENT_TYPE"
export ATMS_WORKER_IMAGE="${ATMS_LIVE_WORKER_IMAGE_PREFIX:-atms-worker:dag-live}-$RUN_KEY"
export ATMS_DAG_COMMAND_ALLOWLIST="${ATMS_DAG_COMMAND_ALLOWLIST:-node,git}"
export ATMS_DAG_ALLOW_DYNAMIC_COMMANDS="${ATMS_DAG_ALLOW_DYNAMIC_COMMANDS:-true}"
if [ "$LIVE_TASK" = "three-worker-showcase" ]; then
  ...
  export ATMS_DAG_WORKER_IDLE_TTL_MS="${ATMS_SHOWCASE_WORKER_IDLE_TTL_MS:-15000}"
fi
if [ -z "${ATMS_DAG_APPROVAL_TOKEN:-}" ]; then
  ATMS_DAG_APPROVAL_TOKEN="$(python3 -c 'import secrets; print(secrets.token_hex(32))')"
  export ATMS_DAG_APPROVAL_TOKEN
fi
if [ -z "${ATMS_DAG_MUTATION_TOKEN:-}" ]; then
  ATMS_DAG_MUTATION_TOKEN="$(python3 -c 'import secrets; print(secrets.token_hex(32))')"
  export ATMS_DAG_MUTATION_TOKEN
fi
if [ -z "${ATMS_MANAGER_ADMIN_TOKEN:-}" ]; then
  ATMS_MANAGER_ADMIN_TOKEN="$(python3 -c 'import secrets; print(secrets.token_hex(32))')"
  export ATMS_MANAGER_ADMIN_TOKEN
fi
PERSISTENT_ARTIFACT_DIR="$ARTIFACT_BASE/$RUN_KEY"
if [ "$LIVE_TASK" = "three-worker-showcase" ]; then
  REPORT_PATH="$PERSISTENT_ARTIFACT_DIR/three-worker-showcase.json"
  UPLOAD_REPORT_PATH="${ATMS_LIVE_REPORT_PATH:-$REPO_ROOT/artifacts/three-worker-showcase/acceptance.json}"
  ...
  UPLOAD_REPORT_PATH="${ATMS_LIVE_REPORT_PATH:-$REPO_ROOT/artifacts/dag-patterns-live.json}"
fi
...
if ! flock -w 60 9; then
  echo "Another Atms live validation is already running on this runner." >&2
  exit 1
fi
ATMS_CLEANUP_LOCK_HELD=1 "$REPO_ROOT/scripts/cleanup-dag-patterns-live-runner.sh"
mkdir -p "$ATMS_HOME"
...
cleanup() {
  ...
  exec 8>&-
  if [ -x "$REPO_ROOT/atms_cli/dist/cli.js" ] || [ -f "$REPO_ROOT/atms_cli/dist/cli.js" ]; then
    node "$REPO_ROOT/atms_cli/dist/cli.js" runtime stop >/dev/null 2>&1
  fi
  ...
  if [ -d "$ATMS_HOME/logs" ]; then
    tar -czf "$PERSISTENT_ARTIFACT_DIR/atms-logs.tgz" -C "$ATMS_HOME" logs >/dev/null 2>&1
  fi
  if [ -f "$REPORT_PATH" ] && [ "$REPORT_PATH" != "$UPLOAD_REPORT_PATH" ]; then
    cp "$REPORT_PATH" "$UPLOAD_REPORT_PATH"
  fi
  case "$ATMS_HOME" in
    "$HOME_BASE"/run-*) rm -rf "$ATMS_HOME" ;;
    *) echo "Refusing to remove unexpected ATMS_HOME: $ATMS_HOME" >&2 ;;
  esac
  exit "$exit_code"
}
trap cleanup EXIT
...
if [ -n "$HOME_TEMPLATE" ]; then
  cp -a --reflink=auto "$HOME_TEMPLATE/." "$ATMS_HOME/"
  chmod 700 "$ATMS_HOME" "$ATMS_HOME/manager" "$ATMS_HOME/manager/secrets"
  chmod 600 "$ATMS_HOME/manager/atms.db" "$ATMS_HOME/manager/secrets/master.key"
fi
...
wake_model() {
  if [ -z "$MODEL_MAC" ]; then
    echo "Model host is offline and ATMS_PATTERN_MODEL_MAC is not configured." >&2
    return 1
  fi
...
addresses = os.environ.get("ATMS_PATTERN_MODEL_BROADCASTS", "255.255.255.255")
...
requested_live_patterns="${ATMS_LIVE_PATTERNS:-}"
requested_live_patterns=",${requested_live_patterns//[[:space:]]/},"
if [ "$LIVE_TASK" = "patterns" ] \
  && { [ -z "${ATMS_LIVE_PATTERNS:-}" ] || [[ "$requested_live_patterns" == *",issue-diagnosis,"* ]]; }; then
  if [ -z "${ATMS_LIVE_ISSUE_REVISION:-}" ]; then
    ATMS_LIVE_ISSUE_REVISION="$(
      HOME="$ATMS_HOME" \
      USERPROFILE="$ATMS_HOME" \
      XDG_CONFIG_HOME="$ATMS_HOME/.config" \
      GIT_ASKPASS='' \
      GIT_TERMINAL_PROMPT=0 \
      GCM_INTERACTIVE=Never \
      git -c credential.helper= -c http.extraHeader= ls-remote --exit-code \
        https://github.com/xiaotianfotos/atms.git refs/heads/main \
        | awk 'NR == 1 { print $1 }'
    )"
  fi
  if [[ ! "$ATMS_LIVE_ISSUE_REVISION" =~ ^([0-9a-f]{40}|[0-9a-f]{64})$ ]]; then
    echo "Could not resolve an exact commit for live issue diagnosis." >&2
    exit 1
  fi
  export ATMS_LIVE_ISSUE_REVISION
fi
...
echo "Building isolated worker image $ATMS_WORKER_IMAGE"
docker build \
  --label "$LIVE_RUN_LABEL=$RUN_KEY" \
  --label "org.atms.live_slot=$LIVE_SLOT" \
  ${ATMS_WORKER_BUILD_NETWORK_ARGS[@]+"${ATMS_WORKER_BUILD_NETWORK_ARGS[@]}"} \
  -t "$ATMS_WORKER_IMAGE" \
  -f "$REPO_ROOT/atms_worker/Dockerfile" \
  "$REPO_ROOT"
...
export ATMS_MANAGER_PORT="$MANAGER_PORT"
export ATMS_MANAGER_URL="http://127.0.0.1:$MANAGER_PORT"
...
node "$REPO_ROOT/atms_cli/dist/cli.js" start --host 0.0.0.0 --no-build-worker-image
flock -u 8
exec 8>&-
SETTING_ID=""
PROFILE_ID=""
if [ -n "$HOME_TEMPLATE" ]; then
  ...
  node "$REPO_ROOT/atms_cli/dist/cli.js" dag sync pr-review
...
if [ "$LIVE_TASK" = "three-worker-showcase" ]; then
  showcase_args=(
    ...
    --stall-timeout-ms "${ATMS_SHOWCASE_STALL_TIMEOUT_MS:-1200000}"
  ...
  # harness. Node/UI processes and the same isolated ATMS_HOME are retained;
  # at least one DAG Worker has already been physically released by idle TTL.
  node "$REPO_ROOT/atms_cli/dist/cli.js" --json runtime status \
    >"$SHOWCASE_RESTART_BEFORE_PATH"
  node "$REPO_ROOT/atms_cli/dist/cli.js" --json runtime restart \
...
if [ "$LIVE_TASK" = "pr-review" ]; then
  REVIEW_INPUT="${ATMS_PR_REVIEW_INPUT:-}"
  REVIEW_ARTIFACT_DIR="${ATMS_PR_REVIEW_ARTIFACT_DIR:-$REPO_ROOT/artifacts/pr-review}"
  REVIEW_TIMEOUT_SECONDS="${ATMS_PR_REVIEW_TIMEOUT_SECONDS:-3300}"
  if [ -z "$REVIEW_INPUT" ]; then
    echo "ATMS_PR_REVIEW_INPUT is required for the pr-review live task." >&2
    exit 1
  fi
  ...
  if ! node "$REPO_ROOT/atms_cli/dist/cli.js" --json "${review_args[@]}" \
  ...
  if [ "$REVIEW_STATUS" != "completed" ]; then
    node "$REPO_ROOT/atms_cli/dist/cli.js" dag quick "$REVIEW_RUN_ID" --events 50 \
      >"$REVIEW_ARTIFACT_DIR/dag-quick.txt" 2>&1 || true
    node "$REPO_ROOT/atms_cli/dist/cli.js" dag chats "$REVIEW_RUN_ID" --tools 20 --raw-tools \
      >"$REVIEW_ARTIFACT_DIR/dag-chats.txt" 2>&1 || true
    node "$REPO_ROOT/atms_cli/dist/cli.js" dag handoffs "$REVIEW_RUN_ID" --content-limit 2000 \
      >"$REVIEW_ARTIFACT_DIR/dag-handoffs.txt" 2>&1 || true
  fi
  node "$REPO_ROOT/atms_cli/dist/cli.js" dag artifact "$REVIEW_RUN_ID" pr-review.json \
  ...
  node "$REPO_ROOT/scripts/validate-pr-review-artifacts.mjs" \
    ...
    "$REVIEW_ARTIFACT_DIR/pr-review.json" \
    "$REVIEW_ARTIFACT_DIR/pr-review.md"
  echo "Atms PR Review artifacts: $REVIEW_ARTIFACT_DIR"
  exit 0
fi
...
validation_args=(
  ...
  --stall-timeout-ms "${ATMS_LIVE_PATTERN_STALL_TIMEOUT_MS:-1200000}"
  --output "$REPORT_PATH"
)
if [ -n "${ATMS_LIVE_PATTERNS:-}" ]; then
  IFS=',' read -ra requested_patterns <<<"$ATMS_LIVE_PATTERNS"
