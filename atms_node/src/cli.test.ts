...
describe("cli arg parsing logic", () => {
  ...
  it("derives default manager URL from ATMS_MANAGER_PORT", () => {
    const args = parseArgs([], { ATMS_MANAGER_PORT: "19222" });
  ...
  it("falls back to env vars", () => {
    const args = parseArgs([], {
      ATMS_MANAGER_WS_URL: "ws://env-host:5555",
      ATMS_PROJECT_ID: "env-proj",
      ATMS_NODE_ID: "env-node",
      ATMS_NODE_PROVIDER: "docker-api",
      ATMS_NODE_CAPABILITIES: "cap1,cap2",
      ATMS_NODE_TOKEN: " node-token ",
  ...
  it("args override env vars", () => {
    const args = parseArgs(
      ...
      { ATMS_PROJECT_ID: "env-proj", ATMS_NODE_PROVIDER: "docker-api" },
    );
