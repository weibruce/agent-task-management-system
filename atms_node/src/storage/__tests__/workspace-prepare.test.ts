...
describe("prepareWorkerWorkspace", () => {
  ...
  beforeEach(() => {
    atmsHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-node-workspace-"));
    process.env.ATMS_HOME = atmsHome;
  });

  afterEach(() => {
    delete process.env.ATMS_HOME;
    fs.rmSync(atmsHome, { recursive: true, force: true });
  });

  it("does nothing when no workspace spec is provided", async () => {
    const result = await prepareWorkerWorkspace("run-1/triage", undefined);
    const expectedRoot = normalizePath(path.join(atmsHome, "workspace", "run-1", "triage"));
  ...
  it("creates an empty workspace for isolated mode", async () => {
    ...
    const expectedRoot = normalizePath(path.join(atmsHome, "workspace", "run-1"));
  ...
  it("creates one run-scoped workspace for shared mode", async () => {
    ...
    const expectedRoot = normalizePath(path.join(atmsHome, "workspace", "run-shared"));
  ...
  it("copies an allowed local worktree into repo under the worker workspace root", async () => {
    const source = fs.mkdtempSync(path.join(os.tmpdir(), "atms-source-worktree-"));
    ...
    try {
      ...
      expect(result.repoPath).toBe(path.join(
        normalizePath(path.join(atmsHome, "workspace", "run-local-copy")),
        "repo",
      ));
  ...
  it("rejects local_copy source paths outside the allowed local roots", async () => {
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "atms-allowed-root-"));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), "atms-outside-root-"));
    try {
      await expect(
  ...
  it("does not reclone an existing git workspace", async () => {
    const root = normalizePath(path.join(atmsHome, "workspace", "run-1"));
