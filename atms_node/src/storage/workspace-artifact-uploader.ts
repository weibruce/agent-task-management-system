    ...
    try {
      const response = await fetch(new URL(spec.upload_url, base), {
        ...
        headers: {
          ...
          "X-Atms-Artifact-Sha256": archive.sha256,
          "X-Atms-Artifact-Uncompressed-Bytes": String(archive.uncompressed_bytes),
          "X-Atms-Artifact-File-Count": String(archive.file_count),
        },
