...
async function resolveLocalCopySource(
  ...
  const configuredRoots = [
    ...splitPathList(process.env.ATMS_ALLOWED_LOCAL_WORKSPACE_ROOTS),
...
async function prepareWorkerWorkspaceInternal(
  ...
  const root = atmsWorkerWorkspacePath(workspaceId);
...
export async function prepareWorkerWorkspace(
  ...
  const key = path.resolve(atmsWorkerWorkspacePath(workspaceId));
