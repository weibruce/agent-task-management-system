...
describe("handleLifecycleRequest", () => {
  ...
  it("container create validates caller supplied mounts", async () => {
    ...
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-node-mount-policy-"));
  ...
  it("container create accepts a caller supplied project mount when policy allows that root", async () => {
    const previousHome = process.env.ATMS_HOME;
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-node-mount-home-"));
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), "atms-node-project-"));
    process.env["ATMS_HOME"] = tempHome;
    const provider = new MockProvider();
    const responses: LifecycleResponse[] = [];
    const send = (msg: LifecycleResponse) => responses.push(msg);
  ...
  it("redacts the Manager credential from lifecycle error responses", async () => {
    ...
    await handleLifecycleRequest(
      makeRequest({
        ...
        spec: {
          image: "alpine:latest",
          env: { ATMS_MANAGER_ADMIN_TOKEN: token },
        },
      }),
      provider,
  ...
  it("rejects Plugin Runtime launch specs that leak Manager authority or omit isolation", async () => {
    ...
    await handleLifecycleRequest(
      makeRequest({
        ...
        spec: {
          ...
          labels: { "atms.resource_type": "plugin-runtime" },
          env: { ATMS_PLUGIN_CAPABILITY_SECRET: capabilitySecret },
        },
      }),
      provider,
      (response) => responses.push(response),
    ...
    expect(responses[0]?.error?.message).toContain("ATMS_PLUGIN_CAPABILITY_SECRET");
  ...
  describe("worker resource_type", () => {
    it("create worker -> returns container info with default image", async () => {
      ...
      expect(container!.config.labels?.["atms.codex_nested_sandbox"]).toBe("false");
      expect(container!.config.mounts).toHaveLength(1);
      expect(container!.config.mounts![0]!.host).toBe("/home/user/.atms/workspace/ws-1");
    });
    ...
    it("maps the trusted Codex nested-sandbox flag to a fixed Docker profile", async () => {
      ...
      await handleLifecycleRequest(
        makeRequest({
          ...
          spec: {
            ...
            labels: { "atms.codex_nested_sandbox": "spoofed" },
          },
        }),
        provider,
      ...
      expect(container!.config.labels?.["atms.codex_nested_sandbox"]).toBe("true");
    ...
    it("create worker overlays declared Git metadata read-only", async () => {
      const previousHome = process.env.ATMS_HOME;
      const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-node-git-mount-"));
      process.env.ATMS_HOME = tempHome;
    ...
    it("rejects a symbolic-link Git metadata mount source", async () => {
      const previousHome = process.env.ATMS_HOME;
      const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-node-git-symlink-"));
      process.env.ATMS_HOME = tempHome;
    ...
    it("create worker accepts isolated workspace mode", async () => {
      const previousHome = process.env.ATMS_HOME;
      const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-node-isolated-"));
      process.env["ATMS_HOME"] = tempHome;
      const provider = new MockProvider();
      const responses: LifecycleResponse[] = [];
      const send = (msg: LifecycleResponse) => responses.push(msg);
    ...
    it("create worker accepts shared workspace mode", async () => {
      const previousHome = process.env.ATMS_HOME;
      const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-node-shared-"));
      process.env["ATMS_HOME"] = tempHome;
      const provider = new MockProvider();
      const responses: LifecycleResponse[] = [];
    ...
    it("start worker -> success", async () => {
      process.env["ATMS_HOME"] = "/home/user/.atms";
      const provider = new MockProvider();
      const created = await provider.create({ image: "atms-worker:latest" });
      const responses: LifecycleResponse[] = [];
      const send = (msg: LifecycleResponse) => responses.push(msg);
