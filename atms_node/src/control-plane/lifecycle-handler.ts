...
function redactLifecycleSecrets(message: string, spec: Record<string, unknown>): string {
  const env = spec.env && typeof spec.env === "object" && !Array.isArray(spec.env)
    ...
    : {};
  const secrets = [env.ATMS_MANAGER_ADMIN_TOKEN, env.ATMS_PLUGIN_CAPABILITY_SECRET]
    ...
    case "create": {
      if (resource_type === "worker") {
        ...
        const config: ContainerConfig = {
          image: (spec.image as string) || "atms-worker:latest",
          env: (spec.env as Record<string, string>) || undefined,
          labels: {
            ...(spec.labels as Record<string, string> || {}),
            "atms.resource_type": "worker",
            "atms.codex_nested_sandbox": String(codexNestedSandbox),
          },
          network: typeof spec.network === "string" && spec.network.length > 0 ? spec.network : undefined,
        ...
        if (spec.workspace_read_only === true && preparedWorkspace.prepared) {
          await mkdir(join(preparedWorkspace.root, ".atms-runtime"), { recursive: true, mode: 0o700 });
        }
...
function assertPluginRuntimeIsolation(config: ContainerConfig): void {
  if (config.labels?.["atms.resource_type"] !== "plugin-runtime") return;
  const env = config.env ?? {};
  for (const secret of ["ATMS_MANAGER_ADMIN_TOKEN", "ATMS_PLUGIN_CAPABILITY_SECRET"]) {
  ...
  if (!config.network || (config.network !== "none" && !/^atms-plugin-broker-[a-z0-9_.-]+$/.test(config.network))) {
