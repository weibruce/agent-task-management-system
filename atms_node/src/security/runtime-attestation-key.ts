...
export function nodeRuntimeAttestationKeyPath(
  ...
  const explicit = env.ATMS_NODE_ATTESTATION_KEY_FILE?.trim();
  if (explicit) return path.resolve(explicit);
  const home = env.ATMS_HOME?.trim() || path.join(os.homedir(), ".atms");
...
export class NodeRuntimeAttestationAuthority {
  ...
  issue(claims: AtmsPluginRuntimeSandboxAttestationClaimsV1, now: Date = new Date()): AtmsPluginRuntimeSandboxAttestationV1 {
    ...
    const validation = validateAtmsPluginRuntimeSandboxAttestation(value, { now_ms: now.getTime(), clock_skew_ms: 0 });
