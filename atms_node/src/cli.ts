...
type CliEnv = Partial<Record<
  ...
  | "ATMS_NODE_ID"
  | "ATMS_NODE_PROVIDER"
  | "ATMS_NODE_CAPABILITIES"
  | "ATMS_NODE_TOKEN"
  | "ATMS_CONTROL_PLANE_TOKEN"
...
function defaultManagerWsUrl(env: CliEnv): string {
  const port = env.ATMS_MANAGER_PORT?.trim() || DEFAULT_MANAGER_PORT;
...
export function parseArgs(argv: string[], env: CliEnv = process.env): CliArgs {
  let managerUrl = env.ATMS_MANAGER_WS_URL ?? defaultManagerWsUrl(env);
  let projectId = env.ATMS_PROJECT_ID ?? "";
  let nodeId = env.ATMS_NODE_ID ?? "";
  let provider = env.ATMS_NODE_PROVIDER ?? "docker-cli";
  const token = (env.ATMS_NODE_TOKEN ?? env.ATMS_CONTROL_PLANE_TOKEN ?? "").trim();
  const allowInsecureRemoteWs = env.ATMS_ALLOW_INSECURE_REMOTE_WS === "1";
  const caps: string[] = [];

  const envCaps = env.ATMS_NODE_CAPABILITIES;
  if (envCaps) {
...
export function resolvePluginRuntimeService(
  ...
  const packageRoots = (env.ATMS_PLUGIN_RUNTIME_PACKAGE_ROOTS ?? "")
    .split(path.delimiter)
    .map((value) => value.trim())
    .filter(Boolean);
  let imageAllowlist: Record<string, string>;
  try {
    const raw = JSON.parse(env.ATMS_PLUGIN_RUNTIME_IMAGE_ALLOWLIST ?? "{}") as unknown;
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) throw new Error("not an object");
    imageAllowlist = raw as Record<string, string>;
  } catch {
    throw new Error("ATMS_PLUGIN_RUNTIME_IMAGE_ALLOWLIST must be a JSON object of image to sha256 digest");
  }
  const seccomp = env.ATMS_PLUGIN_RUNTIME_SECCOMP_PROFILE?.trim();
  if (!seccomp) throw new Error("ATMS_PLUGIN_RUNTIME_SECCOMP_PROFILE is required for plugin-runtime capability");
  const home = path.resolve(env.ATMS_HOME?.trim() || path.join(os.homedir(), ".atms"));
  const authority = new NodeRuntimeAttestationAuthority({
    node_id: args.nodeId,
    ...(env.ATMS_NODE_ATTESTATION_KEY_FILE?.trim()
      ? { key_file: env.ATMS_NODE_ATTESTATION_KEY_FILE.trim() }
      : { env }),
  });
  ...
  return new PluginRuntimeService({
    ...
    allowed_devices: list("ATMS_PLUGIN_RUNTIME_ALLOWED_DEVICES"),
    allowed_gpus: list("ATMS_PLUGIN_RUNTIME_ALLOWED_GPUS"),
    ...(env.ATMS_PLUGIN_RUNTIME_RUNNER?.trim() ? { runtime_runner: env.ATMS_PLUGIN_RUNTIME_RUNNER.trim() } : {}),
  });
}
...
async function main(): Promise<void> {
  ...
  if (!args.managerUrl.trim()) {
    console.error("Error: --manager-url is required (or set ATMS_MANAGER_WS_URL)");
    process.exit(1);
  }
  if (!args.projectId) {
    console.error("Error: --project-id is required (or set ATMS_PROJECT_ID)");
    process.exit(1);
  }
  if (!args.nodeId) {
    console.error("Error: --node-id is required (or set ATMS_NODE_ID)");
    process.exit(1);
  }
  ...
  console.log(`ATMS_NODE_READY node_id=${args.nodeId}`);
