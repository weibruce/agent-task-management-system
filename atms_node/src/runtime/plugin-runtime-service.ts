...
import {
  ATMS_RUNTIME_BLOCKED_SECRET_ENV,
  AtmsPluginPermission,
  atmsPluginRuntimeContainerMeasurementDigestInput,
  stableStringify,
  validateAtmsPluginRuntimeRpcRequest,
  validateAtmsPluginRuntimeRpcResponse,
  type AtmsPluginEffectivePermissionGrantV1,
  type AtmsPluginRuntimeContainerMeasurementV1,
  type AtmsPluginRuntimeArtifactDeclarationV1,
  type AtmsPluginRuntimeArtifactUploadV1,
  type AtmsPluginRuntimeRpcPrepareResultV1,
  type AtmsPluginRuntimeRpcRequestV1,
  type AtmsPluginRuntimeRpcResponseV1,
  type AtmsPluginRuntimeSandboxAttestationClaimsV1,
  type AtmsPluginRuntimeSandboxAttestationV1,
...
export interface PluginRuntimeLaunchResultV1 {
  ...
  image_digest: string;
  attestation: AtmsPluginRuntimeSandboxAttestationV1;
}
...
interface RuntimeLedgerRecordV1 {
  ...
  updated_at: string;
  response?: AtmsPluginRuntimeRpcResponseV1;
...
interface RuntimePreparationRecordV1 {
  ...
  request_digest: string;
  updated_at: string;
  response: AtmsPluginRuntimeRpcPrepareResultV1;
}
...
export class PluginRuntimeService {
  ...
  constructor(options: PluginRuntimeServiceOptions) {
    ...
    this.#runner = options.runtime_runner ?? "/usr/local/bin/atms-plugin-runtime";
  ...
  async launch(raw: unknown): Promise<PluginRuntimeLaunchResultV1> {
    ...
    if (gpuDevices.length && !permissions.has(AtmsPluginPermission.GPU_USE)) {
      throw new Error("Plugin Runtime GPU request exceeds effective grants");
    }
    if (devices.length && ![
      AtmsPluginPermission.DEVICE_CONTROL,
      AtmsPluginPermission.CAMERA_READ,
      AtmsPluginPermission.MICROPHONE_READ,
    ...
    for (const grant of spec.effective_grants) {
      if (!ATMS_RUNTIME_BROKERED_PERMISSIONS.has(grant.permission)) continue;
    ...
    const networkGrant = spec.effective_grants.find((grant) => grant.permission === AtmsPluginPermission.NETWORK_CONNECT);
    const networkHosts = [...(networkGrant?.hosts ?? [])];
    const networkSession = brokerSessions.find((session) => session.permission === AtmsPluginPermission.NETWORK_CONNECT);
    ...
    const config: ContainerConfig = {
      ...
      name: `atms-plugin-runtime-${sha256(spec.runtime_instance_id).slice(0, 20)}`,
      user: "65532:65532",
      readOnlyRootfs: true,
      ...
      gpus: gpuDevices,
      env: {
        ATMS_RUNTIME_INSTANCE_ID: spec.runtime_instance_id,
        ATMS_PLUGIN_ID: spec.binding.plugin_id,
        ATMS_PLUGIN_VERSION: spec.binding.plugin_version,
      },
      labels: {
        "atms.resource_type": "plugin-runtime",
        "atms.node_id": this.#nodeId,
        "atms.runtime_instance_id": spec.runtime_instance_id,
        "atms.plugin_id": spec.binding.plugin_id,
        "atms.plugin_version": spec.binding.plugin_version,
        "atms.package_digest": spec.binding.package_digest,
      },
    };
    ...
    try {
      ...
      const measurementDigest = sha256(atmsPluginRuntimeContainerMeasurementDigestInput(measurement));
      const now = this.#now();
      const identity = this.#authority.publicIdentity();
      const claims: AtmsPluginRuntimeSandboxAttestationClaimsV1 = {
        sandbox_attestation_version: 1,
        issuer: "atms-node",
        audience: "atms-manager",
  ...
  async rpc(runtimeInstanceId: string, raw: unknown): Promise<unknown> {
    ...
    const validation = validateAtmsPluginRuntimeRpcRequest(raw, {
      expected: {
  ...
  async #prepare(
    ...
    request: Extract<AtmsPluginRuntimeRpcRequestV1, { method: "prepare" }>,
  ): Promise<unknown> {
    ...
    const validation = validateAtmsPluginRuntimeRpcResponse(raw, {
      expected: {
        binding: record.binding,
    ...
    const hasArtifactGrant = record.effective_grants.some((grant) => (
      grant.permission === AtmsPluginPermission.ARTIFACT_WRITE
    ));
  ...
  async #execute(record: RuntimeRecord, request: Extract<AtmsPluginRuntimeRpcRequestV1, { method: "execute" }>): Promise<unknown> {
    if (!deepEqual(request.params.authorization.invocation.binding, record.binding)
      || !deepEqual(request.params.authorization.invocation.policy.effective_grants, record.effective_grants)) {
    ...
    const result = validateAtmsPluginRuntimeRpcResponse(response, {
      expected: {
        binding: record.binding,
        source: invocation.source,
        tool: invocation.tool,
  ...
  #reconcile(record: RuntimeRecord, request: Extract<AtmsPluginRuntimeRpcRequestV1, { method: "reconcile" }>): unknown {
  ...
  async #invokeRuntime(record: RuntimeRecord, request: AtmsPluginRuntimeRpcRequestV1, validate = true): Promise<unknown> {
    ...
    const result = validateAtmsPluginRuntimeRpcResponse(raw, { now_ms: this.#now().getTime(), expected: { binding: record.binding } });
  ...
  async #publishBrokerWrites(
    uploads: AtmsPluginRuntimeArtifactUploadV1[],
    ...
    for (let index = 0; index < uploads.length; index += 1) {
      ...
      const result = await fetch(target, {
        ...
        redirect: "error",
        headers: {
          Authorization: `AtmsArtifact ${upload.token}`,
  ...
  async #measure(
  ...
  ): Promise<AtmsPluginRuntimeContainerMeasurementV1> {
    ...
    const requiredEnv = ["ATMS_PLUGIN_ID", "ATMS_PLUGIN_VERSION", "ATMS_RUNTIME_INSTANCE_ID"];
    ...
    if (!requiredEnv.every((name) => actual.envNames.includes(name))
      || actual.envNames.some((name) => !permittedEnv.has(name)
        || /(?:TOKEN|SECRET|PASSWORD|CREDENTIAL|PRIVATE_KEY|API_KEY)/.test(name))
      || ATMS_RUNTIME_BLOCKED_SECRET_ENV.some((name) => actual.envNames.includes(name))) {
    ...
    return {
      ...
      isolation: {
        profile_id: "atms.plugin-runtime.v1",
        uid: 65532,
        gid: 65532,
        ...
        blocked_secret_env: [...ATMS_RUNTIME_BLOCKED_SECRET_ENV],
      },
    };
  }
  ...
  async #assertMeasurementStillCurrent(record: RuntimeRecord): Promise<void> {
    ...
    const measurement: AtmsPluginRuntimeContainerMeasurementV1 = {
      measurement_version: 1,
    ...
    if (
      sha256(atmsPluginRuntimeContainerMeasurementDigestInput(measurement)) !== claims.measurement_digest
  ...
  #assertPreparedUploads(
    ...
    uploads: AtmsPluginRuntimeArtifactUploadV1[] | undefined,
  ): void {
    const hasArtifactGrant = record.effective_grants.some((grant) => (
      grant.permission === AtmsPluginPermission.ARTIFACT_WRITE
    ));
    ...
    for (let index = 0; index < declarations.length; index += 1) {
      ...
      const upload = uploads![index]!;
      const expected: AtmsPluginRuntimeArtifactDeclarationV1 = {
        id: upload.id,
        label: upload.label,
  ...
  #verifyPackage(spec: PluginRuntimeLaunchSpecV1): string {
    ...
    const lockPath = path.join(packagePath, "atms.lock.json");
    ...
    const actualPaths = listPackageFiles(packagePath).filter((file) => !["atms.lock.json", "atms.signature.json"].includes(file));
...
function sanitizedRunnerRequest(request: AtmsPluginRuntimeRpcRequestV1): unknown {
...
function rpcError(
  request: Extract<AtmsPluginRuntimeRpcRequestV1, { method: "prepare" | "execute" | "reconcile" }>,
  code: string,
  message: string,
): unknown {
...
function correlatedResponse(response: AtmsPluginRuntimeRpcResponseV1, rpcId: string): AtmsPluginRuntimeRpcResponseV1 {
