...
describe("lifecycle (with MockProvider)", () => {
  ...
  describe("createContainer", () => {
    ...
    it("rejects mounts outside .atms tree", async () => {
      process.env["ATMS_HOME"] = "/home/user/.atms";
      await expect(
        createContainer({
          config: {
      ...
      ).rejects.toThrow(/outside .atms tree/);
    });
  });
  ...
  describe("createWorkerContainer", () => {
    it("defaults image to atms-worker:latest", async () => {
      process.env["ATMS_HOME"] = "/home/user/.atms";
      ...
      expect(container!.config.image).toBe("atms-worker:latest");
    });
    ...
    it("adds exactly one workspace mount under .atms/workspace", async () => {
      process.env["ATMS_HOME"] = "/home/user/.atms";
      const info = await createWorkerContainer({
        config: { image: "atms-worker:latest" },
        provider,
        workspaceId: "ws-003",
      });
      const container = provider.containers.get(info.id);
      ...
      expect(mount.host).toBe("/home/user/.atms/workspace/ws-003");
    ...
    it("supports run/node workspace IDs", async () => {
      ...
      const container = provider.containers.get(info.id);
      expect(container!.config.mounts![0]!.host).toBe("/home/user/.atms/workspace/run-1/node-1");
    });
