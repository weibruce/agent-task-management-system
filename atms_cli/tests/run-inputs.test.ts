...
describe("CLI immutable run inputs", () => {
  ...
  it("stages local text files before returning scoped run bindings", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-cli-inputs-"));
    roots.push(root);
    ...
    const client = { post } as unknown as AtmsClient;
  ...
  it("requires an explicit scope and safe unique mount paths", async () => {
    const client = { post: vi.fn() } as unknown as AtmsClient;
