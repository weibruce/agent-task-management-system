...
function githubFetch(options: {
  ...
  return vi.fn(async (url: string | URL | Request) => {
    ...
    if (pathname === "/repos/xiaotianfotos/atms") return response({ default_branch: "main" });
    if (pathname === "/repos/xiaotianfotos/atms/pulls/26" && !search) {
      return response({
        draft: options.draft,
...
function managerClient(): AtmsClient {
  return {
...
describe("PR closeout input", () => {
  ...
  it("requires successful remote gates and a zero-finding Atms review before human merge approval", async () => {
    const result = await resolvePrCloseoutInput({
      repo: "xiaotianfotos/atms",
      pr: 26,
      phase: "merge",
