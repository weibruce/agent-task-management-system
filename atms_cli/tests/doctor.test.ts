...
describe("doctor readiness helpers", () => {
    ...
    expect(checks).toEqual([
      {
        name: "docker-cli",
        ok: false,
        detail: "command not found; install Docker Desktop, add docker.exe to PATH, or set ATMS_DOCKER_BIN",
      },
  ...
  it("reports worker image presence when Docker is ready", () => {
    ...
    expect(checks).toEqual([
      ...
      { name: "worker-image", ok: true, detail: "atms-worker:latest present" },
    ]);
  });
});
