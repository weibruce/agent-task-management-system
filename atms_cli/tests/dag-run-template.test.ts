...
describe("PR Review run-template", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "atms-pr-review-cli-"));
    oldHome = process.env.ATMS_HOME;
    oldAssetDir = process.env.ATMS_ASSET_DIR;
    process.env.ATMS_HOME = tmpDir;
  ...
  it("resolves immutable PR metadata from GitHub and wraps a manual envelope", async () => {
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      "https://api.github.com/repos/xiaotianfotos/atms/pulls/25",
      expect.objectContaining({ headers: expect.objectContaining({ "User-Agent": "Atms-PR-Review" }) }),
    );
    ...
    const input = await resolvePrReviewInput(
      {
        ...
        base: "c".repeat(40),
        head: "d".repeat(40),
        base_clone_url: "https://attacker.example/enterprise/atms.git",
      },
      { fetchImpl: fetchImpl as typeof fetch, env: { ATMS_GITHUB_API_BASE_URL: "https://github.example/api/v3/" } },
    );

    expect(fetchImpl).toHaveBeenCalledWith(
      "https://github.example/api/v3/repos/enterprise/atms/pulls/8",
      expect.any(Object),
    );
    expect(input).toMatchObject({
      base: "c".repeat(40),
  ...
  it("rejects credential-bearing clone URLs returned by GitHub metadata", async () => {
    ...
    base.repo.clone_url = "https://token@github.com/xiaotianfotos/atms.git";
  ...
  it("syncs the tracked template and starts a run from structured input", async () => {
    ...
    fs.writeFileSync(path.join(assetRoot, "orchestrations", "pr-review.yaml.template"), "api_version: atms.ai/v1\n");
    const calls: Array<{ url: string; init?: RequestInit }> = [];
    vi.spyOn(globalThis, "fetch").mockImplementation(async (url, init) => {
      calls.push({ url: String(url), init });
      if (String(url) === "https://api.github.com/repos/xiaotianfotos/atms/pulls/25") {
    ...
    await createProgram().parseAsync([
      "node", "atms", "--json", "dag", "run-template", "pr-review",
  ...
  it("waits across transient Manager reconnects without materializing scenario-specific files", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (url) => {
      ...
      if (target === "https://api.github.com/repos/xiaotianfotos/atms/pulls/25") {
        return response(pullMetadata());
      }
  ...
  it("isolates template discovery by ATMS_HOME", () => {
    ...
    process.env.ATMS_HOME = firstHome;
    const first = resolveTemplatePath(orchestrationsDir(), "pr-review");
    process.env.ATMS_HOME = secondHome;
