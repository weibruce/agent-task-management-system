...
const ENV_KEYS = [
  ...
  "ATMS_TEST_MANAGER_SHUTDOWN_DELAY_MS",
] as const;
...
beforeEach(async () => {
  ...
  tempHome = mkdtempSync(join(tmpdir(), "atms-runtime-restart-test-"));
  repoRoot = join(tempHome, "repo");
  managerPort = await availablePort();
  children = [];
  process.env.ATMS_HOME = tempHome;
  process.env.ATMS_REPO_ROOT = repoRoot;

  mkdirSync(join(repoRoot, "atms_manager", "dist"), { recursive: true });
  mkdirSync(join(repoRoot, "atms_node", "dist"), { recursive: true });
  writeFileSync(join(repoRoot, "atms_manager", "dist", "index.js"), dummyManagerSource());
  writeFileSync(join(repoRoot, "atms_node", "dist", "cli.js"), "setInterval(() => {}, 1000);\n");
...
describe("runtime restart --manager-only", () => {
  ...
  it("waits past the Manager five-second forced-shutdown boundary", async () => {
    process.env.ATMS_TEST_MANAGER_SHUTDOWN_DELAY_MS = "5200";
    const oldManagerPid = await startDummyManager();
...
async function startDummyManager(): Promise<number> {
  const child = spawn(process.execPath, [join(repoRoot, "atms_manager", "dist", "index.js")], {
    detached: true,
    env: {
      ...process.env,
      ATMS_HOME: tempHome,
      ATMS_MANAGER_HOST: "127.0.0.1",
...
const home = process.env.ATMS_HOME;
const host = process.env.ATMS_MANAGER_HOST || "127.0.0.1";
const port = Number(process.env.ATMS_MANAGER_PORT);
fs.appendFileSync(
  ...
  JSON.stringify({
    ...
    host,
    port,
    origins: process.env.ATMS_MANAGER_ADMIN_ORIGINS,
  }) + "\\n",
);
...
const shutdownDelayMs = Number(process.env.ATMS_TEST_MANAGER_SHUTDOWN_DELAY_MS || 0);
