...
import { scaffoldPluginProject } from "atms-plugin-sdk";
...
beforeEach(() => {
  ...
  temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), "atms-plugin-cli-"));
  previousHome = process.env.ATMS_HOME;
  previousManagerUrl = process.env.ATMS_MANAGER_URL;
  process.env.ATMS_HOME = path.join(temporaryRoot, "home");
...
afterEach(() => {
  ...
  if (previousManagerUrl === undefined) delete process.env.ATMS_MANAGER_URL;
  else process.env.ATMS_MANAGER_URL = previousManagerUrl;
...
describe("plugin PDK workflow", () => {
  ...
  it("runs empty directory -> scaffold -> codegen -> validate -> dev/test matrix -> pack -> verify", async () => {
    ...
    expect(fs.existsSync(path.join(root, "atms.plugin.json"))).toBe(true);

    await runJson("codegen", root);
    const codegen = readLastJson(log) as Record<string, unknown>;
    expect(codegen.changed).toBe(true);
    expect(fs.existsSync(path.join(root, ".atms", "generated", "plugin-types.d.ts"))).toBe(true);
  ...
  it("serves a live no-store development report", async () => {
    ...
    try {
      ...
      expect(page).toContain("Atms Plugin Development Browser");
...
describe("plugin Manager lifecycle commands", () => {
  ...
  it("uploads the verified .hrp as raw binary with the selected channel", async () => {
    ...
    await createProgram().parseAsync([
      "node", "atms", "--json", "plugin", "install", archive, "--staging",
    ]);
  ...
  it("sets a failing exit code for locally invalid input and Manager rejections", async () => {
    ...
    await createProgram().parseAsync([
      "node", "atms", "plugin", "verify", path.join(temporaryRoot, "missing.hrp"),
    ]);
    ...
    await createProgram().parseAsync([
      "node", "atms", "plugin", "enable", "com.example.rejected",
    ]);
    expect(process.exitCode).toBe(1);
...
async function runJson(...args: string[]): Promise<void> {
  await createProgram().parseAsync(["node", "atms", "--json", "plugin", ...args]);
}
