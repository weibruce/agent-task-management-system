...
import { AtmsClient } from "../src/client.js";
...
describe("multi-round DAG observation", () => {
  ...
  it("reports waiting as a command boundary and exits watch/supervise without polling", async () => {
    ...
    const client = new AtmsClient({ baseUrl: "http://manager.test" });
...
describe("multi-round DAG commands", () => {
  ...
  beforeEach(() => {
    vi.restoreAllMocks();
    process.exitCode = undefined;
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-cli-multiround-"));
    previousHome = process.env.ATMS_HOME;
    previousMutationToken = process.env.ATMS_DAG_MUTATION_TOKEN;
  ...
  afterEach(() => {
    ...
    if (previousMutationToken === undefined) delete process.env.ATMS_DAG_MUTATION_TOKEN;
    else process.env.ATMS_DAG_MUTATION_TOKEN = previousMutationToken;
  ...
  it("lists rounds and commands with an optional round filter", async () => {
    ...
    await createProgram().parseAsync([
      "node", "atms", "--base-url", "http://manager.test", "--json",
      "dag", "rounds", "run/one",
    ]);
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        headers: expect.objectContaining({
          "X-Atms-Dag-Token": "test-mutation-token",
  ...
  it("pairs repeated actors and payloads while preserving plain strings", async () => {
    ...
    await createProgram().parseAsync([
      "node", "atms", "--base-url", "http://manager.test",
  ...
  it("completes only the expected waiting round", async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        headers: expect.objectContaining({ "X-Atms-Dag-Token": "test-mutation-token" }),
