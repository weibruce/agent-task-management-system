...
beforeEach(() => {
  vi.restoreAllMocks();
  tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-cli-evidence-test-"));
...
describe("scorecard command", () => {
  it("renders text with PASS for all-checks-passing response", async () => {
    ...
    await program.parseAsync(["node", "atms", "scorecard", "run-001"]);
  ...
  it("renders text with FAIL for hard errors and exits 1", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "scorecard", "run-002"]);
  ...
  it("renders text with pass_with_warnings for soft warnings", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "scorecard", "run-003"]);
  ...
  it("outputs JSON with --json flag", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "--json", "scorecard", "run-004"]);
  ...
  it("reports API error", async () => {
    ...
    const program = createProgram();
    await program.parseAsync(["node", "atms", "scorecard", "missing-run"]);
...
describe("eval-run command", () => {
  ...
  it("renders text with PASS for passing eval", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "eval-run", "run-100"]);
...
describe("replay command", () => {
  it("renders no improvements needed for all-pass scorecard", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "replay", "run-200"]);
  ...
  it("renders categories and next steps for failures and exits 1", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "replay", "run-201"]);
  ...
  it("degrades gracefully when replay metadata omits acceptance and next steps", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "replay", "run-deploy-diagnosis"]);
...
describe("trace command", () => {
  it("renders sessions with tool calls from API", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "trace", "run-300"]);
  ...
  it("renders single node trace with --node filter", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "trace", "run-301", "--node", "coder"]);
  ...
  it("renders JSON with --json flag", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "--json", "trace", "run-302"]);
  ...
  it("falls back to run-scoped local Worker tool audit when Manager evidence is unavailable", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        event: "tool_use",
        tool_name: "Bash",
        input: { command: "node atms_cli/dist/cli.js doctor" },
        node_id: "diagnose",
        run_id: "run-local",
        ts: 1,
      }) + "\n",
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "trace", "run-local"]);

    const output = logSpy.mock.calls.map((c) => c.join(" ")).join("\n");
    expect(output).toContain("Trace: (1 sessions)");
    expect(output).toContain("node diagnose");
    expect(output).toContain("Bash");
    expect(output).toContain("node atms_cli/dist/cli.js doctor");
    expect(process.exitCode).toBe(0);
  });
  ...
  it("falls back to legacy global Worker tool audit filtered by run_id", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "trace", "run-legacy", "--node", "coder"]);
...
describe("stats command", () => {
  it("shows empty stats message when no stats file exists", async () => {
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "stats"]);
  ...
  it("renders JSON for empty stats with --json", async () => {
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "--json", "stats"]);
  ...
  it("reads stats from ATMS_HOME when configured", async () => {
    const previousHome = process.env.ATMS_HOME;
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-cli-stats-"));
    try {
      process.env.ATMS_HOME = tempHome;
      ...
      if (previousHome === undefined) {
        ...
        process.env.ATMS_HOME = previousHome;
      }
      fs.rmSync(tempHome, { recursive: true, force: true });
    }
  });
