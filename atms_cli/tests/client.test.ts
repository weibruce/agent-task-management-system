...
import { AtmsClient, AtmsHttpError } from "../src/client.js";
import {
...
beforeEach(() => {
  vi.restoreAllMocks();
  tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-client-test-"));
  ...
  delete process.env.ATMS_MANAGER_PORT;
  delete process.env[ATMS_MANAGER_ADMIN_TOKEN];
});
...
describe("local manager port resolution", () => {
  it("uses ATMS_MANAGER_URL for the Manager process port", () => {
    process.env.ATMS_MANAGER_URL = "http://127.0.0.1:34567";
    expect(configuredManagerPort()).toBe(34567);
  });

  it("lets ATMS_MANAGER_PORT override the URL port", () => {
    process.env.ATMS_MANAGER_URL = "http://127.0.0.1:34567";
    process.env.ATMS_MANAGER_PORT = "45678";
    expect(configuredManagerPort()).toBe(45678);
  });
});

describe("AtmsClient.resolveBaseUrl", () => {
  it("uses override when provided", () => {
    const url = AtmsClient.resolveBaseUrl("http://custom:4000");
  ...
  it("uses ATMS_MANAGER_URL env when no override", () => {
    process.env.ATMS_MANAGER_URL = "http://env-host:5555";
    const url = AtmsClient.resolveBaseUrl();
  ...
  it("ignores empty override", () => {
    process.env.ATMS_MANAGER_URL = "http://from-env:7777";
    const url = AtmsClient.resolveBaseUrl("  ");
    expect(url).toBe("http://from-env:7777");
  });

  it("ignores empty env", () => {
    process.env.ATMS_MANAGER_URL = "   ";
    const url = AtmsClient.resolveBaseUrl();
    expect(url).toBe("http://localhost:19191");
  });
});

describe("AtmsClient.get", () => {
  it("calls fetch with correct URL and method", async () => {
    ...
    const client = new AtmsClient({ baseUrl: "http://test:1234", mutationToken: "mutation-secret" });
...
describe("AtmsClient.post", () => {
  it("sends JSON body", async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        headers: expect.objectContaining({ "X-Atms-Dag-Token": "mutation-secret" }),
  ...
  it("sends plugin archives as exact binary bytes instead of JSON or base64", async () => {
    ...
    const client = new AtmsClient({ baseUrl: "http://test:1234" });
    ...
    expect(init).toEqual(expect.objectContaining({
      ...
      headers: expect.objectContaining({
        "Content-Type": "application/vnd.atms.plugin+zip",
      }),
    }));
...
describe("AtmsClient Manager admin token", () => {
  it("injects an environment token into every /api mutation and nowhere else", async () => {
    ...
    process.env[ATMS_MANAGER_ADMIN_TOKEN] = token;
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValue({
      ok: true,
  ...
  it("loads the token from the private local secrets file", async () => {
    ...
    saveLocalSecret(ATMS_MANAGER_ADMIN_TOKEN, token);
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValue({
      ok: true,
      json: async () => ({ success: true }),
    } as unknown as Response);

    await new AtmsClient({ baseUrl: "http://test:1234" })
  ...
  it("keeps process environment precedence over the private secrets file", async () => {
    saveLocalSecret(ATMS_MANAGER_ADMIN_TOKEN, "secret-admin-token-0123456789abcdef");
    const environmentToken = "environment-admin-token-0123456789abcdef";
    process.env[ATMS_MANAGER_ADMIN_TOKEN] = environmentToken;
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValue({
      ok: true,
      json: async () => ({ success: true }),
    } as unknown as Response);

    await new AtmsClient({ baseUrl: "http://test:1234" }).post("/api/runs", {});
    expect(fetchSpy.mock.calls[0][1]?.headers).toEqual(expect.objectContaining({
  ...
  it("refuses an insecure local secrets file", () => {
    saveLocalSecret(ATMS_MANAGER_ADMIN_TOKEN, "cli-local-admin-token-0123456789abcdef");
    if (process.platform === "win32") return;
    fs.chmodSync(getSecretsPath(), 0o644);
    expect(() => new AtmsClient()).toThrow("group/world-accessible");
  });
  ...
  it.runIf(process.platform !== "win32")("refuses a symlinked local secrets file", () => {
    ...
    expect(() => new AtmsClient()).toThrow("non-regular secrets file");
  });
  ...
  it("redacts the credential from Manager and transport errors", async () => {
    ...
    let message = "";
    try {
      await new AtmsClient().post("/api/runs", {});
    } catch (cause) {
...
describe("AtmsClient error handling", () => {
  it("preserves HTTP status while redacting credentials from server errors", async () => {
    ...
    const client = new AtmsClient({ mutationToken: token });
    const error = await client.post("/api/runs/create-and-run", {}).catch(err => err);
    expect(error).toBeInstanceOf(AtmsHttpError);
  ...
  it("throws on timeout", async () => {
    ...
    const client = new AtmsClient({ timeoutMs: 50 });
