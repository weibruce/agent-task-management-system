...
/**
 * These tests exercise each command handler with a mocked AtmsClient.
 * We spy on the global fetch so that AtmsClient never makes real HTTP calls.
 */
...
beforeEach(() => {
  ...
  tempHome = mkdtempSync(join(tmpdir(), "atms-cli-command-test-"));
  ...
  previousManagerUrl = process.env.ATMS_MANAGER_URL;
  previousPublicHost = process.env.ATMS_PUBLIC_HOST;
  previousAssetDir = process.env.ATMS_ASSET_DIR;
  previousUiServeStatic = process.env.ATMS_UI_SERVE_STATIC;
  previousUiPublicUrl = process.env.ATMS_UI_PUBLIC_URL;
  ...
  delete process.env.ATMS_UI_SERVE_STATIC;
  delete process.env.ATMS_UI_PUBLIC_URL;
});
...
describe("credential command", () => {
  it("lists only credential metadata", async () => {
    ...
    await program.parseAsync(["node", "atms", "credential", "list"]);
...
afterEach(() => {
  ...
  restoreEnv("ATMS_MANAGER_URL", previousManagerUrl);
  restoreEnv("ATMS_PUBLIC_HOST", previousPublicHost);
  restoreEnv("ATMS_ASSET_DIR", previousAssetDir);
  restoreEnv("ATMS_UI_SERVE_STATIC", previousUiServeStatic);
  restoreEnv("ATMS_UI_PUBLIC_URL", previousUiPublicUrl);
...
describe("runs command", () => {
  it("prints run table in human-readable mode", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "runs"]);
  ...
  it("prints JSON with --json flag", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "--json", "runs"]);
...
describe("status command", () => {
  it("prints run status in human-readable mode", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "status", "run-123"]);
  ...
  it("prints JSON with --json flag", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "--json", "status", "run-123"]);
...
describe("stop command", () => {
  it("prints confirmation in human-readable mode", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "stop", "run-456"]);
  ...
  it("prints JSON with --json flag", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "--json", "stop", "run-456"]);
  ...
  it("reports error on failure", async () => {
    ...
    const program = createProgram();
    await program.parseAsync(["node", "atms", "stop", "missing-run"]);
...
describe("run command", () => {
  ...
  it("syncs a DAG asset and profile YAML before running", async () => {
    const assetRoot = mkdtempSync(join(tmpdir(), "atms-cli-run-sync-assets-"));
...
`);
  ...
  it("requires --prompt", async () => {
    ...
    try {
      await program.parseAsync(["node", "atms", "run", "template"]);
    } catch {
      // expected
    }
...
describe("--setting-id option guard", () => {
  ...
  it.each(cases)("rejects an explicit blank value for %s before any request", async (_name, args) => {
    ...
    await expect(program.parseAsync(["node", "atms", ...args])).rejects.toMatchObject({
...
describe("templates command", () => {
  it("lists local templates without probing Manager catalog APIs", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "templates", "list"]);
  ...
  it("lists templates from ATMS_ASSET_DIR when configured", async () => {
    const assetRoot = mkdtempSync(join(tmpdir(), "atms-cli-assets-"));
    try {
      mkdirSync(join(assetRoot, "orchestrations"), { recursive: true });
...
`);
      ...
      const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
      const program = createProgram();
      await program.parseAsync(["node", "atms", "--json", "templates", "list"]);
  ...
  it("keeps the local harness deployment diagnosis template single-node and read-only", () => {
    ...
    expect(yaml).toContain("ATMS_WORKER_ID");
...
describe("dag WorkflowSpec commands", () => {
  it("prints structured validation output for AI clients", async () => {
    ...
    writeFileSync(sourcePath, "api_version: atms.ai/v1\nkind: Workflow\n");
    mockFetch({
      success: true,
    ...
    const program = createProgram();
    await program.parseAsync(["node", "atms", "--json", "dag", "validate", sourcePath]);
    ...
    expect(fetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ source: "api_version: atms.ai/v1\nkind: Workflow\n" }),
      }),
    );
  });
  ...
  it("prints source diagnostics and exits non-zero for an invalid workflow", async () => {
    ...
    const program = createProgram();
    await program.parseAsync(["node", "atms", "dag", "validate", sourcePath]);
  ...
  it("fetches the live schema from Manager", async () => {
    ...
    const program = createProgram();
    await program.parseAsync(["node", "atms", "--json", "dag", "schema"]);
...
describe("provider command", () => {
  it("provider list prints provider table in human-readable mode", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "provider", "list"]);
  ...
  it("provider list prints JSON with --json flag", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "--json", "provider", "list"]);
  ...
  it("provider get prints details for a specific provider", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "provider", "get", "openai"]);
  ...
  it("provider get reports error for unknown id", async () => {
    ...
    const errSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "provider", "get", "unknown"]);
...
describe("llm-settings command", () => {
  ...
  it("llm-settings list redacts api_key in human-readable output", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "llm-settings", "list"]);
  ...
  it("llm-settings list --json redacts api_key", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "--json", "llm-settings", "list"]);
  ...
  it("llm-settings add uses ATMS_API_KEY env var", async () => {
    const originalEnv = process.env.ATMS_API_KEY;
    process.env.ATMS_API_KEY = "pk-env-key-999";
    ...
    if (originalEnv === undefined) {
      ...
      process.env.ATMS_API_KEY = originalEnv;
    }
  });
  ...
  it("llm-settings get shows redacted details", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "llm-settings", "get", "set-001"]);
  ...
  it("llm-settings get --json shows redacted details", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "--json", "llm-settings", "get", "set-002"]);
...
describe("model command", () => {
  ...
  it("requires an API key for catalog/custom model configuration", async () => {
    const originalEnv = process.env.ATMS_API_KEY;
    const originalAtmsHome = process.env.ATMS_HOME;
    const tempHome = mkdtempSync(join(tmpdir(), "atms-cli-model-test-"));
    delete process.env.ATMS_API_KEY;
    process.env.ATMS_HOME = tempHome;
    ...
    try {
      ...
      if (originalEnv === undefined) delete process.env.ATMS_API_KEY;
      else process.env.ATMS_API_KEY = originalEnv;
      if (originalAtmsHome === undefined) delete process.env.ATMS_HOME;
      else process.env.ATMS_HOME = originalAtmsHome;
      rmSync(tempHome, { recursive: true, force: true });
    }
  });
  ...
  it("reports incomplete custom model configuration", async () => {
    ...
    const errSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "model", "configure", "unknown"]);
...
describe("doctor command", () => {
  it("passes when manager, runtime, and an active setting are available", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "doctor"]);
    ...
    expect(output).toContain("catalog-provider/catalog-model");
    expect(output).toContain("Atms is ready.");
  });
});

describe("runtime command", () => {
  it("does not expose fixed host Worker startup through atms start", () => {
    const program = createProgram();
  ...
  it("reports the default Agent UI port in JSON status", async () => {
    ...
    await program.parseAsync(["node", "atms", "--json", "ui", "status"]);
  ...
  it("reports the Agent UI port from persisted service state", async () => {
    ...
    writeFileSync(join(pidsDir, "ui-https.json"), JSON.stringify({
      ...
      publicUrl: "https://atms.example.test/ui",
    ...
    writeFileSync(join(pidsDir, "ui.json"), JSON.stringify({
      ...
      protocol: "http",
      managerUrl: "http://localhost:19191",
      publicUrl: "http://atms.example.test/ui",
      textModeEnabled: true,
      startedAt: Date.now(),
    }));
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    ...
    expect(parsed.uiUrl).toBe("https://atms.example.test/ui");
    expect(parsed.uiPublicUrl).toBe("https://atms.example.test/ui");
    expect(parsed.uiHttpsPort).toBe(19292);
    expect(parsed.uiHttpPort).toBe(19293);
    expect(parsed.uiHttpUrl).toBe("http://atms.example.test/ui");
  ...
  it("reports HTTP as the active Agent UI URL when HTTPS is unavailable", async () => {
    ...
    expect(parsed.uiUrl).toBe("http://atms.example.test/ui");
    expect(parsed.uiPublicUrl).toBe("http://atms.example.test/ui");
  });
  ...
  it("uses local HTTPS and HTTP URLs for public UI bind without an explicit public URL", async () => {
    ...
    writeFileSync(join(pidsDir, "ui-https.json"), JSON.stringify({
      ...
      port: 19192,
      protocol: "https",
      managerUrl: "https://atms.example.test",
      startedAt: Date.now(),
    }));
    writeFileSync(join(pidsDir, "ui.pid"), `${process.pid}\n`);
    writeFileSync(join(pidsDir, "ui.json"), JSON.stringify({
  ...
  it("reports Manager bind host and access URL in runtime status", async () => {
    ...
    await program.parseAsync(["node", "atms", "--json", "runtime", "status"]);
    ...
    expect(parsed.serviceControl.service_id).toBe("atms-runtime");
  ...
  it("reports Manager bind host and access URL from persisted service state", async () => {
    ...
    writeFileSync(join(pidsDir, "manager.json"), JSON.stringify({
      ...
      publicUrl: "https://atms.example.test",
      runtimeRoot: "/opt/atms/runtime-a",
      startedAt: Date.now(),
    }));
    mockFetch({ success: true, data: { connected_nodes: 0, connected_workers: 0 } });
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    ...
    expect(parsed.managerAccessUrl).toBe("https://atms.example.test");
    expect(parsed.managerPublicUrl).toBe("https://atms.example.test");
    expect(parsed.managerRuntimeRoot).toBe("/opt/atms/runtime-a");
  });
  ...
  it("aborts start when stored model config cannot be applied", () => {
    ...
    expect(shouldAbortStartForModelConfig({
      applied: false,
      action: "failed",
      detail: "API key is required. Use --api-key or set ATMS_PROVIDER_API_KEY.",
    })).toBe(false);
  });

  it("detects missing model credentials as a first-run configuration gap", () => {
    expect(isMissingModelCredential("API key is required. Use --api-key or set ATMS_PROVIDER_API_KEY.")).toBe(true);
  ...
  it("renders and manages a macOS LaunchAgent service definition without loading it", () => {
    const plist = createLaunchAgentPlist({
      ...
      homeDir: tempHome,
      repoRoot: "/repo/Atms",
      cliPath: "/repo/Atms/atms_cli/dist/cli.js",
      nodePath: "/opt/homebrew/bin/node",
    });
    expect(plist).toContain("com.atms.runtime");
    expect(plist).toContain("<string>/opt/homebrew/bin/node</string>");
    expect(plist).toContain("<string>/repo/Atms/atms_cli/dist/cli.js</string>");
    expect(plist).toContain("<string>start</string>");
    expect(plist).toContain("<string>--ui</string>");
    expect(plist).toContain("<key>ATMS_HOME</key>");
  ...
  it("propagates only an explicit UI public URL into the static UI server environment", () => {
    ...
    expect(staticUiServerEnv({ ...base, explicitPublicUrl: "https://external.example" }))
      .toMatchObject({
        ATMS_HOME: tempHome,
        ATMS_STATIC_UI_DIR: base.staticUiDir,
        ATMS_UI_HOST: "127.0.0.1",
        ATMS_UI_PORT: "19192",
        ATMS_MANAGER_HTTP: "http://localhost:19191",
        ATMS_MANAGER_WS: "ws://localhost:19191",
        ATMS_UI_HTTPS: "1",
        ATMS_UI_PUBLIC_URL: "https://external.example",
      });
    expect(staticUiServerEnv(base)).not.toHaveProperty("ATMS_UI_PUBLIC_URL");
    expect(staticUiServerEnv({ ...base, protocol: "http" as const }))
      .not.toHaveProperty("ATMS_UI_HTTPS");
  ...
  it("resolves one normalized explicit UI Origin across flag, environment, and stored config sources", () => {
    ...
    // ATMS_UI_PUBLIC_URL takes precedence over stored config.
    process.env.ATMS_UI_PUBLIC_URL = " https://Env.Example.test/ ";
  ...
  it("rejects ambiguous explicit UI Origins from every configuration source", () => {
    for (const invalid of [
      ...
      expect(() => resolveExplicitUiPublicOrigin({}, invalid), invalid)
        .toThrow(/exact http\(s\) Origin/);
      process.env.ATMS_UI_PUBLIC_URL = invalid;
  ...
  it("allows forcing Agent UI dev server mode", () => {
    ...
    mkdirSync(join(uiDir, "dist"), { recursive: true });
    writeFileSync(join(uiDir, "dist", "index.html"), "<!doctype html>");
    process.env.ATMS_UI_SERVE_STATIC = "0";
...
describe("dag handoffs command", () => {
  it("prints persisted handoff content from the manager API", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "dag", "handoffs", "run-123"]);
...
describe("dag chats command", () => {
            ...
            messages: [
              {
                ...
                content: {
                  ...
                  tool_id: "tool-1",
                  tool_input: {
                    command: "node atms_cli/dist/cli.js doctor",
                    token: "***REDACTED***",
                  },
                },
              },
  ...
  it("prints redacted tool command details with --raw-tools", async () => {
    ...
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const program = createProgram();
    await program.parseAsync(["node", "atms", "dag", "chats", "run-raw", "--raw-tools"]);

    const output = logSpy.mock.calls.map((c) => c.join(" ")).join("\n");
    expect(output).toContain("Bash: node atms_cli/dist/cli.js doctor");
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        body: JSON.stringify({
          ...
          prompt: "Draft a short checklist for a Windows Atms runtime smoke.",
