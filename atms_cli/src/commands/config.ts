...
export function registerConfigCommand(program: Command): void {
  const configCmd = program
    .command("config")
    .description("Configure local Atms runtime settings")
    .action(async () => {
  ...
  configCmd
    .command("show")
    .description("Show local Atms config with secrets redacted")
    .action(() => {
  ...
  configCmd
    ...
    .argument("<key>", "Config key, for example manager.url or ATMS_MIMO_API_KEY")
    .argument("<value>", "Value to set")
    .action((key: string, value: string) => {
      if (isSecretKey(key)) {
        saveLocalSecret(key.toUpperCase(), value);
        console.log(`Set legacy plaintext ${key.toUpperCase()} in ${getSecretsPath()}`);
        console.log("Run `atms config apply` to import it into the Manager encrypted secret store.");
        return;
      }
      const cfg = loadLocalConfig();
  ...
  configCmd
    ...
    .action(async () => {
      const globalOpts = program.opts() as GlobalOpts;
      const client = new AtmsClient({ baseUrl: globalOpts.baseUrl, timeoutMs: globalOpts.requestTimeout });
...
export async function applyStoredModelConfig(client: AtmsClient): Promise<ApplyResult> {
  void client;
  const cfg = loadLocalConfig();
  return applyModelConfig(cfg);
}

function applyModelConfig(cfg: LocalAtmsConfig): ApplyResult {
  const preset = cfg.model?.preset?.trim();
  if (!preset) {
    return {
      applied: false,
      action: "skipped",
      detail: "no local model alias configured; run `atms model configure <provider-or-endpoint-alias>` when you are ready",
    };
  }
  return {
    applied: false,
    action: "skipped",
    detail: "legacy local model alias config is not auto-applied; resolve it from the Manager provider catalog with `atms model configure <provider-or-endpoint-alias>`",
  };
}

async function runConfigWizard(_globalOpts: GlobalOpts): Promise<void> {
  if (!process.stdin.isTTY) {
    console.error("Error: atms config requires an interactive terminal. Use `atms config set` in non-interactive environments.");
    process.exitCode = 1;
    return;
  }
  ...
  console.log("Model settings are stored in Manager. Use `atms model configure <provider-or-endpoint-alias>` after Manager starts.");
  console.log("Next: atms start");
}
