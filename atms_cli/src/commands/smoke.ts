...
const WINDOWS_SMOKE_PROMPT = "Draft a short checklist for a Windows Atms runtime smoke.";
...
export function registerSmokeCommand(program: Command): void {
  ...
  smokeCmd
    ...
    .option("--profile <profile>", "Runtime profile; only agent_type selection is supported", process.env.ATMS_SMOKE_PROFILE)
      ...
      async (opts: SmokeDagOpts) => {
        ...
        try {
          ...
          if (!opts.wait) {
            ...
            console.log("Next:");
            console.log(`  atms dag supervise ${runId}`);
            console.log(`  atms scorecard ${runId}`);
            console.log(`  atms eval-run ${runId}`);
            return;
          }
          ...
          console.log(`Inspect: atms dag handoffs ${runId}`);
          if (!passed) {
            process.exitCode = 1;
          }
        } catch (err: unknown) {
...
function windowsRuntimeChecks(data: Record<string, unknown>): SmokeCheck[] {
  ...
  return [
    ...
    },
    {
      name: "node",
      ok: connectedNodes > 0,
      detail: connectedNodes > 0 ? `${connectedNodes} connected` : "run: atms start",
    },
    {
      name: "docker-node",
