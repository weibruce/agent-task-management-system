...
import { isAtmsPluginId } from "atms-protocol";
...
export function registerPluginCommand(program: Command): void {
  const plugin = program
    .command("plugin")
    .description("Develop, package, install, and manage Atms plugins");
  ...
  plugin
    ...
    .action(async (
      ...
      output(program, data, [
        ...
        `Channel: ${channel}`,
        "The package remains disabled until `atms plugin enable` succeeds.",
      ]);
    }));
...
function encodePluginId(pluginId: string): string {
  if (!isAtmsPluginId(pluginId)) throw new Error(`Invalid Atms plugin id: ${pluginId}`);
  return encodeURIComponent(pluginId);
}

function encodeRegistryId(registryId: string): string {
  if (!/^[a-z][a-z0-9._-]{0,79}$/.test(registryId)) {
    throw new Error(`Invalid Atms plugin registry id: ${registryId}`);
  }
