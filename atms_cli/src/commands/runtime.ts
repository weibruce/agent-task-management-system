...
import { AtmsClient } from "../client.js";
...
interface UiServiceState {
  ...
  mode?: "dev" | "static";
  /**
   * Absolute directory served by a static UI child. A live child whose
   ...
   * (`ATMS_UI_PUBLIC_URL`), or `undefined` when the child runs with
...
const MANAGER_ADMIN_ORIGINS_ENV = "ATMS_MANAGER_ADMIN_ORIGINS";
...
export function registerRuntimeCommands(program: Command): void {
  ...
  const runtime = program
    .command("runtime")
    .description("Inspect and manage local Atms runtime services");
  ...
  const installCommand = runtime
    .command("install")
    .alias("register")
    .description("Install/register the local Atms runtime service")
  ...
  runtime
    .command("uninstall")
    .alias("unregister")
    .description("Uninstall/delete the local Atms runtime service")
...
async function startRuntime(
  ...
  const assetEnv: Record<string, string> = assetRoot ? { ATMS_ASSET_DIR: assetRoot } : {};
  ...
  const client = new AtmsClient({ baseUrl: managerLocalUrl, timeoutMs: globalOpts.requestTimeout });

  ensureBuiltArtifact("atms_manager/dist/index.js");
  if (!managerOnly) ensureBuiltArtifact("atms_node/dist/cli.js");

  const before = managerOnly ? undefined : await getRuntimeStatus(globalOpts);
  if (managerOnly || !before?.managerHealthy) {
    const pid = startService("manager", "atms_manager/dist/index.js", {
      ATMS_HOME: getAtmsHome(),
      ATMS_MANAGER_PORT: String(managerPort),
      ATMS_MANAGER_HOST: managerHost,
      ATMS_MANAGER_ADMIN_ORIGINS: managerAdminOrigins,
      ATMS_MANAGER_ADMIN_TOKEN: "",
      ...(hasExplicitManagerPublicUrl ? { ATMS_MANAGER_PUBLIC_URL: managerPublicUrl } : {}),
      ATMS_PROJECT_ID: cfg.node?.projectId || "p1",
      ...assetEnv,
    });
    writeManagerState({
      pid,
  ...
  if (applyResult.applied) {
    ...
    if (shouldAbortStartForModelConfig(applyResult)) {
      if (applyResult.detail.includes("Unknown provider_id")) {
        console.error(
          ...
          "This can happen when an older or unmanaged Manager is already listening; run `atms runtime status` " +
    ...
    if (isMissingModelCredential(applyResult.detail)) {
      printMessage("Next: run `atms model configure <provider-or-endpoint-alias>` to add provider credentials before running DAGs.");
    }
  }
  ...
  const runtimeClient = new AtmsClient({ baseUrl: client.baseUrl, timeoutMs: globalOpts.requestTimeout });
  ...
  if (!nodeIds.includes(nodeId)) {
    const env = {
      ATMS_HOME: getAtmsHome(),
      ATMS_MANAGER_WS_URL: managerWsUrl({ ...cfg, manager: { ...cfg.manager, url: client.baseUrl } }),
      ATMS_PROJECT_ID: cfg.node?.projectId || "p1",
      ATMS_NODE_ID: nodeId,
      ATMS_NODE_PROVIDER: cfg.node?.provider || "docker-cli",
      ...assetEnv,
    };
    const pid = startService("node", "atms_node/dist/cli.js", env);
...
async function getRuntimeStatus(globalOpts: GlobalOpts): Promise<RuntimeStatus> {
  ...
  const client = new AtmsClient({ baseUrl: managerUrl, timeoutMs: globalOpts.requestTimeout });
 ...
 * of configuredUiPublicUrl (--public-url flag, ATMS_UI_PUBLIC_URL, then
...
function explicitUiPublicUrl(
  ...
  if (process.env.ATMS_UI_PUBLIC_URL?.trim()) return process.env.ATMS_UI_PUBLIC_URL.trim().replace(/\/+$/, "");
 ...
 * flag, `ATMS_UI_PUBLIC_URL`, or stored `ui.publicUrl` (in that
...
interface StartUiProcessOpts {
   ...
   * ATMS_UI_PUBLIC_URL / ui.publicUrl), only when explicitly set. Derived
...
/**
 * Environment for the zero-dependency static UI server. `ATMS_UI_PUBLIC_URL`
...
export function staticUiServerEnv(opts: {
  ...
  return {
    ATMS_HOME: opts.atmsHome,
    ATMS_STATIC_UI_DIR: opts.staticUiDir,
    ATMS_UI_HOST: opts.host,
    ATMS_UI_PORT: String(opts.port),
    ATMS_MANAGER_HTTP: opts.managerHttp,
    ATMS_MANAGER_WS: opts.managerHttp.replace(/^http/, "ws"),
    ...(opts.protocol === "https"
      ? {
        ATMS_UI_HTTPS: "1",
        ATMS_UI_HTTPS_KEY: opts.certificate?.keyPath || "",
        ATMS_UI_HTTPS_CERT: opts.certificate?.certPath || "",
      }
      : {}),
    ...(opts.explicitPublicUrl ? { ATMS_UI_PUBLIC_URL: opts.explicitPublicUrl } : {}),
  };
}
...
function startUiProcess(opts: StartUiProcessOpts): number {
  ...
  // Production mode: serve the prebuilt agent-ui/dist with a tiny
  // zero-dependency static server (see static-ui-server.ts), avoiding the need
  // to ship agent-ui's full node_modules (vite toolchain).
  const serveStatic = shouldServeStaticAgentUi(agentUiDir);
  const managerHttp = opts.managerUrl || `http://localhost:${opts.managerPort}`;
  let child: import("child_process").ChildProcess;
  if (serveStatic) {
    const serverScript = path.join(resolveRepoRoot(), "atms_cli", "dist", "static-ui-server.js");
    ...
    child = spawn(devServer.command, [
      ...
      env: {
        ...
        ATMS_UI_HOST: opts.host,
        ATMS_UI_PORT: String(opts.port),
        VITE_ATMS_UI_PORT: String(opts.port),
        VITE_ATMS_MANAGER_PORT: opts.managerPort,
        VITE_ATMS_ENABLE_TEXT_MODE: opts.textModeEnabled ? "1" : "0",
        ATMS_MANAGER_HTTP: managerHttp,
...
function hasManagerPublicUrl(config: ReturnType<typeof loadLocalConfig>, override?: string): boolean {
  return Boolean(
    override?.trim() ||
    process.env.ATMS_MANAGER_PUBLIC_URL?.trim() ||
...
function resolveTextModeEnabled(override?: boolean): boolean {
  ...
  return envFlagEnabled(process.env.ATMS_UI_ENABLE_TEXT_MODE) || envFlagEnabled(process.env.VITE_ATMS_ENABLE_TEXT_MODE);
}
...
/**
 * A detached static UI child may outlive its parent process. The PID then
 * remains alive while every static request returns 404 because its old
 * resource root has disappeared. Restart it when the current root differs,
 * when that root vanished, or when upgrading legacy state that did not
 * record the root at all.
 */
...
export function shouldServeStaticAgentUi(
  ...
  const mode = process.env.ATMS_UI_SERVE_STATIC?.trim();
...
function ensureUiCertificate(host: string): UiCertificate {
  ...
  const certDir = path.join(getAtmsHome(), "certs");
...
function managerStatePath(): string {
  const dir = path.join(getAtmsHome(), "pids");
...
async function waitForManager(client: AtmsClient): Promise<void> {
...
async function waitForNode(client: AtmsClient, nodeId: string): Promise<void> {
  const started = Date.now();
...
async function safeRuntimeStatus(client: AtmsClient): Promise<unknown> {
...
function logPath(name: RuntimeServiceName): string {
  const dir = path.join(getAtmsHome(), "logs");
  fs.mkdirSync(dir, { recursive: true });
...
function resolveRepoRoot(): string {
  // Allow deployments with a relocated runtime tree to point the
  // CLI at the correct root instead of relying on the source layout.
