...
async function managerRunSnapshot(client: AtmsClient, runId: string): Promise<PrCloseoutRunSnapshot> {
...
export async function resolvePrCloseoutInput(
  ...
  const apiBaseUrl = (options.apiBaseUrl ?? env.ATMS_GITHUB_API_BASE_URL ?? "https://api.github.com")
    .replace(/\/+$/, "");
