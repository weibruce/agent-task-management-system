...
import { AtmsHttpError, AtmsTransportError, type BaseResponse, type AtmsClient } from "../client.js";
...
export async function resolvePrReviewInput(
  ...
  const apiBaseUrl = (options.apiBaseUrl ?? env.ATMS_GITHUB_API_BASE_URL ?? "https://api.github.com").replace(/\/+$/, "");
...
async function startTemplateRun(
  ...
  try {
    ...
    const ambiguous = err instanceof AtmsTransportError
      || (err instanceof AtmsHttpError && err.status >= 500 && err.status < 600);
...
async function listRunArtifacts(client: AtmsClient, runId: string): Promise<RunArtifactSummary[]> {
...
function printTerminalSummary(
  ...
  if (artifacts.length === 0) {
    ...
    lines.push(`Retrieve with: atms dag artifact ${runId} <name> --output <path>`);
  }
