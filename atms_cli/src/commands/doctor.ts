...
export function dockerReadiness(
  ...
  const image = runner(dockerBin, ["image", "inspect", "atms-worker:latest"], { stdio: "ignore" });
  checks.push({
    name: "worker-image",
    ok: image.status === 0,
    detail: image.status === 0
      ? "atms-worker:latest present"
      : "run: atms start --rebuild-worker-image",
  });
  return checks;
}
  ...
  if (!selected) {
    return {
      ...
      detail: harness === "kimi_code"
        ? "run: atms model configure or atms llm-settings add with an active Kimi setting"
        : harness === "deepseek_harness"
        ? "run: atms model configure or atms llm-settings add with an OpenAI-compatible endpoint"
        : "run: atms model configure or atms llm-settings add with an Anthropic-compatible endpoint",
    };
  }
...
export function registerDoctorCommand(program: Command): void {
  program
    .command("doctor")
    .description("Check whether the local Atms runtime is ready")
    ...
    .action(async (opts: DoctorOpts) => {
      ...
      try {
        ...
        checks.push({
          ...
          detail: model
            ? `${model} configured`
            : "run: atms model configure <provider-or-endpoint-alias>",
        });
      } catch (err) {
        checks.push({
          name: "model",
      ...
      if (globalOpts.json) {
        ...
        console.log(ok ? "Atms is ready." : "Atms is not ready yet.");
      }
