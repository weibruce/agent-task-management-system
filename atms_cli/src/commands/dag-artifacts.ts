...
async function listArtifacts(client: AtmsClient, runId: string): Promise<RunArtifact[]> {
...
export async function cmdDagArtifacts(client: AtmsClient, runId: string, json: boolean): Promise<number> {
  try {
...
export async function cmdDagArtifact(
  ...
  try {
    ...
    if (destination) {
      ...
      stagingDir = fs.mkdtempSync(path.join(os.tmpdir(), "atms-artifact-"));
