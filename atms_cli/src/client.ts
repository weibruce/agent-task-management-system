...
export class AtmsTransportError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "AtmsTransportError";
  }
}

export class AtmsHttpError extends Error {
  constructor(readonly status: number, message: string) {
    super(message);
    this.name = "AtmsHttpError";
  }
}
...
export interface AtmsClientOptions {
  baseUrl?: string;
  timeoutMs?: number;
  adminToken?: string;
...
export class AtmsClient {
  readonly baseUrl: string;
  readonly timeoutMs: number;
  private readonly adminToken?: string;
  readonly mutationToken?: string;

  constructor(opts: AtmsClientOptions = {}) {
    this.baseUrl = AtmsClient.resolveBaseUrl(opts.baseUrl);
    this.timeoutMs = opts.timeoutMs ?? DEFAULT_TIMEOUT_MS;
    this.adminToken = opts.adminToken === undefined
      ? resolveConfiguredManagerAdminToken()
      : opts.adminToken || undefined;
    this.mutationToken = opts.mutationToken ?? process.env.ATMS_DAG_MUTATION_TOKEN;
  }
  ...
  async postBinary<T = BaseResponse>(
    path: string,
    body: Uint8Array,
    contentType = "application/vnd.atms.plugin+zip",
  ): Promise<T> {
  ...
  private async request<T>(
    ...
    try {
      const init: RequestInit = {
        ...
        headers: {
          ...
          ...(method !== "GET" && this.mutationToken
            ? { "X-Atms-Dag-Token": this.mutationToken }
            : {}),
          ...(payload ? {
      ...
      if (!response.ok) {
        ...
        httpRejection = true;
        throw new AtmsHttpError(response.status, message);
      }

      return (await response.json()) as T;
    } catch (err: unknown) {
      if (err instanceof Error && err.name === "AbortError") {
        throw new AtmsTransportError(`Request timed out after ${effectiveTimeout}ms`);
      }
      const redacted = redactClientError(err, this.adminToken, this.mutationToken);
      if (err instanceof AtmsHttpError) {
        throw new AtmsHttpError(err.status, redacted.message);
      }
      if (!httpRejection) throw new AtmsTransportError(redacted.message);
      throw redacted;
    } finally {
      clearTimeout(timer);
    }
