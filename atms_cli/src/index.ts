...
export function readCliPackageVersion(
  ...
  if (typeof metadata.version !== "string" || metadata.version.trim() === "") {
    throw new Error(`Atms CLI package metadata has no valid version: ${packageJsonUrl.href}`);
  }
...
export function createProgram(): Command {
  ...
  program
    .name("atms")
    .description("Atms CLI - DAG orchestration control (TypeScript)")
    .version(CLI_VERSION)
    .option("--base-url <url>", `Manager API URL (default: $ATMS_MANAGER_URL or ${DEFAULT_MANAGER_URL})`)
...
export function getClient(opts: { baseUrl?: string; requestTimeout?: number }): AtmsClient {
  return new AtmsClient({ baseUrl: opts.baseUrl, timeoutMs: opts.requestTimeout });
}
