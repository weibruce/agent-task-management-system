...
export interface LocalAtmsConfig {
  manager?: {
    url?: string;
    port?: number;
    host?: string;
...
export const ATMS_MANAGER_ADMIN_TOKEN = "ATMS_MANAGER_ADMIN_TOKEN";

export function getAtmsHome(): string {
  return process.env.ATMS_HOME?.trim() || path.join(os.homedir(), ".atms");
}

export function getConfigPath(): string {
  return process.env.ATMS_CONFIG_PATH?.trim() || path.join(getAtmsHome(), "config.json");
}

export function getSecretsPath(): string {
  return process.env.ATMS_SECRETS_PATH?.trim() || path.join(getAtmsHome(), "secrets", "env");
}

export function ensureAtmsHome(): void {
  fs.mkdirSync(getAtmsHome(), { recursive: true });
  fs.mkdirSync(path.dirname(getConfigPath()), { recursive: true });
  fs.mkdirSync(path.dirname(getSecretsPath()), { recursive: true, mode: 0o700 });
}

export function defaultLocalConfig(): LocalAtmsConfig {
  return {
    manager: {
...
export function loadLocalConfig(): LocalAtmsConfig {
  const defaults = defaultLocalConfig();
  const filePath = getConfigPath();
  if (!fs.existsSync(filePath)) return defaults;
  try {
    const raw = JSON.parse(fs.readFileSync(filePath, "utf-8")) as LocalAtmsConfig;
...
export function saveLocalConfig(config: LocalAtmsConfig): void {
  ensureAtmsHome();
  const filePath = getConfigPath();
...
export function resolveConfiguredManagerAdminToken(): string | undefined {
  const fromEnvironment = process.env[ATMS_MANAGER_ADMIN_TOKEN];
  if (fromEnvironment !== undefined) return fromEnvironment || undefined;

  const filePath = getSecretsPath();
  if (!fs.existsSync(filePath)) return undefined;
  const value = loadLocalSecrets()[ATMS_MANAGER_ADMIN_TOKEN];
...
export function resolveConfiguredManagerUrl(override?: string): string {
  ...
  if (process.env.ATMS_MANAGER_URL?.trim()) {
    return process.env.ATMS_MANAGER_URL.trim().replace(/\/+$/, "");
  }
...
export function configuredManagerHost(config = loadLocalConfig(), override?: string): string {
  ...
  if (process.env.ATMS_MANAGER_HOST?.trim()) return process.env.ATMS_MANAGER_HOST.trim();
...
export function configuredManagerAccessUrl(config = loadLocalConfig(), override?: string): string {
  if (override?.trim()) return override.trim().replace(/\/+$/, "");
  if (process.env.ATMS_MANAGER_PUBLIC_URL?.trim()) {
    return process.env.ATMS_MANAGER_PUBLIC_URL.trim().replace(/\/+$/, "");
  }
...
export function configuredManagerPort(config = loadLocalConfig()): number {
  const explicitPort = Number(process.env.ATMS_MANAGER_PORT?.trim() || "");
  if (Number.isInteger(explicitPort) && explicitPort > 0 && explicitPort <= 65535) return explicitPort;
  const url = process.env.ATMS_MANAGER_URL?.trim() || config.manager?.url || DEFAULT_MANAGER_URL;
  try {
...
export function configuredUiHost(config = loadLocalConfig(), override?: string): string {
  if (override?.trim()) return override.trim();
  if (process.env.ATMS_UI_HOST?.trim()) return process.env.ATMS_UI_HOST.trim();
...
export function configuredUiPort(
  ...
  const raw =
    override !== undefined && override !== null && String(override).trim()
      ? override
      : process.env.ATMS_UI_PORT?.trim() || config.ui?.port || DEFAULT_UI_PORT;
...
export function configuredUiHttpPort(
  ...
  const raw =
    override !== undefined && override !== null && String(override).trim()
      ? override
      : process.env.ATMS_UI_HTTP_PORT?.trim() || config.ui?.httpPort || DEFAULT_UI_HTTP_PORT;
  const port = typeof raw === "number" ? raw : Number(raw);
...
export function detectedMachineHost(override?: string): string {
  if (override?.trim()) return override.trim();
  if (process.env.ATMS_PUBLIC_HOST?.trim()) return process.env.ATMS_PUBLIC_HOST.trim();
...
export function configuredAssetRoot(config = loadLocalConfig()): string | undefined {
  const explicit = process.env.ATMS_ASSET_DIR?.trim() || config.assets?.root?.trim();
  if (explicit) return path.resolve(explicit);
  const homeAssetRoot = path.join(getAtmsHome(), "asset");
...
export function redactConfig(config: LocalAtmsConfig): LocalAtmsConfig & { secrets: Record<string, string> } {
  const secrets = loadLocalSecrets();
  return {
    ...config,
...
export function setConfigPathValue(config: LocalAtmsConfig, key: string, value: unknown): LocalAtmsConfig {
...
function mergeConfig(defaults: LocalAtmsConfig, raw: LocalAtmsConfig): LocalAtmsConfig {
