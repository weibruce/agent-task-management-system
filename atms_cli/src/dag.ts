/**
 * Core DAG operations logic — TypeScript port of atms_cli/src/dag.rs
 */

import type { AtmsClient, BaseResponse } from "./client.js";
...
export function renderHandoffs(runId: string, handoffs: unknown[]): string {
  ...
  handoffs.forEach((h, i) => {
    const atms = h as Record<string, unknown>;
    const ty = typeof atms.type === "string" ? atms.type : "?";
    const node = typeof atms.node === "string" ? atms.node : "?";
    const port = typeof atms.port === "string" && atms.port ? `:${atms.port}` : "";
    const content = typeof atms.content === "string" ? atms.content : "";
