  ...
  try {
    for (let attempt = 0; attempt < 16; attempt += 1) {
      const candidate = path.join(
        directory,
        `.atms-pack-${process.pid}-${randomBytes(8).toString("hex")}.tmp`,
      );
      try {
...
export function generatePluginPublisherKey(
  ...
  const privateKeyPath = path.join(directory, "atms.publisher.private.pem");
  const descriptorPath = path.join(directory, "atms.publisher.json");
