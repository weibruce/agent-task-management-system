...
function componentAttrs(component: A2uiComponentV1, extra: Record<string, unknown> = {}) {
  ...
  return {
    class: ['atms-a2ui__node', `atms-a2ui__${className(component.component)}`],
...
function renderNode(
  ...
  if (ancestors.includes(component.id) || ancestors.length >= ATMS_A2UI_MAX_DEPTH) {
  ...
  const children = (value: unknown): VNode[] => {
    ...
    return collection.slice(0, ATMS_A2UI_MAX_SOURCE_ITEMS).map((item, index) => child(
      value.componentId,
  ...
  if (component.component === 'Tabs') {
    ...
    return h('section', nodeAttrs(), [
      h('div', { class: 'atms-a2ui__tab-list', role: 'tablist' }, tabs.map((tab, index) => h('button', {
        key: `tab:${index}`,
        type: 'button',
        role: 'tab',
      ...
      activeTab ? h('div', { class: 'atms-a2ui__tab-panel', role: 'tabpanel' }, [child(activeTab.child)]) : null,
    ])
  }
  if (component.component === 'Modal') {
    return h('div', nodeAttrs(), [
      h('div', {
        class: 'atms-a2ui__modal-trigger',
        onClickCapture: (event: MouseEvent) => {
          event.stopPropagation()
          modalOpen.value = true
        },
      }, [child(component.trigger)]),
      modalOpen.value ? h('div', { class: 'atms-a2ui__modal-backdrop', role: 'presentation' }, [
        h('section', { class: 'atms-a2ui__modal-panel', role: 'dialog', 'aria-modal': 'true' }, [
          h('button', {
            class: 'atms-a2ui__modal-close', type: 'button', title: 'Close', 'aria-label': 'Close',
  ...
  if (component.component === 'Divider') {
    return h('atms', nodeAttrs({ 'data-axis': component.axis === 'vertical' ? 'vertical' : 'horizontal' }))
  }
  ...
  if (component.component === 'ChoicePicker') {
    ...
    return h('fieldset', nodeAttrs({
      ...
      component.filterable ? h('input', {
        class: 'atms-a2ui__choice-filter',
        type: 'search',
      ...
      h('div', { class: 'atms-a2ui__choice-options' }, visibleOptions.map(option => {
  ...
  if (component.component === 'HrProgress') {
    ...
    return h('div', nodeAttrs({ 'data-tone': tone(component.tone, runtime, scope) }), [
      ...
      h('div', { class: 'atms-a2ui__progress-track', role: 'progressbar', 'aria-valuemin': 0, 'aria-valuemax': 100, 'aria-valuenow': value }, [
  ...
  if (component.component === 'HrStep') {
    ...
    return h('article', nodeAttrs({ 'data-tone': tone(component.tone, runtime, scope) }), [
      h('div', { class: 'atms-a2ui__step-rail', 'aria-hidden': true }, [h('span', index)]),
      h('div', { class: 'atms-a2ui__step-content' }, [
        h('header', [
  ...
  if (component.component === 'HrTable') {
    ...
    return h('div', nodeAttrs(), [h('table', { class: 'atms-a2ui__table' }, [
  ...
  throw new Error(`Generated A2UI component is not in the Atms Catalog: ${componentName}`)
}
