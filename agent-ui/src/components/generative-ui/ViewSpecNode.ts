...
import { isSafeGenerativeUiArtifactUri, type AtmsViewModelNodeV1 } from 'atms-protocol'
...
function spanStyle(node: AtmsViewModelNodeV1): CSSProperties {
  return { gridColumn: `span ${node.span}` }
}

function children(
  node: AtmsViewModelNodeV1,
...
function renderNode(
  ...
  const base = [`atms-view__node`, `atms-view__${node.type}`]
  ...
  if (node.type === 'divider') return h('atms', attrs)
  if (node.type === 'metric') {
  ...
  if (node.type === 'progress') {
    return h('div', attrs, [
      ...
      h('div', { class: 'atms-view__progress-track', role: 'progressbar', 'aria-valuemin': 0, 'aria-valuemax': 100, 'aria-valuenow': node.progress ?? 0 }, [
  ...
  if (node.type === 'table') {
    return h('div', { ...attrs, class: [...base, 'atms-view__table-scroll'] }, [h('table', { class: 'atms-view__table' }, [
...
const ViewSpecNode = defineComponent({
  name: 'ViewSpecNode',
  props: {
    model: { type: Object as PropType<AtmsViewModelNodeV1>, required: true },
