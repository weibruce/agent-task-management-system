...
import type { AtmsViewModelItemV1 } from 'atms-protocol'

const props = defineProps<{ items: AtmsViewModelItemV1[] }>()
const nodeWidth = 154
const nodeHeight = 64
const columnGap = 52
const rowGap = 14
...
const layout = computed(() => {
  ...
  const level = (id: string): number => {
    ...
    const dependencies = (item?.depends_on ?? []).filter(dependency => ids.has(dependency))
  ...
  const groups = new Map<number, AtmsViewModelItemV1[]>()
  ...
  const edges = props.items.flatMap(item => (item.depends_on ?? []).flatMap(dependency => {
    ...
    return [{ id: `${dependency}:${item.id}`, path: `M ${startX} ${startY} C ${startX + control} ${startY}, ${endX - control} ${endY}, ${endX} ${endY}`, tone: item.tone }]
  }))
  return { width, height, positions, edges }
})

function nodeStyle(item: AtmsViewModelItemV1): CSSProperties {
  const position = layout.value.positions.get(item.id) ?? { x: 0, y: 0 }
  return { left: `${position.x}px`, top: `${position.y}px`, width: `${nodeWidth}px`, height: `${nodeHeight}px`, '--progress': `${item.value ?? 0}%` }
}

function statusIcon(status = '') {
  if (['passed', 'ready', 'resolved', 'verified', 'succeeded'].includes(status)) return CheckCircle2
  if (['failed', 'blocked', 'critical'].includes(status)) return AlertTriangle
  if (['running', 'active'].includes(status)) return LoaderCircle
  return Clock3
}
</script>

<template>
  <div class="atms-view__dag-scroll">
    <div class="atms-view__dag" :style="{ width: `${layout.width}px`, height: `${layout.height}px` }">
      <svg :viewBox="`0 0 ${layout.width} ${layout.height}`" aria-hidden="true">
        <defs><marker id="atms-view-arrow" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0,0 L7,3.5 L0,7 Z" /></marker></defs>
        <path v-for="edge in layout.edges" :key="edge.id" :d="edge.path" :data-tone="edge.tone" marker-end="url(#atms-view-arrow)" />
      </svg>
      <article v-for="item in items" :key="item.id" class="atms-view__dag-node" :data-tone="item.tone" :style="nodeStyle(item)">
        <component :is="statusIcon(item.status)" :size="14" />
