...
const renderState = computed<AtmsUiWidgetDescriptor['render_state']>(() => {
...
function describeRegisteredWidget(): AtmsUiWidgetDescriptor {
...
function syncWidgetRegistration(): void {
  ...
  removeWidgetRegistration = atmsUiWidgetRegistry.register({
    document_id: props.documentId,
    widget_id: props.node.id,
...
.generative-ui-node-host {
  ...
  overflow: hidden;
  outline: none;
  box-shadow: inset 0 1px 0 var(--atms-surface-1);
...
.generative-ui-node-host:focus-within {
  ...
  box-shadow: var(--atms-shadow-panel), inset 0 1px 0 var(--atms-surface-1);
}

.generative-ui-node-host:focus-visible {
  box-shadow: 0 0 0 2px var(--atms-focus-ring);
}

.generative-ui-node-host--selected {
  border-color: var(--atms-accent);
  box-shadow: 0 0 0 2px var(--atms-focus-ring), var(--atms-shadow-panel);
}
...
.generative-ui-node-host--selected::before {
  ...
  height: 7px;
  border-radius: 999px;
  background: var(--atms-accent);
  box-shadow: 0 0 12px var(--atms-focus-ring);
  content: '';
}
...
[data-motion-profile='standard'].generative-ui-node-host--attention::after {
  ...
  inset: 0;
  z-index: 2;
  border: 1px solid var(--atms-accent);
  border-radius: inherit;
  box-shadow: inset 0 0 0 1px var(--atms-accent-soft);
  content: '';
  pointer-events: none;
...
@keyframes generative-ui-standard-attention {
  0% { border-color: var(--atms-border); }
  14% { border-color: var(--atms-accent); box-shadow: 0 0 0 3px var(--atms-focus-ring), var(--atms-shadow-panel); }
  48% { border-color: var(--atms-accent-border); box-shadow: 0 0 0 1px var(--atms-accent-soft), var(--atms-shadow-panel); }
  100% { border-color: var(--atms-border); box-shadow: inset 0 1px 0 var(--atms-surface-1); }
}
...
.generative-ui-node-host__generation-badges span[data-generation-badge='current'] {
  ...
  background: var(--atms-success-soft);
}
...
.generative-ui-node-host__historical-banner {
  ...
  gap: 8px;
  margin-bottom: 12px;
  border-left: 3px solid var(--atms-warning);
  background: var(--atms-warning-soft);
  padding: 8px 10px;
  color: var(--atms-warning);
  font-size: 12px;
...
@keyframes generative-ui-standard-update {
  0% { background-color: var(--atms-accent-soft); }
  100% { background-color: transparent; }
}

@keyframes generative-ui-standard-complete {
  0% { background-color: var(--atms-success-border); }
  45% { background-color: var(--atms-success-soft); }
  100% { background-color: transparent; }
}

@keyframes generative-ui-standard-fail {
  0% { background-color: var(--atms-danger-border); }
  45% { background-color: var(--atms-danger-soft); }
  100% { background-color: transparent; }
}
