...
import type { GenerativeUiStoredNodeV1, AtmsDeclarativeRendererV1 } from 'atms-protocol'
import { buildDeclarativeRendererModel } from '@/generative-ui/declarative-renderer'

const props = defineProps<{ node: GenerativeUiStoredNodeV1; document: AtmsDeclarativeRendererV1 }>()
...
<style scoped>
.declarative-renderer { display:grid; gap:14px; min-width:0; border:1px solid var(--atms-accent-border); border-radius:18px; background:linear-gradient(145deg,var(--atms-panel),var(--atms-bg-raised)); padding:16px; color:var(--atms-text-1); }
header h2, section h3, p, ol, ul, dl { margin:0; }
header h2 { font-size:16px; font-weight:800; }
header p { margin-top:5px; color:var(--atms-text-2); font-size:13px; white-space:pre-wrap; }
.declarative-renderer__sections { display:grid; gap:13px; }
.declarative-renderer__sections > section { display:grid; gap:7px; }
.declarative-renderer__sections h3 { color:var(--atms-accent); font-size:11px; text-transform:uppercase; letter-spacing:.08em; }
.declarative-renderer__text { display:-webkit-box; overflow:hidden; color:var(--atms-text-2); white-space:pre-wrap; -webkit-box-orient:vertical; -webkit-line-clamp:var(--max-lines); }
.declarative-renderer__list,.declarative-renderer__links { display:grid; gap:7px; padding:0; list-style:none; }
.declarative-renderer__list li { display:flex; justify-content:space-between; gap:12px; border-radius:11px; background:var(--atms-surface-1); padding:9px 10px; }
.declarative-renderer__list strong { font-size:13px; }
.declarative-renderer__list p { margin-top:3px; color:var(--atms-text-3); font-size:12px; }
.declarative-renderer__list span { align-self:start; border-radius:999px; background:var(--atms-accent-soft); padding:3px 7px; color:var(--atms-accent); font-size:10px; }
.declarative-renderer__metrics { display:grid; grid-template-columns:repeat(auto-fit,minmax(100px,1fr)); gap:8px; }
.declarative-renderer__metrics div { border-radius:11px; background:var(--atms-surface-1); padding:9px; }
.declarative-renderer__metrics dt { color:var(--atms-text-3); font-size:10px; }
.declarative-renderer__metrics dd { margin:3px 0 0; font-size:16px; font-weight:800; }
.declarative-renderer__links a { color:var(--atms-accent); font-size:12px; text-decoration:underline; text-underline-offset:3px; }
.declarative-renderer__empty { color:var(--atms-text-4); font-size:12px; }
</style>
