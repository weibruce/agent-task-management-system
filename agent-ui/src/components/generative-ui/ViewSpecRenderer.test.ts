...
import type { GenerativeUiCompositionItemV1, GenerativeUiStoredNodeV1, GenerativeUiSurfaceContextV1, AtmsViewSpecV1 } from 'atms-protocol'
...
const placement: GenerativeUiCompositionItemV1 = {
  node_id: 'com.atms.core:view-one', node_revision: 1, surface: 'result', variant: 'detail', rank: 1,
...
function node(view: AtmsViewSpecV1): GenerativeUiStoredNodeV1 {
  return {
    ir_version: 1,
...
function mount(view: AtmsViewSpecV1, inputContext = context, expanded = false): HTMLElement {
...
describe('ViewSpecRenderer', () => {
  it('renders a runtime-authored component tree without a scenario component', () => {
    ...
    expect(mounted.querySelector('.atms-view__metric')?.textContent).toContain('3')
    expect(mounted.querySelector('.atms-view__progress-track')?.getAttribute('aria-valuenow')).toBe('72')
    expect(mounted.querySelectorAll('.atms-view__list li')).toHaveLength(1)
    expect(mounted.querySelectorAll('.atms-view__timeline li')).toHaveLength(1)
    expect(mounted.querySelectorAll('.atms-view__bar_chart > div')).toHaveLength(1)
    expect(mounted.querySelectorAll('.atms-view__dag-node')).toHaveLength(1)
  })
  ...
  it('keeps markdown executable content inert and dispatches only symbolic actions', async () => {
    ...
    expect(mounted.querySelector('.atms-view__markdown strong')?.textContent).toBe('Grounded')
    expect(mounted.querySelector('.atms-view__markdown a')).toBeNull()
    mounted.querySelector<HTMLButtonElement>('.atms-view__action')?.click()
    await nextTick()
    expect(mounted.querySelector('.atms-view__action')?.textContent).toBe('Inspect')
  ...
  it('renders the remaining bounded content and layout primitives from one generic tree', () => {
    ...
    expect(mounted.querySelector('.atms-view__section > header')?.textContent).toBe('Evidence')
    expect(mounted.querySelector('.atms-view__icon svg')).toBeTruthy()
    expect(mounted.querySelector('.atms-view__divider')).toBeTruthy()
    expect(mounted.querySelectorAll('.atms-view__table tbody tr')).toHaveLength(1)
    expect(mounted.querySelector('.atms-view__repeat')?.textContent).toContain('CI')
    expect(mounted.querySelector<HTMLDetailsElement>('.atms-view__disclosure')?.open).toBe(true)
    expect(mounted.querySelector<HTMLAnchorElement>('.atms-view__link')?.href).toBe('https://example.com/evidence')
  })
  ...
  it('marks compact grids and repeats so the renderer can balance odd two-column content', () => {
    ...
    expect(mounted.querySelector('.atms-view-spec')?.getAttribute('data-viewport')).toBe('compact')
    expect(mounted.querySelector<HTMLElement>('.atms-view__grid')?.style.getPropertyValue('--columns')).toBe('3')
    expect(mounted.querySelector<HTMLElement>('.atms-view__grid')?.style.getPropertyValue('--compact-columns')).toBe('2')
    expect(mounted.querySelector('.atms-view__grid')?.getAttribute('data-compact-columns')).toBe('2')
    expect(mounted.querySelectorAll('.atms-view__grid > .atms-view__node')).toHaveLength(3)
    expect(mounted.querySelector<HTMLElement>('.atms-view__repeat')?.style.getPropertyValue('--columns')).toBe('3')
    expect(mounted.querySelector<HTMLElement>('.atms-view__repeat')?.style.getPropertyValue('--compact-columns')).toBe('2')
    expect(mounted.querySelector('.atms-view__repeat')?.getAttribute('data-compact-columns')).toBe('2')
  })
  ...
  it('opens secondary disclosure content when the host expands the Block', () => {
    ...
    expect(mounted.querySelector('.atms-view-spec')?.getAttribute('data-expanded')).toBe('true')
    expect(mounted.querySelector<HTMLDetailsElement>('.atms-view__disclosure')?.open).toBe(true)
  })
  ...
  it('previews image artifacts and renders HTML artifacts without a fixed footer', async () => {
    ...
    const image = mounted.querySelector<HTMLButtonElement>('button.atms-view__artifact')!
  ...
  it('lets a root HTML artifact inherit the full canvas height', () => {
    ...
    const artifact = mounted.querySelector('.atms-view-spec > div.atms-view__artifact')
    expect(artifact?.querySelector('iframe')).toBeTruthy()
    expect(artifact?.children).toHaveLength(1)
    expect(rendererSource).toContain('.atms-view-spec > div.atms-view__artifact { height:100%; }')
    expect(rendererSource).toContain('.atms-view-spec > div.atms-view__artifact > iframe { pointer-events:auto; }')
  })
})
