...
const placement: GenerativeUiCompositionItemV1 = {
  node_id: 'com.atms.core:a2ui-one',
  node_revision: 1,
  surface: 'result',
  variant: 'detail',
  rank: 1,
...
function node(a2ui: AtmsA2uiSurfaceV1, content: Record<string, unknown> = {}): GenerativeUiStoredNodeV1 {
  return {
    ir_version: 1,
    id: placement.node_id,
    kind: 'com.atms.core/generated_view',
    kind_version: 1,
    owner: { id: 'com.atms.core', version: '0.1.0' },
    surface: 'result',
    importance: 'primary',
    content,
    a2ui,
...
function surface(components: AtmsA2uiSurfaceV1['components']): AtmsA2uiSurfaceV1 {
  return { version: 'v1.0', catalogId: ATMS_A2UI_CATALOG_ID, components }
}
...
describe('A2uiRenderer', () => {
  it('renders Text, absolute and relative bindings, template children, and @index', () => {
    ...
    expect(mounted.querySelector('.atms-a2ui__text strong')?.textContent).toBe('Release readiness')
    ...
    expect(mounted.querySelector('.atms-a2ui__text a')).toBeNull()
  ...
  it('renders connected rich steps with template-relative content', () => {
    ...
    const steps = [...mounted.querySelectorAll<HTMLElement>('.atms-a2ui__step')]
    expect(steps).toHaveLength(2)
    expect(steps[0]?.querySelector('.atms-a2ui__step-rail')?.textContent).toBe('1')
    expect(steps[1]?.querySelector('.atms-a2ui__step-rail')?.textContent).toBe('2')
    expect(steps[1]?.querySelector('header')?.textContent).toContain('Anubis')
    expect(steps[1]?.dataset.tone).toBe('positive')
    expect(rendererSource).toContain('.atms-a2ui__step-rail::after')
  })
  ...
  it('applies the semantic palette to tone-bearing collection items', () => {
    ...
    expect([...mounted.querySelectorAll<HTMLElement>('.atms-a2ui__bar-chart > div')].map(item => item.dataset.tone)).toEqual([
      'info', 'positive', 'warning', 'critical',
    ])
    expect(rendererSource).toContain(".atms-a2ui [data-tone='info'] { --tone: var(--atms-info); }")
    expect(rendererSource).toContain(".atms-a2ui [data-tone='positive'] { --tone: var(--atms-success); }")
  })
  ...
  it('renders camelCase Metric, Table, DAG, and passive Artifact components', async () => {
    ...
    expect(mounted.querySelector('.atms-a2ui__metric')?.textContent).toContain('3')
    expect(mounted.querySelectorAll('.atms-a2ui__table tbody tr')).toHaveLength(1)
    expect(mounted.querySelectorAll('.atms-a2ui__dag-node')).toHaveLength(2)
    const source = mounted.querySelector<HTMLAnchorElement>('.atms-a2ui__link')!
    const remoteImage = mounted.querySelector<HTMLImageElement>('[data-a2ui-id="remote"]')!
    const image = mounted.querySelector<HTMLImageElement>('.atms-a2ui__artifact img')!
    const frame = mounted.querySelector<HTMLIFrameElement>('.atms-a2ui__artifact iframe')!
    ...
    expect(frame.hasAttribute('data-atms-artifact-frame')).toBe(true)
    expect(frame.parentElement?.querySelector(':scope > button')).toBeNull()
    expect(frame.parentElement?.querySelector(':scope > p')).toBeNull()

    mounted.querySelector<HTMLButtonElement>('button.atms-a2ui__artifact')?.click()
    await nextTick()
  ...
  it('lets a root HTML Artifact inherit the full canvas height', () => {
    ...
    const artifact = mounted.querySelector('.atms-a2ui > div.atms-a2ui__artifact')
    ...
    expect(rendererSource).toContain('.atms-a2ui > div.atms-a2ui__artifact { height: 100%; }')
    expect(rendererSource).toContain('.atms-a2ui > div.atms-a2ui__artifact > iframe,')
  ...
  it('lets one HTML Artifact nested below root content fill the remaining canvas height', () => {
    ...
    const column = mounted.querySelector<HTMLElement>('.atms-a2ui > .atms-a2ui__column')
    const artifact = column?.querySelector<HTMLElement>(':scope > div.atms-a2ui__artifact')
    expect(column?.dataset.fillHtmlArtifact).toBe('preview')
    expect(artifact?.dataset.artifactKind).toBe('html')
    expect(artifact?.querySelector('iframe')).toBeTruthy()
    expect(rendererSource).toContain('.atms-a2ui > .atms-a2ui__column[data-fill-html-artifact]')
    expect(rendererSource).toContain("div.atms-a2ui__artifact[data-artifact-kind='html']")
  })
  ...
  it('does not promote multiple nested HTML Artifacts into one primary preview', () => {
    ...
    expect(mounted.querySelector('.atms-a2ui > .atms-a2ui__column')?.hasAttribute('data-fill-html-artifact')).toBe(false)
  })
  ...
  it('renders passive Actor media, metrics, comparisons, timelines, and routes without active controls', () => {
    ...
    expect(mounted.querySelector('.atms-a2ui__metric')?.textContent).toContain('84')
    expect(mounted.querySelectorAll('.atms-a2ui__timeline > li')).toHaveLength(1)
    expect(mounted.querySelectorAll('.atms-a2ui__bar-chart > div')).toHaveLength(2)
    expect(mounted.querySelectorAll('.atms-a2ui__dag-node')).toHaveLength(2)
    expect(mounted.querySelector('form, input, .atms-a2ui__button')).toBeNull()
  })
  ...
  it('keeps the stable A2UI root, local tab state, and focus across node revisions', async () => {
    ...
    expect(detailsTab.getAttribute('aria-selected')).toBe('true')
    expect(document.activeElement).toBe(detailsTab)
    expect(root.querySelector('.atms-a2ui__tab-panel')?.textContent).toContain('Updated details')
  })
  ...
  it('keeps invalid inputs editable, binds writable values, and disables failed actions', async () => {
    ...
    expect(mounted.querySelectorAll('[data-a2ui-id="choice"] .atms-a2ui__choice-options label')).toHaveLength(2)
    ...
    await nextTick()
    expect(mounted.querySelectorAll('[data-a2ui-id="choice"] .atms-a2ui__choice-options label')).toHaveLength(1)
  ...
  it('marks compact mobile output, evaluates disclosure.open, and expands details with the host', () => {
    ...
    expect(mounted.querySelector('.atms-a2ui')?.getAttribute('data-device')).toBe('phone')
    expect(mounted.querySelector('.atms-a2ui')?.getAttribute('data-viewport')).toBe('compact')
    expect(mounted.querySelector('.atms-a2ui')?.getAttribute('data-expanded')).toBe('true')
    expect(mounted.querySelector('.atms-a2ui__grid')?.getAttribute('data-compact-columns')).toBe('3')
    expect(rendererSource).toContain(".atms-a2ui__grid[data-compact-columns='3'] .atms-a2ui__image[data-variant='smallFeature']")
    expect(mounted.querySelector<HTMLDetailsElement>('.atms-a2ui__disclosure')?.open).toBe(true)
  })
  ...
  it('keeps one host scroll owner and bounded internal overflow with reduced-motion fallbacks', () => {
    ...
    expect(rendererSource).toContain('.atms-a2ui__table')
    expect(rendererSource).toContain('overflow-x: auto;')
    expect(rendererSource).toContain('overflow-y: visible;')
    expect(rendererSource).toContain('.atms-a2ui__dag-scroll')
