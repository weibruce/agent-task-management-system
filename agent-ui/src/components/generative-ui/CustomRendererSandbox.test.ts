...
const source = 'export function render(payload) { return { version: "v1.0", catalogId: "https://atms.dev/a2ui/catalogs/core/v1", components: [{ id: "root", component: "Text", text: String(payload?.node?.id || "safe") }] } }'
...
describe('CustomRendererSandbox', () => {
  it('creates an opaque bootstrap iframe with no plugin DOM or network authority', async () => {
    ...
    expect(frame!.srcdoc).not.toContain('atms.custom-renderer.action')
  ...
  it('hands parent-validated Worker A2UI to the shared renderer and emits only action name', async () => {
    ...
    expect(root.querySelector('.atms-a2ui')?.textContent).toContain('Sandboxed')
    expect(onSurfaceActions).toHaveBeenCalledWith(['approve'])
    root.querySelector<HTMLButtonElement>('.atms-a2ui__button > button')?.click()
    await nextTick()
