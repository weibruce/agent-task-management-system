...
<template>
  <div class="atms-a2ui__dag-scroll">
    <div class="atms-a2ui__dag" :style="{ width: `${layout.width}px`, height: `${layout.height}px` }">
      <svg :viewBox="`0 0 ${layout.width} ${layout.height}`" aria-hidden="true">
        <defs>
          <marker id="atms-a2ui-arrow" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto">
        ...
        <path
          ...
          :d="edge.path"
          :data-tone="edge.tone"
          marker-end="url(#atms-a2ui-arrow)"
        />
      </svg>
      <article
        v-for="item in items"
        :key="item.id"
        class="atms-a2ui__dag-node"
        :data-tone="item.tone"
