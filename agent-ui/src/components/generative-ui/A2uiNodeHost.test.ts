...
describe('A2UI action bridge', () => {
  it('maps event.name back to the authoritative Atms Action Registry', async () => {
    ...
    expect(root.querySelectorAll('.atms-a2ui__button > button')).toHaveLength(1)
  ...
  it('shows the readable node fallback when the A2UI renderer rejects rich content', async () => {
    ...
    expect(root.querySelector('.atms-a2ui')).toBeNull()
