...
import { atmsUiWidgetRegistry } from '@/browser-tools/widget-registry'
...
describe('GenerativeUiNodeHost', () => {
  it('registers exact widget identity and exposes semantic focus and expansion', async () => {
    ...
    expect(atmsUiWidgetRegistry.describe(target)).toMatchObject({
      ...target,
      kind: 'com.example.plugin/card',
      visible: true,
      focused: false,
    ...
    atmsUiWidgetRegistry.focus(target)
    expect(document.activeElement).toBe(host)
    expect(atmsUiWidgetRegistry.setExpanded(target, true)).toMatchObject({ expanded: true })
    await nextTick()
  ...
  it('fails closed for a widget inside a hidden ancestor', async () => {
    ...
    expect(atmsUiWidgetRegistry.describe(target).visible).toBe(false)
    expect(() => atmsUiWidgetRegistry.focus(target)).toThrow(/not currently visible/)
    expect(() => atmsUiWidgetRegistry.setExpanded(target, true)).toThrow(/not currently visible/)
  })
  ...
  it('removes historical content from the current-revision widget registry', async () => {
    ...
    expect(atmsUiWidgetRegistry.describe(target).widget_revision).toBe(3)

    atmsUiWidgetRegistry.setExpanded(target, true)
    await nextTick()
    ...
    expect(atmsUiWidgetRegistry.snapshot().widgets).toEqual([])
    expect(() => atmsUiWidgetRegistry.describe(target)).toThrow(/not currently rendered/)
  ...
  it('keeps the body locked until every expanded widget is restored', async () => {
    ...
    atmsUiWidgetRegistry.setExpanded(target('node-a'), true)
    atmsUiWidgetRegistry.setExpanded(target('node-b'), true)
    await nextTick()
    expect(document.body.classList.contains('generative-ui-node-expanded')).toBe(true)

    atmsUiWidgetRegistry.setExpanded(target('node-a'), false)
    await nextTick()
    expect(document.body.classList.contains('generative-ui-node-expanded')).toBe(true)

    atmsUiWidgetRegistry.setExpanded(target('node-b'), false)
    await nextTick()
