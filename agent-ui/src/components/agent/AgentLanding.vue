...
<template>
  <div class="flex flex-col items-center justify-center h-screen bg-[var(--atms-bg)] px-4">
    <!-- Title -->
    <div class="text-center mb-10">
      <h1 class="text-4xl font-bold text-[var(--atms-text-1)] tracking-tight mb-2">
        Atms
      </h1>
      <p class="text-sm text-[var(--atms-text-3)]">
    ...
    <div class="w-full max-w-[620px]">
      <div class="mb-2 flex items-center justify-end gap-2 text-xs">
        <span class="text-[var(--atms-text-4)]">Manager</span>
        <select
          :value="store.managerProviderName"
          class="h-8 rounded-md border border-[var(--atms-border)] bg-[var(--atms-bg-raised)] px-2 text-[var(--atms-text-2)] outline-none"
      ...
      <div class="flex items-end gap-2 bg-[var(--atms-surface-1)] rounded-2xl border border-[var(--atms-border)] p-3">
        <textarea
          v-model="inputValue"
          class="flex-1 resize-none bg-transparent text-sm text-[var(--atms-text-1)] placeholder-[var(--atms-text-3)] outline-none max-h-32 min-h-[48px] py-2 px-2"
          :rows="2"
        ...
        <button
          ...
          :class="isSending
            ...
            : 'bg-[var(--atms-accent)] text-[var(--atms-on-accent)] hover:opacity-85'"
      ...
      <button
        class="mt-3 flex w-full items-center justify-center gap-2 rounded-2xl border border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)] px-4 py-3 text-sm text-[var(--atms-accent)] transition hover:bg-[color-mix(in_srgb,var(--atms-accent)_15%,transparent)]"
    ...
    <div class="flex flex-wrap gap-2 mt-6 max-w-[620px] justify-center">
      <button
        ...
        :key="hint"
        class="px-3 py-1.5 rounded-lg text-xs text-[var(--atms-text-3)] bg-[var(--atms-surface-1)] border border-[var(--atms-border)] hover:text-[var(--atms-text-1)] hover:bg-[var(--atms-surface-2)] transition-colors"
    ...
    <!-- History entry -->
    <button
      class="flex items-center gap-1.5 mt-6 text-xs text-[var(--atms-text-4)] hover:text-[var(--atms-text-3)] transition-colors"
      @click="enterHistory"
    >
