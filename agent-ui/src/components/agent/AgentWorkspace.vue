...
function statusColor(status: string): string {
  switch (status) {
    case 'running': return 'text-[var(--atms-success)]'
    case 'completed': return 'text-[var(--atms-info)]'
...
const experienceIngestClass = computed(() => {
  const status = experienceIngest.value?.status
  if (status === 'applied') return 'border-[var(--atms-success-border)] bg-[var(--atms-success-soft)] text-[var(--atms-success)]'
  if (status === 'failed') return 'border-[var(--atms-danger-border)] bg-[var(--atms-danger-soft)] text-[var(--atms-danger)]'
  if (status === 'pending' || status === 'running') return 'border-[var(--atms-warning-border)] bg-[var(--atms-warning-soft)] text-[var(--atms-warning)]'
  return 'border-[var(--atms-border)] bg-[var(--atms-surface-1)] text-[var(--atms-text-3)]'
})
...
<template>
  <div class="agent-workspace flex h-full flex-col bg-transparent">
    ...
    <div class="flex-shrink-0 border-b border-[var(--atms-border)] px-4 py-3">
      <div class="mb-2 text-[11px] font-medium uppercase tracking-[0.2em] text-[var(--atms-text-3)]">{{ t('agent.inspector.title') }}</div>
      ...
      <div v-if="store.dagExecution" class="flex flex-wrap items-center gap-2 text-[10px]">
        <span
          :class="cn(
            ...
            store.isFailed ? 'bg-[var(--atms-danger-soft)] text-[var(--atms-danger)]' :
        ...
        <span class="font-mono text-[var(--atms-text-4)]">{{ store.currentRunId?.slice(-8) }}</span>
        <span class="text-[var(--atms-text-4)]">·</span>
        <span class="text-[var(--atms-text-4)]">{{ t('shell.workspace.nodes', { count: store.nodes.length }) }}</span>
        <span class="text-[var(--atms-text-4)]">·</span>
        <span class="text-[var(--atms-text-4)]">
        ...
        <button
          ...
          class="inline-flex items-center gap-1 rounded-full border border-[var(--atms-border)] px-2 py-1 font-medium text-[var(--atms-text-3)] transition-colors hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-2)]"
      ...
      <div v-else class="text-[11px] text-[var(--atms-text-4)]">{{ t('agent.inspector.noDag') }}</div>
    </div>

    <!-- Tab bar -->
    <div class="flex flex-shrink-0 gap-1 overflow-x-auto border-b border-[var(--atms-border)] px-3 py-2">
      <button
        v-for="tab in tabs"
        :key="tab.key"
        :class="cn(
          ...
          store.inspectorTab === tab.key
            ? 'rounded-full border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)] text-[var(--atms-accent)]'
            : 'rounded-full border-transparent text-[var(--atms-text-3)] hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-2)]'
        )"
    ...
    <div class="min-h-0 flex-1 overflow-y-auto">
      ...
      >
        <div class="max-w-[220px] text-center">
          <Activity class="mx-auto mb-3 h-7 w-7 text-[var(--atms-text-4)]" />
          <div class="text-xs font-medium text-[var(--atms-text-3)]">{{ t('shell.workspace.emptyTitle') }}</div>
          <div class="mt-1 text-[11px] leading-5 text-[var(--atms-text-4)]">
            {{ t('shell.workspace.emptyDescription') }}
          </div>
          <button
            class="mt-4 rounded-full border border-[var(--atms-border)] px-3 py-1.5 text-[11px] text-[var(--atms-text-2)] transition-colors hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]"
      ...
      <div v-else-if="store.inspectorTab === 'nodes'">
        <div v-if="store.nodes.length === 0" class="flex items-center justify-center h-full">
          <div class="text-center space-y-2">
            <Bot class="h-8 w-8 mx-auto opacity-15 text-[var(--atms-text-4)]" />
            <div class="text-xs text-[var(--atms-text-4)]">{{ t('agent.inspector.noNodes') }}</div>
          </div>
        </div>
        ...
        <div v-else class="py-1">
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="text-xs font-medium text-[var(--atms-text-1)] truncate">
                {{ getAgentPersona(node.agent_name).name }}
              </div>
              <div class="text-[10px] text-[var(--atms-text-4)] truncate">{{ node.name }}</div>
            </div>
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
              ...
              <div v-if="node.context_usage_pct != null" class="w-10">
                <div class="flex justify-between text-[9px] mb-0.5">
                  <span class="text-[var(--atms-text-4)]">ctx</span>
                  <span :class="contextUsageText(node.context_usage_pct)">
                    {{ node.context_usage_pct }}%
                  </span>
                </div>
                <div class="h-1 rounded-full bg-[var(--atms-surface-2)]">
                  <div
      ...
      <div v-else-if="store.inspectorTab === 'logs'" class="flex flex-col h-full">
        ...
        <div v-if="!store.selectedNode" class="flex-1 flex items-center justify-center">
          <div class="text-center space-y-2">
            <FileText class="h-8 w-8 mx-auto opacity-20 text-[var(--atms-text-1)]" />
            <div class="text-xs text-[var(--atms-text-4)]">{{ t('agent.inspector.selectNodeLogs') }}</div>
          </div>
        </div>
        ...
        <template v-else>
          ...
          <div class="flex items-center gap-2 border-b border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 py-2.5 flex-shrink-0">
            <div
            ...
            <div class="flex-1 min-w-0">
              <span class="text-[11px] font-medium text-[var(--atms-text-1)]">
                {{ getAgentPersona(store.selectedNode.agent_name).name }}
              </span>
              <span class="text-[10px] text-[var(--atms-text-4)] ml-1">{{ store.selectedNode.name }}</span>
            </div>
            <div
            ...
            >
              ...
              <div class="w-10 h-1 rounded-full bg-[var(--atms-surface-2)]">
                <div
                  class="h-full rounded-full transition-all"
    ...
    <div
      ...
      class="flex-shrink-0 border-t border-[var(--atms-border)] px-3 py-1.5 bg-[var(--atms-surface-1)]"
    >
      <div class="flex items-center gap-3 text-[10px] text-[var(--atms-text-3)]">
        <span>Tokens:</span>
...
<style scoped>
.agent-workspace {
  --workspace-radius: var(--atms-radius-lg);
}
