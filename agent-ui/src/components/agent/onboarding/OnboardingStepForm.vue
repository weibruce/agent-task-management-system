      ...
      <div v-else class="onboarding-step-form__grid">
        <label class="onboarding-step-form__field">
          ...
          >
            ...
            <option v-for="c in credentials" :key="c.key" :value="c.key" class="bg-[var(--atms-bg-raised)] text-[var(--atms-text-1)]">
              {{ credentialLabel(c) }}
            </option>
          </select>
        </label>
        ...
        <label class="onboarding-step-form__field">
          ...
          >
            ...
            <option v-for="m in displayModels" :key="m.id" :value="m.id" class="bg-[var(--atms-bg-raised)] text-[var(--atms-text-1)]">
...
.onboarding-step-form__empty {
  ...
  border: 1px dashed var(--atms-border-strong);
  background: var(--atms-surface-1);
  color: var(--atms-text-3);
  font-size: 0.8rem;
  line-height: 1.5;
...
.onboarding-step-form__input:focus {
  border-color: color-mix(in srgb, var(--atms-accent) 50%, transparent);
  background: var(--atms-accent-soft);
}
...
.onboarding-step-form__submit:not(.onboarding-step-form__submit--disabled):hover {
  background: color-mix(in srgb, var(--atms-accent) 22%, transparent);
