...
<template>
  <div class="agent-chat-surface flex h-full flex-col bg-transparent">
    <div class="agent-chat-header flex h-16 flex-shrink-0 items-center justify-between border-b border-[var(--atms-border)] px-6">
      <div>
        <div class="text-[11px] uppercase tracking-[0.22em] text-[var(--atms-text-3)]">Manager</div>
        <div class="mt-1 text-lg font-semibold text-[var(--atms-text-1)]">Runtime</div>
      </div>
      <div class="flex items-center gap-2 text-xs">
        <select
          :value="store.managerProviderName"
          class="h-9 rounded-full border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 text-xs text-[var(--atms-text-2)] outline-none focus:border-[var(--atms-border-strong)]"
    ...
    >
      <!-- Empty state -->
      <div v-if="store.chatMessages.length === 0" class="flex items-center justify-center h-full text-[var(--atms-text-3)] text-sm">
      ...
      <template v-for="msg in store.chatMessages" :key="msg.id">
        ...
        <div v-if="msg.role === 'system' || msg.role === 'assistant'" class="flex gap-3">
          <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-2xl border border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)]">
            <Bot class="h-4 w-4 text-[var(--atms-accent)]" />
          </div>
          <div class="flex-1 min-w-0 space-y-1">
            <!-- Text -->
            <div v-if="msg.type === 'text'" class="agent-markdown rounded-[22px] border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-4 py-3 text-[15px] leading-7 text-[var(--atms-text-1)] break-words shadow-[inset_0_1px_0_var(--atms-border)]" v-html="renderMarkdown(msg.content)" />

            <!-- Thinking summary -->
            <div
              v-if="msg.type === 'thinking'"
              class="inline-flex h-8 items-center gap-1.5 rounded-full border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3"
            >
              <span class="h-1.5 w-1.5 rounded-full bg-[var(--atms-accent)] animate-pulse" />
              <span class="h-1.5 w-1.5 rounded-full bg-[var(--atms-accent)] animate-pulse [animation-delay:120ms]" />
              <span class="h-1.5 w-1.5 rounded-full bg-[var(--atms-accent)] animate-pulse [animation-delay:240ms]" />
            </div>
            ...
            <div
              ...
              :class="cn(
                ...
                isInvokeRun(msg) && extractRunId(msg)
                  ? 'border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)] cursor-pointer hover:border-[var(--atms-accent)]'
            ...
            >
              ...
              <template v-if="isInvokeRun(msg) && extractRunId(msg)">
                <div class="flex items-center gap-2 px-3 py-2">
                  <GitBranch class="h-4 w-4 text-[var(--atms-accent)] flex-shrink-0" />
                  <div class="flex-1 min-w-0">
                    <div class="text-xs font-medium text-[var(--atms-accent)]">DAG Run</div>
                    <div class="text-[10px] text-[var(--atms-text-4)] font-mono">{{ extractRunId(msg)!.slice(-12) }}</div>
                  </div>
                  <ExternalLink class="h-3 w-3 text-[var(--atms-text-4)]" />
                </div>
              </template>
              <!-- Regular tool call -->
              <template v-else>
                <button
                  class="flex items-center gap-2 w-full px-3 py-2.5 text-xs text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-1)] transition-colors"
                  @click="toggleExpanded(msg.id)"
                >
                  <ChevronDown v-if="expandedToolCalls.has(msg.id)" class="h-3.5 w-3.5" />
                  <ChevronUp v-else class="h-3.5 w-3.5" />
                  <Wrench class="h-3.5 w-3.5 text-[var(--atms-accent)]" />
                  <span class="text-[var(--atms-text-4)]">{{ t('shell.chat.toolCalls') }}</span>
                  <span class="font-medium text-[var(--atms-text-1)]">{{ msg.toolName }}</span>
                  <span v-if="msg.toolSummary" class="text-[var(--atms-text-4)]">{{ msg.toolSummary }}</span>
                  <span
                    :class="cn(
                      'ml-auto text-[10px]',
                      msg.status === 'completed' && 'text-[var(--atms-success)]',
                      msg.status === 'failed' && 'text-[var(--atms-danger)]',
                      msg.status === 'pending' && 'text-[var(--atms-warning)]'
                    )"
                  >
                ...
                <div
                  ...
                  class="border-t border-[var(--atms-border)] px-3 py-2 text-xs text-[var(--atms-text-3)]"
                >
                  {{ toolResultPreview(msg.toolResult) }}
                </div>
                <div v-if="expandedToolCalls.has(msg.id)" class="px-3 pb-3 text-xs text-[var(--atms-text-3)] border-t border-[var(--atms-border)] mt-1 pt-2 whitespace-pre-wrap">
                  <template v-if="msg.content">
                    <div class="text-[var(--atms-text-2)]">Input</div>
                    <div>{{ msg.content }}</div>
                  </template>
                  <template v-if="msg.toolResult">
                    <div class="mt-2 text-[var(--atms-text-2)]">Result</div>
                    <div class="max-h-64 overflow-y-auto rounded-2xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-2">{{ formatToolResult(msg.toolResult) }}</div>
                  </template>
                </div>
              </template>
            </div>
            ...
            <!-- Status -->
            <div v-if="msg.type === 'status'" class="text-xs text-[var(--atms-text-4)] italic">
              {{ msg.content }}
            </div>
          </div>
        </div>
        ...
        <div v-else-if="msg.role === 'user'" class="flex gap-3 flex-row-reverse">
          <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-2xl border border-[var(--atms-border)] bg-[var(--atms-surface-2)]">
            <User class="h-4 w-4 text-[var(--atms-text-2)]" />
          </div>
          <div class="flex-1 min-w-0">
            <div class="ml-auto max-w-[86%] rounded-[22px] rounded-tr-md border border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)] px-4 py-3 text-sm leading-relaxed text-[var(--atms-text-1)] break-words">
              {{ msg.content }}
            </div>
          </div>
        </div>
        ...
        <div v-if="store.managerResponding && !store.hasWsStreamed()" class="flex gap-3">
          ...
          <div class="inline-flex h-8 items-center gap-1.5 rounded-full border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3">
            <span class="h-1.5 w-1.5 rounded-full bg-[var(--atms-accent)] animate-pulse" />
            <span class="h-1.5 w-1.5 rounded-full bg-[var(--atms-accent)] animate-pulse [animation-delay:120ms]" />
            <span class="h-1.5 w-1.5 rounded-full bg-[var(--atms-accent)] animate-pulse [animation-delay:240ms]" />
          </div>
    ...
    <!-- Input Area -->
    <div class="px-7 pb-6 pt-3 bg-gradient-to-t from-[var(--atms-bg)] via-[color-mix(in_srgb,var(--atms-bg)_92%,transparent)] to-[color-mix(in_srgb,var(--atms-bg)_20%,transparent)]">
      <div class="agent-chat-composer mx-auto flex max-w-[820px] items-end gap-2 rounded-[26px] border border-[var(--atms-border)] bg-[var(--atms-panel)] p-3 shadow-2xl backdrop-blur-xl">
        <button
          :class="cn(
            'flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center transition-colors',
            voiceMode ? 'bg-[var(--atms-accent-soft)] text-[var(--atms-accent)] hover:bg-[var(--atms-accent-border)]' : 'bg-transparent text-[var(--atms-text-3)] hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]'
          )"
        ...
        <div v-if="voiceMode" class="min-h-[54px] w-[240px] shrink-0 rounded-[20px] border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 py-2">
          <div class="flex h-8 items-center gap-1 overflow-hidden">
            <span
              v-for="(bar, index) in waveformBars"
              :key="index"
              class="w-1 rounded-full bg-gradient-to-t from-teal-500 via-[var(--atms-accent)] to-[var(--atms-success)] transition-[height,opacity] duration-75"
              :style="{ height: `${Math.max(10, bar * 34)}px`, opacity: 0.38 + bar * 0.62 }"
            />
          </div>
          <div class="mt-1 flex items-center justify-between gap-3 text-[11px]">
            <span class="flex items-center gap-1.5 text-[var(--atms-text-2)]">
              <AudioLines class="h-3.5 w-3.5 text-[var(--atms-accent)]" />
              {{ voiceStatus }}
            </span>
            <span class="font-mono text-[var(--atms-text-3)]">{{ voiceLevelPct }}</span>
          </div>
          <div v-if="voiceError" class="mt-1 text-[11px] text-[var(--atms-danger)]">{{ voiceError }}</div>
        </div>
        <textarea
          v-model="inputValue"
          :class="cn(
            'flex-1 resize-none bg-transparent text-sm text-[var(--atms-text-1)] placeholder:text-[var(--atms-text-4)] outline-none',
        ...
        <button
          :class="cn(
            'flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center transition-colors',
            isSending
              ? 'bg-[var(--atms-danger-soft)] text-[var(--atms-danger)] hover:bg-[var(--atms-danger-soft)]'
              : 'bg-[var(--atms-accent)] text-[var(--atms-on-accent)] hover:bg-[color-mix(in_srgb,var(--atms-accent)_85%,white)]'
          )"
...
@keyframes pulse-ring {
  0% { box-shadow: 0 0 0 0 var(--atms-success-border); }
...
.agent-markdown :deep(blockquote) {
  border-left: 3px solid var(--atms-border-strong);
  padding-left: 0.75em;
  margin: 0.5em 0;
...
.agent-markdown :deep(th) { background: var(--atms-surface-1); font-weight: 600; }
.agent-markdown :deep(atms) { border-color: var(--atms-border); margin: 0.75em 0; }
.agent-markdown :deep(strong) { color: var(--atms-text-1); }
</style>
