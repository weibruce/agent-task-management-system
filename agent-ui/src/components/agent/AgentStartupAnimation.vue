...
const nodes = computed<AnimNode[]>(() => {
  ...
  return labels.value.slice(0, props.nodeCount).map((label, i) => ({
    ...
    label,
    icon: ICONS[i] || Bot,
    color: COLORS[i] || 'var(--atms-text-3)',
    x: positions[i]?.x ?? CX,
    y: positions[i]?.y ?? CY,
  }))
})
...
<template>
  <div class="flex items-center justify-center h-full w-full bg-[var(--atms-bg)] overflow-hidden">
    <svg :viewBox="`0 0 ${W} ${H}`" class="max-w-[700px] max-h-[80vh]" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
      <defs>
        ...
        <linearGradient id="edgeGrad" x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stop-color="var(--atms-info)" stop-opacity="0.5" />
          <stop offset="100%" stop-color="var(--atms-success)" stop-opacity="0.5" />
        </linearGradient>
      </defs>

      <!-- Background grid -->
      <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
        <path d="M 40 0 L 0 0 0 40" fill="none" stroke="var(--atms-border)" stroke-width="0.5" />
      </pattern>
