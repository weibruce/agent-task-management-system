...
function healthClass(value: 'unknown' | 'healthy' | 'failed'): string {
  if (value === 'healthy') return 'bg-[var(--atms-success-soft)] text-[var(--atms-success)]'
  if (value === 'failed') return 'bg-[var(--atms-danger-soft)] text-[var(--atms-danger)]'
  return 'bg-[var(--atms-surface-2)] text-[var(--atms-text-3)]'
}
...
<template>
  <div class="flex h-full min-h-0 flex-col">
    <div class="flex-shrink-0 border-b border-[var(--atms-border)] px-3 py-2">
      <div class="flex items-center justify-between gap-3">
        <div class="flex items-center gap-1.5 text-[11px] font-medium text-[var(--atms-text-1)]">
          <Settings class="h-3.5 w-3.5 text-[var(--atms-accent)]" />
          {{ t('agent.settings.title') }}
        </div>
        <button
          class="rounded p-1 text-[var(--atms-text-3)] hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]"
    ...
    <div class="min-h-0 flex-1 overflow-y-auto p-3">
      <div class="space-y-3">
        <section class="rounded-md border border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)] p-3">
          <div class="mb-2 flex items-center justify-between gap-2">
            <div class="flex items-center gap-1.5 text-[10px] text-[var(--atms-accent)]">
              <Settings class="h-3 w-3" />
              Manager runtime
            </div>
            <span class="text-[10px] text-[var(--atms-text-2)]">{{ store.managerRuntimeLabel }}</span>
          </div>
          <div class="grid grid-cols-2 gap-2">
            <select
              :value="store.managerProviderName"
              class="h-8 rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-2 text-[11px] text-[var(--atms-text-1)] outline-none"
          ...
          <div class="mt-2 text-[10px] leading-relaxed text-[var(--atms-text-3)]">
        ...
        <section class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
          <div class="mb-2 flex items-center justify-between gap-2">
            <div class="flex items-center gap-1.5 text-[10px] text-[var(--atms-text-3)]">
          ...
          <div class="truncate font-mono text-[11px] text-[var(--atms-text-1)]">{{ apiBaseUrl }}</div>
        </section>
        ...
        <section class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
          <div class="mb-2 flex items-center justify-between gap-2">
            ...
            <span class="text-[10px] text-[var(--atms-text-4)]">{{ gitServers.length }}</span>
          </div>
          <div v-if="gitServers.length" class="space-y-2">
            <div
              v-for="server in gitServers"
              :key="server.server_id"
              class="rounded bg-[var(--atms-surface-2)] p-2"
            >
              <div class="mb-1 flex items-center justify-between gap-2">
                <div class="min-w-0">
                  <div class="truncate text-[11px] font-medium text-[var(--atms-text-1)]">{{ server.name }}</div>
                  <div class="truncate text-[10px] text-[var(--atms-text-4)]">{{ server.platform_type }} · {{ server.api_endpoint }}</div>
                </div>
                <button
                  class="rounded border border-[var(--atms-border)] px-2 py-1 text-[10px] text-[var(--atms-text-1)] hover:bg-[var(--atms-surface-2)]"
              ...
              <div class="flex items-center justify-between gap-2 text-[10px]">
                <span class="text-[var(--atms-text-3)]">{{ masked(server.git_user_name) }} / {{ masked(server.git_user_email) }}</span>
                <span :class="server.token_valid ? 'text-[var(--atms-success)]' : 'text-[var(--atms-danger)]'">
          ...
          <div v-else class="text-[11px] text-[var(--atms-text-4)]">{{ t('agent.settings.noGitServers') }}</div>
        </section>
        ...
        <section class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
          <div class="mb-2 flex items-center justify-between gap-2">
            ...
            <span class="text-[10px] text-[var(--atms-text-4)]">{{ providers.length }}</span>
          </div>
          <div v-if="providers.length" class="space-y-1.5">
            ...
            >
              <div class="flex items-center justify-between gap-2">
                <span class="truncate text-[11px] text-[var(--atms-text-1)]">{{ provider.name }}</span>
                <span :class="provider.is_active ? 'text-[var(--atms-success)]' : 'text-[var(--atms-text-4)]'" class="text-[10px]">
                  {{ provider.is_active ? t('agent.settings.active') : t('agent.settings.inactive') }}
                </span>
              </div>
              <div class="mt-0.5 truncate text-[10px] text-[var(--atms-text-4)]">{{ provider.base_url }}</div>
            </div>
          </div>
          <div v-else class="text-[11px] text-[var(--atms-text-4)]">{{ t('agent.settings.noProviders') }}</div>
        </section>
        ...
        <section class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
          <div class="mb-2 flex items-center justify-between gap-2">
            ...
            <span class="text-[10px] text-[var(--atms-text-4)]">{{ activeModels.length }}/{{ llmSettings.length }}</span>
          </div>
          <div v-if="activeModels.length" class="space-y-1.5">
            ...
            >
              <div class="flex items-center justify-between gap-2">
                <span class="truncate text-[11px] text-[var(--atms-text-1)]">{{ setting.display_name || setting.model_name }}</span>
                <span class="text-[10px] text-[var(--atms-text-3)]">{{ setting.provider_name }}</span>
              </div>
              <div class="mt-0.5 truncate text-[10px] text-[var(--atms-text-4)]">{{ setting.model_name }} · {{ setting.api_key_display || '••••' }}</div>
            </div>
          </div>
          <div v-else class="text-[11px] text-[var(--atms-text-4)]">{{ t('agent.settings.noModels') }}</div>
        </section>

        <section class="rounded-md border border-[var(--atms-warning-border)] bg-[var(--atms-warning-soft)] p-3">
          <div class="mb-1 text-[11px] font-medium text-[var(--atms-warning)]">{{ t('agent.settings.defaults') }}</div>
          <div class="text-[11px] leading-relaxed text-[var(--atms-warning)]">
