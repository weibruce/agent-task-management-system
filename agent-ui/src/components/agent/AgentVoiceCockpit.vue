...
const devOnboardingEntryVisible =
  import.meta.env.DEV && flagEnabled(import.meta.env.VITE_ATMS_DEV_ONBOARDING_ENTRY)
const generativeUiShadowPreviewRequested =
  import.meta.env.DEV && flagEnabled(import.meta.env.VITE_ATMS_GENERATIVE_UI_SHADOW_PREVIEW)
...
const VOICE_INPUT_ASSIST_KEY = 'atms.voice.input-assist'
const voiceInputAssist = ref(loadBooleanSetting(VOICE_INPUT_ASSIST_KEY, false))
const VOICE_OUTPUT_ENABLED_KEY = 'atms.voice.output-enabled'
...
const VOICE_TTS_CHANNEL = 'atms.voice.tts.v1'
...
const isNativeShell = computed(() => {
  if (typeof navigator === 'undefined') return false
  return /AtmsShell/i.test(navigator.userAgent)
})
...
function installCodexVoiceTextBridge(): void {
  ...
  ;(window as any).__ATMS_VOICE_AGENT_SEND__ = async (text: string) => {
...
function uninstallCodexVoiceTextBridge(): void {
  if (typeof window === 'undefined') return
  delete (window as any).__ATMS_VOICE_AGENT_SEND__
}
...
function recordTtsDebug(
  ...
  if (import.meta.env.DEV || isAndroidTvShell()) console.info('[AtmsVoiceTts]', code, message)
}
...
const GAMEPAD_NOTICE_SESSION_KEY = 'atms_gamepad_notice_shown'
...
function handleNativeVoiceStatus(status: NativeVoiceStatus): void {
  ...
  if (status.event === 'started' && status.selectedDeviceName) {
    console.info('[AtmsNativeVoice] started', JSON.stringify(status))
  }
}
    ...
    <div class="voice-shell relative flex h-full min-h-0 flex-col p-4" :style="voiceShellStyle">
      ...
      >
          ...
          >
            ...
            <div class="voice-model-menu__row">
              ...
              <div class="voice-model-menu__controls">
                ...
                >
                  <option value="codex_appserver" class="bg-[var(--atms-bg-raised)] text-[var(--atms-text-1)]">Codex</option>
                  <option value="kimi_code" class="bg-[var(--atms-bg-raised)] text-[var(--atms-text-1)]">Kimi Code</option>
                  <option value="claude_agent_sdk" class="bg-[var(--atms-bg-raised)] text-[var(--atms-text-1)]">
                    Claude Code
                  </option>
                </select>
                <select
                ...
                >
                  <option
                    ...
                    disabled
                    class="bg-[var(--atms-bg-raised)] text-[var(--atms-text-1)]"
                  >
                ...
                >
                  <option v-if="!managerAgentModelOptions.length" value="" class="bg-[var(--atms-bg-raised)] text-[var(--atms-text-1)]">
            ...
            >
              ...
              <div class="inline-flex items-center gap-3">
                <span
                  ...
                  :class="codexLiveVoiceEnabled ? 'text-[var(--atms-info)]' : 'text-[var(--atms-text-3)]'"
                >
                  {{
                ...
                <button
                  ...
                  class="relative h-7 w-12 flex-none rounded-full border transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--atms-accent-border)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--atms-bg-raised)] disabled:cursor-not-allowed disabled:opacity-45"
                  :class="
                    codexLiveVoiceEnabled
                      ? 'border-[var(--atms-info-border)] bg-[var(--atms-info)]'
                      : 'border-[var(--atms-border-strong)] bg-[var(--atms-surface-2)]'
                  "
            ...
            <div
              ...
              class="rounded-lg border border-[var(--atms-warning-border)] bg-[var(--atms-warning-soft)] px-3 py-2 text-xs text-[var(--atms-warning)]"
            ...
            <div
              ...
              class="rounded-lg border border-[var(--atms-info-border)] bg-[var(--atms-info-soft)] px-3 py-2 text-xs text-[var(--atms-info)]"
            ...
            <label class="voice-model-menu__row" :class="{ 'opacity-45': codexLiveVoiceEffective }">
              ...
              >
                <option v-if="!asrModelOptions.length" value="" class="bg-[var(--atms-bg-raised)] text-[var(--atms-text-1)]">
        ...
        <button
          ...
          class="voice-runtime-pill voice-runtime-pill--dev-onboarding flex h-9 items-center gap-2 rounded-full border border-[var(--atms-warning-border)] bg-[var(--atms-warning-soft)] px-3 text-xs text-[var(--atms-warning)] transition-colors hover:bg-[var(--atms-warning-soft)] hover:text-[var(--atms-text-1)]"
      ...
      >
        ...
        >
          ...
          <aside
            v-else
            class="flex h-full w-14 flex-col items-center border-r border-[var(--atms-border)] bg-[var(--atms-surface-1)] py-4"
          >
            <button
            ...
            <div class="mt-4 flex flex-1 flex-col items-center gap-2 overflow-hidden">
              ...
              <button
                ...
                class="rounded-full p-2 text-[var(--atms-text-4)] transition-colors hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]"
        ...
        >
          <div
            ...
            class="voice-stage__status pointer-events-none absolute right-5 top-5 z-10 rounded-full border border-[var(--atms-border)] px-3 py-1.5 text-xs backdrop-blur"
          >
            {{ processingText }}
          </div>
          ...
          <div class="voice-control mt-5 border-t border-[var(--atms-border)] pt-4">
                ...
                >
                  <defs>
                    <linearGradient id="codexLiveWaveMainGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                      <stop offset="0%" stop-color="var(--atms-accent)" stop-opacity="0.08" />
                      <stop offset="30%" stop-color="var(--atms-accent)" stop-opacity="0.9" />
                      <stop offset="68%" stop-color="var(--atms-info)" stop-opacity="0.82" />
                      <stop offset="100%" stop-color="var(--atms-info)" stop-opacity="0.08" />
                    </linearGradient>
                    <linearGradient id="codexLiveWaveUpperGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                      <stop offset="0%" stop-color="var(--atms-info)" stop-opacity="0.04" />
                      <stop offset="50%" stop-color="var(--atms-info)" stop-opacity="0.48" />
                      <stop offset="100%" stop-color="var(--atms-accent)" stop-opacity="0.04" />
                    </linearGradient>
                    <linearGradient id="codexLiveWaveLowerGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                      <stop offset="0%" stop-color="var(--atms-speaking)" stop-opacity="0.04" />
                      <stop offset="50%" stop-color="var(--atms-speaking)" stop-opacity="0.42" />
                      <stop offset="100%" stop-color="var(--atms-accent)" stop-opacity="0.04" />
                    </linearGradient>
                  </defs>
                  <path
                ...
                <svg class="voice-wave-svg" viewBox="0 0 360 76" preserveAspectRatio="none">
                  <defs>
                    <linearGradient id="voiceStripMainGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                      <stop offset="0%" stop-color="var(--atms-accent)" stop-opacity="0.1" />
                      <stop offset="30%" stop-color="var(--atms-accent)" stop-opacity="0.85" />
                      <stop offset="62%" stop-color="var(--atms-info)" stop-opacity="0.75" />
                      <stop offset="100%" stop-color="var(--atms-speaking)" stop-opacity="0.12" />
                    </linearGradient>
                    <linearGradient id="voiceStripUpperGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                      <stop offset="0%" stop-color="var(--atms-info)" stop-opacity="0.06" />
                      <stop offset="50%" stop-color="var(--atms-info)" stop-opacity="0.4" />
                      <stop offset="100%" stop-color="var(--atms-accent)" stop-opacity="0.06" />
                    </linearGradient>
                    <linearGradient id="voiceStripLowerGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                      <stop offset="0%" stop-color="var(--atms-speaking)" stop-opacity="0.08" />
                      <stop offset="44%" stop-color="var(--atms-speaking)" stop-opacity="0.42" />
                      <stop offset="100%" stop-color="var(--atms-accent)" stop-opacity="0.06" />
                    </linearGradient>
                  </defs>
                  <path
                  ...
                  <svg class="voice-wave-svg" viewBox="0 0 360 76" preserveAspectRatio="none">
                    <defs>
                      <linearGradient id="voiceWaveMainGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                        <stop offset="0%" stop-color="var(--atms-accent)" stop-opacity="0.12" />
                        <stop offset="30%" stop-color="var(--atms-accent)" stop-opacity="0.82" />
                        <stop offset="62%" stop-color="var(--atms-info)" stop-opacity="0.74" />
                        <stop offset="100%" stop-color="var(--atms-info)" stop-opacity="0.14" />
                      </linearGradient>
                      <linearGradient id="voiceWaveUpperGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                        <stop offset="0%" stop-color="var(--atms-info)" stop-opacity="0.08" />
                        <stop offset="50%" stop-color="var(--atms-info)" stop-opacity="0.44" />
                        <stop offset="100%" stop-color="var(--atms-accent)" stop-opacity="0.08" />
                      </linearGradient>
                      <linearGradient id="voiceWaveLowerGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                        <stop offset="0%" stop-color="var(--atms-speaking)" stop-opacity="0.1" />
                        <stop offset="44%" stop-color="var(--atms-speaking)" stop-opacity="0.46" />
                        <stop offset="100%" stop-color="var(--atms-accent)" stop-opacity="0.08" />
                      </linearGradient>
                    </defs>
                    <path
...
<style scoped>
/* ==========================================================================
   Atms Voice Cockpit — design tokens
...
.voice-cockpit {
  ...
  --vc-bg: var(--atms-bg);
  --vc-surface-1: var(--atms-surface-1);
  --vc-surface-2: var(--atms-surface-2);
  --vc-surface-3: var(--atms-surface-3);
  --vc-panel: var(--atms-panel);
  --vc-border: var(--atms-border);
  --vc-border-strong: var(--atms-border-strong);
  --vc-text-1: var(--atms-text-1);
  --vc-text-2: var(--atms-text-2);
  --vc-text-3: var(--atms-text-3);
  --vc-text-4: var(--atms-text-4);
  --vc-accent: var(--atms-accent);
  --vc-accent-soft: var(--atms-accent-soft);
  --vc-accent-border: var(--atms-accent-border);
  --vc-speaking: var(--atms-speaking);
  --vc-speaking-soft: var(--atms-speaking-soft);
  --vc-speaking-border: var(--atms-speaking-border);
  --vc-success: var(--atms-success);
  --vc-success-soft: var(--atms-success-soft);
  --vc-success-border: var(--atms-success-border);
  --vc-warning: var(--atms-warning);
  --vc-warning-soft: var(--atms-warning-soft);
  --vc-warning-border: var(--atms-warning-border);
  --vc-danger: var(--atms-danger);
  --vc-danger-soft: var(--atms-danger-soft);
  --vc-danger-border: var(--atms-danger-border);
  --vc-info: var(--atms-info);
  --vc-info-soft: var(--atms-info-soft);
  --vc-info-border: var(--atms-info-border);
  --vc-radius-lg: var(--atms-radius-lg);
  --vc-radius-md: var(--atms-radius-md);
  --vc-radius-sm: var(--atms-radius-sm);
  --ambient-a: color-mix(in srgb, var(--atms-accent) 9%, transparent);
  --ambient-b: color-mix(in srgb, var(--atms-info) 7%, transparent);
  width: 100vw;
  height: 100vh;
  height: 100svh;
  height: 100dvh;
...
:global(#app.voice-cockpit-root-active) {
  background: var(--atms-bg, #06080d) !important;
}
...
.voice-cockpit--listening {
  --ambient-a: color-mix(in srgb, var(--atms-accent) 17%, transparent);
  --ambient-b: color-mix(in srgb, var(--atms-info) 10%, transparent);
}

.voice-cockpit--speaking {
  --ambient-a: color-mix(in srgb, var(--atms-speaking) 15%, transparent);
  --ambient-b: color-mix(in srgb, var(--atms-speaking) 10%, transparent);
}

.voice-cockpit--processing,
.voice-cockpit--thinking {
  --ambient-a: color-mix(in srgb, var(--atms-warning) 10%, transparent);
  --ambient-b: color-mix(in srgb, var(--atms-accent) 7%, transparent);
}
...
.voice-fullscreen-gate {
  ...
  background:
    ...
    color-mix(in srgb, var(--atms-overlay) 88%, var(--vc-bg));
  color: var(--vc-text-1);
  text-align: center;
...
.voice-immersive-exit-zone {
  position: fixed;
  top: var(--atms-electron-titlebar-height, 0px);
  left: 50%;
  z-index: 220;
...
.voice-model-menu__popover {
  ...
  background: var(--atms-bg-raised);
  background-image: linear-gradient(
    145deg,
    color-mix(in srgb, var(--atms-accent) 3%, transparent),
    transparent 42%
  );
  padding: 12px;
  box-shadow: var(--atms-shadow-floating), 0 0 0 1px var(--atms-border) inset;
...
.voice-stage {
  ...
  background:
    ...
    var(--vc-surface-1);
  box-shadow: var(--atms-shadow-panel);
  transition:
...
.voice-composer__row:focus-within {
  border-color: var(--vc-accent-border);
  box-shadow: 0 0 0 3px var(--atms-focus-ring);
}
...
.voice-composer__mic--active {
  ...
  background: var(--vc-accent-soft);
  color: var(--vc-accent);
  box-shadow: 0 0 0 4px var(--atms-focus-ring);
}
...
.voice-input-zone--active .voice-input-zone__mic {
  ...
  background: var(--vc-accent-soft);
  color: var(--vc-accent);
  box-shadow: 0 0 0 8px var(--atms-focus-ring);
}
...
.voice-gamepad-connect__svg {
  ...
  flex: 0 0 210px;
  height: 130px;
  filter: drop-shadow(0 12px 24px var(--atms-border-strong));
