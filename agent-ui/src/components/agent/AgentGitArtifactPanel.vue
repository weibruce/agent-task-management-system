    ...
    <div class="flex-shrink-0 border-b border-[var(--atms-border)] px-3 py-2">
      <div class="flex items-center justify-between gap-3">
        <div class="flex items-center gap-1.5 text-[11px] font-medium text-[var(--atms-text-1)]">
          <GitPullRequest class="h-3.5 w-3.5 text-[var(--atms-success)]" />
      ...
      <div class="space-y-3">
        <section class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
          ...
          <div class="space-y-1 text-[11px]">
            <div class="flex items-center justify-between gap-2">
              <span class="text-[var(--atms-text-3)]">{{ t('agent.artifacts.current') }}</span>
              <span class="truncate font-mono text-[var(--atms-text-1)]">{{ summary.branch || t('agent.artifacts.unavailable') }}</span>
            </div>
            <div class="flex items-center justify-between gap-2">
              <span class="text-[var(--atms-text-3)]">{{ t('agent.artifacts.base') }}</span>
              <span class="truncate font-mono text-[var(--atms-text-2)]">{{ summary.baseBranch || t('agent.artifacts.unavailable') }}</span>
            </div>
          </div>
        </section>
        ...
        <section class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
          <div class="mb-2 flex items-center justify-between gap-2">
            ...
            <span
              :class="cn(
                ...
                summary.reviewStatus === 'approved' ? 'bg-[var(--atms-success-soft)] text-[var(--atms-success)]' :
                summary.reviewStatus === 'changes_requested' ? 'bg-[var(--atms-danger-soft)] text-[var(--atms-danger)]' :
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            class="flex items-center gap-1.5 truncate text-[11px] text-[var(--atms-info)] hover:text-[var(--atms-info)]"
          >
            <ExternalLink class="h-3 w-3 flex-shrink-0" />
            <span class="truncate">{{ primaryPrUrl }}</span>
          </a>
          <div v-else class="text-[11px] text-[var(--atms-text-4)]">{{ t('agent.artifacts.noPullRequest') }}</div>
        </section>
        ...
        <section class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
          ...
          <div v-else class="text-[11px] text-[var(--atms-text-4)]">{{ t('agent.artifacts.noIssue') }}</div>
          <div v-if="summary.changeTitle" class="mt-2 line-clamp-2 text-[11px] text-[var(--atms-text-2)]">
            {{ summary.changeTitle }}
          </div>
        </section>
        ...
        <section class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
          <div class="mb-2 flex items-center justify-between gap-2">
            ...
            <span class="text-[10px] text-[var(--atms-text-4)]">{{ summary.changedFiles.length }}</span>
          </div>
          <div v-if="summary.changedFiles.length" class="space-y-1">
            <div
              v-for="file in summary.changedFiles"
              :key="file"
              class="truncate rounded bg-[var(--atms-surface-2)] px-2 py-1 font-mono text-[10px] text-[var(--atms-text-1)]"
            >
              {{ file }}
            </div>
          </div>
          <div v-else class="text-[11px] text-[var(--atms-text-4)]">{{ t('agent.artifacts.noChangedFiles') }}</div>
        </section>
        ...
        <section class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
          ...
          <div class="space-y-1">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              class="flex items-center gap-1.5 truncate text-[10px] text-[var(--atms-info)] hover:text-[var(--atms-info)]"
            >
              <ExternalLink class="h-3 w-3 flex-shrink-0" />
              <span class="truncate">{{ url }}</span>
            </a>
            <div v-if="summary.prUrls.length + summary.issueUrls.length <= 1" class="text-[11px] text-[var(--atms-text-4)]">
