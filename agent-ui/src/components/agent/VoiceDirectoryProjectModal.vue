...
<template>
  <Teleport :to="teleportTarget">
    ...
    <div
      ...
      class="fixed inset-0 z-[180] flex items-center justify-center bg-[var(--atms-overlay)] px-6 backdrop-blur-sm"
    >
      <section class="flex h-[min(720px,82vh)] w-[min(980px,92vw)] overflow-hidden rounded-[24px] border border-[var(--atms-border-strong)] bg-[var(--atms-panel)] text-[var(--atms-text-1)]" :style="{ boxShadow: 'var(--atms-shadow-floating)' }">
      <aside class="flex w-64 shrink-0 flex-col border-r border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4">
        <div class="mb-4 flex items-center justify-between">
          ...
          <button class="rounded-full p-2 text-[var(--atms-text-3)] hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]" @click="$emit('update:open', false)">
            <X class="h-4 w-4" />
          </button>
        </div>

        <label class="mb-2 text-xs text-[var(--atms-text-3)]">{{ t('voice.directoryPicker.server') }}</label>
        <div class="mb-4 flex items-center gap-2 rounded-xl border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 py-2">
          <Server class="h-4 w-4 text-[var(--atms-accent)]" />
          <select v-model="serverId" class="min-w-0 flex-1 bg-transparent text-sm text-[var(--atms-text-1)] outline-none">
            <option v-for="server in servers" :key="server.id" :value="server.id" class="bg-[var(--atms-bg-raised)] text-[var(--atms-text-1)]">
              {{ server.name }}
            </option>
          </select>
        </div>
        <div class="mb-3 text-xs text-[var(--atms-text-4)]">
        ...
        <div v-if="roots.length" class="min-h-0 flex-1" data-testid="voice-directory-existing-projects">
          <div class="mb-2 text-xs text-[var(--atms-text-3)]">{{ t('voice.directoryPicker.existingProjects') }}</div>
          <div class="space-y-1 overflow-y-auto">
            <button
              v-for="root in roots"
              :key="root.id"
              data-testid="voice-directory-project-root"
              class="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]"
      ...
      <div class="flex min-w-0 flex-1 flex-col">
        <header class="flex shrink-0 items-center gap-3 border-b border-[var(--atms-border)] px-5 py-4">
          <button
            class="flex items-center gap-2 rounded-full border border-[var(--atms-border)] px-3 py-2 text-sm text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)] disabled:opacity-35"
          ...
          <input
            v-model="pathInput"
            class="min-w-0 flex-1 rounded-xl border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 py-2 font-mono text-sm text-[var(--atms-accent)] outline-none focus:border-[var(--atms-accent-border)]"
          ...
          />
          <label class="flex items-center gap-2 rounded-full border border-[var(--atms-border)] px-3 py-2 text-xs text-[var(--atms-text-3)]">
        ...
        <main class="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_300px]">
          <div class="min-h-0 overflow-y-auto p-4">
            <div v-if="loading" class="flex h-full items-center justify-center text-[var(--atms-text-3)]">
              <Loader2 class="mr-2 h-4 w-4 animate-spin" />
              {{ t('voice.directoryPicker.loading') }}
            </div>
            <div v-else-if="error" class="rounded-2xl border border-[var(--atms-danger-border)] bg-[var(--atms-danger-soft)] p-4 text-sm text-[var(--atms-danger)]">
              {{ error }}
            </div>
            <div v-else class="space-y-1.5">
              <button
                v-if="parentPath"
                class="group flex w-full min-w-0 items-center gap-3 rounded-xl border border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)] px-3 py-2.5 text-left hover:border-[var(--atms-accent-border)] hover:bg-[var(--atms-accent-soft)]"
                :title="t('voice.directoryPicker.parentTitle')"
                @click="browse(parentPath)"
              >
                <CornerUpLeft class="h-4 w-4 shrink-0 text-[var(--atms-accent)]" />
                <div class="min-w-0 flex-1">
                  <div class="text-sm font-semibold text-[var(--atms-accent)]">{{ t('voice.directoryPicker.parentDirectory') }}</div>
                  <div class="mt-0.5 break-all font-mono text-[10px] leading-4 text-[var(--atms-text-4)]">{{ parentPath }}</div>
                </div>
              </button>
              <button
                v-for="entry in entries"
                :key="entry.path"
                class="group flex w-full min-w-0 items-center gap-3 rounded-xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 py-2.5 text-left hover:border-[var(--atms-accent-border)] hover:bg-[var(--atms-accent-soft)]"
                @click="browse(entry.path)"
              >
                <FolderOpen class="h-4 w-4 shrink-0 text-[var(--atms-accent)]" />
                <div class="flex min-w-0 flex-1 items-center gap-3">
                  <div class="min-w-0 flex-1 break-all text-sm leading-5 text-[var(--atms-text-1)] group-hover:text-[var(--atms-text-1)]">{{ entry.name }}</div>
                  <div class="flex shrink-0 gap-2 text-[10px] text-[var(--atms-text-4)]">
          ...
          <aside class="overflow-y-auto border-l border-[var(--atms-border)] bg-[var(--atms-control)] p-5">
            <div class="mb-4">
              <div class="text-xs tracking-[0.18em] text-[var(--atms-accent)]">{{ t('voice.directoryPicker.add') }}</div>
              <h2 class="mt-1 text-xl font-semibold">{{ t('voice.directoryPicker.newDirectory') }}</h2>
            </div>
            <label class="mb-1 block text-xs text-[var(--atms-text-3)]">{{ t('voice.project.namePlaceholder') }}</label>
            <input
              :value="projectName"
            ...
            <textarea
              v-model="description"
              class="mb-4 h-24 w-full resize-none rounded-xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 py-2 text-sm outline-none focus:border-[var(--atms-accent-border)]"
              :placeholder="t('voice.project.optional')"
            />
            <div class="mb-4 rounded-2xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3 text-xs text-[var(--atms-text-3)]">
              <div class="mb-1 text-[var(--atms-text-2)]">{{ t('voice.directoryPicker.currentDirectory') }}</div>
            ...
            <div class="mb-4 rounded-2xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
              ...
              <label class="mb-1 block text-xs text-[var(--atms-text-3)]">Gitea / Git Server</label>
              <select
                :value="selectedGitServerId"
                class="mb-3 h-10 w-full rounded-xl border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm text-[var(--atms-text-1)] outline-none focus:border-[var(--atms-accent-border)] disabled:opacity-45"
              ...
              <select
                ...
                class="h-10 w-full rounded-xl border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm text-[var(--atms-text-1)] outline-none focus:border-[var(--atms-accent-border)] disabled:opacity-45"
              ...
              <div v-if="gitError" class="mt-2 text-xs text-[var(--atms-danger)]">{{ gitError }}</div>
              <div v-else class="mt-2 text-xs text-[var(--atms-text-4)]">
                {{ t('voice.directoryPicker.tokenHint') }}
              </div>
            </div>
            <button
              class="sticky bottom-0 z-10 mt-2 flex h-11 w-full items-center justify-center gap-2 rounded-full bg-[var(--atms-accent)] font-medium text-[var(--atms-on-accent)] shadow-[var(--atms-shadow-panel)] transition hover:bg-[var(--atms-accent-hover)] disabled:cursor-not-allowed disabled:opacity-45"
              :disabled="!canCreate"
              @click="handleCreate"
            >
