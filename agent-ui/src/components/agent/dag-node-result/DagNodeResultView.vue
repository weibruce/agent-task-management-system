...
<template>
  <div class="dag-node-result">
    <div v-if="loading && !result" class="py-6 text-center text-sm text-[var(--atms-text-4)]">
      {{ t('dag.result.loading') }}
    </div>
    <div v-else-if="!result || !result.latest" class="py-6 text-center text-sm text-[var(--atms-text-4)]">
      {{ t('dag.result.empty') }}
    </div>
