...
<template>
  <div v-if="payload" class="flex flex-col gap-3 px-6 py-4">
    <div class="flex items-center gap-3 text-sm">
      <span class="rounded-full border border-[var(--atms-info-border)] bg-[var(--atms-info-soft)] px-2.5 py-1 text-xs font-medium text-[var(--atms-info)]">
        {{ payload.completed ? t('dag.result.loopCompleted') : t('dag.result.loopIterating') }}
      </span>
      <span v-if="payload.total != null" class="font-mono text-xs text-[var(--atms-text-3)]">
        <template v-if="payload.index != null">{{ payload.index + 1 }}/</template>{{ payload.total }}
      </span>
    </div>

    <div v-if="payload.item !== undefined" class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 py-2">
      <div class="mb-1 text-[10px] uppercase tracking-wide text-[var(--atms-text-4)]">{{ t('dag.result.loopItem') }}</div>
      <pre class="whitespace-pre-wrap break-all font-mono text-xs text-[var(--atms-text-1)]">{{ prettyValue(payload.item) }}</pre>
    </div>

    <div v-if="results.length" class="rounded-lg border border-[var(--atms-border)]">
