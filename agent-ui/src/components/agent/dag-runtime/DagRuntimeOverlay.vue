...
<template>
  <transition name="overlay-zoom">
    <div class="dag-runtime-overlay fixed inset-0 z-[200] flex flex-col overflow-hidden bg-[var(--atms-bg)]">
      <!-- 氛围层 -->
      <div class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_22%_18%,var(--atms-canvas-ambient-primary),transparent_38%),radial-gradient(circle_at_78%_72%,var(--atms-canvas-ambient-secondary),transparent_42%)]" />
      ...
      <template v-else>
        <div class="relative z-10 min-h-0 flex-1">
          ...
          <div
            v-if="metricsLoading"
            class="pointer-events-none absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full border border-[var(--atms-border)] bg-[var(--atms-panel)] px-3 py-1.5 text-[10px] text-[var(--atms-text-3)] backdrop-blur"
          >
      ...
      <div
        ...
        class="pointer-events-none absolute bottom-4 left-1/2 z-20 -translate-x-1/2 rounded-full border border-[var(--atms-border)] bg-[var(--atms-panel)] px-6 py-2.5 text-sm text-[var(--atms-text-2)] backdrop-blur"
      >
