...
const embeddedAppearance = computed(() => {
  ...
  return {
    ...
    text: styles.getPropertyValue('--atms-text-1').trim() || '#edf4fc',
  }
})
...
.voice-dynamic-widget {
  ...
  overflow: auto;
  background:
    radial-gradient(circle at 78% 8%, var(--atms-info-soft), transparent 34%),
    var(--atms-panel);
...
.voice-dynamic-widget__node[data-status*="active"] {
  ...
  box-shadow: 0 0 22px var(--atms-success-soft);
}
...
.voice-dynamic-widget__node-ring {
  ...
  height: 38px;
  border-radius: 999px;
  color: var(--atms-on-accent);
  background: conic-gradient(from 180deg, var(--atms-accent), var(--atms-speaking), var(--atms-surface-2));
  font-size: 10px;
  font-weight: 900;
}
...
.voice-dynamic-widget__time-item::before {
  ...
  box-shadow: 0 0 16px var(--atms-focus-ring);
}
...
.voice-dynamic-widget__bar-row i {
  ...
  background: linear-gradient(90deg, var(--atms-accent), var(--atms-speaking));
}
...
.voice-dynamic-widget__steps-item--active {
  color: var(--atms-accent) !important;
  background: var(--atms-accent-soft) !important;
}
</style>
