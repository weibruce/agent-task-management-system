...
<template>
  <div class="flex h-full min-h-0 flex-col">
    <div class="flex-shrink-0 border-b border-[var(--atms-border)] px-3 py-2">
      <div class="mb-2 flex items-center justify-between gap-3">
        <div class="flex items-center gap-1.5 text-[11px] font-medium text-[var(--atms-text-1)]">
          <FileText class="h-3.5 w-3.5 text-[var(--atms-info)]" />
          {{ t('agent.evidence.title') }}
        </div>
        <div v-if="loading" class="flex items-center gap-1 text-[10px] text-[var(--atms-text-3)]">
      ...
      <div class="grid grid-cols-4 gap-1.5 text-[10px]">
        <div class="rounded bg-[var(--atms-surface-2)] px-2 py-1.5">
          <div class="text-[var(--atms-text-3)]">{{ t('agent.evidence.toolCalls') }}</div>
          <div class="mt-0.5 text-[var(--atms-text-1)]">{{ totals.toolCalls }}</div>
        </div>
        <div class="rounded bg-[var(--atms-surface-2)] px-2 py-1.5">
          <div class="text-[var(--atms-text-3)]">{{ t('agent.evidence.toolResults') }}</div>
          <div class="mt-0.5 text-[var(--atms-text-1)]">{{ totals.toolResults }}</div>
        </div>
        <div class="rounded bg-[var(--atms-surface-2)] px-2 py-1.5">
          <div class="text-[var(--atms-text-3)]">{{ t('agent.evidence.handoffs') }}</div>
          <div class="mt-0.5 text-[var(--atms-info)]">{{ totals.handoffs }}</div>
        </div>
        <div class="rounded bg-[var(--atms-surface-2)] px-2 py-1.5">
          <div class="text-[var(--atms-text-3)]">{{ t('agent.evidence.errors') }}</div>
          <div :class="cn('mt-0.5', totals.errors ? 'text-[var(--atms-danger)]' : 'text-[var(--atms-text-1)]')">{{ totals.errors }}</div>
        </div>
      </div>
      <div v-if="error" class="mt-2 rounded border border-[var(--atms-danger-border)] bg-[var(--atms-danger-soft)] px-2 py-1 text-[10px] text-[var(--atms-danger)]">
        {{ error }}
      </div>
    </div>
    ...
    <div v-if="store.nodes.length === 0" class="flex flex-1 items-center justify-center">
      <div class="text-center">
        <Bot class="mx-auto h-8 w-8 text-[var(--atms-text-4)]" />
        <div class="mt-2 text-xs text-[var(--atms-text-4)]">{{ t('agent.inspector.noNodes') }}</div>
      </div>
    </div>

    <div v-else class="grid min-h-0 flex-1 grid-cols-[160px_minmax(0,1fr)]">
      <div class="min-h-0 overflow-y-auto border-r border-[var(--atms-border)] py-1">
        <button
          v-for="item in evidenceList"
          :key="item.node.id"
          :class="cn(
            'w-full px-2 py-2 text-left transition-colors',
            store.selectedNodeId === item.node.id ? 'bg-[var(--atms-info-soft)]' : 'hover:bg-[var(--atms-surface-2)]'
          )"
        ...
        >
          <div class="flex items-center gap-2">
            ...
            <div class="min-w-0 flex-1">
              <div class="truncate text-[11px] font-medium text-[var(--atms-text-1)]">{{ item.node.name }}</div>
              <div class="truncate text-[10px] text-[var(--atms-text-4)]">{{ item.node.id }}</div>
            </div>
            <component
              :is="statusIcon(item.node.status)"
              :class="cn(
                'h-3.5 w-3.5 flex-shrink-0',
                item.node.status === 'failed' ? 'text-[var(--atms-danger)]' :
                item.node.status === 'completed' ? 'text-[var(--atms-info)]' :
                item.node.status === 'running' ? 'text-[var(--atms-success)] animate-spin' :
                'text-[var(--atms-text-3)]'
              )"
            />
          </div>
          <div class="mt-1 flex items-center gap-1 text-[9px] text-[var(--atms-text-3)]">
            <span>{{ item.toolCalls.length }} tools</span>
            <span v-if="item.handoffs.length" class="text-[var(--atms-info)]">{{ item.handoffs.length }} handoff</span>
            <span v-if="item.errors.length" class="text-[var(--atms-danger)]">{{ item.errors.length }} error</span>
          </div>
        </button>
      </div>

      <div class="min-h-0 overflow-y-auto">
        <div v-if="!selectedEvidence" class="flex h-full items-center justify-center text-xs text-[var(--atms-text-4)]">
          {{ t('agent.evidence.selectNode') }}
        </div>

        <div v-else class="space-y-3 p-3">
          <section class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
            <div class="mb-2 flex items-center justify-between gap-3">
              <div class="min-w-0">
                <div class="truncate text-xs font-medium text-[var(--atms-text-1)]">{{ selectedEvidence.node.name }}</div>
                <div class="mt-0.5 truncate text-[10px] text-[var(--atms-text-4)]">{{ selectedEvidence.node.id }}</div>
              </div>
              <span
                :class="cn(
                  'rounded-full px-2 py-0.5 text-[10px]',
                  selectedEvidence.node.status === 'failed' ? 'bg-[var(--atms-danger-soft)] text-[var(--atms-danger)]' :
                  selectedEvidence.node.status === 'completed' ? 'bg-[var(--atms-info-soft)] text-[var(--atms-info)]' :
                  selectedEvidence.node.status === 'running' ? 'bg-[var(--atms-success-soft)] text-[var(--atms-success)]' :
                  'bg-[var(--atms-surface-2)] text-[var(--atms-text-2)]'
                )"
              >
                {{ selectedEvidence.node.status }}
              </span>
            </div>
            <div class="text-[11px] leading-relaxed text-[var(--atms-text-2)]">
              {{ selectedEvidence.latestText || t('agent.evidence.noText') }}
            </div>
          </section>

          <section v-if="selectedEvidence.handoffs.length" class="rounded-md border border-[var(--atms-info-border)] bg-[var(--atms-info-soft)] p-3">
            <div class="mb-2 flex items-center gap-1.5 text-[11px] font-medium text-[var(--atms-info)]">
            ...
            <div class="space-y-1.5">
              <pre
                ...
                class="max-h-28 overflow-auto whitespace-pre-wrap break-words rounded bg-[var(--atms-bg)] p-2 text-[10px] leading-relaxed text-[var(--atms-info)]"
              >{{ compact(toolDetail(message), 900) }}</pre>
            </div>
          </section>

          <section v-if="selectedEvidence.errors.length" class="rounded-md border border-[var(--atms-danger-border)] bg-[var(--atms-danger-soft)] p-3">
            <div class="mb-2 flex items-center gap-1.5 text-[11px] font-medium text-[var(--atms-danger)]">
            ...
            <div class="space-y-1.5">
              <pre
                ...
                class="max-h-32 overflow-auto whitespace-pre-wrap break-words rounded bg-[var(--atms-bg)] p-2 text-[10px] leading-relaxed text-[var(--atms-danger)]"
              >{{ compact(toolDetail(message), 1100) }}</pre>
            </div>
          </section>

          <section class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
            <div class="mb-2 flex items-center gap-1.5 text-[11px] font-medium text-[var(--atms-text-1)]">
            ...
            <div v-if="selectedEvidence.toolCalls.length" class="space-y-1.5">
              <details
                ...
                class="rounded bg-[var(--atms-surface-2)] px-2 py-1.5"
              >
                <summary class="cursor-pointer text-[11px] text-[var(--atms-text-1)]">
                  <Wrench class="mr-1 inline h-3 w-3 text-[var(--atms-text-3)]" />
                  {{ toolLabel(message) }}
                </summary>
                <pre class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap break-words text-[10px] leading-relaxed text-[var(--atms-text-2)]">{{ compact(toolDetail(message), 1200) }}</pre>
              </details>
            </div>
            <div v-else class="text-[11px] text-[var(--atms-text-4)]">{{ t('agent.evidence.noToolCalls') }}</div>
          </section>
            ...
            <div v-if="selectedEvidence.messages.length" class="space-y-1.5">
              <div
                ...
                :class="cn(
                  ...
                  isError(message) ? 'border-[var(--atms-danger-border)] bg-[var(--atms-danger-soft)]' :
                  isHandoff(message) ? 'border-[var(--atms-info-border)] bg-[var(--atms-info-soft)]' :
                  'border-[var(--atms-border)] bg-[var(--atms-surface-1)]'
                )"
              >
                <div class="mb-1 flex items-center justify-between gap-2 text-[9px] text-[var(--atms-text-4)]">
                  <span>{{ message.type }} · {{ toolLabel(message) }}</span>
                  <span>{{ message.timestamp ? new Date(message.timestamp).toLocaleTimeString() : '' }}</span>
                </div>
                <pre class="max-h-24 overflow-auto whitespace-pre-wrap break-words text-[10px] leading-relaxed text-[var(--atms-text-2)]">{{ compact(toolDetail(message), 900) }}</pre>
              </div>
            </div>
            <div v-else class="text-[11px] text-[var(--atms-text-4)]">{{ t('agent.evidence.noMessages') }}</div>
          </section>
        </div>
      </div>
    </div>
