...
<template>
  <section data-testid="agent-settings-section-general" class="mt-8 space-y-6">
    <div>
      <h2 class="text-lg font-semibold text-[var(--atms-text-1)]">{{ t('settings.general.title') }}</h2>
      <p class="mt-1 text-sm text-[var(--atms-text-3)]">{{ t('settings.general.description') }}</p>
    </div>

    <div class="border-b border-[var(--atms-border)] pb-6">
      <div class="flex items-start gap-3">
        ...
        <div class="min-w-0 flex-1">
          <div class="text-sm font-medium text-[var(--atms-text-1)]">{{ t('settings.general.language.title') }}</div>
          <div class="mt-1 text-sm text-[var(--atms-text-3)]">{{ t('settings.general.language.description') }}</div>
          ...
          <div class="mt-4 grid max-w-2xl gap-2 sm:grid-cols-3" role="radiogroup" :aria-label="t('settings.general.language.title')">
            <button
              ...
              :class="uiStore.locale === option.code
                ...
                : 'border-[var(--atms-settings-divider)] bg-[var(--atms-settings-card)] text-[var(--atms-text-2)] hover:border-[var(--atms-border-strong)] hover:bg-[var(--atms-settings-card-hover)] hover:text-[var(--atms-text-1)]'"
            ...
            >
              <span class="min-w-0 flex-1">
                ...
                <span class="mt-0.5 block text-xs text-[var(--atms-text-3)]">{{ t(option.translationKey) }}</span>
              </span>
              <Check v-if="uiStore.locale === option.code" class="h-4 w-4 flex-shrink-0 text-[var(--atms-accent)]" />
            </button>
          </div>
        </div>
      </div>
