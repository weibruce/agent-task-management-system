...
import { applyAppearanceToDocument } from '@/appearance/appearance-registry'
...
describe('GeneralSettings', () => {
  ...
  it('switches the interface to English and persists the preference', async () => {
    ...
    expect(root.textContent).toContain('选择 Atms 界面使用的语言')
    ...
    expect(root.textContent).toContain('Choose the language used by the Atms interface')
  ...
  it('switches to Traditional Chinese with a script-based locale', async () => {
    ...
    expect(root.textContent).toContain('選擇 Atms 界面使用的語言')
