...
<template>
  <section data-testid="agent-settings-section-providers" class="mt-10 space-y-6">
    <div class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div class="mt-1 text-xs text-[var(--atms-text-3)]">{{ t('settings.models.runtimeSubtitle') }}</div>
        </div>
        <span
          data-testid="agent-settings-manager-runtime-count"
          class="rounded-full border border-[var(--atms-border)] px-3 py-1 text-xs text-[var(--atms-text-2)]"
      ...
      <div v-if="currentManagerUsesCodex" class="mt-4 grid gap-3 md:grid-cols-2">
        <div
          ...
          class="flex h-10 items-center justify-between rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm"
        >
          <span class="inline-flex items-center gap-2">
            Codex
            <em v-if="codexVersion" class="font-mono text-[10px] not-italic text-[var(--atms-text-3)]">
              {{ codexVersion }}
            </em>
          </span>
          <span class="inline-flex items-center gap-1 text-xs text-[var(--atms-text-2)]">
        ...
        >
          ...
          <span class="text-xs text-[var(--atms-text-3)]">
      ...
      <div v-else class="mt-4 grid gap-3 md:grid-cols-2">
        <select
          ...
          class="h-10 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none"
      ...
      <div v-if="currentManagerUsesCodex" class="mt-3 text-xs leading-relaxed text-[var(--atms-text-3)]">
        {{ t('settings.models.codexRuntimeHint') }}
      </div>
      <div v-else class="mt-3 text-xs leading-relaxed text-[var(--atms-text-3)]">
        {{ t('settings.models.runtimeHint') }}
      </div>
      <div
        v-if="liveVoiceSupported"
        data-testid="agent-settings-live-voice"
        class="mt-4 rounded-lg border border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)] p-3"
      >
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2 text-sm font-medium text-[var(--atms-text-1)]">
              <span>{{ t('settings.models.liveVoiceTitle') }}</span>
              <span class="rounded-full bg-[var(--atms-surface-1)] px-2 py-0.5 text-[10px] text-[var(--atms-accent)]">
                {{ t('settings.models.liveVoiceExperimental') }}
              </span>
              <span v-if="codexVersion" class="font-mono text-[10px] text-[var(--atms-text-3)]">
                Codex {{ codexVersion }}
              </span>
            </div>
            <p class="mt-1 text-xs leading-relaxed text-[var(--atms-text-3)]">
          ...
          <div class="inline-flex shrink-0 items-center gap-3">
            <span
              ...
              :class="liveVoiceEnabled ? 'text-[var(--atms-info)]' : 'text-[var(--atms-text-3)]'"
            >
        ...
        <label
          class="mt-3 grid gap-2 border-t border-[var(--atms-accent-border)] pt-3 sm:grid-cols-[minmax(0,1fr)_220px] sm:items-center"
          data-testid="agent-settings-live-voice-voice-row"
        >
          <span>
            <strong class="block text-sm font-medium text-[var(--atms-text-1)]">
              {{ t('settings.models.liveVoiceVoiceTitle') }}
            </strong>
            <span class="mt-0.5 block text-xs text-[var(--atms-text-3)]">
      ...
      <div
        ...
        class="mt-4 rounded-lg border border-[var(--atms-warning-border)] bg-[var(--atms-warning-soft)] p-3 text-xs text-[var(--atms-warning)]"
      >
        {{
    ...
    <div class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)]">
      <div
        class="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--atms-border)] px-4 py-3"
      >
        <div>
        ...
        <div class="relative">
          <button
            ...
            class="inline-flex items-center gap-2 rounded-md bg-[var(--atms-accent)] px-3 py-2 text-sm text-[var(--atms-on-accent)] hover:bg-[var(--atms-accent-hover)] disabled:opacity-50"
            :disabled="loading"
            aria-haspopup="menu"
          ...
          <div
            v-if="createMenuOpen"
            class="absolute right-0 top-full z-20 mt-2 w-56 rounded-md border border-[var(--atms-border-strong)] bg-[var(--atms-surface-1)] p-1.5 shadow-2xl"
            role="menu"
          >
            <button
              ...
              type="button"
              class="flex w-full items-center gap-3 rounded px-3 py-2.5 text-left text-sm text-[var(--atms-text-1)] hover:bg-[var(--atms-surface-1)]"
              role="menuitem"
              @click="openCreate(option.id)"
            >
              <component :is="option.icon" class="h-4 w-4 text-[var(--atms-accent)]" />
      ...
      <div
        ...
        class="px-4 py-8 text-center text-sm text-[var(--atms-text-3)]"
      >
        <Loader2 class="mx-auto h-5 w-5 animate-spin" />
        <div class="mt-2">{{ t('settings.models.loading') }}</div>
      </div>

      <div v-else-if="!configuredModelCount" class="px-4 py-8 text-center text-sm text-[var(--atms-text-3)]">
      ...
      <template v-else>
        <div
          ...
          class="border-b border-[var(--atms-border)] px-4 py-4"
        >
          <div class="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
            <div class="min-w-0 flex-1">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span
                  ...
                  class="rounded-full bg-[var(--atms-success-soft)] px-2 py-0.5 text-[10px] text-[var(--atms-success)]"
                  >{{ t('settings.models.currentManager') }}</span
                >
                <span
                  v-if="codexModels.length"
                  class="rounded-full bg-[var(--atms-accent-soft)] px-2 py-0.5 text-[10px] text-[var(--atms-accent)]"
                  >{{ t('settings.models.autoDetected') }}</span
                >
                <span class="rounded-full bg-[var(--atms-surface-1)] px-2 py-0.5 text-[10px] text-[var(--atms-text-2)]">{{
                  t('settings.models.readOnly')
                }}</span>
              </div>
              <div class="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-[var(--atms-text-3)]">
              ...
              <div class="mt-2 flex flex-wrap items-center gap-2">
                ...
                <span
                  v-if="currentManagerUsesCodex"
                  class="rounded-full bg-[var(--atms-surface-1)] px-2 py-0.5 text-[10px] text-[var(--atms-text-2)]"
                >
            ...
            <div class="inline-flex items-center gap-1.5 text-xs text-[var(--atms-text-3)] xl:pt-1">
              <LockKeyhole class="h-3.5 w-3.5" />
        ...
        <div
          ...
          :key="setting.id"
          class="group border-b border-[var(--atms-border)] px-4 py-4 last:border-0"
          ...
          <div class="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-[var(--atms-text-3)]">
                ...
                <span>·</span>
                <span v-if="setting.models && setting.models.length > 1" class="text-[var(--atms-text-2)]">
                  {{
              ...
              <div class="mt-2 flex flex-wrap items-center gap-2">
                ...
                <span
                  ...
                  class="rounded-full bg-[var(--atms-warning-soft)] px-2 py-0.5 text-[10px] text-[var(--atms-warning)]"
            ...
            <div class="flex flex-wrap items-center gap-2 xl:justify-end">
              <button
                class="rounded-md border border-[var(--atms-border)] px-3 py-1.5 text-xs text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-1)]"
              ...
              <div>
                <button
                  ...
                  class="rounded-md border border-[var(--atms-danger-border)] px-2 py-1.5 text-[var(--atms-danger)] hover:bg-[var(--atms-danger-soft)]"
                  :class="deleteConfirmId === setting.id ? 'bg-[var(--atms-danger-soft)]' : ''"
        ...
        <div
          ...
          class="border-t border-dashed border-[var(--atms-border)] px-4 py-2"
        >
          <div class="text-xs text-[var(--atms-text-3)]">
        ...
        <div
          ...
          :key="setting.id"
          class="group border-b border-[var(--atms-border)] px-4 py-3 opacity-60 last:border-0 hover:opacity-80"
          :data-testid="`agent-settings-model-item-${setting.id}`"
        >
          <div class="flex flex-col gap-2 xl:flex-row xl:items-center xl:justify-between">
            <div class="min-w-0 flex-1">
              <div class="text-sm font-medium text-[var(--atms-text-2)]">
                {{ setting.display_name || setting.model_name }}
              </div>
              <div class="mt-0.5 text-xs text-[var(--atms-text-3)]">
                {{ setting.provider_name }}
                <span
                  v-if="setting.plan_type === 'custom' || setting.endpoint_id?.endsWith('_custom')"
                  class="ml-1 rounded bg-[var(--atms-warning-soft)] px-1 text-[var(--atms-warning)]"
                  >{{ t('settings.models.custom') }}</span
                >
                · {{ localizedPlanLabel(setting.plan_type) }} ·
                {{
      ...
      <transition name="settings-panel">
        ...
        >
          <div class="flex h-full flex-col">
            <div
              class="flex items-center justify-between border-b border-[var(--atms-border)] px-5 py-4 flex-shrink-0"
            >
              <h2 class="text-base font-semibold text-[var(--atms-text-1)]">
                {{
                  editingSetting ? t('settings.models.editTitle') : t('settings.models.addTitle')
                }}
              </h2>
              <button
                class="rounded-md border border-[var(--atms-border)] px-3 py-1.5 text-sm text-[var(--atms-text-2)] transition-colors hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]"
                @click="closeDialog"
              >
...
.settings-model-panel {
  ...
  z-index: 60;
  /* 匹配设置页主题：--atms-bg 基底 + --atms-panel 主区 + 毛玻璃 */
