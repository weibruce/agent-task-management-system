...
<template>
  <section class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)]">
    <header
    ...
    >
      <div>
        ...
        <p class="mt-1 text-xs text-[var(--atms-text-3)]">{{ t('settings.models.providers.subtitle') }}</p>
      </div>
      <button
        type="button"
        class="inline-flex items-center gap-2 rounded-md border border-[var(--atms-border-strong)] px-3 py-2 text-sm text-[var(--atms-text-1)] hover:bg-[var(--atms-surface-1)]"
        @click="startCreate"
      >
        <Plus class="h-4 w-4" />
    ...
    <div class="grid min-h-80 md:grid-cols-[220px_minmax(0,1fr)]">
      <nav
        class="border-b border-[var(--atms-border)] p-2 md:border-b-0 md:border-r"
        ...
        <button
          ...
          :class="
            selectedId === provider.id && !creating
              ...
              : 'text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-1)] hover:text-[var(--atms-text-1)]'
          "
          @click="loadProvider(provider)"
        >
          <span class="truncate">{{ provider.name }}</span>
          <span class="text-[10px] text-[var(--atms-text-3)]">{{
            settings.filter(setting => setting.provider_id === provider.id).length
          }}</span>
        </button>
        <div v-if="!customProviders.length" class="px-3 py-6 text-center text-xs text-[var(--atms-text-3)]">
      ...
      <form class="space-y-4 p-4" @submit.prevent="save">
        <div class="grid gap-3 sm:grid-cols-2">
          <label class="space-y-1">
            <span class="text-xs text-[var(--atms-text-3)]">Provider ID</span>
            <input
              v-model="providerId"
              :disabled="!creating"
              class="h-10 w-full rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none disabled:text-[var(--atms-text-3)]"
              placeholder="custom-openai"
            />
          </label>
          <label class="space-y-1">
            <span class="text-xs text-[var(--atms-text-3)]">{{ t('settings.models.form.displayName') }}</span>
            <input
              v-model="providerName"
              class="h-10 w-full rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none focus:border-[var(--atms-accent-border)]"
            />
          </label>
        </div>
        <label class="block space-y-1">
          <span class="text-xs text-[var(--atms-text-3)]">Base URL</span>
          <input
            v-model="baseUrl"
            class="h-10 w-full rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none focus:border-[var(--atms-accent-border)]"
        ...
        <details v-if="supportsLlm" class="border-t border-[var(--atms-border)] pt-3">
          <summary class="cursor-pointer text-xs text-[var(--atms-text-3)]">
            {{ t('settings.models.providers.protocolUrls') }}
          </summary>
          <div class="mt-3 grid gap-3">
            <input
              v-model="chatCompletionsBaseUrl"
              class="h-9 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-xs"
        ...
        <div v-if="supportsTts || supportsAsr" class="space-y-3 border-t border-[var(--atms-border)] pt-4">
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
              <div class="text-xs text-[var(--atms-text-3)]">
                {{ t('settings.models.providers.voiceEndpoints') }}
              </div>
              <div class="mt-1 text-[11px] text-[var(--atms-text-3)]">
            ...
            <button
              ...
              class="inline-flex h-8 items-center gap-1.5 rounded-md border border-[var(--atms-border-strong)] px-2.5 text-xs text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-2)] disabled:cursor-not-allowed disabled:opacity-40"
            ...
            <div class="space-y-1">
              <div class="flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  type="button"
                  class="inline-flex items-center gap-1 text-[10px] text-[var(--atms-accent)] hover:underline"
              ...
              <input
                ...
                class="h-9 w-full rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 font-mono text-xs outline-none focus:border-[var(--atms-accent-border)]"
          ...
          <div
            ...
            class="rounded-md border border-[var(--atms-danger-border)] bg-[var(--atms-danger-soft)] px-3 py-2 text-xs text-[var(--atms-danger)]"
          >
            {{ endpointTestError }}
          </div>
        </div>
        <div class="space-y-2">
          <div class="text-xs text-[var(--atms-text-3)]">{{ t('settings.models.form.capabilities') }}</div>
        ...
        <div v-if="!creating && selectedProvider" class="space-y-3 border-t border-[var(--atms-border)] pt-4">
          <div>
            <div class="text-xs text-[var(--atms-text-3)]">
            ...
            <div v-if="referenceCount" class="mt-2 flex flex-wrap gap-1.5">
              <span
                ...
                :key="setting.id"
                class="rounded border border-[var(--atms-warning-border)] bg-[var(--atms-warning-soft)] px-2 py-1 text-xs text-[var(--atms-warning)]"
              >
          ...
          <div class="flex flex-wrap justify-end gap-2">
            <button
              ...
              :key="option.purpose"
              type="button"
              class="inline-flex h-8 items-center gap-1.5 rounded-md border border-[var(--atms-border)] px-2.5 text-xs text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-1)]"
        ...
        <div class="flex flex-wrap items-end justify-between gap-3 pt-2">
          <div v-if="!creating" class="flex flex-wrap items-end gap-2">
            <button
              ...
              class="inline-flex items-center gap-2 rounded-md border border-[var(--atms-danger-border)] px-3 py-2 text-xs text-[var(--atms-danger)] disabled:cursor-not-allowed disabled:opacity-35"
              :class="deleteConfirmOpen ? 'bg-[var(--atms-danger-soft)]' : ''"
              :disabled="saving"
          ...
          <button
            type="submit"
            class="inline-flex items-center gap-2 rounded-md bg-[var(--atms-accent)] px-4 py-2 text-sm font-medium text-[var(--atms-on-accent)] hover:bg-[color:color-mix(in_srgb,var(--atms-accent)_85%,white)] disabled:opacity-40"
