...
import {
  listAtmsPlugins,
  setAtmsPluginEnabled,
  type AtmsPluginRegistrySummaryV1,
} from '@/api/services/plugin-api'

const { t } = useI18n()
const registry = ref<AtmsPluginRegistrySummaryV1 | null>(null)
const loading = ref(false)
const saving = ref<string | null>(null)
const error = ref('')

function adoptRegistry(next: AtmsPluginRegistrySummaryV1): void {
...
async function refresh(): Promise<void> {
  ...
  error.value = ''
  try {
    adoptRegistry((await listAtmsPlugins()).data)
  } catch (cause) {
...
async function toggle(pluginId: string, enabled: boolean): Promise<void> {
  ...
  error.value = ''
  try {
    adoptRegistry((await setAtmsPluginEnabled(
      pluginId,
      enabled,
...
<template>
  <section data-testid="agent-settings-section-plugins" class="mt-8 space-y-5">
    <div class="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-4 py-3">
      <div>
        <div class="flex items-center gap-2 text-sm font-semibold text-[var(--atms-text-1)]">
          <Boxes class="h-4 w-4 text-[var(--atms-accent)]" />
          {{ t('settings.plugins.registry') }}
          <span
            data-testid="agent-settings-plugins-count"
            class="rounded-full bg-[var(--atms-accent-soft)] px-2 py-0.5 text-xs text-[var(--atms-accent)]"
          >
            {{ t('settings.plugins.detected', { count: registry?.plugins.length ?? 0 }) }}
          </span>
        </div>
        <p class="mt-1 text-xs text-[var(--atms-text-3)]">
      ...
      <button
        ...
        class="inline-flex h-9 items-center gap-2 rounded-full border border-[var(--atms-border)] px-3 text-xs text-[var(--atms-text-1)] hover:bg-[var(--atms-surface-2)] disabled:opacity-50"
    ...
    <p
      ...
      role="alert"
      aria-live="polite"
      class="rounded-xl border border-[var(--atms-danger-border)] bg-[var(--atms-danger-soft)] px-4 py-3 text-sm text-[var(--atms-danger)]"
    >
      {{ error }}
    </p>
    ...
    <div class="grid gap-3">
      <article
        ...
        class="grid gap-4 rounded-2xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-center"
      >
        <div class="min-w-0">
          <div class="flex flex-wrap items-center gap-2">
            <h2 class="truncate font-semibold text-[var(--atms-text-1)]">{{ plugin.name }}</h2>
            <span class="rounded-full bg-[var(--atms-surface-1)] px-2 py-0.5 font-mono text-[10px] text-[var(--atms-text-3)]">{{ plugin.version }}</span>
            <span v-if="plugin.locked" class="inline-flex items-center gap-1 rounded-full bg-[var(--atms-surface-2)] px-2 py-0.5 text-[10px] text-[var(--atms-text-2)]">
              <LockKeyhole class="h-3 w-3" /> {{ t('settings.plugins.core') }}
            </span>
          </div>
          <p class="mt-1 break-all font-mono text-xs text-[var(--atms-text-3)]">{{ plugin.id }}</p>
          <div class="mt-3 flex flex-wrap gap-2 text-[11px] text-[var(--atms-text-2)]">
        ...
        <label class="inline-flex items-center justify-between gap-3 md:justify-end">
          <span class="text-xs" :class="plugin.enabled ? 'text-[var(--atms-success)]' : 'text-[var(--atms-text-3)]'">
          ...
          <span
            aria-hidden="true"
            class="relative inline-flex h-6 w-11 flex-none rounded-full border transition-colors duration-150 peer-focus-visible:ring-2 peer-focus-visible:ring-[var(--atms-accent-border)] peer-focus-visible:ring-offset-2 peer-focus-visible:ring-offset-black"
            :class="[
              plugin.enabled ? 'border-[var(--atms-accent)] bg-[var(--atms-accent)]' : 'border-[var(--atms-border-strong)] bg-[var(--atms-surface-2)]',
    ...
    <p v-if="!loading && registry && !registry.plugins.length" class="rounded-2xl border border-dashed border-[var(--atms-border-strong)] px-4 py-8 text-center text-sm text-[var(--atms-text-3)]">
