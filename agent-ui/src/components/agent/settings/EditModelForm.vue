...
<template>
  <div class="space-y-5 p-5">
    <section class="space-y-3 border-b border-[var(--atms-border)] pb-5">
      <div class="text-xs font-medium text-[var(--atms-text-2)]">{{ t('settings.models.edit.provider') }}</div>
      <div
        class="flex min-h-11 items-center justify-between gap-3 rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3"
      >
        <div class="min-w-0">
          <div class="truncate text-sm text-[var(--atms-text-1)]">{{ setting.provider_name }}</div>
          <div class="truncate text-[11px] text-[var(--atms-text-3)]">
            {{ setting.provider_id }} · {{ setting.endpoint_name || setting.endpoint_id }}
          </div>
        </div>
        <span class="inline-flex flex-shrink-0 items-center gap-1 text-xs text-[var(--atms-text-3)]">
    ...
    <section class="grid gap-3 sm:grid-cols-2">
      <label class="space-y-1">
        <span class="text-xs text-[var(--atms-text-3)]">{{ t('settings.models.form.modelName') }}</span>
        <div
          class="flex h-10 items-center rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 font-mono text-sm text-[var(--atms-text-2)]"
        >
          {{ setting.model_name }}
        </div>
      </label>
      <label class="space-y-1">
        <span class="text-xs text-[var(--atms-text-3)]">{{ t('settings.models.form.aliasPlaceholder') }}</span>
        <input
          v-model="displayName"
    ...
    <section class="space-y-2">
      <div class="text-xs font-medium text-[var(--atms-text-2)]">
    ...
    <section v-if="setting.supports_tts" class="grid gap-3 sm:grid-cols-3">
      <label class="space-y-1">
        <span class="text-xs text-[var(--atms-text-3)]">{{ t('settings.models.form.voice') }}</span>
        <input
          v-model="ttsVoice"
          class="h-10 w-full rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none focus:border-[var(--atms-accent-border)]"
        />
      ...
      <label class="space-y-1">
        <span class="text-xs text-[var(--atms-text-3)]">{{ t('settings.models.edit.format') }}</span>
        <input
          v-model="ttsFormat"
          class="h-10 w-full rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none focus:border-[var(--atms-accent-border)]"
        />
      ...
      <label class="space-y-1">
        <span class="text-xs text-[var(--atms-text-3)]">{{ t('settings.models.edit.sampleRate') }}</span>
        <input
    ...
    <label class="block space-y-1">
      <span class="text-xs font-medium text-[var(--atms-text-2)]">API Key</span>
      <input
        v-model="apiKey"
        type="password"
        class="h-10 w-full rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none focus:border-[var(--atms-accent-border)]"
    ...
    <div class="flex justify-end gap-2 pt-2">
      <button
        type="button"
        class="rounded-md border border-[var(--atms-border)] px-4 py-2 text-sm text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-1)]"
        @click="emit('cancel')"
      >
      ...
      <button
        type="button"
        class="inline-flex items-center gap-2 rounded-md bg-[var(--atms-accent)] px-4 py-2 text-sm font-medium text-[var(--atms-on-accent)] hover:bg-[color:color-mix(in_srgb,var(--atms-accent)_85%,white)] disabled:opacity-50"
        :disabled="submitting"
        @click="submit"
      >
