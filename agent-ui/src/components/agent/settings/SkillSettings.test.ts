...
async function mount(): Promise<HTMLElement> {
  listDetectedManagerSkills.mockResolvedValue({
    ...
    data: {
      total: 2,
      root: '/home/test/.atms/skills',
      skills: [{
        id: 'atms-cli',
        name: 'atms-cli',
        description: 'Operate Atms through its local CLI.',
        relative_path: 'skills/atms-cli/SKILL.md',
        source: 'home',
        enabled: true,
      }, {
        id: 'palquery',
...
describe('SkillSettings', () => {
  it('renders a read-only static catalog using SKILL.md front matter metadata', async () => {
    ...
    expect(mounted.querySelector('[data-testid="agent-settings-skills-count"]')?.textContent)
      .toContain('2')
    expect(mounted.textContent).toContain('Operate Atms through its local CLI.')
    expect(mounted.textContent).toContain('查询幻兽帕鲁资料和配种路线。')
    expect(mounted.textContent).toContain('/home/test/.atms/skills')
