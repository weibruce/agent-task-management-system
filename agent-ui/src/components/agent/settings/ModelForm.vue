...
<template>
  <div class="space-y-5 p-5">
    ...
    <div class="contents">
      <div>
        <div class="mb-2 flex items-center justify-between">
          <span class="text-xs font-medium text-[var(--atms-text-2)]">{{
            t('settings.models.form.credentialType')
          }}</span>
          <div class="relative">
            <Search
              class="pointer-events-none absolute left-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[var(--atms-text-3)]"
            />
            <input
              v-model="searchQuery"
              class="h-8 w-44 rounded-lg border border-[var(--atms-border)] bg-[var(--atms-control)] pl-7 pr-2 text-xs text-[var(--atms-text-1)] outline-none focus:border-[var(--atms-accent-border)]"
        ...
        <div
          class="max-h-64 space-y-3 overflow-y-auto rounded-xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3"
        >
          <div v-for="group in groupedCredentials" :key="group.provider.id">
            <div
              class="mb-1.5 px-1 text-[11px] font-semibold uppercase tracking-wider text-[var(--atms-text-3)]"
            >
              {{ group.provider.name }}
            </div>
              ...
              <button
                ...
                :class="[
                  ...
                  isSelectedCredentialOption(item)
                    ? 'border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)] shadow-[0_0_16px_color-mix(in_srgb,var(--atms-accent)_12%,transparent)]'
                    : 'border-[var(--atms-border)] bg-[var(--atms-surface-1)] hover:border-[var(--atms-border-strong)] hover:bg-[var(--atms-surface-1)]'
                ]"
              ...
              >
                <span
                  :class="[
                    ...
                    isSelectedCredentialOption(item)
                      ? 'border-[var(--atms-accent)] bg-[var(--atms-accent)]'
                      : 'border-[var(--atms-border-strong)]'
                  ]"
                >
                  <span
                    v-if="isSelectedCredentialOption(item)"
                    class="h-1.5 w-1.5 rounded-full bg-[var(--atms-on-accent)]"
                  />
                </span>
                <div class="min-w-0 flex-1">
                  <div class="text-sm font-medium text-[var(--atms-text-1)]">
                    {{ localizedPlanLabel(item.planType) }}
                  </div>
                  <div class="mt-0.5 flex items-center gap-2 text-[11px] text-[var(--atms-text-3)]">
                    <span>{{
          ...
          <div v-if="!groupedCredentials.length" class="py-6 text-center text-xs text-[var(--atms-text-3)]">
        ...
        <div
          ...
          class="mt-3 space-y-1.5 rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 py-2.5"
        >
          <div class="flex items-center gap-2 text-[11px] text-[var(--atms-text-3)]">
            <span>{{ t('settings.models.form.supportedProtocols') }}</span>
            <span
              v-for="p in selectedCredential.protocols"
              :key="p"
              class="rounded bg-[var(--atms-surface-1)] px-1.5 py-0.5 text-[var(--atms-text-2)]"
          ...
          <div
            ...
            :key="row.label"
            class="flex items-center gap-2 text-[11px] text-[var(--atms-text-3)]"
          >
            <span>{{ row.label }}：</span>
            <span class="truncate font-mono text-[var(--atms-text-2)]">{{ row.value }}</span>
          </div>
          <template v-if="isArkVoiceCredential">
            <div class="flex items-center gap-2 text-[11px] text-[var(--atms-text-3)]">
              <span>TTS HTTP：</span>
              <span class="truncate font-mono text-[var(--atms-text-2)]">{{
      ...
      <div v-if="selectedCredential" class="space-y-2">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            <span v-if="probing" class="flex items-center gap-1 text-[11px] text-[var(--atms-accent)]">
              <Loader2 class="h-3 w-3 animate-spin" /> {{ t('settings.models.form.probing') }}
            </span>
            <span v-else-if="probedModels.length" class="text-[11px] text-[var(--atms-success)]">
              {{ t('settings.models.form.probedCount', { count: probedModels.length }) }}
            </span>
            <button
              v-if="canProbeModels && !probing"
              type="button"
              class="flex items-center gap-1 text-[11px] text-[var(--atms-text-3)] transition-colors hover:text-[var(--atms-accent)]"
              :title="t('settings.models.form.reprobe')"
              @click="runProbe"
            >
              <RefreshCw class="h-3 w-3" />
            </button>
            <span class="text-[11px] text-[var(--atms-text-3)]">{{
        ...
        <div
          v-if="probeError"
          class="rounded-md border border-[var(--atms-warning-border)] bg-[var(--atms-warning-soft)] px-3 py-1.5 text-[11px] text-[var(--atms-warning)]"
        >
        ...
        <div v-if="availableDisplayModels.length" class="grid gap-2 sm:grid-cols-2">
          <button
            ...
            :class="[
              ...
              m.probed
                ? 'border-[var(--atms-success-border)] bg-[var(--atms-success-soft)] text-[var(--atms-success)] hover:border-[var(--atms-success-border)]'
                : 'border-[var(--atms-border)] bg-[var(--atms-surface-1)] text-[var(--atms-text-2)] hover:border-[var(--atms-accent-border)] hover:bg-[var(--atms-accent-soft)]'
            ]"
            @click="addModel(m.id)"
          >
            <span class="min-w-0">
              <span class="block truncate">{{ m.display_name }}</span>
              <span v-if="m.probed" class="mt-0.5 block text-[9px] text-[var(--atms-success)]">{{
                t('settings.models.form.probed')
              }}</span>
            </span>
            <span
              class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full border border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)] text-[var(--atms-accent)]"
            >
        ...
        <div
          v-else
          class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 py-4 text-center text-xs text-[var(--atms-text-3)]"
        >
        ...
        <div
          ...
          class="space-y-2 rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3"
        >
          <div class="flex items-center justify-between gap-3">
            <div class="text-[11px] text-[var(--atms-text-3)]">
          ...
          <div
            ...
            :key="model.id"
            class="grid gap-2 rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-2 sm:grid-cols-[minmax(0,180px)_minmax(0,1fr)_32px]"
          >
            <span class="min-w-0 truncate py-1.5 text-sm text-[var(--atms-text-2)]" :title="model.id">
              {{ model.displayName }}
            </span>
            <input
              :value="model.alias"
              class="h-8 flex-1 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-2 text-xs text-[var(--atms-text-1)] outline-none focus:border-[var(--atms-accent-border)]"
              :placeholder="t('settings.models.form.aliasPlaceholder')"
              @input="setModelAlias(model.id, ($event.target as HTMLInputElement).value)"
            />
            <button
              type="button"
              class="flex h-8 w-8 items-center justify-center rounded-full border border-[var(--atms-danger-border)] text-[var(--atms-danger)] transition hover:bg-[var(--atms-danger-soft)] hover:text-[var(--atms-danger)]"
    ...
    <div
      ...
      class="flex items-center gap-3 rounded-lg border border-[var(--atms-success-border)] bg-[var(--atms-success-soft)] px-3 py-2.5"
    >
      <KeyRound class="h-4 w-4 flex-shrink-0 text-[var(--atms-success)]" />
      <div class="min-w-0 flex-1">
        <div class="text-xs font-medium text-[var(--atms-success)]">
          {{ t('settings.models.form.reuseCredential') }}
        </div>
        <div class="mt-0.5 truncate font-mono text-[11px] text-[var(--atms-text-3)]">
          {{ reusableCredentialDisplay }}
        </div>
      </div>
      <button
        type="button"
        class="flex-shrink-0 text-xs text-[var(--atms-text-2)] transition-colors hover:text-[var(--atms-text-1)]"
    ...
    <label v-else class="block space-y-1">
      <span class="flex items-center justify-between gap-3 text-xs font-medium text-[var(--atms-text-2)]">
        <span>API Key</span>
        <button
          v-if="canReuseCredential"
          type="button"
          class="font-normal text-[color:color-mix(in_srgb,var(--atms-accent)_70%,transparent)] transition-colors hover:text-[var(--atms-accent)]"
      ...
      <input
        v-model="apiKey"
        type="password"
        class="h-10 w-full rounded-lg border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none focus:border-[var(--atms-accent-border)]"
    ...
    <div class="space-y-2">
      ...
      <div class="flex flex-wrap gap-2">
        <span
          class="rounded-full border border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)] px-2.5 py-1 text-xs font-medium uppercase text-[var(--atms-accent)]"
          >{{ purpose }}</span
        >
    ...
    <div class="flex items-center gap-6">
      <label class="flex cursor-pointer items-center gap-2">
        <input
          type="checkbox"
          v-model="isDefault"
          class="h-4 w-4 rounded border-[var(--atms-border-strong)] bg-[var(--atms-control)]"
        />
        <span class="text-sm text-[var(--atms-text-2)]">{{ t('settings.models.setDefault') }}</span>
      </label>
      <label class="flex cursor-pointer items-center gap-2">
        <input
          type="checkbox"
          ...
          class="h-4 w-4 rounded border-[var(--atms-border-strong)] bg-[var(--atms-control)]"
        />
        <span class="text-sm text-[var(--atms-text-2)]">{{ t('settings.actions.enable') }}</span>
      </label>
    </div>
    ...
    <div class="flex justify-end gap-2 pt-2">
      <button
        type="button"
        class="rounded-lg border border-[var(--atms-border)] px-4 py-2 text-sm text-[var(--atms-text-2)] transition-colors hover:bg-[var(--atms-surface-1)] hover:text-[var(--atms-text-1)]"
        @click="emit('cancel')"
      >
      ...
      <button
        ...
        :class="[
          ...
          canSubmit && !isSubmitting
            ? 'bg-[var(--atms-accent)] text-[var(--atms-on-accent)] hover:bg-[color:color-mix(in_srgb,var(--atms-accent)_85%,white)]'
            : 'cursor-not-allowed bg-[var(--atms-control)] text-[var(--atms-text-3)]'
        ]"
        @click="submit"
      >
