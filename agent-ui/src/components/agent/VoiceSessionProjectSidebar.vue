...
function statusColor(status: string): string {
  switch (status) {
    case 'running':
      return 'text-[var(--atms-accent)]'
    case 'completed':
      return 'text-[var(--atms-success)]'
    case 'failed':
      return 'text-[var(--atms-danger)]'
    default:
      return 'text-[var(--atms-text-3)]'
  }
}
...
<template>
  <aside
    class="voice-left-rail flex h-full min-h-0 w-full shrink-0 flex-col border-r border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-4 py-5"
  >
    <div class="voice-left-rail__header mb-4 flex items-center justify-between">
      <div class="voice-left-rail__title min-w-0">
        <div class="truncate text-xl font-semibold text-[var(--atms-text-1)]">{{ t('voice.sidebar.workspace') }}</div>
      </div>
      <div class="voice-left-rail__header-actions flex items-center gap-2">
        <button
          type="button"
          class="voice-left-rail__header-button flex h-11 w-11 shrink-0 touch-manipulation items-center justify-center rounded-full border border-[var(--atms-border)] text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]"
    ...
    <div class="relative mb-4">
      <Search
        class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--atms-text-3)]"
      />
      <input
        v-model="projectQuery"
        class="h-10 w-full rounded-full border border-[var(--atms-border)] bg-[var(--atms-surface-1)] pl-9 pr-3 text-sm text-[var(--atms-text-1)] outline-none placeholder:text-[var(--atms-text-4)] focus:border-[var(--atms-accent-border)]"
        :placeholder="t('voice.sidebar.search')"
      />
    </div>

    <section class="flex min-h-0 flex-1 flex-col">
      <div class="mb-2 flex items-center justify-between px-1 text-xs text-[var(--atms-text-3)]">
      ...
      <div class="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
        <div v-for="project in filteredProjects" :key="project.id" class="group rounded-2xl">
            ...
            :class="
              cn(
                ...
                project.id === store.managerProjectId
                  ? 'border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)] text-[var(--atms-accent)]'
                  : 'border-transparent text-[var(--atms-text-2)] hover:border-[var(--atms-border)] hover:bg-[var(--atms-surface-1)] hover:text-[var(--atms-text-1)]'
              )
            "
          ...
          >
            <Folder class="h-4 w-4 shrink-0 text-[var(--atms-accent)]" />
            <div class="min-w-0 flex-1">
              <div class="truncate text-sm font-medium">{{ project.name }}</div>
              <div class="mt-0.5 truncate text-[11px] text-[var(--atms-text-4)]">
            ...
            >
              <button
                class="rounded-full p-1.5 text-[var(--atms-text-3)] hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]"
              ...
              <button
                class="rounded-full p-1.5 text-[var(--atms-text-4)] hover:bg-[var(--atms-danger-soft)] hover:text-[var(--atms-danger)] disabled:opacity-45"
                :class="confirmDeleteProjectId === project.id ? 'bg-[var(--atms-danger-soft)] text-[var(--atms-danger)] opacity-100' : ''"
          ...
          <div
            ...
            class="mt-2 rounded-2xl border border-[var(--atms-danger-border)] bg-[var(--atms-panel)] p-4 text-[var(--atms-text-1)] shadow-xl shadow-black/25"
            @click.stop
          >
            <div class="flex items-start gap-3">
              <div class="mt-0.5 rounded-full bg-[var(--atms-danger-soft)] p-2 text-[var(--atms-danger)]">
                <Trash2 class="h-4 w-4" />
              </div>
              <div class="min-w-0 flex-1">
                <div class="text-sm font-semibold text-[var(--atms-text-1)]">{{ t('voice.sidebar.removeDirectoryConfirm', { name: project.name }) }}</div>
                <div class="mt-1 break-all font-mono text-xs text-[var(--atms-text-3)]">
                  {{ projectPath(project) || project.id }}
                </div>
                <div class="mt-3 space-y-1.5 text-xs leading-relaxed text-[var(--atms-text-2)]">
            ...
            <div class="mt-4 flex gap-2">
              <button
                class="flex-1 rounded-full bg-[var(--atms-danger-soft)] px-4 py-2 text-sm font-medium text-[var(--atms-danger)] transition hover:bg-[var(--atms-danger-soft)] disabled:opacity-45"
                :disabled="deletingProjectId === project.id"
              ...
              <button
                class="flex-1 rounded-full border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-4 py-2 text-sm text-[var(--atms-text-2)] transition hover:bg-[var(--atms-surface-2)]"
          ...
          <div class="ml-7 mt-1 space-y-1">
            <button
              ...
              :class="
                cn(
                  'flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm text-[var(--atms-text-2)] transition hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]',
              ...
              :class="
                cn(
                  ...
                  session.session_id === props.activeSessionId
                    ? 'border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)]'
                    : 'border-transparent hover:border-[var(--atms-border)] hover:bg-[var(--atms-surface-1)]'
                )
              "
            ...
            >
              ...
              >
                ...
                />
                <div class="min-w-0 flex-1">
                  <div class="truncate text-sm text-[var(--atms-text-1)]">{{ sessionLabel(session) }}</div>
                </div>
                <span class="shrink-0 text-xs text-[var(--atms-text-3)]">{{
                  timeAgo(session.start_time)
                }}</span>
              </button>
              <button
                class="shrink-0 rounded-full p-1.5 text-[var(--atms-text-4)] transition hover:bg-[var(--atms-danger-soft)] hover:text-[var(--atms-danger)] group-hover/session:opacity-100 disabled:opacity-45"
              ...
              <div
                ...
                class="absolute right-0 top-full z-30 mt-1 w-48 rounded-2xl border border-[var(--atms-border)] bg-[var(--atms-panel)] p-3 backdrop-blur-md"
              >
                <p class="pb-2.5 text-xs leading-relaxed text-[var(--atms-text-2)]">
                  {{ t('voice.sidebar.deleteSessionConfirm') }}
                </p>
                <div class="flex gap-2">
                  <button
                    class="flex-1 rounded-full bg-[var(--atms-danger-soft)] px-3 py-1.5 text-xs font-medium text-[var(--atms-danger)] transition hover:bg-[var(--atms-danger-soft)] disabled:opacity-45"
                    :disabled="deletingSessionId === session.session_id"
                  ...
                  <button
                    class="flex-1 rounded-full border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 py-1.5 text-xs text-[var(--atms-text-2)] transition hover:bg-[var(--atms-surface-2)]"
                  ...
                  :class="
                    cn(
                      ...
                      runId === store.currentRunId
                        ...
                        : 'text-[var(--atms-text-3)] hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]/70'
                    )
                  "
            ...
            <button
              ...
              :class="
                cn(
                  'flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm text-[var(--atms-text-3)] transition hover:bg-[var(--atms-surface-1)] hover:text-[var(--atms-text-1)]',
            ...
            <div
              ...
              class="px-3 py-2 text-xs text-[var(--atms-text-4)]"
            >
        ...
        <div
          ...
          class="rounded-2xl border border-[var(--atms-border)] p-4 text-center text-xs text-[var(--atms-text-3)]"
        >
  ...
  <div
    v-if="editingProject"
    class="fixed inset-0 z-[75] flex items-center justify-center bg-[var(--atms-overlay)] px-6 backdrop-blur-sm"
  >
    <section
      class="w-[min(520px,92vw)] rounded-[24px] border border-[var(--atms-border)] bg-[var(--atms-panel)] p-5 text-[var(--atms-text-1)] shadow-2xl"
    >
      <div class="mb-5 flex items-start justify-between gap-4">
        <div>
          <div class="text-xs tracking-[0.18em] text-[var(--atms-accent)]">{{ t('voice.sidebar.directories') }}</div>
          <h2 class="mt-1 text-xl font-semibold">{{ t('voice.project.configuration') }}</h2>
        </div>
        <button
          class="rounded-full p-2 text-[var(--atms-text-3)] hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]"
      ...
      <label class="mb-1 block text-xs text-[var(--atms-text-3)]">{{ t('voice.project.name') }}</label>
      <input
        v-model="editName"
        class="mb-3 h-10 w-full rounded-xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 text-sm outline-none focus:border-[var(--atms-accent-border)]"
        :placeholder="t('voice.project.namePlaceholder')"
      />

      <label class="mb-1 block text-xs text-[var(--atms-text-3)]">{{ t('voice.project.description') }}</label>
      <textarea
        v-model="editDescription"
        class="mb-3 h-20 w-full resize-none rounded-xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 py-2 text-sm outline-none focus:border-[var(--atms-accent-border)]"
        :placeholder="t('voice.project.optional')"
      />

      <label class="mb-1 block text-xs text-[var(--atms-text-3)]">{{ t('voice.project.path') }}</label>
      <input
        v-model="editWorkspacePath"
        class="mb-2 h-10 w-full rounded-xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 font-mono text-sm outline-none focus:border-[var(--atms-accent-border)]"
        placeholder="/path/to/directory"
      />
      <div class="mb-5 text-xs text-[var(--atms-text-3)]">
        {{ t('voice.project.contextHint') }}
      </div>

      <div class="mb-5 rounded-2xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
        <div class="mb-3 flex items-center gap-2 text-sm font-medium text-[var(--atms-text-1)]">
          <GitBranch class="h-4 w-4 text-[var(--atms-accent)]" />
          {{ t('voice.project.gitRepository') }}
        </div>
        <label class="mb-1 block text-xs text-[var(--atms-text-3)]">Git Token</label>
        <select
          :value="selectedGitServerId || ''"
          class="mb-3 h-10 w-full rounded-xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 text-sm text-[var(--atms-text-1)] outline-none focus:border-[var(--atms-accent-border)]"
        ...
        <label class="mb-1 block text-xs text-[var(--atms-text-3)]">{{ t('voice.project.repository') }}</label>
        <select
          v-model="selectedRepoFullName"
          class="h-10 w-full rounded-xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 text-sm text-[var(--atms-text-1)] outline-none focus:border-[var(--atms-accent-border)] disabled:opacity-45"
        ...
        <div class="mt-2 text-xs text-[var(--atms-text-3)]">
      ...
      <div class="flex justify-end gap-2">
        <button
          class="rounded-full px-4 py-2 text-sm text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]"
          @click="closeProjectSettings"
        >
          {{ t('settings.actions.cancel') }}
        </button>
        <button
          class="flex items-center gap-2 rounded-full bg-[var(--atms-accent)] px-4 py-2 text-sm font-medium text-[var(--atms-on-accent)] hover:opacity-85 disabled:opacity-45"
...
.voice-session-sidebar__gamepad-focus {
  ...
  outline-offset: 2px;
  box-shadow:
    0 0 0 1px var(--atms-accent-border),
    0 0 22px var(--atms-accent-soft);
}
</style>
