...
function mcpRuntimeStatusClass(server: MCPServer): string {
  ...
  if (status === 'runtime_available') return 'bg-[var(--atms-success-soft)] text-[var(--atms-success)]'
  if (status === 'configured') return 'bg-[var(--atms-surface-1)] text-[var(--atms-text-2)]'
  if (status === 'enabled') return 'bg-[var(--atms-info-soft)] text-[var(--atms-info)]'
  if (status === 'unavailable_with_reason') return 'bg-[var(--atms-danger-soft)] text-[var(--atms-danger)]'
  return 'bg-[var(--atms-warning-soft)] text-[var(--atms-warning)]'
}
...
function wireGuardStatusClass(): string {
  if (wireGuardStatus.value.connected) return 'bg-[var(--atms-success-soft)] text-[var(--atms-success)]'
  if (wireGuardStatus.value.lastError) return 'bg-[var(--atms-danger-soft)] text-[var(--atms-danger)]'
  if (wireGuardConfigured.value || wireGuardStatus.value.configured) return 'bg-[var(--atms-warning-soft)] text-[var(--atms-warning)]'
  return 'bg-[var(--atms-surface-2)] text-[var(--atms-text-2)]'
}
...
<template>
  <div class="agent-settings-shell relative flex h-screen flex-col overflow-hidden bg-[var(--atms-bg)] p-2 text-[var(--atms-text-1)] md:flex-row md:p-4">
    <div class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_24%_12%,var(--atms-ambient-accent),transparent_32%),radial-gradient(circle_at_82%_4%,var(--atms-ambient-info),transparent_28%)]" />
    <aside class="relative z-10 flex w-full flex-shrink-0 flex-col rounded-t-[24px] border border-b-0 border-[var(--atms-settings-divider)] bg-[var(--atms-settings-sidebar)] shadow-[var(--atms-shadow-panel)] backdrop-blur-xl md:w-[224px] md:rounded-l-[30px] md:rounded-tr-none md:border-b md:border-r-0 xl:w-[338px]">
      <div class="hidden h-10 flex-shrink-0 md:block" />
      <div class="px-3 pt-3 md:px-4 md:pt-0">
        <button class="flex h-11 w-full items-center gap-2 rounded-full border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 text-left text-base text-[var(--atms-text-1)] hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]" @click="back">
      ...
      <nav class="mt-3 flex flex-none gap-1 overflow-x-auto px-3 pb-2 text-sm md:mt-6 md:block md:flex-1 md:overflow-y-auto md:px-4 md:pb-0 md:text-[15px]">
        <div class="mb-3 hidden px-1 text-[11px] tracking-[0.22em] text-[var(--atms-text-3)] md:block">Atms</div>
        <button
          ...
          :class="activeTab === tab.id ? 'border border-[var(--atms-settings-active-border)] bg-[var(--atms-settings-active)] text-[var(--atms-text-1)]' : 'border border-transparent text-[var(--atms-text-2)] hover:border-[var(--atms-settings-divider)] hover:bg-[var(--atms-settings-card-hover)] hover:text-[var(--atms-text-1)]'"
      ...
      <div class="hidden border-t border-[var(--atms-border)] px-4 py-4 text-xs text-[var(--atms-text-3)] md:block">
        API {{ apiBaseUrl }}
      </div>
    </aside>

    <main class="relative z-10 min-h-0 min-w-0 flex-1 overflow-y-auto rounded-b-[24px] border border-[var(--atms-settings-divider)] bg-[var(--atms-panel)] shadow-[var(--atms-shadow-panel)] backdrop-blur-xl md:rounded-bl-none md:rounded-r-[30px]">
      <div class="mx-auto w-full max-w-[1040px] px-4 py-5 sm:px-5 sm:py-7 xl:px-10 xl:py-12">
        <div class="flex items-start justify-between gap-4">
          <div>
            <h1 class="text-2xl font-semibold tracking-normal text-[var(--atms-text-1)] sm:text-3xl">{{ activeTabLabel }}</h1>
            <p v-if="activeTabDescription" class="mt-3 text-sm text-[var(--atms-text-3)]">{{ activeTabDescription }}</p>
          </div>
          <button class="flex h-10 flex-shrink-0 items-center gap-2 rounded-full border border-[var(--atms-border)] px-3 text-sm text-[var(--atms-text-1)] hover:bg-[var(--atms-surface-2)] sm:h-11 sm:px-4" @click="refreshAll">
        ...
        <section v-if="activeTab === 'workspace'" data-testid="agent-settings-section-workspace" class="mt-10 space-y-6">
          <div class="grid gap-3 sm:grid-cols-3">
            <div class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4">
              <div class="text-sm text-[var(--atms-text-3)]">Workspace Path</div>
            ...
            <div class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4">
              <div class="text-sm text-[var(--atms-text-3)]">Connected Nodes</div>
              <div class="mt-2 text-2xl font-semibold">{{ runtimeStatus?.connected_nodes ?? 0 }}</div>
            </div>
            <div class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4">
              <div class="text-sm text-[var(--atms-text-3)]">Active Runs</div>
              <div class="mt-2 text-2xl font-semibold">{{ runtimeStatus?.active_runs ?? 0 }}</div>
            </div>
          </div>
          <div class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)]">
            <div class="border-b border-[var(--atms-border)] px-4 py-3">
              <div class="font-semibold">当前项目 / 工作区</div>
              <div class="mt-1 text-sm text-[var(--atms-text-2)]">当前后端没有本地目录导入 API，这里显示现有 Project、Storage 和 Git 绑定状态。</div>
            </div>
            <div class="grid gap-4 p-4 md:grid-cols-[300px_1fr]">
              <div class="space-y-2">
                <button
                  ...
                  :class="selectedProjectId === project.id ? 'border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)]' : 'border-[var(--atms-border)] bg-[var(--atms-surface-1)] hover:bg-[var(--atms-surface-1)]'"
                  @click="selectProject(project.id)"
                >
                  <div class="truncate text-sm font-medium">{{ project.name }}</div>
                  <div class="mt-1 text-xs text-[var(--atms-text-3)]">{{ project.active_changes }} active changes</div>
                </button>
                <div v-if="!projects.length" data-testid="agent-settings-workspace-projects-empty" class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3 text-sm text-[var(--atms-text-3)]">暂无 Project。</div>
                <div class="rounded-md border border-[var(--atms-warning-border)] bg-[var(--atms-warning-soft)] p-3 text-sm text-[var(--atms-warning)]" data-testid="workspace-directory-import-tracked-unsupported">
              ...
              <div v-if="selectedProject" class="space-y-4">
                <div class="grid gap-3 sm:grid-cols-2">
                  <div class="rounded-md bg-[var(--atms-surface-1)] p-3">
                    <div class="text-xs text-[var(--atms-text-3)]">Project ID</div>
                    <div class="mt-1 break-all font-mono text-sm">{{ selectedProject.id }}</div>
                  </div>
                  <div class="rounded-md bg-[var(--atms-surface-1)] p-3">
                    <div class="text-xs text-[var(--atms-text-3)]">Git</div>
                    <div class="mt-1 text-sm">{{ splitRepo(selectedProject.git_repo_name) }}</div>
                  </div>
                  <div class="rounded-md bg-[var(--atms-surface-1)] p-3">
                    <div class="text-xs text-[var(--atms-text-3)]">Storage</div>
                    <div class="mt-1 text-sm">{{ projectStorages.length }} 个挂载配置</div>
                  </div>
                  <div class="rounded-md bg-[var(--atms-surface-1)] p-3">
                    <div class="text-xs text-[var(--atms-text-3)]">Updated</div>
                    <div class="mt-1 text-sm">{{ formatDate(selectedProject.updated_at) }}</div>
                  </div>
                </div>
                <div class="space-y-2">
                  <div v-for="storage in projectStorages" :key="storage.id" class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3 text-sm">
                    <div class="font-medium">{{ storage.name }}</div>
                    <div class="mt-1 text-[var(--atms-text-3)]">{{ storage.storage_type }} · {{ storage.mount_point || storage.storage_path }}</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section v-if="activeTab === 'git'" data-testid="agent-settings-section-git" class="mt-8 space-y-5">
          <div class="grid gap-3 md:grid-cols-3">
            <div class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4">
              <div class="text-xs text-[var(--atms-text-3)]">Git Servers</div>
              <div class="mt-2 text-2xl font-semibold text-[var(--atms-text-1)]">{{ gitServers.length }}</div>
            </div>
            <div class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4">
              <div class="text-xs text-[var(--atms-text-3)]">{{ t('settings.git.tokenStatus') }}</div>
              <div class="mt-2 text-2xl font-semibold text-[var(--atms-success)]">{{ validGitServerCount }}/{{ gitServers.length }}</div>
            </div>
            <div class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4">
              <div class="text-xs text-[var(--atms-text-3)]">{{ t('settings.git.projectLink') }}</div>
              <div class="mt-2 text-sm font-medium text-[var(--atms-text-1)]">{{ t('settings.git.selectWhenAdding') }}</div>
              <div class="mt-1 text-xs text-[var(--atms-text-3)]">{{ t('settings.git.linkHint') }}</div>
            </div>
          </div>

          <section class="overflow-hidden rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)]">
            <div class="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--atms-border)] px-4 py-3.5">
              <div>
                <h2 class="text-base font-semibold text-[var(--atms-text-1)]">{{ t('settings.git.savedTitle') }}</h2>
                <p class="mt-1 text-sm text-[var(--atms-text-3)]">{{ t('settings.git.savedSummary', { count: gitServers.length }) }}</p>
              </div>
              <button
                class="inline-flex h-10 items-center gap-2 rounded-md bg-[var(--atms-accent)] px-4 text-sm text-[var(--atms-on-accent)] hover:bg-[var(--atms-accent-hover)] disabled:opacity-40"
                :disabled="loading"
                @click="openGitDrawer"
            ...
            <div v-if="gitServers.length" class="divide-y divide-[var(--atms-border)]">
              <div v-for="server in gitServers" :key="server.server_id" :data-testid="`agent-settings-git-server-item-${server.server_id}`" class="grid gap-3 px-4 py-3.5 md:grid-cols-[minmax(0,1fr)_150px_150px] md:items-center">
                <div class="min-w-0">
                  <div class="flex flex-wrap items-center gap-2">
                    <div class="truncate font-medium text-[var(--atms-text-1)]">{{ server.name }}</div>
                    <span class="rounded-full border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-2 py-0.5 text-[11px] uppercase tracking-wide text-[var(--atms-text-2)]">{{ server.platform_type }}</span>
                  </div>
                  <div class="mt-1 truncate font-mono text-xs text-[var(--atms-text-3)]">{{ server.api_endpoint }}</div>
                  <div class="mt-1 text-xs text-[var(--atms-text-4)]">
                    {{ server.git_user_name || t('settings.git.userUnset') }} · {{ server.git_user_email || t('settings.git.emailUnset') }}
                  </div>
                </div>
                <div class="flex items-center gap-1.5 text-sm" :class="server.token_valid ? 'text-[var(--atms-success)]' : 'text-[var(--atms-danger)]'" :data-testid="`agent-settings-git-validity-${server.server_id}`">
                ...
                <div class="flex justify-start gap-2 md:justify-end">
                  <button class="inline-flex h-9 items-center rounded-md border border-[var(--atms-border)] px-3 text-sm text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-1)] disabled:opacity-40" :disabled="saving === `verify-git-${server.server_id}`" @click="verifyGit(server)" :data-testid="`agent-settings-git-verify-${server.server_id}`">
                    <Loader2 v-if="saving === `verify-git-${server.server_id}`" class="mr-2 h-4 w-4 animate-spin" />
                    {{ t('settings.actions.verify') }}
                  </button>
                  <button class="inline-flex h-9 w-9 items-center justify-center rounded-md border border-[var(--atms-danger-border)] text-[var(--atms-danger)] hover:bg-[var(--atms-danger-soft)] disabled:opacity-40" :disabled="saving === `delete-git-${server.server_id}`" @click="removeGit(server)" :data-testid="`agent-settings-git-delete-${server.server_id}`"><Trash2 class="h-4 w-4" /></button>
                </div>
              </div>
            </div>
            <div v-else class="px-4 py-8 text-sm text-[var(--atms-text-3)]">
        ...
        >
          <section class="space-y-4">
            <div class="flex flex-wrap items-start justify-between gap-4">
              <div>
                <h2 class="text-base font-semibold text-[var(--atms-text-1)]">{{ t('settings.wireguard.connection') }}</h2>
              </div>
              <span
            ...
            <div class="overflow-hidden rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)]">
              <div class="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--atms-border)] px-4 py-3.5">
                <div class="grid min-w-[240px] gap-1.5 text-sm text-[var(--atms-text-2)]">
                  <label class="text-[var(--atms-text-2)]" for="agent-settings-wireguard-profile-select">Profile</label>
                  <select
                    ...
                    class="h-10 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm text-[var(--atms-text-1)] outline-none"
                  >
                ...
                <div class="flex flex-wrap items-center gap-2">
                  <button
                    class="inline-flex h-10 items-center rounded-md bg-[var(--atms-accent)] px-3 py-2 text-sm text-[var(--atms-on-accent)] hover:bg-[var(--atms-accent-hover)] disabled:opacity-40"
                  ...
                  <button
                    class="inline-flex h-10 items-center rounded-md border border-[var(--atms-border)] px-3 py-2 text-sm text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-1)] disabled:opacity-40"
              ...
              <div class="grid gap-2 border-b border-[var(--atms-border)] px-4 py-3 text-xs text-[var(--atms-text-3)] sm:grid-cols-2 xl:grid-cols-4">
                <div>
                  endpoint:
                  <span class="font-mono text-[var(--atms-text-2)]">{{ wireGuardStatus.endpoint || wireGuardForm.endpoint || '-' }}</span>
                </div>
                <div>
                  tunnel:
                  <span class="font-mono text-[var(--atms-text-2)]">{{ wireGuardStatus.tunnelName || wireGuardProfileName || '-' }}</span>
                </div>
                <div>
                  rx/tx:
                ...
                <div>
                  last handshake:
                  <span class="font-mono text-[var(--atms-text-2)]">{{ formatWireGuardHandshake(wireGuardStatus.latestHandshakeEpochMillis) }}</span>
                </div>
                <div v-if="wireGuardStatus.lastError" class="sm:col-span-2 xl:col-span-4">
                  error:
                  <span class="font-mono text-[var(--atms-danger)]">{{ wireGuardStatus.lastError }}</span>
                </div>
              </div>
              <div class="grid gap-3 px-4 py-4 xl:grid-cols-2">
                <label class="grid gap-2 text-sm text-[var(--atms-text-2)]">
                ...
                <label class="grid gap-2 text-sm text-[var(--atms-text-2)] xl:col-span-2">
                  Allowed IPs
                  <input
                ...
                <label class="grid gap-2 text-sm text-[var(--atms-text-2)] xl:col-span-2">
                  ...
                  <textarea
                    ...
                    class="min-h-[320px] resize-y rounded-md border border-[var(--atms-border)] bg-[var(--atms-code-bg)] px-3 py-2 font-mono text-sm leading-6 text-[var(--atms-text-1)] outline-none"
              ...
              <div class="flex flex-wrap items-center gap-2 border-t border-[var(--atms-border)] px-4 py-3.5">
                <button
                  class="inline-flex h-10 items-center rounded-md border border-[var(--atms-border)] px-3 py-2 text-sm text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-1)]"
                ...
                <button
                  class="inline-flex h-10 items-center rounded-md border border-[var(--atms-danger-border)] px-3 py-2 text-sm text-[var(--atms-danger)] hover:bg-[var(--atms-danger-soft)] disabled:opacity-40"
              ...
              <div
                ...
                class="border-t border-[var(--atms-danger-border)] bg-[var(--atms-danger-soft)] px-4 py-3 text-sm text-[var(--atms-danger)]"
              >
                {{ wireGuardError }}
              </div>
            </div>
        ...
        <section v-if="activeTab === 'voice'" data-testid="agent-settings-section-voice" class="mt-10 space-y-8">
          <div
            ...
            class="rounded-xl border border-[var(--atms-info-border)] bg-[var(--atms-info-soft)] px-4 py-3 text-sm text-[var(--atms-info)]"
          ...
          <section class="space-y-3">
            <div>
              <h2 class="text-base font-semibold text-[var(--atms-text-1)]">{{ t('settings.voice.narration') }}</h2>
              <p class="mt-1 text-sm text-[var(--atms-text-3)]">{{ t('settings.voice.narrationDescription') }}</p>
            </div>
            <div
              class="overflow-hidden rounded-xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)]"
              :class="{ 'opacity-45': codexLiveVoiceEffective }"
            >
              <div class="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-5 px-4 py-3.5">
                <div class="min-w-0">
                  <div class="font-medium text-[var(--atms-text-1)]">{{ t('settings.voice.commentary') }}</div>
                  <div class="mt-1 text-sm leading-6 text-[var(--atms-text-3)]">
                ...
                <button
                  ...
                  :class="commentaryNarrationEnabled ? 'border-[var(--atms-info-border)] bg-[var(--atms-info)]' : 'border-[var(--atms-border-strong)] bg-[var(--atms-surface-2)]'"
          ...
          <section class="space-y-3">
            <div>
              <h2 class="text-base font-semibold text-[var(--atms-text-1)]">{{ t('settings.voice.controls') }}</h2>
              <p class="mt-1 text-sm text-[var(--atms-text-3)]">{{ t('settings.voice.controlsDescription') }}</p>
            </div>
            <div class="overflow-hidden rounded-xl border border-[var(--atms-border)] bg-[var(--atms-surface-1)]">
              <div class="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-5 px-4 py-3.5">
                <div>
                  <div class="font-medium text-[var(--atms-text-1)]">{{ t('settings.voice.inputButton') }}</div>
                  <div class="mt-1 text-sm leading-6 text-[var(--atms-text-3)]">
                ...
                <span
                  class="rounded-full px-3 py-1.5 text-sm"
                  :class="voiceButtonBindingMode === 'hid' ? 'bg-[var(--atms-accent-soft)] text-[var(--atms-accent)]' : 'bg-[var(--atms-success-soft)] text-[var(--atms-success)]'"
                >
                  {{ voiceButtonBindingMode === 'hid' ? 'WebHID' : t('settings.voice.keyboardButton') }}
                </span>
              </div>
              <div class="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-5 border-t border-[var(--atms-border)] px-4 py-3.5">
                <div class="min-w-0">
                  <div class="flex items-center gap-2 font-medium text-[var(--atms-text-1)]">
                    <Mic class="h-4 w-4 text-[var(--atms-accent)]" />
                    {{ t('settings.voice.microphone') }}
                  </div>
                  <div class="mt-1 text-sm leading-6 text-[var(--atms-text-3)]">
                    {{ t('settings.voice.microphoneDescription') }}
                  </div>
                  <div v-if="voiceAudioInputStatus" class="mt-1 text-xs text-[var(--atms-text-2)]">{{ voiceAudioInputStatus }}</div>
                </div>
                <div class="flex min-w-0 items-center gap-2">
                  <select
                    ...
                    class="h-10 min-w-[260px] rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 text-sm text-[var(--atms-text-1)] outline-none"
                  ...
                  <div
                    ...
                    class="max-w-[260px] truncate rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 py-2 text-sm text-[var(--atms-text-2)]"
                  ...
                  <button
                    class="rounded-lg border border-[var(--atms-border)] px-3 py-2 text-sm text-[var(--atms-text-2)] transition hover:bg-[var(--atms-surface-2)] disabled:opacity-40"
              ...
              <div class="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-5 border-t border-[var(--atms-border)] px-4 py-3.5">
                <div>
                  <div class="font-medium text-[var(--atms-text-1)]">{{ t('settings.voice.hidBinding') }}</div>
                  <div class="mt-1 text-sm text-[var(--atms-text-3)]">{{ t('settings.voice.hidBindingDescription') }}</div>
                </div>
                <div class="min-w-0 max-w-[360px] truncate font-mono text-sm text-[var(--atms-text-2)]" :title="formatVoiceHidBinding(voiceHidBinding)">
                  {{ formatVoiceHidBinding(voiceHidBinding) }}
                </div>
              </div>
              <div class="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-5 border-t border-[var(--atms-border)] px-4 py-3.5">
                <div>
                  <div class="font-medium text-[var(--atms-text-1)]">{{ t('settings.voice.keyboardBinding') }}</div>
                  <div class="mt-1 text-sm text-[var(--atms-text-3)]">{{ t('settings.voice.keyboardBindingDescription') }}</div>
                </div>
                <div class="min-w-0 max-w-[260px] truncate font-mono text-sm text-[var(--atms-text-2)]" :title="formatVoiceKeyboardBinding(voiceKeyboardBinding)">
                  {{ formatVoiceKeyboardBinding(voiceKeyboardBinding) }}
                </div>
              </div>
              <div class="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-5 border-t border-[var(--atms-border)] px-4 py-3.5">
                <div>
                  <div class="font-medium text-[var(--atms-text-1)]">{{ t('settings.voice.status') }}</div>
                  <div class="mt-1 text-sm" :class="voiceHidSecureContext ? 'text-[var(--atms-success)]' : 'text-[var(--atms-warning)]'">
                  ...
                  <div v-if="voiceHidBindingStatus" class="mt-1 text-xs text-[var(--atms-text-2)]">{{ voiceHidBindingStatus }}</div>
                </div>
                <div class="flex items-center gap-2">
                  <button
                    class="rounded-lg bg-[var(--atms-surface-3)] px-3.5 py-2 text-sm font-medium text-[var(--atms-text-1)] transition hover:bg-[var(--atms-surface-3)] disabled:opacity-40"
                  ...
                  <button
                    class="rounded-lg border border-[var(--atms-border)] px-3.5 py-2 text-sm text-[var(--atms-text-2)] transition hover:bg-[var(--atms-surface-2)] disabled:opacity-40"
              ...
              <div
                class="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-5 border-t border-[var(--atms-border)] px-4 py-3.5"
                :class="{ 'opacity-45': codexLiveVoiceEffective }"
              >
                <div>
                  <label class="font-medium text-[var(--atms-text-1)]" for="voice-vad-silence-seconds">{{ t('settings.voice.sendDelay') }}</label>
                  <div class="mt-1 text-sm text-[var(--atms-text-3)]">{{ t('settings.voice.sendDelayDescription') }}</div>
                </div>
                <div class="flex items-center gap-2">
                  <input
                    ...
                    class="h-9 w-24 rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 text-sm text-[var(--atms-text-1)] outline-none"
                    @change="saveVoiceVadSilenceSetting"
                    @blur="saveVoiceVadSilenceSetting"
                  />
                  <span class="text-sm text-[var(--atms-text-3)]">{{ t('settings.voice.seconds') }}</span>
                </div>
              </div>
            </div>
          </section>
        ...
        <section v-if="activeTab === 'mcp'" data-testid="agent-settings-section-mcp" class="mt-10 space-y-6">
          <div class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4" data-testid="agent-settings-asset-diagnostics">
            <h2 class="font-semibold">新增 MCP Server</h2>
            <div class="mt-4 grid gap-3 md:grid-cols-2">
              <input v-model="newMcp.name" data-testid="agent-settings-mcp-create-name" class="h-10 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none" placeholder="名称" />
              <select v-model="newMcp.type" data-testid="agent-settings-mcp-create-type" class="h-10 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none">
                <option value="STDIO">STDIO</option>
                <option value="SSE">SSE</option>
              </select>
              <input v-model="newMcp.command" data-testid="agent-settings-mcp-create-command" class="h-10 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none" placeholder="STDIO command" />
              <input v-model="newMcp.url" data-testid="agent-settings-mcp-create-url" class="h-10 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none" placeholder="SSE URL" />
              <input v-model="newMcp.arguments" data-testid="agent-settings-mcp-create-arguments" class="h-10 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none" placeholder="arguments" />
              <input v-model="newMcp.environment_variables" data-testid="agent-settings-mcp-create-env" class="h-10 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none" placeholder="env JSON / string" />
            </div>
            <button data-testid="agent-settings-mcp-create-submit" class="mt-3 rounded-md bg-[var(--atms-accent)] px-3 py-2 text-sm text-[var(--atms-on-accent)]" @click="createMcpServer">添加 MCP</button>
          </div>
          <div class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)]">
            <div v-for="server in mcpServers" :key="server.id" :data-testid="`agent-settings-mcp-server-item-${server.id}`" class="grid gap-3 border-b border-[var(--atms-border)] px-4 py-3 last:border-0 md:grid-cols-[1fr_100px_120px]">
              <div>
                <div class="font-medium">{{ server.name }}</div>
                <div class="mt-1 text-xs text-[var(--atms-text-3)]">{{ server.type }} · {{ server.url || server.command || '-' }} · {{ formatValue(server.arguments) }}</div>
                ...
                <div v-if="server.runtime_status === 'unavailable_with_reason'" :data-testid="`agent-settings-mcp-availability-diagnostic-${server.id}`" class="mt-1 rounded border border-[var(--atms-danger-border)] bg-[var(--atms-danger-soft)] px-2 py-1 text-xs text-[var(--atms-danger)]">
                  {{ server.runtime_message }}
                </div>
              </div>
              <button :data-testid="`agent-settings-mcp-toggle-${server.id}`" class="rounded-md border border-[var(--atms-border)] px-3 py-1.5 text-sm" @click="toggleMcp(server)">{{ server.enabled ? '停用' : '启用' }}</button>
              <div class="flex items-center justify-end gap-2">
                <button
                  :data-testid="`agent-settings-mcp-refresh-runtime-${server.id}`"
                  class="rounded-md border border-[var(--atms-border)] px-2 py-1.5 text-[var(--atms-text-1)] hover:bg-[var(--atms-surface-1)] disabled:cursor-not-allowed disabled:opacity-50"
                ...
                <button :data-testid="`agent-settings-mcp-delete-${server.id}`" class="rounded-md border border-[var(--atms-danger-border)] px-2 py-1.5 text-[var(--atms-danger)] hover:bg-[var(--atms-danger-soft)]" @click="removeMcp(server)"><Trash2 class="h-4 w-4" /></button>
              </div>
            </div>
            <div v-if="!mcpServers.length" data-testid="agent-settings-mcp-empty-state" class="p-4 text-sm text-[var(--atms-text-3)]">暂无 MCP Server。</div>
          </div>
          <div class="rounded-md border border-[var(--atms-warning-border)] bg-[var(--atms-warning-soft)] p-3 text-sm text-[var(--atms-warning)]">
        ...
        <section v-if="activeTab === 'memory'" data-testid="agent-settings-section-memory" class="mt-10 space-y-6">
          <div class="grid gap-3 sm:grid-cols-3">
            <div class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4" data-testid="agent-settings-memory-stats-total">
              <div class="text-sm text-[var(--atms-text-3)]">总记忆</div>
              <div class="mt-2 text-2xl font-semibold">{{ memoryStats?.total_memories ?? memories.length }}</div>
            </div>
            <div class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4" data-testid="agent-settings-memory-stats-weight">
              <div class="text-sm text-[var(--atms-text-3)]">平均权重</div>
              <div class="mt-2 text-2xl font-semibold">{{ memoryStats?.avg_weight ?? 0 }}</div>
            </div>
            <div class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4" data-testid="agent-settings-experience-graph-summary">
              <div class="text-sm text-[var(--atms-text-3)]">Run experience KG</div>
              <div class="mt-2 flex items-center gap-2 text-sm" :class="experienceGraph?.available ? 'text-[var(--atms-success)]' : 'text-[var(--atms-warning)]'">
              ...
              <div v-if="!experienceGraph?.available" class="mt-1" data-testid="agent-settings-experience-graph-tracked-gap">
                <span class="rounded-full bg-[var(--atms-warning-soft)] px-2 py-0.5 text-xs text-[var(--atms-warning)]">tracked_gap: 经验图谱数据依赖后台 ingest</span>
              </div>
              <div class="mt-2 text-xs" :class="experienceGraphError ? 'text-[var(--atms-danger)]' : 'text-[var(--atms-text-3)]'">{{ experienceGraphError || (experienceGraph?.updated_at ? `更新于 ${formatDate(experienceGraph.updated_at)}` : experienceGraph?.reason || '等待自动 ingest') }}</div>
              <button class="mt-3 flex h-9 w-full items-center justify-center gap-2 rounded-md border border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)] text-sm text-[var(--atms-accent)] hover:bg-[color:color-mix(in_srgb,var(--atms-accent)_18%,transparent)]" @click="router.push('/agent/experience')">
          ...
          <div class="grid gap-4 lg:grid-cols-[1fr_1fr]">
            <div class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4" data-testid="agent-settings-section-decorative-marker">
              <div class="flex items-center justify-between gap-3">
                <div>
                  <h2 class="font-semibold">经验图谱结构</h2>
                  <div class="mt-1 text-xs text-[var(--atms-text-3)]">{{ experienceGraph?.structure_coverage?.message || '尚未读取图谱状态。' }}</div>
                </div>
                <button class="rounded-md border border-[var(--atms-border)] px-3 py-1.5 text-sm hover:bg-[var(--atms-surface-1)]" title="刷新经验图谱" aria-label="刷新经验图谱" @click="refreshMemory">
                  <RefreshCw class="h-4 w-4" />
                </button>
              </div>
              <div class="mt-3 rounded-md border border-[var(--atms-accent-border)] bg-[var(--atms-accent-soft)] px-3 py-2 text-xs text-[var(--atms-text-2)]">
                这里展示的是运行经验图谱：Run 与模板、评分、失败根因和经验的关系。单次 DAG 原始拓扑仍在 DAG 运行页面查看。
              </div>
              <div class="mt-4 grid gap-2 sm:grid-cols-2">
                <div v-for="(ok, name) in experienceGraph?.structure_coverage?.checks || {}" :key="name" class="flex items-center justify-between rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 py-2 text-sm">
                  <span class="text-[var(--atms-text-2)]">{{ coverageLabel(String(name)) }}</span>
                  <span :class="ok ? 'text-[var(--atms-success)]' : 'text-[var(--atms-warning)]'">{{ boolLabel(ok) }}</span>
                </div>
              </div>
              <div class="mt-4 grid grid-cols-3 gap-2 text-sm">
                <div class="rounded-md bg-[var(--atms-surface-1)] p-3">
                  <div class="text-xs text-[var(--atms-text-3)]">节点</div>
                  <div class="mt-1 font-semibold">{{ experienceGraph?.node_count ?? 0 }}</div>
                </div>
                <div class="rounded-md bg-[var(--atms-surface-1)] p-3">
                  <div class="text-xs text-[var(--atms-text-3)]">关系</div>
                  <div class="mt-1 font-semibold">{{ experienceGraph?.relationship_count ?? 0 }}</div>
                </div>
                <div class="rounded-md bg-[var(--atms-surface-1)] p-3">
                  <div class="text-xs text-[var(--atms-text-3)]">失败运行</div>
                  <div class="mt-1 font-semibold">{{ experienceGraph?.failed_runs ?? 0 }}</div>
                </div>
              </div>
            </div>
            <div class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4">
              <h2 class="font-semibold">DAG 模板成功率</h2>
              <div class="mt-3 space-y-3">
                <div v-for="item in experienceGraph?.template_stats || []" :key="item.template" class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
                  <div class="flex items-center justify-between gap-3">
                    <div class="min-w-0 truncate text-sm font-medium">{{ item.template }}</div>
                    <div class="text-sm" :class="item.success_rate >= 0.8 ? 'text-[var(--atms-success)]' : item.success_rate >= 0.5 ? 'text-[var(--atms-warning)]' : 'text-[var(--atms-danger)]'">{{ percent(item.success_rate) }}</div>
                  </div>
                  <div class="mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--atms-surface-2)]">
                    <div class="h-full rounded-full bg-[var(--atms-success)]" :style="{ width: percent(item.success_rate) }"></div>
                  </div>
                  <div class="mt-2 text-xs text-[var(--atms-text-3)]">{{ item.runs }} runs · {{ item.failures }} failures · scorecard pass {{ item.scorecard_passes }}</div>
                  <div v-if="item.problem_categories.length" class="mt-2 flex flex-wrap gap-1">
                    <span v-for="category in item.problem_categories" :key="category" class="rounded bg-[var(--atms-danger-soft)] px-2 py-0.5 text-xs text-[var(--atms-danger)]">{{ category }}</span>
                  </div>
                </div>
                <div v-if="!(experienceGraph?.template_stats?.length)" class="text-sm text-[var(--atms-text-3)]">暂无模板运行统计。</div>
              </div>
            </div>
          </div>
          <div class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4">
            <div class="flex items-start justify-between gap-4">
              <div>
                ...
                <p class="mt-1 text-xs text-[var(--atms-text-3)]">展示当前 asset 解析路径、symlink 状态和子目录可用性。</p>
              </div>
              <span
                class="rounded-full px-2 py-1 text-xs"
                :class="assetDiagnostics?.exists ? 'bg-[var(--atms-success-soft)] text-[var(--atms-success)]' : 'bg-[var(--atms-danger-soft)] text-[var(--atms-danger)]'"
              >
                {{ assetDiagnostics?.env_source || '...' }}
              </span>
            </div>
            <div v-if="assetDiagnosticsError" class="mt-3 rounded-md border border-[var(--atms-danger-border)] bg-[var(--atms-danger-soft)] px-3 py-2 text-sm text-[var(--atms-danger)]">{{ assetDiagnosticsError }}</div>
            <div v-if="assetDiagnostics" class="mt-4 space-y-3">
              <div class="grid gap-3 sm:grid-cols-2">
                <div class="rounded-md bg-[var(--atms-surface-1)] p-3">
                  <div class="text-xs text-[var(--atms-text-3)]">Asset Root</div>
                  <div class="mt-1 break-all font-mono text-sm" data-testid="agent-settings-asset-root">{{ assetDiagnostics.asset_root }}</div>
                </div>
                <div class="rounded-md bg-[var(--atms-surface-1)] p-3">
                  <div class="text-xs text-[var(--atms-text-3)]">Repo Seed</div>
                  <div class="mt-1 break-all font-mono text-sm">{{ assetDiagnostics.repo_seed_path }}</div>
                </div>
              </div>
              <div class="flex flex-wrap gap-2 text-xs">
                <span class="rounded-full px-2 py-1" :class="assetDiagnostics.exists ? 'bg-[var(--atms-success-soft)] text-[var(--atms-success)]' : 'bg-[var(--atms-danger-soft)] text-[var(--atms-danger)]'">
                  {{ assetDiagnostics.exists ? '目录存在' : '目录缺失' }}
                </span>
                <span v-if="assetDiagnostics.is_symlink" class="rounded-full bg-[var(--atms-accent-soft)] px-2 py-1 text-[var(--atms-accent)]">
                  symlink → {{ assetDiagnostics.symlink_target }}
                </span>
                <span v-if="assetDiagnostics.catalog_path" class="rounded-full bg-[var(--atms-success-soft)] px-2 py-1 text-[var(--atms-success)]">catalog.yaml</span>
                <span v-if="assetDiagnostics.experience_graph_path" class="rounded-full bg-[var(--atms-success-soft)] px-2 py-1 text-[var(--atms-success)]">db/graph.json</span>
              </div>
              <div class="grid gap-2 sm:grid-cols-3">
                <div
                  ...
                  :class="status.exists ? 'border-[var(--atms-success-border)] bg-[var(--atms-success-soft)]' : 'border-[var(--atms-danger-border)] bg-[var(--atms-danger-soft)]'"
                >
                  <span class="text-[var(--atms-text-2)]">{{ name }}</span>
                  <CheckCircle2 v-if="status.exists" class="h-3.5 w-3.5 text-[var(--atms-success)]" />
                  <XCircle v-else class="h-3.5 w-3.5 text-[var(--atms-danger)]" />
                </div>
              </div>
              <div class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3" data-testid="agent-settings-orchestration-templates">
                <div class="flex items-center justify-between gap-3">
                  <div>
                    <div class="text-sm font-medium text-[var(--atms-text-1)]">DAG 模板目录</div>
                    <div class="mt-1 text-xs text-[var(--atms-text-3)]">默认只展示 catalog primary 模板；legacy/compat 不进入主要入口。</div>
                  </div>
                  <span class="rounded-full bg-[var(--atms-accent-soft)] px-2 py-1 text-xs text-[var(--atms-accent)]">{{ orchestrationTemplates.length }} primary</span>
                </div>
                <div v-if="orchestrationTemplatesError" class="mt-3 rounded-md border border-[var(--atms-danger-border)] bg-[var(--atms-danger-soft)] px-3 py-2 text-sm text-[var(--atms-danger)]">{{ orchestrationTemplatesError }}</div>
                <div class="mt-3 grid gap-2">
                  <div
                    v-for="template in orchestrationTemplates"
                    :key="template.id"
                    :data-testid="`agent-settings-orchestration-template-${template.id}`"
                    class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] px-3 py-2"
                  >
                    <div class="flex items-start justify-between gap-3">
                      <div class="min-w-0">
                        <div class="truncate text-sm font-medium text-[var(--atms-text-1)]">{{ template.name || template.id }}</div>
                        <div class="mt-1 truncate font-mono text-xs text-[var(--atms-text-3)]">{{ template.id }}</div>
                      </div>
                      <div class="flex flex-shrink-0 items-center gap-1 text-xs">
                        <span class="rounded bg-[var(--atms-success-soft)] px-2 py-0.5 text-[var(--atms-success)]">{{ template.category }}</span>
                        <span class="rounded bg-[var(--atms-surface-2)] px-2 py-0.5 text-[var(--atms-text-1)]">{{ template.node_count }} nodes</span>
                      </div>
                    </div>
                    <div class="mt-2 flex flex-wrap gap-1 text-xs">
                      <span
                        ...
                        :key="profile"
                        class="rounded bg-[var(--atms-info-soft)] px-2 py-0.5 text-[var(--atms-info)]"
                      >
                        {{ profile }}
                      </span>
                      <span v-if="!(template.supported_profiles?.length)" class="rounded bg-[var(--atms-warning-soft)] px-2 py-0.5 text-[var(--atms-warning)]">profile 未声明</span>
                    </div>
                    <div class="mt-2 break-all font-mono text-[11px] text-[var(--atms-text-3)]">{{ template.path }}</div>
                  </div>
                  <div v-if="!orchestrationTemplates.length && !orchestrationTemplatesError" class="text-sm text-[var(--atms-text-3)]">暂无 primary DAG 模板。</div>
                </div>
              </div>
            </div>
          </div>
          <div class="grid gap-4 lg:grid-cols-[1fr_1fr]">
            <div class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4">
              ...
              <div class="mt-3 space-y-3">
                <div v-for="problem in experienceGraph?.problems || []" :key="problem.category" class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
                  <div class="flex items-center justify-between gap-3">
                    <div class="text-sm font-medium">{{ problem.category }}</div>
                    <div class="text-xs text-[var(--atms-text-3)]">{{ problem.count }} 次 · {{ problem.severity || 'unknown' }}</div>
                  </div>
                  <div class="mt-1 text-sm text-[var(--atms-text-2)]">{{ problem.description || '无描述' }}</div>
                  <div v-if="problem.lesson_actions.length" class="mt-2 text-xs text-[var(--atms-success)]">{{ problem.lesson_actions[0] }}</div>
                </div>
                <div v-if="!(experienceGraph?.problems?.length)" class="text-sm text-[var(--atms-text-3)]">暂无失败根因记录。</div>
              </div>
            </div>
            <div class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4">
              <h2 class="font-semibold">最近经验</h2>
              <div class="mt-3 space-y-3">
                <div v-for="lesson in experienceGraph?.lessons || []" :key="lesson.id" class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
                  <div class="text-sm font-medium">{{ lesson.summary || lesson.id }}</div>
                  <div class="mt-1 text-sm text-[var(--atms-text-2)]">{{ lesson.action || '无行动项' }}</div>
                </div>
                <div v-if="!(experienceGraph?.lessons?.length)" class="text-sm text-[var(--atms-text-3)]">暂无可复用经验。</div>
              </div>
            </div>
          </div>
          <div class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-4">
            <div class="grid gap-3 md:grid-cols-[1fr_160px_100px]">
              <input v-model="memoryQuery" class="h-10 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none" placeholder="搜索记忆" data-testid="agent-settings-memory-search" />
              <select v-model="memoryKind" class="h-10 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none">
              ...
              <button class="rounded-md border border-[var(--atms-border)] px-3 py-2 text-sm hover:bg-[var(--atms-surface-1)]" @click="refreshMemory">搜索</button>
            </div>
            <div class="mt-4 grid gap-3 md:grid-cols-[1fr_150px_150px_80px]">
              <input v-model="newMemory.content" class="h-10 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none" placeholder="新增记忆" />
              <input v-model="newMemory.kind" class="h-10 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none" placeholder="kind" />
              <input v-model="newMemory.topic" class="h-10 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm outline-none" placeholder="topic" />
              <button class="rounded-md bg-[var(--atms-accent)] px-3 py-2 text-sm text-[var(--atms-on-accent)]" @click="addMemory" data-testid="agent-settings-memory-create-button">添加</button>
            </div>
          </div>
          <div class="rounded-lg border border-[var(--atms-border)] bg-[var(--atms-surface-1)]">
            <div v-for="memory in memories" :key="memory.id" class="grid gap-3 border-b border-[var(--atms-border)] px-4 py-3 last:border-0 md:grid-cols-[80px_1fr_110px_80px]" :data-testid="`agent-settings-memory-item-${memory.id}`">
              <div class="font-mono text-xs text-[var(--atms-text-3)]">#{{ memory.id }}</div>
              <div>
                <div class="text-sm">{{ memory.content }}</div>
                <div class="mt-1 text-xs text-[var(--atms-text-3)]">{{ formatDate(memory.created_at) }}</div>
              </div>
              <div class="text-sm text-[var(--atms-text-2)]">{{ memory.kind }} · {{ memory.weight.toFixed(2) }}</div>
              <button class="justify-self-end rounded-md border border-[var(--atms-danger-border)] px-2 py-1.5 text-[var(--atms-danger)] hover:bg-[var(--atms-danger-soft)]" @click="removeMemory(memory)" :data-testid="`agent-settings-memory-delete-${memory.id}`"><Trash2 class="h-4 w-4" /></button>
            </div>
            <div v-if="!memories.length" data-testid="agent-settings-memory-empty-state" class="p-4 text-sm text-[var(--atms-text-3)]">暂无记忆。</div>
          </div>
        </section>
      </div>
    </main>
      ...
      <transition name="settings-git-panel">
        <aside v-if="gitDrawerOpen" class="settings-git-panel" data-testid="agent-settings-git-create-drawer" @click.stop>
          <form class="flex h-full flex-col" @submit.prevent="createGit">
            <header class="flex shrink-0 items-center justify-between border-b border-[var(--atms-border)] px-5 py-4">
              <div>
                <div class="text-xs tracking-[0.18em] text-[var(--atms-text-3)]">Git Server</div>
                <h2 class="mt-1 text-base font-semibold text-[var(--atms-text-1)]">{{ t('settings.git.drawerTitle') }}</h2>
              </div>
              <button
                type="button"
                class="inline-flex h-9 w-9 items-center justify-center border border-[var(--atms-border)] text-[var(--atms-text-2)] transition-colors hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]"
            ...
            <div class="min-h-0 flex-1 overflow-y-auto px-5 py-5">
              <div class="grid gap-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                  <label class="grid min-w-0 gap-2 text-sm text-[var(--atms-text-2)]">
                    Git user
                    <input v-model.trim="newGitServer.git_user_name" class="h-10 min-w-0 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm text-[var(--atms-text-1)] outline-none" :placeholder="t('settings.git.optional')" />
                  </label>
                  <label class="grid min-w-0 gap-2 text-sm text-[var(--atms-text-2)]">
                    Git email
                    <input v-model.trim="newGitServer.git_user_email" class="h-10 min-w-0 rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 text-sm text-[var(--atms-text-1)] outline-none" :placeholder="t('settings.git.optional')" />
                  </label>
                </div>
                <label class="grid gap-2 text-sm text-[var(--atms-text-2)]">
                  Token
                ...
                <label class="grid gap-2 text-sm text-[var(--atms-text-2)]">
                  ...
                  <textarea
                    ...
                    class="min-h-[86px] resize-none rounded-md border border-[var(--atms-border)] bg-[var(--atms-control)] px-3 py-2 text-sm text-[var(--atms-text-1)] outline-none"
            ...
            <footer class="flex shrink-0 justify-end gap-2 border-t border-[var(--atms-border)] px-5 py-4">
              <button
                type="button"
                class="h-10 border border-[var(--atms-border)] px-4 text-sm text-[var(--atms-text-2)] hover:bg-[var(--atms-surface-1)] hover:text-[var(--atms-text-1)]"
                @click="closeGitDrawer"
              >
              ...
              <button
                type="submit"
                class="inline-flex h-10 items-center justify-center gap-2 bg-[var(--atms-accent)] px-4 text-sm text-[var(--atms-on-accent)] hover:bg-[var(--atms-accent-hover)] disabled:opacity-40"
...
.agent-settings-shell {
  ...
  --settings-field-bg: var(--atms-control);
  --settings-border: var(--atms-settings-divider);
}
...
.agent-settings-shell :deep([tabindex]:focus-visible) {
  outline: 2px solid var(--atms-accent);
  outline-offset: 3px;
  box-shadow: 0 0 0 6px color-mix(in srgb, var(--atms-accent) 12%, transparent);
}
...
.agent-settings-shell :deep(.bg-\[\#343434\]) {
  background: var(--atms-settings-card);
}

.agent-settings-shell :deep(.bg-\[var\(--atms-surface-1\)\]) {
  background-color: var(--atms-settings-card);
}

.agent-settings-shell :deep(.border-\[var\(--atms-border\)\]) {
  border-color: var(--atms-settings-divider);
}
...
.settings-git-panel {
  ...
  border-left: 1px solid var(--atms-border);
  background: var(--atms-panel);
  color: var(--atms-text-1);
