...
function stateColor(state: ProgressState): string {
  if (state === 'done') return 'text-[var(--atms-success)]'
  if (state === 'running') return 'text-[var(--atms-info)]'
  if (state === 'failed') return 'text-[var(--atms-danger)]'
  return 'text-[var(--atms-text-4)]'
}
  ...
  <div class="space-y-4 p-3">
    <section class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
      <div class="flex items-center justify-between gap-3">
        <div>
          <div class="text-xs font-medium text-[var(--atms-text-1)]">{{ t('agent.progress.title') }}</div>
          <div class="mt-0.5 text-[10px] text-[var(--atms-text-4)]">
        ...
        <div
          :class="cn(
            ...
            store.isCompleted ? 'bg-[var(--atms-success-soft)] text-[var(--atms-success)]' :
            store.isRunning ? 'bg-[var(--atms-info-soft)] text-[var(--atms-info)]' :
            store.isCancelled ? 'bg-[var(--atms-warning-soft)] text-[var(--atms-warning)]' :
            'bg-[var(--atms-surface-2)] text-[var(--atms-text-3)]'
          )"
        >
      ...
      <div class="mt-3 h-1.5 overflow-hidden rounded-full bg-[var(--atms-surface-2)]">
        <div
          :class="cn('h-full rounded-full transition-all', store.isFailed ? 'bg-[var(--atms-danger)]' : store.isCancelled ? 'bg-[var(--atms-warning)]' : 'bg-[var(--atms-success)]')"
          :style="{ width: `${progressPercent}%` }"
        />
      </div>
      <div class="mt-2 flex justify-between text-[10px] text-[var(--atms-text-3)]">
    ...
    <section class="space-y-1.5">
      <div
        ...
        :key="item.key"
        class="flex items-start gap-2 rounded-md px-2 py-2 hover:bg-[var(--atms-surface-2)]"
      >
        <component
        ...
        <div class="min-w-0 flex-1">
          <div class="text-[11px] font-medium text-[var(--atms-text-1)]">{{ item.label }}</div>
          <div class="mt-0.5 truncate text-[10px] text-[var(--atms-text-4)]">{{ item.detail }}</div>
        </div>
      </div>
    </section>
    ...
    <section
      ...
      class="rounded-md border border-[var(--atms-danger-border)] bg-[var(--atms-danger-soft)] p-3"
    >
      <div class="mb-2 flex items-center gap-2 text-xs font-medium text-[var(--atms-danger)]">
      ...
      <button
        ...
        :key="node.id"
        class="flex w-full items-center justify-between rounded px-2 py-1 text-left text-[11px] text-[var(--atms-danger)] hover:bg-[var(--atms-danger-soft)]"
        @click="store.selectNode(node.id); store.inspectorTab = 'evidence'"
      >
        <span class="truncate">{{ node.name }}</span>
        <span class="ml-2 text-[10px] text-[var(--atms-danger)]">{{ t('agent.progress.viewEvidence') }}</span>
      </button>
    </section>

    <section class="grid grid-cols-2 gap-2">
      <div class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
        <div class="mb-2 flex items-center gap-1.5 text-[10px] text-[var(--atms-text-3)]">
          <TerminalSquare class="h-3 w-3" />
          {{ t('agent.progress.tokens') }}
        </div>
        <div class="space-y-1 text-[11px] text-[var(--atms-text-1)]">
      ...
      <div class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
        ...
        <div class="text-lg font-semibold text-[var(--atms-text-1)]">{{ fmtCost(audit?.total_cost_usd) }}</div>
        <div class="mt-1 text-[10px] text-[var(--atms-text-4)]">
    ...
    <section class="rounded-md border border-[var(--atms-border)] bg-[var(--atms-surface-1)] p-3">
      <div class="mb-3 flex items-center justify-between gap-3">
        ...
        <div
          :class="cn(
            'rounded-full px-2 py-0.5 text-[10px] font-medium',
            !scorecard ? 'bg-[var(--atms-surface-2)] text-[var(--atms-text-3)]' :
            scorecard.passed ? 'bg-[var(--atms-success-soft)] text-[var(--atms-success)]' :
            'bg-[var(--atms-danger-soft)] text-[var(--atms-danger)]'
          )"
        >
      ...
      <div v-if="scorecard?.failures.length" class="mb-3 space-y-1">
        <button
          ...
          class="w-full rounded bg-[var(--atms-danger-soft)] px-2 py-1.5 text-left hover:bg-[var(--atms-danger-soft)]"
          @click="openFailureEvidence(failure)"
        >
          <div class="truncate text-[11px] text-[var(--atms-danger)]">{{ failure.message }}</div>
          <div class="mt-0.5 truncate text-[10px] text-[var(--atms-danger)]">{{ failure.rule }}</div>
        </button>
      </div>
      <div v-else class="mb-3 text-[11px] text-[var(--atms-text-3)]">
      ...
      <div class="grid grid-cols-2 gap-2 text-[11px]">
        <div class="rounded bg-[var(--atms-surface-2)] px-2 py-1.5">
          <div class="text-[var(--atms-text-3)]">{{ t('agent.progress.toolCalls') }}</div>
          <div class="mt-0.5 text-[var(--atms-text-1)]">{{ toolTotals.toolUse }}</div>
        </div>
        <div class="rounded bg-[var(--atms-surface-2)] px-2 py-1.5">
          <div class="text-[var(--atms-text-3)]">{{ t('agent.progress.toolResults') }}</div>
          <div class="mt-0.5 text-[var(--atms-text-1)]">{{ toolTotals.toolResult }}</div>
        </div>
      </div>
    </section>
