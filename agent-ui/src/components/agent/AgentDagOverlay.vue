...
const statusStyle: Record<string, { dot: string; border: string; shadow?: string; bg?: string; text?: string }> = {
  pending:   { dot: 'bg-[var(--atms-text-3)]', border: 'border-[var(--atms-border-strong)]', bg: 'bg-[var(--atms-surface-1)]', text: 'text-[var(--atms-text-2)]' },
  ready:     { dot: 'bg-[var(--atms-info)]',            border: 'border-[var(--atms-info-border)]',       bg: 'bg-[var(--atms-info-soft)]',  text: 'text-[var(--atms-info)]' },
  running:   { dot: 'bg-[var(--atms-success)] animate-pulse', border: 'border-[var(--atms-success-border)]', shadow: 'shadow-[0_0_18px_var(--atms-success-soft)]', bg: 'bg-[var(--atms-success-soft)]', text: 'text-[var(--atms-success)]' },
  completed: { dot: 'bg-[var(--atms-info)]',            border: 'border-[var(--atms-info-border)]',          bg: 'bg-[var(--atms-info-soft)]', text: 'text-[var(--atms-info)]' },
  failed:    { dot: 'bg-[var(--atms-danger)]',             border: 'border-[var(--atms-danger-border)]',           bg: 'bg-[var(--atms-danger-soft)]',  text: 'text-[var(--atms-danger)]' },
  skipped:   { dot: 'bg-[var(--atms-warning)]',          border: 'border-[var(--atms-warning-border)]',        bg: 'bg-[var(--atms-warning-soft)]', text: 'text-[var(--atms-warning)]' },
}

const edgeStyles: Record<string, { stroke: string; dashed: boolean; label: string }> = {
  always:     { stroke: 'var(--atms-text-3)', dashed: false, label: '' },
  on_success: { stroke: 'var(--atms-success)', dashed: false, label: '✓' },
  on_failure: { stroke: 'var(--atms-danger)', dashed: true,  label: '✗' },
}
  ...
  <div class="h-full">
    ...
    >
      <template #node-dagNode="nodeProps">
        <div
          class="px-3 py-2 rounded-lg border-2 bg-[var(--atms-bg-raised)] shadow-md min-w-[160px] transition-all cursor-pointer hover:shadow-lg"
          :class="[
            (statusStyle[nodeProps.data?.status]?.border || 'border-[var(--atms-border-strong)]'),
        ...
        >
          ...
          <div class="flex items-center gap-2">
            ...
            />
            <span class="font-medium text-sm text-[var(--atms-text-1)] truncate">
          ...
          <div class="flex items-center justify-between mt-1">
            <span
              ...
              :class="statusStyle[nodeProps.data?.status]?.text || 'text-[var(--atms-text-3)]'"
            >
              {{ formatDuration(nodeProps.data?.startedAt, nodeProps.data?.completedAt) }}
            </span>
            <span v-else class="text-[10px] font-mono text-[var(--atms-text-4)]">--</span>
          </div>

          <!-- Context usage bar -->
          <div v-if="nodeProps.data?.contextUsagePct != null" class="mt-1">
            <div class="flex justify-between text-[9px]">
              <span class="text-[var(--atms-text-3)]">ctx</span>
              <span :class="contextUsageText(nodeProps.data.contextUsagePct)">
                {{ nodeProps.data.contextUsagePct }}%
              </span>
            </div>
            <div class="h-1 rounded-full bg-[var(--atms-surface-2)] mt-0.5">
              <div
