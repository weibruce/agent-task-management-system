  ...
  <div ref="wrapperRef" class="message-list-wrapper">
    ...
    <div class="message-list">
      ...
      <div v-if="loading" class="loading-state">
        <div class="inline-block h-6 w-6 border-2 border-[var(--atms-accent)] border-t-transparent rounded-full animate-spin"></div>
        <p class="text-sm text-[var(--atms-text-3)] mt-2">加载消息中...</p>
      </div>
      ...
      >
        <slot name="empty">
          <p class="text-sm text-[var(--atms-text-3)]">{{ emptyText || '暂无消息' }}</p>
        </slot>
      </div>
...
.round-nav-fixed {
  @apply flex items-center gap-3 px-4 py-2 bg-[var(--atms-panel)] backdrop-blur border-b border-[var(--atms-border)];
  position: sticky;
  top: 0;
  z-index: 50;
  flex-shrink: 0;
...
.round-nav-title {
  @apply text-xs font-medium text-[var(--atms-text-3)] whitespace-nowrap;
}

.round-count {
  @apply bg-[var(--atms-surface-2)] px-2 py-0.5 rounded;
}
...
.round-nav-btn {
  ...
  @apply bg-[var(--atms-surface-1)] text-[var(--atms-text-2)];
  @apply hover:bg-[var(--atms-accent-hover)] hover:text-[var(--atms-on-accent)];
}

.round-nav-btn.active {
  @apply bg-[var(--atms-accent)] text-[var(--atms-on-accent)];
}
