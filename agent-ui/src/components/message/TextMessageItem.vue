...
<template>
  <div :class="itemClass">
    ...
    <template v-if="isThinking">
      <div
        class="thinking-header cursor-pointer flex items-center gap-1.5 px-1 py-1 text-xs text-[var(--atms-text-3)] hover:text-[var(--atms-text-1)] transition-colors"
      ...
      <div v-if="thinkingExpanded" class="thinking-content ml-5 px-3 py-2 text-xs text-[var(--atms-text-3)] rounded max-h-[50vh] overflow-y-auto whitespace-pre-wrap border-l-2 border-[var(--atms-border-strong)]">
        {{ displayContent }}
      </div>
    </template>
...
.text-message-item.user-message {
  background-color: var(--atms-accent-soft);
  border-left: 3px solid var(--atms-accent);
}
...
.status-dot {
  ...
  flex-shrink: 0;
  margin-top: 6px;
  background-color: var(--atms-text-2);
}
...
.thinking-content::-webkit-scrollbar-thumb {
  background-color: var(--atms-border-strong);
  border-radius: 4px;
}
</style>
