...
<style scoped>
.pr-closeout { display:grid; grid-auto-rows:max-content; align-content:start; gap:16px; min-width:0; color:var(--atms-text-1); }
.pr-closeout__backdrop { position:fixed; inset:0; z-index:999; background:var(--atms-overlay); backdrop-filter:blur(8px); }
.pr-closeout[data-expanded="true"] { position:fixed; inset:18px; z-index:1000; overflow:auto; border:1px solid var(--atms-accent-border); border-radius:8px; padding:22px; background:var(--atms-bg-raised); box-shadow:var(--atms-shadow-floating); }
.pr-closeout__hero { display:grid; grid-template-columns:minmax(0,1fr) auto; gap:18px 24px; align-items:start; border-bottom:1px solid var(--atms-border); padding:2px 2px 18px; animation:pr-closeout-enter .28s ease-out both; }
.pr-closeout__identity { min-width:0; }
.pr-closeout__eyebrow { display:inline-flex; align-items:center; gap:7px; color:var(--atms-accent); font-size:12px; font-weight:800; }
.pr-closeout__identity h2 { margin:8px 0 0; color:var(--atms-text-1); font-size:clamp(22px,2.4vw,32px); line-height:1.14; letter-spacing:0; text-wrap:balance; }
.pr-closeout__identity > p { max-width:920px; margin:9px 0 0; color:var(--atms-text-2); font-size:13px; line-height:1.6; }
.pr-closeout__meta { display:flex; flex-wrap:wrap; gap:7px; margin-top:11px; }
.pr-closeout__meta span { border:1px solid var(--atms-border); border-radius:999px; padding:4px 8px; color:var(--atms-text-3); background:var(--atms-surface-1); font-size:10px; font-weight:750; }
.pr-closeout__sha { font-family:ui-monospace,SFMono-Regular,Menlo,monospace; }
.pr-closeout__decision { display:grid; min-width:162px; grid-template-columns:auto 1fr; gap:2px 9px; align-items:center; border-left:3px solid var(--atms-border-strong); padding:7px 12px; background:var(--atms-surface-1); }
.pr-closeout__decision > svg { grid-row:1 / span 3; color:var(--atms-text-3); }
.pr-closeout__decision span,.pr-closeout__decision small { color:var(--atms-text-3); font-size:10px; }
.pr-closeout__decision strong { font-size:15px; }
.pr-closeout__decision[data-status="ready"] { border-color:var(--atms-success); background:var(--atms-success-soft); }
.pr-closeout__decision[data-status="ready"] > svg { color:var(--atms-success); }
.pr-closeout__decision[data-status="blocked"] { border-color:var(--atms-warning); background:var(--atms-warning-soft); }
.pr-closeout__decision[data-status="blocked"] > svg { color:var(--atms-warning); }
.pr-closeout__decision[data-status="changes_requested"] { border-color:var(--atms-danger); background:var(--atms-danger-soft); }
.pr-closeout__decision[data-status="changes_requested"] > svg { color:var(--atms-danger); }
.pr-closeout__actions { grid-column:1 / -1; display:flex; flex-wrap:wrap; gap:8px; }
.pr-closeout__action { display:inline-flex; min-height:36px; align-items:center; justify-content:center; gap:7px; border:1px solid var(--atms-border-strong); border-radius:6px; padding:0 12px; color:var(--atms-text-2); background:var(--atms-control); font:inherit; font-size:12px; font-weight:800; text-decoration:none; cursor:pointer; transition:border-color .18s ease,background .18s ease,transform .18s ease; }
.pr-closeout__action:hover { border-color:var(--atms-accent-border); color:var(--atms-accent); background:var(--atms-accent-soft); transform:translateY(-1px); }
.pr-closeout__action--primary { border-color:var(--atms-accent-border); color:var(--atms-accent); background:var(--atms-accent-soft); }
...
.pr-closeout__compact-metrics { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); border:1px solid var(--atms-border); border-radius:7px; overflow:hidden; }
.pr-closeout__compact-metrics > div { display:grid; grid-template-columns:auto 1fr; gap:1px 6px; align-items:center; min-width:0; padding:8px; border-right:1px solid var(--atms-border); }
.pr-closeout__compact-metrics > div:last-child { border-right:0; }
.pr-closeout__compact-metrics svg { grid-row:1 / span 2; color:var(--atms-success); }
.pr-closeout__compact-metrics [data-tone="warning"] svg { color:var(--atms-warning); }
.pr-closeout__compact-metrics strong { font-size:16px; line-height:1; }
.pr-closeout__compact-metrics span { overflow:hidden; color:var(--atms-text-3); font-size:8px; text-overflow:ellipsis; white-space:nowrap; }
...
.pr-closeout__compact-flow > div { display:flex; flex:0 0 auto; align-items:center; gap:5px; color:var(--atms-text-2); font-size:9px; font-weight:750; }
.pr-closeout__compact-flow i { display:grid; place-items:center; width:22px; height:22px; border:1px solid var(--atms-border); border-radius:999px; color:var(--atms-text-3); background:var(--atms-surface-1); }
.pr-closeout__compact-flow [data-status="passed"] i { border-color:var(--atms-success-border); color:var(--atms-success); }
.pr-closeout__compact-flow [data-status="running"] i { border-color:var(--atms-accent-border); color:var(--atms-accent); }
.pr-closeout__compact-flow [data-status="blocked"] i,.pr-closeout__compact-flow [data-status="failed"] i { border-color:var(--atms-warning-border); color:var(--atms-warning); }
.pr-closeout__compact-flow b { color:var(--atms-text-4); font-size:12px; }
.pr-closeout__compact-footer { display:grid; gap:7px; border-top:1px solid var(--atms-border); padding-top:8px; }
.pr-closeout__compact-footer > p { display:grid; grid-template-columns:auto minmax(0,1fr); gap:7px; align-items:start; margin:0; color:var(--atms-warning); }
.pr-closeout__compact-footer > p > span { display:-webkit-box; overflow:hidden; color:var(--atms-text-3); font-size:8px; line-height:1.35; -webkit-box-orient:vertical; -webkit-line-clamp:2; }
.pr-closeout__compact-footer strong { display:block; margin-bottom:2px; color:var(--atms-text-1); font-size:9px; }
.pr-closeout__compact-platforms { display:flex; flex-wrap:wrap; gap:5px; }
.pr-closeout__compact-platforms span { display:inline-flex; align-items:center; gap:4px; border:1px solid var(--atms-border); border-radius:999px; padding:3px 6px; color:var(--atms-text-3); font-size:8px; }
.pr-closeout__compact-platforms span[data-status="passed"] { border-color:var(--atms-success-border); color:var(--atms-success); }
.pr-closeout__compact-platforms span[data-status="running"] { border-color:var(--atms-accent-border); color:var(--atms-accent); }
.pr-closeout__metrics { display:grid; grid-template-columns:repeat(5,minmax(0,1fr)); border:1px solid var(--atms-border); border-radius:8px; overflow:hidden; background:var(--atms-surface-1); }
.pr-closeout__metrics > div { display:grid; grid-template-columns:auto 1fr; gap:1px 8px; align-items:center; min-width:0; padding:12px; border-right:1px solid var(--atms-border); animation:pr-closeout-enter .28s ease-out both; }
.pr-closeout__metrics > div:last-child { border-right:0; }
.pr-closeout__metrics svg { grid-row:1 / span 2; color:var(--atms-text-3); }
.pr-closeout__metrics strong { color:var(--atms-text-1); font-size:20px; line-height:1; }
.pr-closeout__metrics span { overflow:hidden; color:var(--atms-text-3); font-size:10px; text-overflow:ellipsis; white-space:nowrap; }
.pr-closeout__metrics [data-tone="positive"] svg { color:var(--atms-success); }
.pr-closeout__metrics [data-tone="negative"] svg { color:var(--atms-danger); }
.pr-closeout__metrics [data-tone="warning"] svg { color:var(--atms-warning); }
.pr-closeout__metrics [data-tone="info"] svg { color:var(--atms-info); }
...
.pr-closeout__section { min-width:0; border-top:1px solid var(--atms-border); padding-top:13px; }
.pr-closeout__section > header { display:flex; align-items:start; justify-content:space-between; gap:12px; margin-bottom:11px; }
.pr-closeout__section > header span { color:var(--atms-accent); font-size:10px; font-weight:850; text-transform:uppercase; }
.pr-closeout__section > header h3 { margin:3px 0 0; color:var(--atms-text-1); font-size:15px; letter-spacing:0; }
.pr-closeout__section > header > em { border-radius:999px; padding:3px 7px; color:var(--atms-text-3); background:var(--atms-surface-1); font-size:10px; font-style:normal; font-weight:800; }
.pr-closeout__graph-scroll { overflow-x:auto; border:1px solid var(--atms-border); border-radius:8px; background:linear-gradient(180deg,var(--atms-surface-1),var(--atms-code-bg)); scrollbar-color:var(--atms-scrollbar) transparent; }
.pr-closeout__graph { position:relative; min-height:260px; background-image:linear-gradient(var(--atms-border) 1px,transparent 1px),linear-gradient(90deg,var(--atms-border) 1px,transparent 1px); background-size:28px 28px; }
.pr-closeout__graph > svg { position:absolute; inset:0; width:100%; height:100%; overflow:visible; }
.pr-closeout__graph > svg > path { fill:none; stroke:var(--atms-border-strong); stroke-width:1.5; stroke-dasharray:5 5; animation:pr-closeout-edge-flow 1.4s linear infinite; }
.pr-closeout__graph > svg marker path { fill:var(--atms-text-4); }
.pr-closeout__graph > svg > path[data-status="running"] { stroke:var(--atms-accent); stroke-width:2; }
.pr-closeout__graph > svg > path[data-status="failed"],.pr-closeout__graph > svg > path[data-status="blocked"] { stroke:var(--atms-warning); }
.pr-closeout__graph > svg > path[data-status="passed"] { stroke:var(--atms-success); }
.pr-closeout__flow-node { position:absolute; display:grid; grid-template-columns:auto minmax(0,1fr); gap:5px 8px; align-content:center; border:1px solid var(--atms-border-strong); border-radius:7px; padding:9px 10px 12px; background:var(--atms-panel); box-shadow:var(--atms-shadow-panel); animation:pr-closeout-enter .3s ease-out both; }
.pr-closeout__flow-node > svg { color:var(--atms-text-3); }
...
.pr-closeout__flow-node strong { color:var(--atms-text-1); font-size:11px; }
.pr-closeout__flow-node span { margin-top:3px; color:var(--atms-text-3); font-size:9px; }
.pr-closeout__flow-node > i { position:absolute; right:9px; bottom:6px; left:9px; height:2px; overflow:hidden; border-radius:999px; background:var(--atms-surface-2); }
.pr-closeout__flow-node > i b { display:block; width:var(--progress); height:100%; border-radius:inherit; background:var(--atms-accent); transition:width .28s ease; }
.pr-closeout__flow-node[data-status="passed"] { border-color:var(--atms-success-border); }
.pr-closeout__flow-node[data-status="passed"] > svg { color:var(--atms-success); }
.pr-closeout__flow-node[data-status="running"] { border-color:var(--atms-accent-border); box-shadow:0 0 0 1px var(--atms-accent-soft),var(--atms-shadow-panel); animation:pr-closeout-enter .3s ease-out both,pr-closeout-pulse 1.8s ease-in-out infinite; }
.pr-closeout__flow-node[data-status="running"] > svg { color:var(--atms-accent); animation:pr-closeout-spin 1.2s linear infinite; }
.pr-closeout__flow-node[data-status="failed"],.pr-closeout__flow-node[data-status="blocked"] { border-color:var(--atms-warning-border); }
.pr-closeout__flow-node[data-status="failed"] > svg,.pr-closeout__flow-node[data-status="blocked"] > svg { color:var(--atms-warning); }
.pr-closeout__platforms,.pr-closeout__checks,.pr-closeout__reviews { display:grid; gap:6px; }
.pr-closeout__platforms > div,.pr-closeout__checks > div,.pr-closeout__reviews > div { display:grid; grid-template-columns:auto minmax(0,1fr) auto; gap:8px; align-items:center; min-width:0; border-bottom:1px solid var(--atms-border); padding:8px 2px; }
...
.pr-closeout__platforms svg,.pr-closeout__checks svg,.pr-closeout__reviews svg { color:var(--atms-text-3); }
...
.pr-closeout__platforms span,.pr-closeout__checks span,.pr-closeout__reviews span { margin-top:2px; color:var(--atms-text-3); font-size:9px; }
.pr-closeout__platforms em,.pr-closeout__reviews em,.pr-closeout__checks small { color:var(--atms-text-3); font-size:9px; font-style:normal; }
.pr-closeout__platforms [data-status="passed"] svg,.pr-closeout__checks [data-status="passed"] svg,.pr-closeout__reviews [data-status="resolved"] svg { color:var(--atms-success); }
.pr-closeout__platforms [data-status="failed"] svg,.pr-closeout__checks [data-status="failed"] svg { color:var(--atms-danger); }
.pr-closeout__platforms [data-status="running"] svg,.pr-closeout__checks [data-status="running"] svg { color:var(--atms-accent); animation:pr-closeout-spin 1.2s linear infinite; }
.pr-closeout__checks > div,.pr-closeout__reviews > div { grid-template-columns:auto minmax(0,1fr) auto auto; }
.pr-closeout__checks a,.pr-closeout__reviews a,.pr-closeout__timeline a { display:grid; place-items:center; width:26px; height:26px; border-radius:5px; color:var(--atms-accent); background:var(--atms-accent-soft); }
.pr-closeout__blockers { display:grid; gap:7px; }
.pr-closeout__blockers article { display:grid; grid-template-columns:auto minmax(0,1fr); gap:9px; border-left:3px solid var(--atms-warning); border-radius:6px; padding:9px; background:var(--atms-warning-soft); animation:pr-closeout-enter .25s ease-out both; }
.pr-closeout__blockers article[data-severity="critical"] { border-color:var(--atms-danger); background:var(--atms-danger-soft); }
.pr-closeout__blockers svg { color:var(--atms-warning); }
.pr-closeout__blockers strong { display:block; font-size:11px; }
.pr-closeout__blockers p { margin:4px 0 0; color:var(--atms-text-2); font-size:10px; line-height:1.45; }
.pr-closeout__blockers small { display:block; margin-top:5px; color:var(--atms-warning); font-size:9px; }
.pr-closeout__clear { display:flex; align-items:center; gap:8px; min-height:48px; color:var(--atms-success); font-size:11px; font-weight:800; }
...
.pr-closeout__timeline li { position:relative; display:grid; grid-template-columns:auto minmax(0,1fr) auto; gap:7px 9px; align-items:start; min-width:0; border:1px solid var(--atms-border); border-radius:7px; padding:10px; background:var(--atms-surface-1); animation:pr-closeout-enter .28s ease-out both; }
.pr-closeout__timeline i { display:grid; grid-row:1 / span 2; place-items:center; width:24px; height:24px; border-radius:999px; color:var(--atms-text-3); background:var(--atms-surface-2); }
.pr-closeout__timeline time { grid-column:2; color:var(--atms-text-3); font-size:9px; }
.pr-closeout__timeline div { grid-column:2; min-width:0; }
.pr-closeout__timeline strong { display:block; font-size:11px; }
.pr-closeout__timeline p { margin:3px 0 0; color:var(--atms-text-3); font-size:9px; line-height:1.4; }
.pr-closeout__timeline li[data-status="verified"] i { color:var(--atms-success); background:var(--atms-success-soft); }
.pr-closeout__timeline li[data-status="failed"] i { color:var(--atms-danger); background:var(--atms-danger-soft); }
.pr-closeout__empty { margin:0; padding:16px 0; color:var(--atms-text-4); font-size:10px; }
...
@keyframes pr-closeout-pulse { 50% { box-shadow:0 0 0 4px var(--atms-accent-soft),var(--atms-shadow-panel); } }
@keyframes pr-closeout-spin { to { transform:rotate(360deg); } }
@media (max-width:980px) { .pr-closeout__primary-grid { grid-template-columns:1fr; } .pr-closeout__metrics { grid-template-columns:repeat(3,minmax(0,1fr)); } .pr-closeout__metrics > div:nth-child(3) { border-right:0; } .pr-closeout__metrics > div:nth-child(-n+3) { border-bottom:1px solid var(--atms-border); } }
@media (max-width:640px) { .pr-closeout { gap:13px; } .pr-closeout[data-expanded="true"] { inset:6px; padding:14px; } .pr-closeout:not([data-compact="true"]) .pr-closeout__hero { grid-template-columns:1fr; } .pr-closeout:not([data-compact="true"]) .pr-closeout__decision { min-width:0; } .pr-closeout__metrics { grid-template-columns:repeat(2,minmax(0,1fr)); } .pr-closeout__metrics > div { border-right:1px solid var(--atms-border); border-bottom:1px solid var(--atms-border); } .pr-closeout__metrics > div:nth-child(even) { border-right:0; } .pr-closeout__metrics > div:last-child { grid-column:1 / -1; border-bottom:0; } .pr-closeout__detail-grid { grid-template-columns:1fr; } .pr-closeout__evidence-section { grid-column:auto; } .pr-closeout__timeline { grid-template-columns:1fr; } }
