...
describe('appearance registry', () => {
  ...
  it('migrates both legacy appearance storage formats', () => {
    localStorage.setItem('atms.skin', 'paper')
  ...
  it('applies color scheme, document state, and theme metadata together', () => {
    ...
    expect(document.documentElement.dataset.atmsAppearance).toBe('paper')
  ...
  it('broadcasts the resolved appearance to marked Artifact frames', () => {
    ...
    frame.dataset.atmsArtifactFrame = ''
  ...
  it('supports self-contained plugin tokens and clears them when switching', () => {
    ...
    expect(document.documentElement.style.getPropertyValue('--atms-bg')).toBe('#faf7f2')
    expect(document.documentElement.style.getPropertyValue('--atms-text-1')).toBe('#241b12')

    applyAppearanceToDocument('cockpit')
    expect(document.documentElement.style.getPropertyValue('--atms-bg')).toBe('')
    expect(document.documentElement.style.getPropertyValue('--atms-text-1')).toBe('')
    unsubscribe()
  })
