...
describe('appearance contract', () => {
  it('defines every public semantic token for both built-in appearances', () => {
    const css = source('styles/atms-theme.css')
  ...
  it('keeps the paper appearance on subdued surfaces instead of pure white', () => {
    const css = source('styles/atms-theme.css')
    const paper = css.slice(css.indexOf(":root[data-atms-appearance='paper']"), css.indexOf('::selection'))

    expect(paper).toContain('--atms-bg: #e9edef')
    expect(paper).toContain('--atms-panel: #f7f8f7')
    expect(paper).not.toMatch(/--atms-(?:bg-raised|surface-1|panel):\s*#fff(?:fff)?\b/i)
  })
  ...
  it('themes native select popup menus with semantic colors on Windows and other host UIs', () => {
    ...
    expect(css).toMatch(
      /select option,\s*select optgroup\s*\{[^}]*background-color:\s*var\(--atms-bg-raised\);[^}]*color:\s*var\(--atms-text-1\)/s,
    )
    expect(css).toMatch(
  ...
  it('prepaints the fixed cockpit appearance before the application entry module', () => {
    const html = readFileSync(resolve(sourceRoot, '../index.html'), 'utf8')
    expect(html).toContain("dataset.atmsAppearance = 'cockpit'")
    expect(html.indexOf("dataset.atmsAppearance = 'cockpit'")).toBeLessThan(html.indexOf('/src/main.ts'))
    expect(html).toContain('data-atms-appearance="cockpit"')
  })
