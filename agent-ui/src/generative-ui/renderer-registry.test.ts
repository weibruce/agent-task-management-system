...
import type { GenerativeUiStoredNodeV1, AtmsPluginUiProjectionV1 } from 'atms-protocol'
...
describe('GenerativeUiRendererRegistry', () => {
  ...
  it.each(['phone', 'desktop', 'tv'] as const)('projects the PR closeout renderer for %s', device => {
    ...
    projection.kinds[0] = {
      ...projection.kinds[0],
      plugin_id: 'com.atms.pr-closeout',
  ...
  it('removes disabled and unknown built-in renderers without dynamic loading', () => {
    ...
    expect(runtime.unresolved_renderer_ids).toEqual(['com.atms.topic-outline:topic-outline-main'])
  })
  ...
  it('keeps the old topic kind inside the isolated legacy compatibility path', () => {
    ...
    const legacyTopic = node({
      ...
      owner: { id: 'com.atms.content', version: '0.1.0' },
...
function topicProjection(enabled: boolean): AtmsPluginUiProjectionV1 {
