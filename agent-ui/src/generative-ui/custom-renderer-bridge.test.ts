...
import { ATMS_A2UI_CATALOG_ID } from 'atms-protocol'
...
describe('custom Renderer A2UI Worker bridge', () => {
  it('accepts only ready, full-catalog a2ui, and error messages', () => {
    expect(read(message('atms.custom-renderer.ready'))).toEqual({
      type: 'atms.custom-renderer.ready',
      ...identity,
    })
    expect(read(message('atms.custom-renderer.a2ui', { request_id: 'render-1', a2ui }))).toEqual({
      type: 'atms.custom-renderer.a2ui',
      ...identity,
      request_id: 'render-1',
      a2ui,
    })
    expect(read(message('atms.custom-renderer.error', { message: 'safe failure' }))).toMatchObject({
      type: 'atms.custom-renderer.error',
      message: 'safe failure',
    })
    expect(read(message('atms.custom-renderer.resize', { height: 420 }))).toBeUndefined()
    expect(read(message('atms.custom-renderer.action', { action_id: 'approve' }))).toBeUndefined()
  })

  it.each([
    ['foreign window', message('atms.custom-renderer.ready'), { source: {} as WindowProxy }],
    ['foreign origin', message('atms.custom-renderer.ready'), { origin: 'https://attacker.invalid' }],
    ['missing nonce', (() => { const value = message('atms.custom-renderer.ready'); delete value.nonce; return value })(), {}],
    ['wrong nonce', { ...message('atms.custom-renderer.ready'), nonce: 'wrong' }, {}],
    ['wrong plugin', { ...message('atms.custom-renderer.ready'), plugin_id: 'com.attacker.plugin' }, {}],
    ['wrong version', { ...message('atms.custom-renderer.ready'), plugin_version: '2.0.0' }, {}],
    ['wrong renderer', { ...message('atms.custom-renderer.ready'), renderer_id: 'other' }, {}],
    ['wrong digest', { ...message('atms.custom-renderer.ready'), renderer_digest: 'd'.repeat(64) }, {}],
    ['wrong node', { ...message('atms.custom-renderer.ready'), node_id: 'node-two' }, {}],
    ['stale revision', { ...message('atms.custom-renderer.ready'), node_revision: 3 }, {}],
    ['unknown field', { ...message('atms.custom-renderer.ready'), admin_token: 'steal-me' }, {}],
    ['invalid request id', message('atms.custom-renderer.a2ui', { request_id: 'stale', a2ui }), {}],
    ['oversized error', message('atms.custom-renderer.error', { message: 'x'.repeat(501) }), {}],
  ...
  it('rejects non-JSON, cyclic, prototype-bearing, and oversized Worker output', () => {
    ...
    cyclic.self = cyclic
    expect(read(message('atms.custom-renderer.a2ui', { request_id: 'render-1', a2ui: cyclic }))).toBeUndefined()
    expect(read(message('atms.custom-renderer.a2ui', {
  ...
  it('builds a no-DOM bootstrap with deny-by-default CSP and inert source embedding', () => {
    ...
    const srcdoc = buildCustomRendererSrcdoc({
      ...
      nonce,
      identity,
      parent_origin: 'https://ui.atms.test',
    })
    ...
    expect(srcdoc).not.toContain('atms.custom-renderer.action')
    expect(srcdoc).not.toContain('atms.custom-renderer.resize')
  ...
  it('creates an immutable parent init envelope without a second action/schema channel', () => {
    ...
    expect(envelope).toMatchObject({
      type: 'atms.custom-renderer.init',
      nonce,
      node_id: 'node-one',
  ...
  it('strictly binds Manager source responses to the projected identity', () => {
    ...
    const response = {
      ...
      content: `export function render(payload) { return { version: 'v1.0', catalogId: '${ATMS_A2UI_CATALOG_ID}', components: [{ id: 'root', component: 'Text', text: String(payload?.node?.id || 'ok') }] } }`,
    }
