...
import { validateAtmsA2uiSurface } from 'atms-protocol'
import {
  ...
  it('contains untrusted code and returns only native A2UI JSON without iframe DOM', async () => {
    ...
    const bootstrapProbe = await page.evaluate(({ bootstrap, probeIdentity, probeNonce }) => new Promise<string>((resolve) => {
      ...
      worker.postMessage({
        ...
        source: `export function render(payload) { return { version: 'v1.0', catalogId: 'https://atms.dev/a2ui/catalogs/core/v1', components: [{ id: 'root', component: 'Text', text: String(payload?.node?.id || 'probe') }] } }`,
      })
      setTimeout(() => finish('timeout'), 2_000)
    }), { bootstrap: CUSTOM_RENDERER_WORKER_BOOTSTRAP, probeIdentity: identity, probeNonce: nonce })
    expect(bootstrapProbe).toBe('atms.custom-renderer.worker.ready')
    ...
    try {
      ...
      await page.waitForFunction(() => (window as any).__customRendererMessages
        .some((message: any) => message?.type === 'atms.custom-renderer.a2ui'), undefined, {
        polling: 100,
        timeout: 10_000,
      })
    } catch (cause) {
    ...
    expect(messages.map((message: any) => message?.type)).toEqual([
      'atms.custom-renderer.ready',
      'atms.custom-renderer.a2ui',
    ])
    const a2uiMessage = messages.find((message: any) => message?.type === 'atms.custom-renderer.a2ui')
    expect(validateAtmsA2uiSurface(a2uiMessage.a2ui, {
