...
export const CUSTOM_RENDERER_MAX_OUTPUT_BYTES = ATMS_A2UI_MAX_BYTES
...
export type CustomRendererBridgeMessageV1 =
  | ({ type: 'atms.custom-renderer.ready' } & CustomRendererIdentityV1)
  | ({ type: 'atms.custom-renderer.a2ui'; request_id: string; a2ui: unknown } & CustomRendererIdentityV1)
  | ({ type: 'atms.custom-renderer.error'; message: string } & CustomRendererIdentityV1)
...
export function readCustomRendererBridgeMessage(
  ...
  if (value.type === 'atms.custom-renderer.ready' && exactKeys(value)) {
    return { type: value.type, ...identity }
  }
  if (value.type === 'atms.custom-renderer.a2ui'
  ...
  if (value.type === 'atms.custom-renderer.error'
...
(() => {
  ...
  const jsonClone = raw => {
    ...
    if (encoded === undefined || safeEncode(encoded).byteLength > ${ATMS_A2UI_MAX_BYTES}) throw new Error('Renderer output exceeds its byte limit');
  ...
  const fail = (requestId, cause) => {
    ...
    send('atms.custom-renderer.worker.error', { ...(requestId ? { request_id: requestId } : {}), message });
  };
  safeAddEventListener('message', async event => {
    const value = event.data;
    if (!configured) {
      if (!exact(value, ['worker_protocol','type','nonce','identity','source'])
        || value.worker_protocol !== 1 || value.type !== 'atms.custom-renderer.worker.configure'
    ...
    try {
      ...
      send('atms.custom-renderer.worker.a2ui', {
        request_id: value.request_id,
...
(() => {
  ...
  const exactInit = value => value && typeof value === 'object' && !Array.isArray(value)
    ...
    && value.bridge_version === 1 && value.type === 'atms.custom-renderer.init'
  ...
  const worker = new Worker(workerUrl, { name: 'atms-custom-renderer' });
  const stopWorker = () => {
    worker.terminate();
  ...
  const fail = cause => {
    ...
    stopWorker();
    send('atms.custom-renderer.error', { message: String(cause && cause.message ? cause.message : cause || 'Renderer failed').slice(0, 500) });
  };
  const dispatchRender = payload => {
    ...
    worker.postMessage({
      worker_protocol: 1,
      type: 'atms.custom-renderer.worker.render',
      nonce,
      identity,
  ...
  worker.addEventListener('message', event => {
    ...
    if (value.type === 'atms.custom-renderer.worker.ready'
      && keys === ['identity','nonce','type','worker_protocol'].join('\\0')) {
      workerReady = true;
      clearTimeout(bootstrapTimer);
      URL.revokeObjectURL(workerUrl);
      send('atms.custom-renderer.ready');
      if (pendingPayload) {
    ...
    if (value.type === 'atms.custom-renderer.worker.a2ui'
      && keys === ['a2ui','identity','nonce','request_id','type','worker_protocol'].join('\\0')
      && value.request_id === activeRequest) {
      clearTimeout(requestTimer);
      send('atms.custom-renderer.a2ui', { request_id: value.request_id, a2ui: value.a2ui });
      stopWorker();
      return;
    }
    if (value.type === 'atms.custom-renderer.worker.error'
  ...
  worker.postMessage({
    worker_protocol: 1,
    type: 'atms.custom-renderer.worker.configure',
    nonce,
    identity,
    source: pluginSource,
  });
