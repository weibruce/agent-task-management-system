...
import { resolveAtmsBuiltinRendererComponent } from '@/plugins/renderer-component-catalog'
...
export function buildProjectedGenerativeUiRegistry(
  value: AtmsPluginUiProjectionV1,
): ProjectedGenerativeUiRegistryV1 {
  const validation = validateAtmsPluginUiProjection(toRaw(value))
  ...
  for (const renderer of projection.renderers) {
    ...
    const catalogEntry = resolveAtmsBuiltinRendererComponent(renderer.source.id)
    if (!catalogEntry) {
      unresolved.add(`${renderer.plugin_id}:${renderer.renderer_id}`)
      continue
    }
