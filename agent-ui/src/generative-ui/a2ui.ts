...
export type { A2uiComponentV1, AtmsA2uiSurfaceV1 } from 'atms-protocol'
...
/** Atms collection field selectors are always relative to one source item. */
...
export function validateA2uiSurfaceForNode(
  ...
  const validation = validateAtmsA2uiSurface(value, {
...
export function indexA2uiSurface(surface: AtmsA2uiSurfaceV1): ReadonlyMap<string, A2uiComponentV1> {
  return new Map(surface.components.map(component => [component.id, component]))
}

export function a2uiActionNames(surface: AtmsA2uiSurfaceV1): ReadonlySet<string> {
