// 在文件顶部添加日志
// console.log('前端应用启动...');
// console.log('环境变量 VITE_LITE_MODE:', import.meta.env.VITE_LITE_MODE);
// console.log('window.LITE_MODE:', (window as any).LITE_MODE);

import { createApp } from 'vue'
import { createPinia } from 'pinia'
import mitt from 'mitt'
import App from '@/App.vue'
import router from '@/router'
import { i18n } from '@/plugins/i18n'
import { applyAppearanceToDocument, DEFAULT_APPEARANCE_ID } from '@/appearance/appearance-registry'
import { installTouchActivation } from '@/input/touch-activation'

import './styles/atms-theme.css'
import './styles/tailwind.css'
import './style.css'
import './styles/main.css'
import './styles/scrollbar.css'
import Toast from './components/controls/Toast.vue'
import '@vue-flow/core/dist/style.css';

// Appearance is fixed to the default cockpit (dark) theme.
applyAppearanceToDocument(DEFAULT_APPEARANCE_ID)

const removeTouchActivation = installTouchActivation(document)
if (import.meta.hot) {
  import.meta.hot.dispose(removeTouchActivation)
}

const app = createApp(App)
const pinia = createPinia()

// 创建 mitt eventBus
const eventBus = mitt()

// 使用 provide 提供 eventBus
app.provide('eventBus', eventBus)

app.use(router)

app.use(pinia)
app.use(i18n)
app.component('Toast', Toast)

// 在应用挂载前添加

app.mount('#app')
