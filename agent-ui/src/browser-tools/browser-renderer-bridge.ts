...
import { executeAtmsUiTool } from './ui-surface-controller'

const UI_SESSION_ID_KEY = 'atms.browserTools.uiSessionId.v1'
const TAB_ID_KEY = 'atms.browserTools.tabId.v1'
...
export type BrowserToolsTransport = 'renderer' | 'none'
...
export function currentBrowserToolsTurnBinding(): BrowserToolsTurnBinding {
  ...
  return { browser_tools_transport: 'none' }
}
...
export class AtmsBrowserRendererBridge {
  readonly identity: BrowserRendererPageIdentity

  private readonly controller: AtmsUiSurfaceController
  ...
  private handleInvocation(message: Record<string, unknown>): void {
    ...
    const contract = ATMS_UI_TOOL_CONTRACTS.find(candidate => candidate.name === message.tool_name)
