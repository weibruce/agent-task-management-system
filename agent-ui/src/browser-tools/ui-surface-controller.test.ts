...
import {
  ...
  type AtmsUiSurfaceController,
} from './ui-surface-controller'
import type { AtmsUiWidgetDescriptor, AtmsUiWidgetTarget } from './widget-registry'

const widget: AtmsUiWidgetDescriptor = {
  document_id: 'document-one',
  document_revision: 4,
  widget_id: 'widget-one',
  widget_revision: 2,
  kind: 'com.atms.test/card',
  render_state: 'stable',
  status_phase: 'ready',
...
function widgetMethods() {
  return {
    ...
    setWidgetExpanded: vi.fn((_target: AtmsUiWidgetTarget, expanded: boolean) => ({ ...widget, expanded })),
  }
}
...
describe('Atms UI surface controller', () => {
  ...
  it('opens and closes the DAG status surface through semantic actions', async () => {
    ...
    const controller: AtmsUiSurfaceController = {
      getState: () => ({
    ...
    const opened = await executeAtmsUiTool('ui_open_surface', {
      surface: 'dag_status',
      query: 'PR Review',
    }, controller)
    ...
    const closed = await executeAtmsUiTool('ui_close_surface', {
      surface: 'dag_status',
    }, controller)
  ...
  it('describes, focuses, and expands only an exact widget revision', async () => {
    ...
    await expect(executeAtmsUiTool('ui_describe_widget', target, controller))
      .resolves.toMatchObject({ ok: true, widget: { widget_id: 'widget-one' } })
    await expect(executeAtmsUiTool('ui_focus_widget', target, controller))
      .resolves.toMatchObject({ ok: true, widget: { focused: true } })
    await expect(executeAtmsUiTool('ui_set_widget_expanded', {
      ...target,
      expanded: true,
  ...
  it('propagates the commit boundary into a widget action before post-action failure', async () => {
    ...
    await expect(executeAtmsUiTool('ui_focus_widget', {
  ...
  it.each([
    ...
    await expect(executeAtmsUiTool(name, input, controller)).rejects.toThrow()
