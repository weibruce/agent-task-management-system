...
import type { AtmsUiSurfaceController } from './ui-surface-controller'
import {
  createBrowserRendererPageIdentity,
  currentBrowserToolsTurnBinding,
  AtmsBrowserRendererBridge,
...
function controller(
  overrides: Partial<AtmsUiSurfaceController> = {},
...
function readyMessage(
  bridge: AtmsBrowserRendererBridge,
...
async function connectedBridge(
  surfaceController: AtmsUiSurfaceController,
): Promise<{ bridge: AtmsBrowserRendererBridge; socket: FakeSocket }> {
  const socket = new FakeSocket()
  const webSocketFactory = vi.fn(() => socket as unknown as WebSocket)
  const ids = ['ui-session-1', 'tab-1', 'navigation-1']
  const bridge = new AtmsBrowserRendererBridge(surfaceController, {
...
describe('browser renderer bridge', () => {
  ...
  it('does not let an obsolete ticket completion clear or authenticate a newer start', async () => {
    ...
    const bridge = new AtmsBrowserRendererBridge(controller(), {
      sessionStorage: new MemoryStorage(),
  ...
  it('always obtains the ticket and WebSocket through the current UI origin', async () => {
    ...
    expect(socket.sent[0]).toEqual({
      ...
      ...bridge.identity,
      contracts: ATMS_UI_TOOL_CONTRACTS.map(contract => ({
        name: contract.name,
  ...
  it('cancels an invocation before its first visible action and replies once', async () => {
    ...
    const contract = ATMS_UI_TOOL_CONTRACTS.find(item => item.name === 'ui_open_surface')!
    socket.message({
      type: 'tool.invoke',
      version: 1,
      call_id: 'call-1',
  ...
  it('reports indeterminate when a committed result exceeds the size limit', async () => {
    ...
    const contract = ATMS_UI_TOOL_CONTRACTS.find(item => item.name === 'ui_close_surface')!
    socket.message({
      type: 'tool.invoke',
      version: 1,
