...
const textModeEnabled = flagEnabled(import.meta.env.VITE_ATMS_ENABLE_TEXT_MODE)
...
<template>
  ...
  <div v-else-if="textModeEnabled" class="agent-shell relative flex h-screen overflow-hidden bg-[var(--atms-bg)] p-4 text-[var(--atms-text-1)]">
    <div class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_22%_12%,color-mix(in_srgb,var(--atms-accent)_14%,transparent),transparent_34%),radial-gradient(circle_at_78%_20%,color-mix(in_srgb,var(--atms-accent)_10%,transparent),transparent_31%),linear-gradient(180deg,var(--atms-surface-1),transparent_24%)]" />
    <div class="relative z-10 flex min-h-0 flex-1 flex-col gap-3">
      ...
      <div class="relative flex min-h-0 flex-1 overflow-hidden rounded-[30px] border border-[var(--atms-border)] bg-[var(--atms-panel)] shadow-[var(--atms-shadow-floating)] backdrop-blur-xl">
      <!-- Left: session sidebar (collapsible w-56/w-10) -->
      <AgentSessionSidebar />

      <!-- Center: compact manager runtime conversation -->
      <div class="min-w-0 flex-1 flex flex-col border-r border-[var(--atms-border)] bg-[var(--atms-bg)]">
        <AgentChatPanel />
      </div>
      ...
      >
        <!-- Collapse toggle -->
        <div class="flex h-14 items-center justify-start px-3 flex-shrink-0 border-b border-[var(--atms-border)] bg-[var(--atms-surface-1)]">
          <button
            class="rounded-full border border-[var(--atms-border)] p-2 text-[var(--atms-text-3)] transition-colors hover:bg-[var(--atms-surface-2)] hover:text-[var(--atms-text-1)]"
