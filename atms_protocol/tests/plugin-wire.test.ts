...
import {
  ...
  validateAtmsPluginUiProjection,
  validateAtmsResolvedPluginDescriptorWire,
  applyAtmsDirectUiProjection,
  validateAtmsDirectUiProjection,
  validateAtmsDeclarativeRenderer,
  buildAtmsDeclarativeRendererModel,
  validateAtmsPluginToolInput,
  executeAtmsPluginTool,
  validateAtmsPluginToolExecutionEnvelope,
...
function descriptor(): AtmsResolvedPluginDescriptorV1 {
  return {
...
function context(): AtmsPluginTurnContextV1 {
  return {
    context_version: 1,
    registry_revision: 1,
...
describe("Atms plugin wire contracts", () => {
  it("validates the expression-free declarative Renderer DSL", () => {
    ...
    expect(validateAtmsDeclarativeRenderer(renderer)).toMatchObject({ valid: true });
    expect(validateAtmsDeclarativeRenderer({ ...renderer, script: "alert(1)" }).valid).toBe(false);
    expect(validateAtmsDeclarativeRenderer({
      ...renderer,
      sections: [renderer.sections[0], renderer.sections[0]],
    }).errors).toContainEqual(expect.objectContaining({ keyword: "uniqueSectionId" }));
    expect(buildAtmsDeclarativeRendererModel(renderer, {
      title: "Release 1.2.3",
      summary: "Ready.",
  ...
  it("rejects plugin schema regexes before evaluating potentially exponential input", () => {
    ...
    expect(validateAtmsPluginToolInput(unsafe, { value: `${"a".repeat(30)}!` })).toMatchObject({
      valid: false,
      errors: [expect.objectContaining({ keyword: "safePattern" })],
    });
    expect(validateAtmsPluginToolInput({
      type: "object",
  ...
  it("reuses bounded Tool validators without trusting mutable schema identity", () => {
    ...
    try {
      ...
      expect(validateAtmsPluginToolInput(schema, { value: "four" })).toMatchObject({ valid: true });
      expect(compile.mock.calls.length).toBe(initialCompiles + 1);
      expect(validateAtmsPluginToolInput(schema, { value: "four" })).toMatchObject({ valid: true });
      expect(compile.mock.calls.length).toBe(initialCompiles + 1);

      schema.properties.value.maxLength = 8;
      expect(validateAtmsPluginToolInput(schema, { value: "12345678" })).toMatchObject({ valid: true });
  ...
  it("bounds runtime schema validation diagnostics", () => {
    ...
    const result = validateAtmsPluginToolInput({
      type: "object",
      properties,
  ...
  it("validates an archived descriptor without following package paths", () => {
    expect(validateAtmsResolvedPluginDescriptorWire(descriptor())).toMatchObject({ valid: true });
    expect(validateAtmsResolvedPluginDescriptorWire({
      ...descriptor(),
      extra: true,
    }).valid).toBe(false);
  });
  ...
  it("requires canonical qualified identities and enabled plugin references", () => {
    expect(validateAtmsPluginTurnContext(context())).toMatchObject({ valid: true });
    const invalid = context();
    invalid.skills[0].qualified_id = "com.example.notes:other";
    expect(validateAtmsPluginTurnContext(invalid).errors).toContainEqual(expect.objectContaining({
  ...
  it("requires canonical ordering and unique harness wire ids", () => {
    ...
    expect(validateAtmsPluginTurnContext(invalid).errors).toEqual(expect.arrayContaining([
  ...
  it("keeps qualified Tool identity plus a maximum manifest description bounded", () => {
    ...
    expect(validateAtmsPluginTurnContext(bounded)).toMatchObject({ valid: true });
    bounded.tools[0].description = "x".repeat(601);
    expect(validateAtmsPluginTurnContext(bounded).valid).toBe(false);
  });

  it("validates a strict empty UI registry projection", () => {
    expect(validateAtmsPluginUiProjection({
      registry_revision: 0,
  ...
  it("applies a bounded declarative Domain-to-UI projection without code", () => {
    ...
    expect(validateAtmsDirectUiProjection(projection)).toMatchObject({ valid: true });
    expect(validateAtmsPluginToolInput({
      type: "object",
    ...
    expect(applyAtmsDirectUiProjection({
      projection,
    ...
    const envelope = executeAtmsPluginTool(descriptor, {
    ...
    expect(validateAtmsPluginToolExecutionEnvelope(envelope)).toMatchObject({ valid: true });
    expect(executeAtmsPluginTool(descriptor, {
      id: "task-draft",
    ...
    expect(validateAtmsPluginToolExecutionEnvelope({ ...envelope, unknown: true }).valid).toBe(false);
    const wrongQualified = structuredClone(envelope);
    wrongQualified.tool.qualified_id = "com.example.notes:other";
    expect(validateAtmsPluginToolExecutionEnvelope(wrongQualified).errors).toContainEqual(
    ...
    expect(validateAtmsPluginToolExecutionEnvelope(wrongOwner).errors).toContainEqual(
    ...
    expect(validateAtmsPluginToolExecutionEnvelope(wrongLegacyId).errors).toContainEqual(
    ...
    expect(validateAtmsPluginToolExecutionEnvelope(nestedUnknown).valid).toBe(false);
    expect(() => executeAtmsPluginTool({ ...descriptor, confirmation: "always" }, {
      id: "com.example.notes:note-1", title: "Research notes", note: "Keep the ABI semantic.", questions: [],
  ...
  it("keeps projection Action declarations non-authoritative until Manager resolves the manifest binding", () => {
    ...
    const projected = applyAtmsDirectUiProjection({
      projection,
      plugin: { id: "com.example.notes", version: "1.0.0" },
      arguments: {
        id: "com.example.notes:current",
