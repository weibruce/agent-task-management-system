...
function claims(): AtmsPluginArtifactWriteCapabilityClaimsV1 {
  return {
...
describe("Plugin Artifact Broker capability protocol", () => {
  it("accepts one exact content-addressed Tool and document-scope binding", () => {
    const value = claims();
    expect(validateAtmsPluginArtifactWriteCapabilityClaims(value, {
      now_ms: now,
      expected: {
    ...
    expect(atmsPluginArtifactCapabilitySigningInput(value)).toBe(
      atmsPluginArtifactCapabilitySigningInput(structuredClone(value)),
    );
  });
  ...
  it("rejects scope, package, request, MIME, size, expiry and extension drift", () => {
    ...
    unsafe.extra = true;
    expect(validateAtmsPluginArtifactWriteCapabilityClaims(unsafe, { now_ms: now }).valid).toBe(false);

    const expired = claims();
    expect(validateAtmsPluginArtifactWriteCapabilityClaims(expired, {
