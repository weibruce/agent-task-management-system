...
describe("Manager Agent harness contract", () => {
  ...
  it("renders Atms skill metadata and the pattern-first DAG workflow", () => {
    const prompt = buildManagerAgentSystemPrompt({
    ...
    expect(prompt).toContain("Available Atms Skills");
    expect(prompt).toContain("atms-dag-patterns: Select and instantiate reusable DAG patterns. [home]");
    ...
    expect(prompt).toContain("load atms-dag-patterns");
    ...
    expect(prompt).toContain("Atms CLI is a complete supported control surface");
    expect(prompt).toContain("ATMS_CLI_ENTRYPOINT");
  ...
  it("embeds already-selected Skill bodies without asking the Agent to reload them", () => {
    ...
    expect(prompt).toContain("com.atms.core:voice-generative-ui: Build structured voice UI. [plugin] [already loaded]");
    expect(prompt).toContain("## Loaded Atms Skill: com.atms.core:voice-generative-ui");
  ...
  it("catalogs harness-native Skill projections without inlining their bodies", () => {
    ...
    expect(prompt).toContain(
      "com.atms.core:voice-generative-ui: Build truthful generated interfaces and supervised live panels. [plugin] [native]",
    );
  ...
  it("parses Atms prompt-mode tool markers as shared protocol data", () => {
    const create = formatAtmsPromptToolCall({
      name: "create_and_run",
      input: { yamlPath: "assets/orchestrations/public-two-node.yaml.template" },
    });
    const finish = formatAtmsPromptToolCall({
      name: "finish",
      input: { text: "done" },
    });
    const handoff = formatAtmsPromptHandoff({
      port: "done",
      content: { ok: true },
      summary: "ok",
    });
    ...
    expect(parseAtmsPromptToolCalls(text)).toEqual([
      {
        name: "create_and_run",
        input: { yamlPath: "assets/orchestrations/public-two-node.yaml.template" },
      },
    ...
    expect(parseAtmsPromptHandoff(text)).toEqual({
      port: "done",
      content: { ok: true },
      summary: "ok",
    });
    expect(stripAtmsPromptMarkers(text)).toBe("before\n\n\n\nafter");
  });
});
