...
import {
  ...
  ATMS_A2UI_MAX_BYTES,
  ATMS_A2UI_PURE_FUNCTIONS,
  ATMS_A2UI_UPSTREAM_COMMIT,
  analyzeAtmsA2uiSurfaceSemantics,
  applyAtmsDirectUiProjection,
  createAtmsA2uiCreateSurfaceMessage,
  atmsA2uiCatalogComponentSchemas,
  atmsA2uiCatalogDefinition,
  atmsA2uiCatalogFunctionSchemas,
  atmsA2uiComponentSchema,
  isSafeGenerativeUiExternalUri,
  validateGenerativeUiNode,
  validateAtmsA2uiCreateSurfaceMessage,
  validateAtmsA2uiSurface,
  type GenerativeUiNodeV1,
  type AtmsA2uiSurfaceV1,
} from "../src/index.js";

function surface(): AtmsA2uiSurfaceV1 {
  return {
    version: "v1.0",
...
function node(a2ui: AtmsA2uiSurfaceV1 = surface()): GenerativeUiNodeV1 {
  return {
    ir_version: 1,
...
function templateSurface(itemCount: number): { a2ui: AtmsA2uiSurfaceV1; content: Record<string, unknown> } {
  return {
    a2ui: {
      version: "v1.0",
      catalogId: ATMS_A2UI_CATALOG_ID,
...
describe("direct A2UI v1.0 protocol", () => {
  it("exports one standard custom Catalog from the runtime component and function schemas", () => {
    const runtimeComponents = atmsA2uiComponentSchema.oneOf.map((schema) => (
      (schema.properties.component as { const: string }).const
    ));
    expect(atmsA2uiCatalogDefinition).toMatchObject({
    ...
    expect(Object.keys(atmsA2uiCatalogComponentSchemas)).toEqual(runtimeComponents);
    expect(Object.keys(atmsA2uiCatalogFunctionSchemas).sort()).toEqual(
      ATMS_A2UI_PURE_FUNCTIONS.filter((name) => name !== "@index").sort(),
    );

    for (const [name, schema] of Object.entries(atmsA2uiCatalogComponentSchemas)) {
    ...
    expect(JSON.stringify(atmsA2uiCatalogDefinition)).not.toContain("atms-a2ui-dynamic-");
    expect(JSON.stringify(atmsA2uiCatalogDefinition)).not.toContain("atms-a2ui-data-binding");
  });

  it("pins the upstream contract and validates the direct Atms surface", () => {
    expect(ATMS_A2UI_UPSTREAM_COMMIT).toBe("16425ca82061f756e420d2453e066d0c7c0295c1");
    expect(validateAtmsA2uiSurface(surface(), { data_model: node().content })).toMatchObject({
      valid: true,
      errors: [],
    });
  ...
  it("creates the exact standard v1 createSurface envelope with host-owned id and node content", () => {
    const value = node();
    const message = createAtmsA2uiCreateSurfaceMessage(value);
    ...
    expect(validateAtmsA2uiCreateSurfaceMessage(message).valid).toBe(true);

    expect(validateAtmsA2uiCreateSurfaceMessage({
      version: "v1.0",
      createSurface: {
    ...
    expect(validateAtmsA2uiCreateSurfaceMessage({ ...message, extra: true }).valid).toBe(false);
    expect(validateAtmsA2uiCreateSurfaceMessage({
      ...message,
  ...
  it("uses exact standard DataBinding and FunctionCall shapes", () => {
    expect(ATMS_A2UI_PURE_FUNCTIONS).toEqual([
      "@index",
      "and",
      "email",
      "formatCurrency",
    ...
    expect(validateAtmsA2uiSurface({
      version: "v1.0",
      catalogId: ATMS_A2UI_CATALOG_ID,
    ...
    const nestedRegex: AtmsA2uiSurfaceV1 = {
      version: "v1.0",
      catalogId: ATMS_A2UI_CATALOG_ID,
      components: [{
        id: "root",
  ...
  it("narrows literal formatString expressions to pointers and escaped openers", () => {
    ...
    expect(validateAtmsA2uiSurface(escapedAndAbsolute, {
    ...
    expect(validateAtmsA2uiSurface(templateRelative.a2ui, {
    ...
    expect(validateAtmsA2uiSurface(indexInterpolation, {
    ...
    for (const value of [
      ...
      expect(validateAtmsA2uiSurface(invalid).valid).toBe(false);
    }
    ...
    expect(validateAtmsA2uiSurface(dangerous).errors.some((error) => (
  ...
  it("validates template-relative bindings and materialized component budgets", () => {
    ...
    expect(validateAtmsA2uiSurface(valid.a2ui, { data_model: valid.content }).valid).toBe(true);

    const rootRelative = structuredClone(valid.a2ui);
    rootRelative.components = [{ id: "root", component: "Text", text: { path: "name" } }];
    expect(validateAtmsA2uiSurface(rootRelative).errors.some((error) => (
    ...
    expect(validateAtmsA2uiSurface(indexBinding.a2ui, {
    ...
    expect(validateAtmsA2uiSurface(pendingSource.a2ui, { data_model: {} }).valid).toBe(true);
    expect(validateAtmsA2uiSurface(pendingSource.a2ui, {
  ...
  it("enforces refs, cycles, unreachable components, one root, depth, and direct children", () => {
    ...
    expect(validateAtmsA2uiSurface(missing).errors.some((error) => (
      error.keyword === "a2uiComponentReference"
    ))).toBe(true);

    const cyclic: AtmsA2uiSurfaceV1 = {
      version: "v1.0",
      catalogId: ATMS_A2UI_CATALOG_ID,
      components: [
    ...
    expect(validateAtmsA2uiSurface(cyclic).errors.some((error) => (
    ...
    expect(validateAtmsA2uiSurface(unreachable).errors.some((error) => (
      error.keyword === "a2uiUnreachableComponent"
    ))).toBe(true);

    const tooManyDeclared: AtmsA2uiSurfaceV1 = {
      version: "v1.0",
      catalogId: ATMS_A2UI_CATALOG_ID,
    ...
    expect(analyzeAtmsA2uiSurfaceSemantics(tooManyDeclared).some((error) => (
    ...
    const deepComponents: AtmsA2uiSurfaceV1["components"] = Array.from(
      { length: 9 },
      (_, index) => index === 8
    ...
    expect(analyzeAtmsA2uiSurfaceSemantics(wide as AtmsA2uiSurfaceV1)
      .some((error) => error.keyword === "maxA2uiDirectChildren")).toBe(true);
    expect(validateAtmsA2uiSurface(wide).valid).toBe(false);

    const invalidWeight: AtmsA2uiSurfaceV1 = {
      version: "v1.0",
      catalogId: ATMS_A2UI_CATALOG_ID,
      components: [
    ...
    expect(validateAtmsA2uiSurface(invalidWeight).errors.some((error) => (
    ...
    expect(validateAtmsA2uiSurface(validWeight).valid).toBe(true);
  });
  ...
  it("requires DateTimeInput to enable at least one native picker", () => {
    ...
    expect(validateAtmsA2uiSurface(disabled, { data_model: { value: "" } }).valid).toBe(false);
    expect(validateAtmsA2uiSurface({
      ...disabled,
  ...
  it("maps event names to node actions and rejects local action calls or responses", () => {
    const interactive: AtmsA2uiSurfaceV1 = {
      version: "v1.0",
      catalogId: ATMS_A2UI_CATALOG_ID,
      components: [
    ...
    expect(validateAtmsA2uiSurface(response).errors.some((error) => (
    ...
    const unsupportedErrors = validateAtmsA2uiSurface(unsupportedEventFields, {
    ...
    expect(validateAtmsA2uiSurface(emptyResponsePath, {
      action_ids: new Set(["inspect"]),
    ...
    expect(validateAtmsA2uiSurface(local).errors.some((error) => (
  ...
  it("rejects unsafe static and bound media/artifact URIs", () => {
    for (const component of [
      ...
      expect(validateAtmsA2uiSurface({
        version: "v1.0",
        catalogId: ATMS_A2UI_CATALOG_ID,
        components: [component] as AtmsA2uiSurfaceV1["components"],
      }).errors.some((error) => error.keyword === "artifactUri")).toBe(true);
    }

    const bound: AtmsA2uiSurfaceV1 = {
      version: "v1.0",
      catalogId: ATMS_A2UI_CATALOG_ID,
      components: [{ id: "root", component: "HrArtifact", kind: "image", uri: { path: "/url" } }],
    };
    expect(validateAtmsA2uiSurface(bound, { data_model: { url: "javascript:alert(1)" } })
      .errors.some((error) => error.keyword === "artifactUri")).toBe(true);
    expect(validateAtmsA2uiSurface(bound, {
  ...
  it("accepts only credential-free HTTP(S) links in HrLink", () => {
    ...
    expect(validateAtmsA2uiSurface(link, {
  ...
  it("rejects prototype-polluting pointer segments across bindings, templates, sources, and item fields", () => {
    ...
    expect(validateAtmsA2uiSurface(binding).errors.some((error) => (
      error.keyword === "a2uiPointerSegment"
    ))).toBe(true);
    ...
    expect(validateAtmsA2uiSurface(nestedBinding).errors.some((error) => (
      error.keyword === "a2uiPointerSegment"
    ))).toBe(true);
    ...
    expect(validateAtmsA2uiSurface(template).errors.some((error) => (
      error.keyword === "a2uiPointerSegment"
    ))).toBe(true);
    ...
    expect(validateAtmsA2uiSurface(itemPath).errors.some((error) => (
      error.keyword === "a2uiPointerSegment"
    ))).toBe(true);
  });

  it("enforces the 64 KiB surface budget and camelCase Atms fields", () => {
    const oversized: AtmsA2uiSurfaceV1 = {
      version: "v1.0",
      catalogId: ATMS_A2UI_CATALOG_ID,
      components: [
        {
        ...
        ...Array.from({ length: 17 }, (_, index) => ({
          id: `chunk-${index}`,
          component: "Text" as const,
          text: "x".repeat(Math.ceil(ATMS_A2UI_MAX_BYTES / 16)),
        })),
      ],
    };
    expect(validateAtmsA2uiSurface(oversized).errors.some((error) => (
    ...
    expect(validateAtmsA2uiSurface(snakeCase).valid).toBe(false);
  });

  it("projects A2UI separately from semantic content and host presentation", () => {
    const result = applyAtmsDirectUiProjection({
