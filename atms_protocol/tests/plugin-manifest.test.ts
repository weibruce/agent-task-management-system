...
import {
  ATMS_PLUGIN_API_VERSION,
  ATMS_PLUGIN_MANIFEST_VERSION,
  ATMS_RENDERER_API_VERSION,
  collectAtmsPluginFileReferences,
  isSafeAtmsPluginPackagePath,
  validateAtmsPluginCompatibility,
  validateAtmsPluginManifest,
  type AtmsPluginManifestV1,
} from "../src/plugins/index.js";
import { validateMessage } from "../src/validation.js";

function manifest(): AtmsPluginManifestV1 {
  return {
    manifest_version: ATMS_PLUGIN_MANIFEST_VERSION,
    id: "com.atms.topic-outline",
    version: "1.0.0",
    name: "Topic Outline",
    publisher: { id: "com.atms", name: "Atms" },
    license: "MIT",
    compatibility: {
      atms: { min: "0.1.0", max_exclusive: "0.2.0" },
      plugin_api: [ATMS_PLUGIN_API_VERSION],
      ui_ir: [1],
      renderer_api: [ATMS_RENDERER_API_VERSION],
    },
    capabilities: [{
      id: "compose",
    ...
    actions: [],
    runtime: { trust: "data_only", plugin_api: ATMS_PLUGIN_API_VERSION },
    permissions: { required: [], optional: [] },
    state: { schema_version: 1, migrations: [] },
  };
}

describe("Atms Plugin Manifest V1", () => {
  it("accepts a strict vertical capability manifest without mutating it", () => {
    const input = manifest();
    const before = structuredClone(input);
    expect(validateAtmsPluginManifest(input)).toEqual({
      valid: true,
      value: before,
      errors: [],
    });
  ...
  it("rejects unknown fields and malformed semantic versions", () => {
    expect(validateAtmsPluginManifest({ ...manifest(), surprise: true }).valid).toBe(false);
    expect(validateAtmsPluginManifest({ ...manifest(), version: "v1" }).valid).toBe(false);
    expect(validateAtmsPluginManifest({ ...manifest(), version: "1.0.0-01" }).valid).toBe(false);

    const invalidRange = manifest();
    invalidRange.compatibility.atms.max_exclusive = "0.1.0";
    expect(validateAtmsPluginManifest(invalidRange).errors).toContainEqual(expect.objectContaining({
      keyword: "compatibilityRange",
    }));
  });

  it("normalizes paths and enforces minimum side-effect policy", () => {
    expect(isSafeAtmsPluginPackagePath("skills/valid/SKILL.md")).toBe(true);
    expect(isSafeAtmsPluginPackagePath("skills/bad\nname/SKILL.md")).toBe(false);
    expect(isSafeAtmsPluginPackagePath("skills/bad\0name/SKILL.md")).toBe(false);

    const destructive = manifest();
    destructive.tools[0].effect = "destructive";
    expect(validateAtmsPluginManifest(destructive).errors).toContainEqual(expect.objectContaining({
    ...
    expect(validateAtmsPluginManifest(network).errors).toContainEqual(expect.objectContaining({
  ...
  it("rejects duplicate declarations, dangling references, and foreign namespaces", () => {
    ...
    expect(validateAtmsPluginManifest(duplicate).errors).toContainEqual(expect.objectContaining({
      keyword: "uniqueDeclaration",
    }));

    const dangling = manifest();
    dangling.capabilities[0].tools = ["missing_tool"];
    expect(validateAtmsPluginManifest(dangling).errors).toContainEqual(expect.objectContaining({
      keyword: "toolReference",
    }));

    const foreign = manifest();
    foreign.kinds[0].kind = "com.example.other/outline";
    expect(validateAtmsPluginManifest(foreign).errors).toContainEqual(expect.objectContaining({
  ...
  it("rejects duplicate workflow URI ownership before package installation", () => {
    ...
    expect(validateAtmsPluginManifest(value).errors).toContainEqual(expect.objectContaining({
      path: "/workflows/1",
      keyword: "uniqueDeclaration",
  ...
  it("requires contiguous kind versions and adjacent migrations", () => {
    ...
    expect(validateAtmsPluginManifest(input).errors).toEqual(expect.arrayContaining([
  ...
  it("bounds version axes without allocating from hostile values", () => {
    ...
    expect(() => validateAtmsPluginManifest(hostile)).not.toThrow();
    expect(validateAtmsPluginManifest(hostile).valid).toBe(false);

    const hostileState = manifest();
    hostileState.state.schema_version = Number.MAX_SAFE_INTEGER;
    expect(() => validateAtmsPluginManifest(hostileState)).not.toThrow();
    expect(validateAtmsPluginManifest(hostileState).valid).toBe(false);
  });
  ...
  it("requires permissions to be declared and handlers to match the trust tier", () => {
    ...
    expect(validateAtmsPluginManifest(permission).errors).toContainEqual(expect.objectContaining({
      keyword: "permissionReference",
    }));

    const runtime = manifest();
    runtime.tools[0].handler = { type: "runtime", method: "compose" };
    expect(validateAtmsPluginManifest(runtime).errors).toContainEqual(expect.objectContaining({
    ...
    expect(validateAtmsPluginManifest(trustedRuntime).errors)
  ...
  it("rejects unsafe package paths and collects the complete deterministic file set", () => {
    ...
    expect(validateAtmsPluginManifest(unsafe).valid).toBe(false);

    expect(collectAtmsPluginFileReferences(manifest())).toEqual([
  ...
  it("does not let the generic schema entrypoint bypass manifest semantics", () => {
    const unsafe = manifest();
    unsafe.skills[0].path = "../outside/SKILL.md";
    expect(validateMessage(unsafe, "atms-plugin-manifest-v1")).toMatchObject({
      valid: false,
  ...
  it("evaluates every compatibility axis independently", () => {
    const input = manifest();
    expect(validateAtmsPluginCompatibility(input, {
      atms: "0.1.0",
      plugin_api: 1,
      ui_ir: 1,
      renderer_api: 1,
  ...
  it("fails safely for cyclic, accessor-backed, and oversized inputs", () => {
    ...
    cyclic.self = cyclic;
    expect(validateAtmsPluginManifest(cyclic).errors[0]?.keyword).toBe("jsonValue");
    ...
    expect(validateAtmsPluginManifest(accessor).errors[0]?.keyword).toBe("jsonValue");

    const oversized = manifest();
    oversized.capabilities[0].summary = "x".repeat(600 * 1024);
    expect(validateAtmsPluginManifest(oversized).errors[0]?.keyword).toBe("maxPayloadBytes");
  });
});
