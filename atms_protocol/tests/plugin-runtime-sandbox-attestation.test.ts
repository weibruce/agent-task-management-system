...
describe("Plugin Runtime sandbox attestation protocol", () => {
  ...
  function attestation(): AtmsPluginRuntimeSandboxAttestationV1 {
    return {
      claims: {
        ...
        isolation: {
          ...
          mounts: [{ source: "package:runtime", target: "/opt/atms/plugin", mode: "ro" }],
  ...
  it("accepts an exact non-root, no-secret, canonical launch claim", () => {
    ...
    expect(validateAtmsPluginRuntimeSandboxAttestation(value, { now_ms: now }))
      .toMatchObject({ valid: true, errors: [] });
    expect(atmsPluginRuntimeSandboxAttestationSigningInput(value.claims))
  ...
  it("rejects root, writable-root, secret, network, expiry, and extension drift", () => {
    ...
    unsafe.claims.isolation.blocked_secret_env = ["ATMS_MANAGER_ADMIN_TOKEN"];
    unsafe.claims.isolation.network = {
      mode: "none", hosts: ["example.com"], network_name: null, internal: true,
    };
    const paths = validateAtmsPluginRuntimeSandboxAttestation(unsafe, { now_ms: now })
    ...
    expect(validateAtmsPluginRuntimeSandboxAttestation(value, {
      now_ms: Date.parse("2026-07-12T10:03:00.000Z"),
      clock_skew_ms: 0,
    }).errors).toEqual(expect.arrayContaining([
      expect.objectContaining({ path: "/claims/expires_at" }),
    ]));
    expect(validateAtmsPluginRuntimeSandboxAttestation({ ...value, extra: true }, { now_ms: now }).valid)
      .toBe(false);
  });
});
