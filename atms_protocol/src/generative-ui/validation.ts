...
import {
  ...
  type AtmsA2uiSemanticOptionsV1,
  type AtmsA2uiSurfaceV1,
} from "./a2ui.js";
import {
  ATMS_VIEW_SPEC_MAX_BYTES,
  analyzeAtmsViewSpecSemantics,
  type AtmsViewSpecV1,
} from "./view-spec.js";

const CORE_GENERATED_VIEW_KIND = "com.atms.core/generated_view";
...
function jsonLimitsForSchema(schemaName: string): Partial<GenerativeUiJsonValueLimits> {
  if (schemaName === "atms-view-spec-v1") return { max_bytes: ATMS_VIEW_SPEC_MAX_BYTES };
  if (schemaName === "atms-a2ui-surface-v1") return { max_bytes: ATMS_A2UI_MAX_BYTES };
...
function nodeSemanticErrors(
  ...
  if (node.view) {
    if (node.kind !== CORE_GENERATED_VIEW_KIND || node.kind_version !== LEGACY_VIEW_SPEC_KIND_VERSION) {
      errors.push({
        path: `${path}/view`,
        message: "legacy ViewSpec is supported only for com.atms.core/generated_view@1",
        keyword: "legacyViewSpecVersion",
      });
    }
    const viewAnalysis = analyzeGenerativeUiJsonValue(node.view, {
      path: `${path}/view`,
      limits: { max_bytes: ATMS_VIEW_SPEC_MAX_BYTES },
    });
    if (!viewAnalysis.valid) {
      errors.push({
        path: viewAnalysis.error?.path || `${path}/view`,
        message: viewAnalysis.error?.message || `view exceeds ${ATMS_VIEW_SPEC_MAX_BYTES} bytes`,
        keyword: viewAnalysis.error?.keyword || "maxPayloadBytes",
      });
    } else {
      errors.push(...analyzeAtmsViewSpecSemantics(node.view, {
  ...
  if (node.a2ui) {
    if (node.kind === CORE_GENERATED_VIEW_KIND && node.kind_version !== NATIVE_A2UI_KIND_VERSION) {
      errors.push({
        path: `${path}/a2ui`,
        message: "native A2UI presentation for com.atms.core/generated_view requires kind version 2",
        keyword: "nativeA2uiVersion",
      });
    }
    const a2uiAnalysis = analyzeGenerativeUiJsonValue(node.a2ui, {
      path: `${path}/a2ui`,
      limits: { max_bytes: ATMS_A2UI_MAX_BYTES },
    });
    if (!a2uiAnalysis.valid) {
      errors.push({
        path: a2uiAnalysis.error?.path || `${path}/a2ui`,
        message: a2uiAnalysis.error?.message || `a2ui exceeds ${ATMS_A2UI_MAX_BYTES} bytes`,
        keyword: a2uiAnalysis.error?.keyword || "maxPayloadBytes",
      });
    } else {
      errors.push(...analyzeAtmsA2uiSurfaceSemantics(node.a2ui, {
        action_ids: new Set((node.actions ?? []).map((action) => action.id)),
...
export function validateAtmsViewSpec(
  value: unknown,
  options: { action_ids?: ReadonlySet<string> } = {},
): GenerativeUiValidationResult<AtmsViewSpecV1> {
  const validation = validate<AtmsViewSpecV1>("atms-view-spec-v1", value);
  ...
  return withSemanticErrors(validation, analyzeAtmsViewSpecSemantics(validation.value, options));
}

export function validateAtmsA2uiSurface(
  value: unknown,
  options: Omit<AtmsA2uiSemanticOptionsV1, "path"> = {},
): GenerativeUiValidationResult<AtmsA2uiSurfaceV1> {
  const validation = validate<AtmsA2uiSurfaceV1>("atms-a2ui-surface-v1", value);
  if (!validation.value) return validation;
  const analysis = analyzeGenerativeUiJsonValue(validation.value, {
    limits: { max_bytes: ATMS_A2UI_MAX_BYTES },
  });
  ...
  return withSemanticErrors(validation, analyzeAtmsA2uiSurfaceSemantics(validation.value, options));
}

export function validateAtmsA2uiCreateSurfaceMessage(
  value: unknown,
): GenerativeUiValidationResult<A2uiCreateSurfaceMessageV1> {
  return validate<A2uiCreateSurfaceMessageV1>(
    "atms-a2ui-create-surface-message-v1",
    value,
  );
}
    ...
    if (operation.op === "patch") {
      ...
      if (operation.changes.view) {
        ...
        if (!viewAnalysis.valid) {
          ...
          errors.push(...analyzeAtmsViewSpecSemantics(operation.changes.view, {
      ...
      if (operation.changes.a2ui) {
        ...
        if (!a2uiAnalysis.valid) {
          ...
          errors.push(...analyzeAtmsA2uiSurfaceSemantics(operation.changes.a2ui, {
            ...(operation.changes.actions
              ? { action_ids: new Set(operation.changes.actions.map((action) => action.id)) }
