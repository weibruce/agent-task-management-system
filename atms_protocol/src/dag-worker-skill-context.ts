...
import type { AtmsA2uiSurfaceV1 } from "./generative-ui/a2ui.js";
import { ATMS_PLUGIN_ID_PATTERN } from "./plugins/types.js";
import {
...
function visualDataKeys(a2ui: AtmsA2uiSurfaceV1): string[] {
...
function parseVisualProfile(value: unknown, path: string): DagWorkerSkillVisualProfileV1 {
  ...
  if (value.views !== undefined) {
    ...
    profile.views = value.views.map((entry, index) => {
      ...
      ids.add(id);
      const validation = validateAtmsA2uiSurface(entry.a2ui);
      if (!validation.valid || !validation.value) {
        issue(`${entryPath}/a2ui`, "failed Atms A2UI surface validation", "a2ui");
      }
...
function parsePlugin(value: unknown, path: string): DagWorkerSkillPluginV1 {
  ...
  if (!ATMS_PLUGIN_ID_PATTERN.test(id)) issue(`${path}/id`, "has an invalid plugin id", "pattern");
