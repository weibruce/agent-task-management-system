/**
 * Pure validation for Atms PR Review evidence consumed by PR closeout.
 *
...
export async function resolvePrCloseout(
  ...
  if (phase === "merge") {
    ...
    if (!prReview) {
      blockers.push({ code: "pr_review_missing", message: "No conclusive zero-finding Atms PR Review is bound to the current head." });
    }
  }
