...
export function validateMessage(data: unknown, schemaName: string): ValidationResult {
  if (schemaName === "atms-plugin-manifest-v1") {
    const result = validateAtmsPluginManifest(data);
    return { valid: result.valid, errors: result.errors };
  }
  if (schemaName === "atms-plugin-turn-context-v1") {
    const result = validateAtmsPluginTurnContext(data);
    return { valid: result.valid, errors: result.errors };
  }
  if (schemaName === "atms-plugin-ui-projection-v1") {
    const result = validateAtmsPluginUiProjection(data);
    return { valid: result.valid, errors: result.errors };
  }
  if (schemaName === "atms-resolved-plugin-descriptor-v1") {
    const result = validateAtmsResolvedPluginDescriptorWire(data);
    return { valid: result.valid, errors: result.errors };
  }
  if (schemaName === "atms-direct-ui-projection-v1") {
    const result = validateAtmsDirectUiProjection(data);
    return { valid: result.valid, errors: result.errors };
  }
  const ajv = getAjv();
...
export function validateDagActorSurfacePatchV1(value: unknown): ValidationResult {
  ...
  const a2ui = validateAtmsA2uiSurface(patch.body.a2ui, {
