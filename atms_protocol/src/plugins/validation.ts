...
import { atmsPluginSchemas } from "./schemas.js";
import {
  ATMS_ACTION_ARGUMENT_MAX_BYTES,
  ATMS_ACTION_CAPABILITY_MAX_TTL_MS,
  ATMS_ACTION_CONFIRMATION_MAX_TTL_MS,
  ATMS_ACTION_REQUEST_MAX_TTL_MS,
  ATMS_RUNTIME_DOMAIN_OUTPUT_MAX_BYTES,
  AtmsPluginRendererMode,
  AtmsPluginRuntimeTrust,
  type AtmsPluginToolBindingV1,
  type AtmsPluginToolCapabilityClaimsV1,
  type AtmsPluginToolConfirmationChallengeV1,
  type AtmsPluginToolConfirmationDecisionV1,
  ...
  type AtmsPluginCompatibilityTargetV1,
  type AtmsDeclarativeRendererV1,
  type AtmsDirectUiProjectionV1,
  type AtmsPluginHandlerV1,
  type AtmsPluginManifestV1,
  type AtmsPluginEffectivePermissionGrantV1,
  type AtmsPluginPermission,
  type AtmsPluginTurnContextV1,
  type AtmsPluginToolExecutionEnvelopeV1,
  type AtmsPluginUiProjectionV1,
  type AtmsPluginRuntimeArtifactV1,
  type AtmsPluginRuntimeLogEntryV1,
  type AtmsPluginRuntimeRpcRequestV1,
  type AtmsPluginRuntimeRpcResponseV1,
  type AtmsResolvedPluginDescriptorV1,
  type AtmsPluginValidationError,
  type AtmsPluginValidationResult,
} from "./types.js";
...
function createValidator() {
  ...
  for (const [name, schema] of Object.entries(atmsPluginSchemas)) {
    ajv.addSchema(schema, name);
  }
  return ajv;
}

function normalizeErrors(errors: ErrorObject[] | null | undefined): AtmsPluginValidationError[] {
...
function duplicateErrors<T>(
  ...
  const errors: AtmsPluginValidationError[] = [];
...
export function isSafeAtmsPluginPackagePath(value: string): boolean {
...
export function isCanonicalAtmsPluginSemver(value: unknown): value is string {
...
function permissionSet(manifest: AtmsPluginManifestV1): Set<AtmsPluginPermission> {
  return new Set([
...
function referencedPermissionErrors(
  ...
  declared: Set<AtmsPluginPermission>,
  path: string,
): AtmsPluginValidationError[] {
...
function handlerErrors(
  ...
  const errors: AtmsPluginValidationError[] = [];
  if (handler.type === "projection" && !isSafeAtmsPluginPackagePath(handler.file)) {
    errors.push(error(`${path}/file`, "must be a package-relative POSIX path", "packagePath"));
  }
  if (
    handler.type === "runtime"
    && trust !== AtmsPluginRuntimeTrust.SANDBOXED_RUNTIME
    && trust !== AtmsPluginRuntimeTrust.TRUSTED_BUILTIN
  ) {
    errors.push(error(`${path}/type`, "runtime handlers require sandboxed_runtime or trusted_builtin", "runtimeTrust"));
  }
  if (handler.type === "builtin" && trust !== AtmsPluginRuntimeTrust.TRUSTED_BUILTIN) {
    errors.push(error(`${path}/type`, "builtin handlers require trusted_builtin", "runtimeTrust"));
  }
  return errors;
}

function semanticErrors(manifest: AtmsPluginManifestV1): AtmsPluginValidationError[] {
  const errors: AtmsPluginValidationError[] = [];
  errors.push(
  ...
  if (compareSemver(manifest.compatibility.atms.min, manifest.compatibility.atms.max_exclusive) !== -1) {
    errors.push(error(
      "/compatibility/atms",
  ...
  manifest.skills.forEach((skill, index) => {
    if (!isSafeAtmsPluginPackagePath(skill.path) || !skill.path.endsWith("/SKILL.md")) {
      errors.push(error(
        `/skills/${index}/path`,
    ...
    if (skill.visual_profile !== undefined && (
      !isSafeAtmsPluginPackagePath(skill.visual_profile)
  ...
  manifest.schemas.forEach((schema, index) => {
    if (!isSafeAtmsPluginPackagePath(schema.file) || !schema.file.endsWith(".json")) {
      errors.push(error(
        `/schemas/${index}/file`,
        "must be a package-relative JSON file",
        "schemaPath",
  ...
  manifest.kinds.forEach((kind, kindIndex) => {
    ...
    kind.migrations.forEach((migration, migrationIndex) => {
      if (!isSafeAtmsPluginPackagePath(migration.file)) {
        errors.push(error(
  ...
  manifest.renderers.forEach((renderer, index) => {
    ...
    if (
      (renderer.mode === AtmsPluginRendererMode.BUILTIN && renderer.source.type !== "builtin")
      || (renderer.mode === AtmsPluginRendererMode.DECLARATIVE && renderer.source.type !== "declarative")
      || (renderer.mode === AtmsPluginRendererMode.CUSTOM && renderer.source.type !== "custom")
    ) {
      errors.push(error(`/renderers/${index}/source`, "renderer source must match renderer mode", "rendererMode"));
    }
    if (renderer.source.type !== "builtin" && !isSafeAtmsPluginPackagePath(renderer.source.file)) {
    ...
    if (renderer.fallback.type === "core_projection" && !isSafeAtmsPluginPackagePath(renderer.fallback.file)) {
  ...
  if (manifest.runtime.trust === AtmsPluginRuntimeTrust.SANDBOXED_RUNTIME) {
  ...
  if (manifest.runtime.entrypoint && !isSafeAtmsPluginPackagePath(manifest.runtime.entrypoint.file)) {
...
export function resetAtmsPluginValidator(): void {
  validator = undefined;
}

export function validateAtmsPluginManifest(
  value: unknown,
): AtmsPluginValidationResult<AtmsPluginManifestV1> {
  ...
  const validateFn: ValidateFunction | undefined = validator.getSchema("atms-plugin-manifest-v1");
  if (!validateFn) {
  ...
  const manifest = stableValue as AtmsPluginManifestV1;
  let errors: AtmsPluginValidationError[];
  try {
...
export function validateAtmsPluginCompatibility(
  manifest: AtmsPluginManifestV1,
  target: AtmsPluginCompatibilityTargetV1,
): AtmsPluginValidationError[] {
  const errors: AtmsPluginValidationError[] = [];
  const lower = compareSemver(target.atms, manifest.compatibility.atms.min);
  const upper = compareSemver(target.atms, manifest.compatibility.atms.max_exclusive);
  if (lower === null || upper === null || lower < 0 || upper >= 0) {
    errors.push(error(
      "/compatibility/atms",
      `Atms ${target.atms} is outside the supported range`,
      "incompatibleVersion",
    ));
  }
...
export function collectAtmsPluginFileReferences(
  manifest: AtmsPluginManifestV1,
): string[] {
  ...
  const addHandler = (handler: AtmsPluginHandlerV1) => {
...
export function atmsPluginTurnContextDigestInput(
  value: AtmsPluginTurnContextV1,
): Omit<AtmsPluginTurnContextV1, "context_digest"> {
  const { context_digest: _digest, ...input } = structuredClone(value);
  return input;
}

export function validateAtmsPluginTurnContext(
  value: unknown,
): AtmsPluginValidationResult<AtmsPluginTurnContextV1> {
  const validation = validatePluginWireValue<AtmsPluginTurnContextV1>(
    "atms-plugin-turn-context-v1",
    value,
  );
  ...
  context.tools.forEach((entry, index) => {
    ...
    if (entry.handler.type === "projection") {
      ...
      const projection = validateAtmsDirectUiProjection(entry.handler.document);
      if (!projection.valid) {
...
export function validateAtmsPluginUiProjection(
  value: unknown,
): AtmsPluginValidationResult<AtmsPluginUiProjectionV1> {
  const validation = validatePluginWireValue<AtmsPluginUiProjectionV1>(
    "atms-plugin-ui-projection-v1",
    value,
  );
  if (!validation.value) return validation;
  const projection = validation.value;
  const kindKey = (entry: AtmsPluginUiProjectionV1["kinds"][number]) => (
    `${entry.plugin_id}\0${entry.kind}\0${String(entry.kind_version).padStart(2, "0")}`
  );
  const rendererKey = (entry: AtmsPluginUiProjectionV1["renderers"][number]) => (
...
export function validateAtmsResolvedPluginDescriptorWire(
  value: unknown,
): AtmsPluginValidationResult<AtmsResolvedPluginDescriptorV1> {
  const validation = validatePluginWireValue<AtmsResolvedPluginDescriptorV1>(
    "atms-resolved-plugin-descriptor-v1",
    value,
  );
  if (!validation.value) return validation;
  const descriptor = validation.value;
  const manifest = validateAtmsPluginManifest(descriptor.manifest);
  ...
  const expectedFiles = collectAtmsPluginFileReferences(descriptor.manifest);
  if (
...
export function validateAtmsDirectUiProjection(
  value: unknown,
): AtmsPluginValidationResult<AtmsDirectUiProjectionV1> {
  const validation = validatePluginWireValue<AtmsDirectUiProjectionV1>(
    "atms-direct-ui-projection-v1",
    value,
  );
  if (!validation.value) return validation;
  const projection = validation.value;
...
export function validateAtmsDeclarativeRenderer(
  value: unknown,
): AtmsPluginValidationResult<AtmsDeclarativeRendererV1> {
  const validation = validatePluginWireValue<AtmsDeclarativeRendererV1>(
    "atms-declarative-renderer-v1",
    value,
  );
  if (!validation.value) return validation;
...
export function validateAtmsPluginToolExecutionEnvelope(
  value: unknown,
): AtmsPluginValidationResult<AtmsPluginToolExecutionEnvelopeV1> {
  const validation = validatePluginWireValue<AtmsPluginToolExecutionEnvelopeV1>(
    "atms-plugin-tool-execution-envelope-v1",
    value,
  );
  if (!validation.value) return validation;
...
const ACTION_BINDING_FIELDS: Array<keyof AtmsPluginToolBindingV1> = [
  "plugin_id",
  "plugin_version",
  "manifest_digest",
  "package_digest",
...
const ACTION_TARGET_FIELDS: Array<keyof AtmsPluginActionTargetV1> = [
  "document_id",
  "document_revision",
  "node_id",
  "node_revision",
...
function equalPermissions(
  left: AtmsPluginPermission[],
  right: AtmsPluginPermission[],
): boolean {
...
function effectivePermissionGrantErrors(
  grants: AtmsPluginEffectivePermissionGrantV1[],
  permissions: AtmsPluginPermission[],
  path: string,
): AtmsPluginValidationError[] {
  const errors: AtmsPluginValidationError[] = [];
...
function equalEffectivePermissionGrants(
  left: AtmsPluginEffectivePermissionGrantV1[],
  right: AtmsPluginEffectivePermissionGrantV1[],
): boolean {
...
function actionBindingErrors(
  actual: AtmsPluginToolBindingV1,
  expected: AtmsPluginToolBindingV1,
  path: string,
): AtmsPluginValidationError[] {
...
function actionTargetErrors(
  actual: AtmsPluginActionTargetV1,
  expected: AtmsPluginActionTargetV1,
  path: string,
): AtmsPluginValidationError[] {
...
function toolIdentityErrors(
  actual: AtmsPluginToolInvocationV1["tool"],
  expected: AtmsPluginToolInvocationV1["tool"],
  path: string,
): AtmsPluginValidationError[] {
  const errors: AtmsPluginValidationError[] = [];
...
function toolSourceErrors(
  actual: AtmsPluginToolInvocationV1["source"],
  expected: AtmsPluginToolInvocationV1["source"],
  path: string,
): AtmsPluginValidationError[] {
...
export function atmsPluginToolInvocationDigestInput(
  value: AtmsPluginToolInvocationV1,
): Omit<AtmsPluginToolInvocationV1, "request_digest"> {
  const { request_digest: _digest, ...input } = structuredClone(value);
  return input;
}

export function validateAtmsPluginToolInvocation(
  value: unknown,
  options: AtmsPluginToolValidationOptionsV1 = {},
): AtmsPluginValidationResult<AtmsPluginToolInvocationV1> {
  const validation = validatePluginWireValue<AtmsPluginToolInvocationV1>(
    "atms-plugin-tool-invocation-v1",
    value,
  );
  if (!validation.value) return validation;
  ...
  if (invokedAt !== null && deadlineAt !== null) {
    ...
    if (lifetime <= 0 || lifetime > ATMS_ACTION_REQUEST_MAX_TTL_MS) {
      errors.push(error(
        "/deadline_at",
        `Tool deadline must be after invocation and within ${ATMS_ACTION_REQUEST_MAX_TTL_MS} ms`,
        "requestLifetime",
      ));
    }
  ...
  const argumentAnalysis = analyzeGenerativeUiJsonValue(invocation.arguments, {
    path: "/arguments",
    limits: {
      max_bytes: ATMS_ACTION_ARGUMENT_MAX_BYTES,
      max_values: 4_096,
      max_depth: 32,
    },
  });
  if (!argumentAnalysis.valid) {
    errors.push(argumentAnalysis.error ?? error(
      "/arguments",
      `Tool arguments exceed ${ATMS_ACTION_ARGUMENT_MAX_BYTES} bytes`,
      "maxPayloadBytes",
    ));
  }
...
export function validateAtmsPluginToolCapabilityClaims(
  value: unknown,
  invocation?: AtmsPluginToolInvocationV1,
  options: AtmsPluginToolValidationOptionsV1 = {},
): AtmsPluginValidationResult<AtmsPluginToolCapabilityClaimsV1> {
  const validation = validatePluginWireValue<AtmsPluginToolCapabilityClaimsV1>(
    "atms-plugin-tool-capability-claims-v1",
    value,
  );
  if (!validation.value) return validation;
  ...
  if (issuedAt !== null && expiresAt !== null) {
    ...
    if (lifetime <= 0 || lifetime > ATMS_ACTION_CAPABILITY_MAX_TTL_MS) {
      errors.push(error(
        "/expires_at",
        `capability must expire after issuance and within ${ATMS_ACTION_CAPABILITY_MAX_TTL_MS} ms`,
        "capabilityLifetime",
      ));
    }
...
export function validateAtmsPluginToolConfirmationChallenge(
  value: unknown,
  invocation?: AtmsPluginToolInvocationV1,
  options: AtmsPluginToolValidationOptionsV1 = {},
): AtmsPluginValidationResult<AtmsPluginToolConfirmationChallengeV1> {
  const validation = validatePluginWireValue<AtmsPluginToolConfirmationChallengeV1>(
    "atms-plugin-tool-confirmation-challenge-v1",
    value,
  );
  if (!validation.value) return validation;
  ...
  if (issuedAt !== null && expiresAt !== null) {
    const lifetime = expiresAt - issuedAt;
    if (lifetime <= 0 || lifetime > ATMS_ACTION_CONFIRMATION_MAX_TTL_MS) {
      errors.push(error(
        "/expires_at",
        `confirmation challenge must expire within ${ATMS_ACTION_CONFIRMATION_MAX_TTL_MS} ms`,
...
export function validateAtmsPluginToolConfirmationDecision(
  value: unknown,
  invocation?: AtmsPluginToolInvocationV1,
  challenge?: AtmsPluginToolConfirmationChallengeV1,
): AtmsPluginValidationResult<AtmsPluginToolConfirmationDecisionV1> {
  const validation = validatePluginWireValue<AtmsPluginToolConfirmationDecisionV1>(
    "atms-plugin-tool-confirmation-decision-v1",
    value,
  );
  if (!validation.value) return validation;
...
export function validateAtmsPluginAuthorizedToolInvocation(
  value: unknown,
  options: AtmsPluginToolValidationOptionsV1 = {},
): AtmsPluginValidationResult<AtmsPluginAuthorizedToolInvocationV1> {
  const validation = validatePluginWireValue<AtmsPluginAuthorizedToolInvocationV1>(
    "atms-plugin-authorized-tool-invocation-v1",
    value,
  );
  if (!validation.value) return validation;
  const authorization = validation.value;
  const invocation = validateAtmsPluginToolInvocation(authorization.invocation, options);
  const capability = validateAtmsPluginToolCapabilityClaims(
  ...
  if (confirmation) {
    const challenge = validateAtmsPluginToolConfirmationChallenge(
      confirmation.challenge,
      authorization.invocation,
      options,
    );
    const decision = validateAtmsPluginToolConfirmationDecision(
...
export function validateAtmsPluginRuntimeRpcRequest(
  value: unknown,
  options: AtmsPluginToolValidationOptionsV1 = {},
): AtmsPluginValidationResult<AtmsPluginRuntimeRpcRequestV1> {
  const validation = validatePluginWireValue<AtmsPluginRuntimeRpcRequestV1>(
    "atms-plugin-runtime-rpc-request-v1",
    value,
  );
  if (!validation.value) return validation;
  ...
  if (request.method === "execute" || request.method === "prepare") {
    const authorization = validateAtmsPluginAuthorizedToolInvocation(
...
export function validateAtmsPluginRuntimeRpcResponse(
  value: unknown,
  options: AtmsPluginToolValidationOptionsV1 = {},
): AtmsPluginValidationResult<AtmsPluginRuntimeRpcResponseV1> {
  const validation = validatePluginWireValue<AtmsPluginRuntimeRpcResponseV1>(
    "atms-plugin-runtime-rpc-response-v1",
    value,
  );
  if (!validation.value) return validation;
  ...
  if (response.method === "prepare") {
    ...
    if (response.output.type === "domain_output") {
      const output = analyzeGenerativeUiJsonValue(response.output.output, {
        path: "/output/output",
        limits: {
          max_bytes: ATMS_RUNTIME_DOMAIN_OUTPUT_MAX_BYTES,
          max_values: 20_000,
          max_depth: 32,
        },
      });
      if (!output.valid) {
        errors.push(output.error ?? error(
          "/output/output",
          `domain output exceeds ${ATMS_RUNTIME_DOMAIN_OUTPUT_MAX_BYTES} bytes`,
          "maxPayloadBytes",
        ));
      }
    } else {
