...
export const ATMS_PLUGIN_SCHEMA_MAX_BYTES = 256 * 1024;
export const ATMS_PLUGIN_SKILL_MAX_BYTES = 256 * 1024;
export const ATMS_PLUGIN_DESCRIPTOR_MAX_BYTES = 4 * 1024 * 1024;
export const ATMS_PLUGIN_SCHEMA_MAX_DEPTH = 32;
export const ATMS_PLUGIN_SCHEMA_MAX_VALUES = 20_000;
export const ATMS_PLUGIN_SCHEMA_MAX_ARRAY_ITEMS = 256;
export const ATMS_PLUGIN_SCHEMA_MAX_STRING_LENGTH = 16_384;
export const ATMS_PLUGIN_SCHEMA_MAX_PROPERTIES = 128;
export const ATMS_PLUGIN_SCHEMA_MAX_ENUM_VALUES = 64;

export interface AtmsPluginSchemaPolicyIssue {
  path: string;
  message: string;
  keyword: "schemaBudget" | "schemaRef" | "safePattern" | "schemaComplexity";
}

export function decodeAtmsPluginUtf8(value: Uint8Array, label = "plugin text file"): string {
  try {
...
export function analyzeAtmsPluginSchemaPolicy(schema: unknown): AtmsPluginSchemaPolicyIssue[] {
  const issues: AtmsPluginSchemaPolicyIssue[] = [];
  const analysis = analyzeGenerativeUiJsonValue(schema, {
    limits: {
      max_bytes: ATMS_PLUGIN_SCHEMA_MAX_BYTES,
      max_depth: ATMS_PLUGIN_SCHEMA_MAX_DEPTH,
      max_values: ATMS_PLUGIN_SCHEMA_MAX_VALUES,
    },
  });
  if (!analysis.valid) {
    issues.push({
  ...
  const visit = (value: unknown, pointer: string): void => {
    ...
    if (object.type === "array" && (
      ...
      || Number(object.maxItems) > ATMS_PLUGIN_SCHEMA_MAX_ARRAY_ITEMS
    )) issues.push({
      path: `${pointer}/maxItems`,
      message: `arrays require maxItems <= ${ATMS_PLUGIN_SCHEMA_MAX_ARRAY_ITEMS}`,
    ...
    if (object.type === "string" && !Array.isArray(object.enum) && !("const" in object) && (
      ...
      || Number(object.maxLength) > ATMS_PLUGIN_SCHEMA_MAX_STRING_LENGTH
    )) issues.push({
      path: `${pointer}/maxLength`,
      message: `strings require maxLength <= ${ATMS_PLUGIN_SCHEMA_MAX_STRING_LENGTH}`,
      keyword: "schemaComplexity",
    });
    if (object.type === "object") {
      if (
        ...
        && !(
          ...
          && Number(object.maxProperties) <= ATMS_PLUGIN_SCHEMA_MAX_PROPERTIES
        )
      ) issues.push({
        path: `${pointer}/additionalProperties`,
        message: `objects must be closed or bound maxProperties <= ${ATMS_PLUGIN_SCHEMA_MAX_PROPERTIES}`,
        keyword: "schemaComplexity",
      });
      const propertyCount = object.properties && typeof object.properties === "object" && !Array.isArray(object.properties)
        ...
        : 0;
      if (propertyCount > ATMS_PLUGIN_SCHEMA_MAX_PROPERTIES) issues.push({
        path: `${pointer}/properties`,
        message: `objects may declare at most ${ATMS_PLUGIN_SCHEMA_MAX_PROPERTIES} properties`,
        keyword: "schemaComplexity",
      });
      if (Array.isArray(object.required) && object.required.length > ATMS_PLUGIN_SCHEMA_MAX_PROPERTIES) {
        complexity(`${pointer}/required`, `required may contain at most ${ATMS_PLUGIN_SCHEMA_MAX_PROPERTIES} fields`);
      }
    ...
    if (Array.isArray(object.enum) && object.enum.length > ATMS_PLUGIN_SCHEMA_MAX_ENUM_VALUES) issues.push({
      path: `${pointer}/enum`,
      message: `enum may contain at most ${ATMS_PLUGIN_SCHEMA_MAX_ENUM_VALUES} values`,
      keyword: "schemaComplexity",
    });
    ...
    if (object.definitions && typeof object.definitions === "object" && !Array.isArray(object.definitions)) {
      ...
      if (definitions.length > ATMS_PLUGIN_SCHEMA_MAX_PROPERTIES) {
        complexity(`${pointer}/definitions`, `definitions may contain at most ${ATMS_PLUGIN_SCHEMA_MAX_PROPERTIES} schemas`);
      }
