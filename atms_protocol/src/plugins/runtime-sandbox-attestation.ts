...
export const ATMS_RUNTIME_SANDBOX_ATTESTATION_VERSION = 1 as const;
export const ATMS_RUNTIME_SANDBOX_ATTESTATION_MAX_TTL_MS = 10 * 60 * 1000;
export const ATMS_RUNTIME_BLOCKED_SECRET_ENV = [
  "ATMS_MANAGER_ADMIN_TOKEN",
  "ATMS_PLUGIN_CAPABILITY_SECRET",
] as const;

export interface AtmsPluginRuntimeSandboxIsolationV1 {
  profile_id: string;
  uid: number;
  gid: number;
...
export interface AtmsPluginRuntimeContainerMeasurementV1 {
  measurement_version: 1;
  container_id: string;
  image_digest: string;
  command: string[];
  env_names: string[];
  isolation: AtmsPluginRuntimeSandboxIsolationV1;
}

export interface AtmsPluginRuntimeSandboxAttestationClaimsV1 {
...
export interface AtmsPluginRuntimeSandboxAttestationV1 {
  claims: AtmsPluginRuntimeSandboxAttestationClaimsV1;
  signature: string;
}

export interface AtmsPluginRuntimeSandboxAttestationValidationResultV1 {
  valid: boolean;
  errors: Array<{ path: string; message: string }>;
  value?: AtmsPluginRuntimeSandboxAttestationV1;
}
...
const PERMISSIONS = new Set<string>(Object.values(AtmsPluginPermission));
...
function validGrant(value: unknown): value is AtmsPluginEffectivePermissionGrantV1 {
  ...
  if (value.permission === AtmsPluginPermission.NETWORK_CONNECT && !canonicalStrings(value.hosts, false)) return false;
  return true;
}

export function atmsPluginRuntimeSandboxAttestationSigningInput(
  claims: AtmsPluginRuntimeSandboxAttestationClaimsV1,
): string {
  return stableStringify(claims);
}

export function atmsPluginRuntimeContainerMeasurementDigestInput(
  measurement: AtmsPluginRuntimeContainerMeasurementV1,
): string {
  return stableStringify(measurement);
}

export function validateAtmsPluginRuntimeSandboxAttestation(
  raw: unknown,
  options: { now_ms?: number; clock_skew_ms?: number } = {},
): AtmsPluginRuntimeSandboxAttestationValidationResultV1 {
  ...
  if (claims.issuer !== "atms-node") errors.push({ path: "/claims/issuer", message: "invalid issuer" });
  if (claims.audience !== "atms-manager") errors.push({ path: "/claims/audience", message: "invalid audience" });
  ...
  if (Number.isFinite(issuedAt) && Number.isFinite(expiresAt)) {
    if (expiresAt <= issuedAt || expiresAt - issuedAt > ATMS_RUNTIME_SANDBOX_ATTESTATION_MAX_TTL_MS) {
  ...
  const value = raw as unknown as AtmsPluginRuntimeSandboxAttestationV1;
...
function validateIsolation(
  ...
  if (!Array.isArray(isolation.blocked_secret_env)
    || stableStringify(isolation.blocked_secret_env) !== stableStringify(ATMS_RUNTIME_BLOCKED_SECRET_ENV)) {
