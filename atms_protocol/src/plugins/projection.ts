...
import { analyzeAtmsPluginSchemaPolicy } from "./schema-policy.js";
import type {
  AtmsDirectUiProjectionResultV1,
  AtmsDirectUiProjectionV1,
  AtmsPluginValidationError,
  AtmsPluginValidationResult,
  AtmsPluginToolDescriptorV1,
  AtmsPluginToolExecutionEnvelopeV1,
} from "./types.js";
...
function normalizedAjvErrors(errors: ErrorObject[] | null | undefined): AtmsPluginValidationError[] {
...
export function validateAtmsPluginToolInput(
  schema: Record<string, unknown>,
  value: unknown,
): AtmsPluginValidationResult<Record<string, unknown>> {
  const schemaIssues = analyzeAtmsPluginSchemaPolicy(schema);
...
function projectedFallbackItems(
  value: unknown,
  projections: AtmsDirectUiProjectionV1["fallback"]["item_projections"],
): string[] {
  const items: string[] = [];
...
function legacyPriority(importance: AtmsDirectUiProjectionV1["defaults"]["importance"]): string {
  if (importance === "critical" || importance === "primary") return "high";
  if (importance === "ambient") return "low";
  return "normal";
}

export function applyAtmsDirectUiProjection(input: {
  projection: unknown;
  plugin: { id: string; version: string };
  arguments: Record<string, unknown>;
}): AtmsDirectUiProjectionResultV1 {
  const projectionValidation = validateAtmsDirectUiProjection(input.projection);
...
export function executeAtmsPluginTool(
  ...
  const input = validateAtmsPluginToolInput(descriptor.input_schema, argumentsValue);
  ...
  const output = validateAtmsPluginToolInput(descriptor.output_schema, projection.node.content);
  if (!output.valid) {
  ...
  const validation = validateAtmsPluginToolExecutionEnvelope(envelope);
  if (!validation.valid || !validation.value) {
