...
export const ATMS_ARTIFACT_BROKER_CAPABILITY_VERSION = 1 as const;
export const ATMS_ARTIFACT_BROKER_MAX_TTL_MS = 5 * 60 * 1000;
export const ATMS_ARTIFACT_BROKER_MAX_BYTES = 16 * 1024 * 1024;
export const ATMS_ARTIFACT_BROKER_MEDIA_TYPES = [
  "application/json",
  "image/jpeg",
  "image/png",
  "image/webp",
...
export type AtmsPluginArtifactMediaTypeV1 =
  (typeof ATMS_ARTIFACT_BROKER_MEDIA_TYPES)[number];

export interface AtmsPluginArtifactWriteCapabilityClaimsV1 {
  artifact_capability_version: 1;
  capability_id: string;
  audience: "atms.artifact-broker";
  ...
  artifact: {
    label: string;
    media_type: AtmsPluginArtifactMediaTypeV1;
    digest: string;
    size_bytes: number;
  };
  issued_at: string;
...
export interface AtmsPluginArtifactCapabilityValidationOptionsV1 {
  now_ms?: number;
...
const MEDIA_TYPES = new Set<string>(ATMS_ARTIFACT_BROKER_MEDIA_TYPES);
...
function issue(path: string, message: string, keyword = "artifactCapability"): AtmsPluginValidationError {
...
function bindingErrors(value: unknown): AtmsPluginValidationError[] {
  ...
  if (!isAtmsPluginId(value.plugin_id)) errors.push(issue("/binding/plugin_id", "invalid plugin id"));
...
export function atmsPluginArtifactCapabilitySigningInput(
  claims: AtmsPluginArtifactWriteCapabilityClaimsV1,
): string {
  return stableStringify(claims);
}

export function validateAtmsPluginArtifactWriteCapabilityClaims(
  raw: unknown,
  options: AtmsPluginArtifactCapabilityValidationOptionsV1 = {},
): AtmsPluginValidationResult<AtmsPluginArtifactWriteCapabilityClaimsV1> {
  ...
  if (raw.audience !== "atms.artifact-broker") errors.push(issue("/audience", "invalid audience"));
  ...
  if (!isRecord(raw.artifact) || !exactKeys(raw.artifact, ["label", "media_type", "digest", "size_bytes"])) {
    ...
    if (!Number.isSafeInteger(raw.artifact.size_bytes)
      ...
      || Number(raw.artifact.size_bytes) > ATMS_ARTIFACT_BROKER_MAX_BYTES) {
  ...
  if (Number.isFinite(issuedAt) && Number.isFinite(expiresAt)) {
    if (expiresAt <= issuedAt || expiresAt - issuedAt > ATMS_ARTIFACT_BROKER_MAX_TTL_MS) {
  ...
  const value = raw as unknown as AtmsPluginArtifactWriteCapabilityClaimsV1;
