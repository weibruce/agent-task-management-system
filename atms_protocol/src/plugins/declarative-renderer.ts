...
import type { AtmsDeclarativeRendererV1 } from "./types.js";
import { validateAtmsDeclarativeRenderer } from "./validation.js";

export type AtmsDeclarativeSectionModel =
...
export interface AtmsDeclarativeRendererModelV1 {
  title: string;
  subtitle?: string;
  empty_message: string;
  sections: AtmsDeclarativeSectionModel[];
}
 ...
 */
export function buildAtmsDeclarativeRendererModel(
  documentValue: AtmsDeclarativeRendererV1,
  content: Record<string, unknown>,
  options: { locale?: string } = {},
): AtmsDeclarativeRendererModelV1 {
  const validation = validateAtmsDeclarativeRenderer(documentValue);
  ...
  const sections: AtmsDeclarativeSectionModel[] = [];
