...
import { validateAtmsPluginToolInput } from "./plugins/projection.js";
...
export function managerAgentSkillViewPresentToolDefinition(): AgentToolDefinition {
  return {
    ...
    description: [
      "Run the trusted presenter shipped by an enabled local Atms Skill and publish its validated visual or start its supervised DAG in one call.",
      ...
      "Atms executes without a shell. Visual results are validated and committed; supervised DAG results may load only Workflow/Profile assets inside that Skill before the run starts.",
...
export function managerAgentSkillViewRenderToolDefinition(): AgentToolDefinition {
  return {
    ...
    description: [
      "Render or update a validated visual template owned by an enabled local Atms Skill.",
      "Use the skill_id and template_id returned by the natively loaded Skill or its presenter,",
      "and pass the presenter's semantic data unchanged. Atms owns and validates the A2UI layout.",
    ...
    input_schema: {
      ...
      properties: {
        skill_id: {
          ...
          minLength: 1,
          maxLength: 200,
          description: "Enabled local Skill id, for example the id shown in the Atms Skill catalog.",
        },
        template_id: {
          type: "string",
...
function skillViewToolDefinition(
  ...
  return {
    ...
    description: [
      ...
      "Provide semantic data only; Atms owns the layout. data.title is used as the Block title.",
      "Atms A2UI formatString accepts only ${/absolute/pointer} and template-relative ${pointer} interpolation; escape a literal opener as \\${. Nested functions and other expressions are rejected.",
      "Reuse the current selected Block id for follow-up updates. When this schema matches, use this Tool; the raw generated-view Tool rejects matching data so Atms can preserve this layout.",
    ].join(" "),
...
export function matchingManagerAgentSkillViewToolDefinition(
  ...
  return definitions.find((definition) =>
    validateAtmsPluginToolInput(definition.input_schema, candidate).valid
  );
}
...
export function materializeManagerAgentSkillViewInput(
  ...
  const validation = validateAtmsPluginToolInput(definition.input_schema, input);
  ...
  const content = { data };
  const a2uiValidation = validateAtmsA2uiSurface(definition.template.a2ui, {
    action_ids: new Set(),
    data_model: content,
  });
