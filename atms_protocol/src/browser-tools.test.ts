...
describe("browser tools contracts", () => {
  ...
  it("publishes a small stable page-tool catalog", () => {
    ...
    expect(ATMS_UI_TOOL_CONTRACTS.map((contract) => contract.name)).toEqual([
      "ui_get_state",
      "ui_open_surface",
      "ui_close_surface",
      "ui_describe_widget",
    ...
    expect(atmsUiToolContract("ui_open_surface")?.effect).toBe("presentational");
    expect(atmsUiToolContract("unknown")).toBeUndefined();
  });

  it("separates trusted contract and page descriptor digests", () => {
    const contract = atmsUiToolContract("ui_open_surface")!;
  ...
  it("freezes the cross-repository first-slice contract digests", () => {
    expect(Object.fromEntries(ATMS_UI_TOOL_CONTRACTS.map((contract) => [
      contract.name,
  ...
  it("validates and normalizes every frozen tool input", () => {
    expect(validateAtmsUiToolInput("ui_get_state", {})).toEqual({});
    expect(validateAtmsUiToolInput("ui_open_surface", {
      surface: " dag_status ",
      entity_id: " run-001 ",
    })).toEqual({ surface: "dag_status", entity_id: "run-001" });
    expect(validateAtmsUiToolInput("ui_close_surface", {
      surface: "dag_status",
    ...
    expect(validateAtmsUiToolInput("ui_describe_widget", widgetTarget)).toEqual(widgetTarget);
    expect(validateAtmsUiToolInput("ui_set_widget_expanded", {
      ...widgetTarget,
      expanded: true,
    })).toEqual({
      ...widgetTarget,
    ...
    expect(validateAtmsUiToolInput("ui_describe_widget", {
      ...widgetTarget,
      widget_id: "widget-one",
  ...
  it.each([
    ...
    expect(() => validateAtmsUiToolInput(name, input)).toThrow(error);
  });
});
