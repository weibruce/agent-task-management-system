/**
 * Experimental Atms Browser Tools contracts.
 *
...
export const ATMS_UI_SURFACES = ["dag_status"] as const;
export type AtmsUiSurface = (typeof ATMS_UI_SURFACES)[number];

export const ATMS_UI_TOOL_NAMES = [
  "ui_get_state",
  "ui_open_surface",
  "ui_close_surface",
  "ui_describe_widget",
...
export type AtmsUiToolName = (typeof ATMS_UI_TOOL_NAMES)[number];
 ...
 */
export const ATMS_UI_TOOL_CONTRACTS: readonly UiToolContract[] = Object.freeze([
  Object.freeze({
    name: "ui_get_state",
    description: "Read the visible Atms surface and a bounded list of trusted widget identities and revisions.",
  ...
  Object.freeze({
    ...
    description: "Open a trusted Atms surface directly. For dag_status, provide an exact run id, a unique run query, or neither to open the run list.",
  ...
  Object.freeze({
    ...
    description: "Close a trusted Atms surface without changing DAG or workspace business state.",
    input_schema: Object.freeze({
      type: "object",
...
export function atmsUiToolContract(name: string): UiToolContract | undefined {
  return ATMS_UI_TOOL_CONTRACTS.find((contract) => contract.name === name);
}
 ...
 */
export function validateAtmsUiToolInput(
  name: AtmsUiToolName,
  value: unknown,
  ...
  const allowed = name === "ui_get_state"
    ...
    : name === "ui_open_surface"
      ...
      : name === "ui_close_surface"
        ...
        : null;
  if (!allowed) throw new Error(`Unknown Atms UI tool: ${String(name)}`);
...
/**
 * Browser-hosted renderer transport for the same frozen Atms UI contracts.
 *
