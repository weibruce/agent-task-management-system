...
export function managerAgentRequiredToolObjectivePrompt(value: unknown): string {
  ...
  return [
    "Atms runtime objective for this turn:",
...
export function managerAgentOutcomeObjectivePrompt(value: unknown): string {
  ...
  return [
    "Atms externally observable outcome contract for this turn:",
    ...
    "- Atms verifies committed Tool evidence after the turn and rejects unsupported completion claims.",
  ].join("\n");
}
...
export function managerAgentPluginToolCallName(
  descriptor: Pick<AtmsPluginToolDescriptorV1, "local_id" | "wire_id">,
  descriptors: readonly Pick<AtmsPluginToolDescriptorV1, "local_id" | "wire_id">[],
): string {
...
export const ATMS_PROMPT_TOOL_CALL_PROTOCOL = "atms_tool_call";
export const ATMS_PROMPT_HANDOFF_PROTOCOL = "atms_handoff";
...
export interface AtmsPromptToolCall {
  name: string;
  input: Record<string, unknown>;
}

export interface AtmsPromptHandoff {
  port: string;
  content: unknown;
  summary?: string;
}
...
export const MANAGER_AGENT_TOOL_SPECS: Record<ManagerAgentToolName, AgentToolDefinition> = {
  ui_get_state: {
    name: "ui_get_state",
    description: atmsUiToolContract("ui_get_state")!.description,
    input_schema: atmsUiToolContract("ui_get_state")!.input_schema,
  },
  ui_open_surface: {
    name: "ui_open_surface",
    description: atmsUiToolContract("ui_open_surface")!.description,
    input_schema: atmsUiToolContract("ui_open_surface")!.input_schema,
  },
  ui_close_surface: {
    name: "ui_close_surface",
    description: atmsUiToolContract("ui_close_surface")!.description,
    input_schema: atmsUiToolContract("ui_close_surface")!.input_schema,
  },
  ui_describe_widget: {
    name: "ui_describe_widget",
    description: atmsUiToolContract("ui_describe_widget")!.description,
    input_schema: atmsUiToolContract("ui_describe_widget")!.input_schema,
  },
  ui_focus_widget: {
    name: "ui_focus_widget",
    description: atmsUiToolContract("ui_focus_widget")!.description,
    input_schema: atmsUiToolContract("ui_focus_widget")!.input_schema,
  },
  ui_set_widget_expanded: {
    name: "ui_set_widget_expanded",
    description: atmsUiToolContract("ui_set_widget_expanded")!.description,
    input_schema: atmsUiToolContract("ui_set_widget_expanded")!.input_schema,
  },
  list_projects: {
    name: "list_projects",
  ...
  list_skills: {
    name: "list_skills",
    description: "List all enabled Atms skills discovered from ATMS_HOME/skills with built-in fallbacks.",
    input_schema: emptyObjectSchema,
  },
  read_skill: {
    name: "read_skill",
    description: "Load the complete SKILL.md instructions for one Atms skill before applying it.",
    input_schema: {
      type: "object",
  ...
  list_orchestrations: {
    ...
    description: "List Atms orchestration YAML templates available to the current runtime.",
    input_schema: emptyObjectSchema,
  },
  list_dag_patterns: {
  ...
  run_pr_review: {
    name: "run_pr_review",
    description: "Resolve immutable GitHub PR metadata in code and start Atms's built-in read-only pr-review DAG. Prefer this over manually calling gh, curl, or create_and_run for PR reviews.",
    input_schema: {
      type: "object",
      properties: {
  ...
  run_pr_closeout: {
    ...
    description: "Resolve GitHub and persisted Atms evidence in code, then start the deterministic pr-closeout DAG. This tool never merges a PR and does not accept model-asserted local test evidence.",
    input_schema: {
      type: "object",
      properties: {
        repo: { type: "string", description: "GitHub repository in owner/name form" },
  ...
  publish_artifact: {
    ...
    description: [
      ...
      "Pass a project-relative source_path. The result includes a stable artifact_id, revision, and preview_url; bind preview_url from a Atms A2UI HrArtifact component.",
...
export function formatAtmsPromptToolCall(call: AtmsPromptToolCall): string {
  return `<${ATMS_PROMPT_TOOL_CALL_PROTOCOL}>${JSON.stringify({
    name: call.name,
    input: call.input ?? {},
  })}</${ATMS_PROMPT_TOOL_CALL_PROTOCOL}>`;
}

export function parseAtmsPromptToolCalls(text: string): AtmsPromptToolCall[] {
  return extractJsonMarkerObjects(text, ATMS_PROMPT_TOOL_CALL_PROTOCOL)
    .map((parsed): AtmsPromptToolCall | null => {
    ...
    .filter((item): item is AtmsPromptToolCall => Boolean(item));
}

export function formatAtmsPromptHandoff(handoff: AtmsPromptHandoff): string {
  return `<${ATMS_PROMPT_HANDOFF_PROTOCOL}>${JSON.stringify({
    port: handoff.port,
    content: handoff.content,
    ...(handoff.summary ? { summary: handoff.summary } : {}),
  })}</${ATMS_PROMPT_HANDOFF_PROTOCOL}>`;
}

export function parseAtmsPromptHandoff(text: string): AtmsPromptHandoff | null {
  const parsed = extractJsonMarkerObjects(text, ATMS_PROMPT_HANDOFF_PROTOCOL)[0];
...
export function stripAtmsPromptMarkers(text: string): string {
  return decodePromptMarkerText(text)
    .replace(new RegExp(`<${ATMS_PROMPT_TOOL_CALL_PROTOCOL}>[\\s\\S]*?<\\/${ATMS_PROMPT_TOOL_CALL_PROTOCOL}>`, "g"), "")
    .replace(new RegExp(`<${ATMS_PROMPT_HANDOFF_PROTOCOL}>[\\s\\S]*?<\\/${ATMS_PROMPT_HANDOFF_PROTOCOL}>`, "g"), "")
    .trim();
}
