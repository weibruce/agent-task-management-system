...
Call the current Tool catalog entry for `com.atms.topic-outline:upsert_topic_outline` with the complete current outline. Its harness-safe wire name is supplied by the current turn and must not be guessed. Reuse one stable plugin-owned `id` such as `com.atms.topic-outline:current` across turns so a revision replaces the same semantic node without colliding with Core or another plugin. A partial outline is valid: record what is known, preserve useful prior fields, and put only unresolved decisions in `questions`.
