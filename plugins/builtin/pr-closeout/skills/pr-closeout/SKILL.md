...
Call the current Tool catalog entry for `com.atms.pr-closeout:upsert_pr_closeout` using the harness-safe wire name supplied by the current turn. Reuse one stable plugin-owned `id`, such as `com.atms.pr-closeout:owner-repo-pr-21`, so later evidence replaces the same report.
