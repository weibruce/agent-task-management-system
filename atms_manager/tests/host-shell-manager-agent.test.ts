...
describe("host-shell Manager Agent worker fingerprint", () => {
  ...
  beforeEach(async () => {
    ...
    oldPort = process.env.ATMS_MANAGER_AGENT_HOST_PORT;
    oldEntry = process.env.ATMS_MANAGER_AGENT_HOST_ENTRY;
    oldShell = process.env.ATMS_MANAGER_AGENT_SHELL;
    oldWorkspace = process.env.ATMS_PROJECT_WORKSPACE;
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "atms-host-shell-"));
    process.env.ATMS_HOME = tmpDir;
    process.env.ATMS_MANAGER_AGENT_HOST_PORT = String(await availablePort());
    process.env.ATMS_MANAGER_AGENT_SHELL = process.platform === "win32" ? process.execPath : "/bin/sh";
    process.env.ATMS_PROJECT_WORKSPACE = tmpDir;
  });
  ...
  afterEach(async () => {
    ...
    if (oldPort === undefined) delete process.env.ATMS_MANAGER_AGENT_HOST_PORT;
    else process.env.ATMS_MANAGER_AGENT_HOST_PORT = oldPort;
    if (oldEntry === undefined) delete process.env.ATMS_MANAGER_AGENT_HOST_ENTRY;
    else process.env.ATMS_MANAGER_AGENT_HOST_ENTRY = oldEntry;
    if (oldShell === undefined) delete process.env.ATMS_MANAGER_AGENT_SHELL;
    else process.env.ATMS_MANAGER_AGENT_SHELL = oldShell;
    if (oldWorkspace === undefined) delete process.env.ATMS_PROJECT_WORKSPACE;
    else process.env.ATMS_PROJECT_WORKSPACE = oldWorkspace;
  });

  it("changes when the worker entry build artifact changes", () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "atms-host-shell-fingerprint-"));
