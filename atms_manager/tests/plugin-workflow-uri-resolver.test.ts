...
describe("plugin workflow URI resolver", () => {
  ...
  beforeEach(() => {
    ...
    home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workflow-resolver-home-"));
    source = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workflow-resolver-source-"));
  ...
  it("rejects non-canonical URIs, unknown fields and capabilities outside the selected context", () => {
    ...
    expect(() => resolvePluginWorkflowUri({
      ...request,
      capability_id: "com.atms.core:voice-generative-ui",
