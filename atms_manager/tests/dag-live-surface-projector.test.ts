...
describe("DAG Live Surface Projector", () => {
  ...
  beforeEach(() => {
    ...
    home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-live-surface-"));
  ...
  it("projects three actors into three stable native A2UI blocks without cross-over", () => {
    ...
    expect(document.nodes.every((node) => node.kind === "com.atms.core/generated_view" && node.kind_version === 2))
      .toBe(true);
    expect(document.nodes.every((node) => node.a2ui?.catalogId === "https://atms.dev/a2ui/catalogs/core/v1"))
      .toBe(true);
  ...
  it("uses a fixed bounded A2UI catalog while retaining only recent findings", () => {
    ...
    expect(node.a2ui?.components.length).toBeLessThanOrEqual(ATMS_A2UI_MAX_COMPONENTS);
