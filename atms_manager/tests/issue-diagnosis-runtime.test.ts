  ...
  evidence: [
    {
      id: "repro-e001",
      type: "test",
      locator: "atms_manager/tests/llm-providers.test.ts:420-480",
      observation: "The focused API test accepts reuse_existing_api_key and rejects the reserved sentinel.",
    },
    {
      id: "repro-e002",
      type: "command",
      locator: "npm --prefix atms_manager exec vitest run tests/llm-providers.test.ts",
      observation: "The focused regression test passed.",
    },
  ],
  tests: [{
    command: "npm --prefix atms_manager exec vitest run tests/llm-providers.test.ts",
    status: "passed",
...
const dataflowReview = {
  ...
  evidence: [
    ...
    },
    {
      id: "dataflow-e002",
      type: "source",
      locator: "atms_manager/src/server/llm-settings.ts:438-447",
  ...
  evidence: [
    {
      id: "history-e001",
      type: "history",
      locator: "git show e2ad824^:atms_manager/src/persistence/llm-settings.ts",
      observation: "The parent reused a key for api_key=__reuse_existing__.",
    },
    {
      id: "history-e002",
      type: "history",
      locator: "git show e2ad824:atms_manager/src/server/llm-settings.ts",
...
function prepareDiagnosisRun(
  ...
  const runWorkspace = path.join(process.env.ATMS_HOME!, "workspace", ...runId.split("/"));
...
describe("issue-diagnosis pattern runtime", () => {
  ...
  beforeEach(() => {
    ...
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-issue-diagnosis-"));
  ...
  it("captures bounded line-numbered focus evidence without reading traversal paths", () => {
    ...
    expect(snapshot).toMatchObject({
      ...
      tested_revision: revision,
      files: [{
        path: "atms_manager/src/orchestration/dag-patterns.ts",
