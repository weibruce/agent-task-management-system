  ...
  return {
    ...
    nodes: Array.from({ length: nodeCount }, (_, index) => ({
      ...
      a2ui: {
        ...
        catalogId: "https://atms.dev/a2ui/catalogs/core/v1" as const,
...
describe("Generative UI canvas turn context", () => {
  ...
  it("labels canvas values as data and keeps the user message last", () => {
    ...
    expect(prompt).toContain("Atms binds that Tool to selected_node_id");
  ...
  it("keeps attached DAG context separate from canvas data and before the user message", () => {
    ...
    expect(prompt).toContain("Current Atms DAG context");
