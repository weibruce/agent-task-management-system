...
vi.mock("../src/server/edge-tts.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    synthesizeEdgeTts: vi.fn(async () => Buffer.from("ID3-atms-edge")),
  };
});
...
function browserRendererContracts(): Array<{ name: string; contract_digest: string }> {
  return ATMS_UI_TOOL_CONTRACTS.map((contract) => ({
    name: contract.name,
...
function installVoiceHostManagerStub(input: {
    ...
    const server = http.createServer((req, res) => {
      if (req.method === 'GET' && req.url === '/health') {
        ...
        res.end(JSON.stringify({
          ...
          fingerprint: process.env.ATMS_MANAGER_AGENT_FINGERPRINT,
          process_id: process.pid,
          project_id: process.env.PROJECT_ID || null,
          worker_id: process.env.ATMS_WORKER_ID,
        }));
        return;
      }
  ...
  `, "utf-8");
  process.env.ATMS_MANAGER_AGENT_HOST_ENTRY = entry;
  process.env.ATMS_MANAGER_AGENT_SHELL = process.platform === "win32" ? process.execPath : "/bin/sh";
}
...
describe("voice bootstrap routes", () => {
  ...
  beforeEach(async () => {
    ...
    oldLocalNodeAutostart = process.env.ATMS_LOCAL_NODE_AUTOSTART;
    oldHostPort = process.env.ATMS_MANAGER_AGENT_HOST_PORT;
    oldHostEntry = process.env.ATMS_MANAGER_AGENT_HOST_ENTRY;
    oldHostShell = process.env.ATMS_MANAGER_AGENT_SHELL;
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-voice-bootstrap-"));
    process.env.ATMS_HOME = tmpHome;
    process.env.ATMS_LOCAL_NODE_AUTOSTART = "0";
    process.env.ATMS_MANAGER_AGENT_HOST_PORT = String(await findAvailableManagerAgentPort());
    delete process.env.ATMS_MANAGER_AGENT_HOST_ENTRY;
    delete process.env.ATMS_MANAGER_AGENT_SHELL;
    vi.mocked(synthesizeEdgeTts).mockReset().mockResolvedValue(Buffer.from("ID3-atms-edge"));
    _clearStoredConfig();
  ...
  afterEach(async () => {
    ...
    if (oldLocalNodeAutostart === undefined) delete process.env.ATMS_LOCAL_NODE_AUTOSTART;
    else process.env.ATMS_LOCAL_NODE_AUTOSTART = oldLocalNodeAutostart;
    if (oldHostPort === undefined) delete process.env.ATMS_MANAGER_AGENT_HOST_PORT;
    else process.env.ATMS_MANAGER_AGENT_HOST_PORT = oldHostPort;
    if (oldHostEntry === undefined) delete process.env.ATMS_MANAGER_AGENT_HOST_ENTRY;
    else process.env.ATMS_MANAGER_AGENT_HOST_ENTRY = oldHostEntry;
    if (oldHostShell === undefined) delete process.env.ATMS_MANAGER_AGENT_SHELL;
    else process.env.ATMS_MANAGER_AGENT_SHELL = oldHostShell;
    delete process.env.ATMS_MIMO_API_KEY;
    delete process.env.ATMS_TTS_API_KEY;
  ...
  it("passes the selected project workspace to host Codex voice turns", async () => {
    const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "atms-voice-selected-workspace-"));
  ...
  it("passes only the selected persisted canvas Block into the next Manager turn", async () => {
    ...
    const turn = await fetch(`${baseUrl}/api/voice-agent/sessions/${createdBody.data.session_id}/turn`, {
      ...
      body: JSON.stringify({
        text: "深入第二条",
        selected_node_id: "com.atms.core:ai-news",
      }),
    });
  ...
  it("removes a canonical generated-view Block returned by the Manager Agent", async () => {
    _setHostCodexManagerAgentRunnerForTest(async (input) => ({
      ...
      tool_calls: [{
        ...
        input: { id: "com.atms.core:motion-check" },
      }],
      ...
      voice_surface: {
        ...
        task_draft: null,
        widgets: [],
        remove_widget_ids: ["com.atms.core:motion-check"],
      },
      worker_id: "host-codex",
    }));
    const port = await listen(server);
    ...
    const turn = await fetch(`${baseUrl}/api/voice-agent/sessions/${createdBody.data.session_id}/turn`, {
      ...
      body: JSON.stringify({
        text: "移除临时卡",
        selected_node_id: "com.atms.core:motion-check",
      }),
    });
  ...
  it("publishes and serves a standalone project artifact across requests", async () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), "atms-voice-artifact-project-"));
    ...
    try {
      ...
      expect(previewHtml).toContain("data-atms-artifact-appearance-bridge");
  ...
  it("serves voice endpoints and built-in Edge TTS without requiring local defaults", async () => {
    ...
    expect(Buffer.from(await speech.arrayBuffer()).toString()).toBe("ID3-atms-edge");
  ...
  it("routes codex_appserver voice turns through the real Manager Agent handoff", async () => {
    ...
    const turn = await fetch(`${baseUrl}/api/voice-agent/sessions/${createdBody.data.session_id}/turn`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: "Atms 语音测试，请创建一个一句话任务摘要，不要执行真实操作。" }),
    });
    const body = await turn.json() as {
      data: {
        spoken_text: string;
    ...
    expect(turn.status).toBe(200);
    expect(body.data.spoken_text).toBe("host codex handled: Atms 语音测试，请创建一个一句话任务摘要，不要执行真实操作。");
    ...
    expect(body.data.workspace.conversation).toContainEqual(expect.objectContaining({
      role: "assistant",
      text: "host codex handled: Atms 语音测试，请创建一个一句话任务摘要，不要执行真实操作。",
      spoken_text: "host codex handled: Atms 语音测试，请创建一个一句话任务摘要，不要执行真实操作。",
      channel: "final",
    }));
  ...
  it("does not synthesize generated UI from verbose manager output without voice tools", async () => {
    const verbose = [
      "## Manager Agent",
      "- 启动 / 管理 Atms DAG 工作流：查看模板、创建运行、查询状态。",
      "- 代码与项目协作：读代码、改文件、跑测试。",
