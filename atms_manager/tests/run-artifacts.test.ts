...
describe("run artifacts", () => {
  ...
  beforeEach(() => {
    ...
    home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-manager-artifacts-"));
    ...
    try {
      ...
      const uploaded = await fetch(`${base}/api/runs/artifact-upload-run/artifacts/evidence.tar.gz/upload`, {
        ...
        headers: {
          ...
          "X-Atms-Artifact-Sha256": sha256,
          "X-Atms-Artifact-Uncompressed-Bytes": "123",
          "X-Atms-Artifact-File-Count": "4",
        },
        body: bytes,
      });
