...
describe("Manager Agent signed turn envelope", () => {
  it("forwards a signed, target-scoped payload while exposing only the public key", async () => {
    ...
    expect(Object.keys(environment).sort()).toEqual([
      "ATMS_MANAGER_TURN_ENVELOPE_REQUIRED",
      ATMS_MANAGER_TURN_KEY_ID_ENV,
      ATMS_MANAGER_TURN_PUBLIC_KEY_ENV,
    ].sort());
    expect(envelope.claims.key_id).toBe(environment[ATMS_MANAGER_TURN_KEY_ID_ENV]);
    expect(envelope.claims.scope).toMatchObject({
      browser_tools_transport: "renderer",
      browser_tools_target: payload.browser_tools_target,
    });
    const publicKey = createPublicKey({
      key: Buffer.from(environment[ATMS_MANAGER_TURN_PUBLIC_KEY_ENV]!, "base64url"),
      format: "der",
      type: "spki",
    });
    expect(verify(
