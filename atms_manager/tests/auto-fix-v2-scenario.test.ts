...
describe("Auto Fix v2 local-test convergence architecture", () => {
  ...
  beforeEach(() => {
    ...
    previousCommandAllowlist = process.env.ATMS_DAG_COMMAND_ALLOWLIST;
    home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-autofix-v2-"));
    ...
    git(repository, ["config", "user.name", "Atms Test"]);
    git(repository, ["config", "user.email", "atms@example.invalid"]);
  ...
  afterEach(() => {
    ...
    if (previousCommandAllowlist === undefined) delete process.env.ATMS_DAG_COMMAND_ALLOWLIST;
    else process.env.ATMS_DAG_COMMAND_ALLOWLIST = previousCommandAllowlist;
  ...
  function writeReport(workspace: string, phase: "aggregate" | "fix", status: "passed" | "failed") {
    const relative = `.atms/test-reports/${phase}.json`;
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      ...
      try {
        process.env.ATMS_HOME = aliasedHome;
        closeDb();
        _clearActiveRuns();
