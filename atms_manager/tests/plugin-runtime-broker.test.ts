...
describe("plugin Runtime RPC broker", () => {
  ...
  beforeEach(() => {
    ...
    home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-runtime-home-"));
    source = fs.mkdtempSync(path.join(os.tmpdir(), "atms-runtime-source-"));
  ...
  function invocation(suffix: string): AtmsPluginToolInvocationV1 {
    const value: AtmsPluginToolInvocationV1 = {
      tool_bus_version: 1,
    ...
    value.request_digest = pluginJsonDigest(atmsPluginToolInvocationDigestInput(value));
  ...
  function authorization(
    ...
    request: AtmsPluginToolInvocationV1,
  ) {
  ...
  it("returns a validated correlated result and consumes the capability", async () => {
    ...
    let captured: AtmsPluginRuntimeRpcRequestV1 | undefined;
  ...
  it("accepts only a Node-signed sandbox attestation bound to launch isolation and exact grants", async () => {
    ...
    const baseClaims: AtmsPluginRuntimeSandboxAttestationClaimsV1 = {
      ...
      isolation: {
        ...
        mounts: [{ source: `package:${request.binding.package_digest}`, target: "/opt/atms/plugin", mode: "ro" }],
    ...
    const signed = (claims: AtmsPluginRuntimeSandboxAttestationClaimsV1): AtmsPluginRuntimeSandboxAttestationV1 => ({
      claims,
      signature: sign(
        null,
        Buffer.from(atmsPluginRuntimeSandboxAttestationSigningInput(claims), "utf8"),
        privateKey,
      ).toString("base64url"),
    });
    const gate = new VerifiedPluginRuntimeSandboxGate({
      trusted_nodes: [{ key_id: "node-key-one", node_id: "node-one", public_key: publicKey }],
      allowed_profile_ids: new Set(["atms.plugin-runtime.v1"]),
    });
    ...
    expect(() => gate.assertAllowed({
      ...
      attestation: signed({
        ...
        isolation: {
          ...
          network: { mode: "brokered", hosts: ["example.com"], network_name: "atms-plugin-broker-test", internal: true },
        },
      }),
      transport_identity: { node_id: "node-one", runtime_instance_id: "runtime-instance-one", container_id: "container-one", measurement_digest: "e".repeat(64), image_digest: `sha256:${"d".repeat(64)}` },
      now,
    ...
    const registration = (
      ...
      sandbox_attestation: {
        claims: {
          ...
          issuer: "atms-node" as const,
          audience: "atms-manager" as const,
  ...
  it("correlates cancel and health through the exact bound Runtime", async () => {
    ...
    const seen: AtmsPluginRuntimeRpcRequestV1[] = [];
    const transports = new PluginRuntimeTransportRegistry();
    transports.register("com.example.runtime", "0.1.0", {
      request: async (rpc) => {
        seen.push(rpc);
