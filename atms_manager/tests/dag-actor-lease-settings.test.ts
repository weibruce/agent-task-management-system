...
const ENV_NAMES = [
  "ATMS_DAG_WORKER_IDLE_TTL_MS",
  "ATMS_DAG_ACTOR_RETENTION_TTL_MS",
] as const;
...
describe("DAG actor lease settings", () => {
  ...
  it("accepts bounded base-10 environment overrides", () => {
    expect(loadDagActorLeaseSettings({
      ATMS_DAG_WORKER_IDLE_TTL_MS: "1500",
      ATMS_DAG_ACTOR_RETENTION_TTL_MS: "900000",
    })).toEqual({
  ...
  it.each(["", "0", "-1", "1.5", "1e3", "Infinity", "NaN"])(
    ...
    (value) => {
      expect(() => loadDagActorLeaseSettings({
        ATMS_DAG_WORKER_IDLE_TTL_MS: value,
      })).toThrow(/ATMS_DAG_WORKER_IDLE_TTL_MS/);
    },
  );
