...
describe("workspace retention", () => {
  ...
  beforeEach(() => {
    ...
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-workspace-retention-"));
  ...
  it("unlinks an expired workspace symlink without touching its target", async () => {
    ...
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), "atms-retention-outside-"));
  ...
  it("rejects workspace paths that escape through an intermediate symlink", async () => {
    ...
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), "atms-retention-parent-outside-"));
...
`));
  ...
  it("persists settings and defaults cleanup to dry-run through the HTTP API", async () => {
    process.env.ATMS_DAG_MUTATION_TOKEN = "retention-secret";
    ...
    const headers = {
      ...
      "x-atms-dag-token": "retention-secret",
    };
