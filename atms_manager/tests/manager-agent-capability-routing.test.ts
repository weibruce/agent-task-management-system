...
import type { GenerativeUiCanvasContextV1 } from "atms-protocol";
...
function writeHomeSkillA2uiTemplate(home: string, id: string): void {
  const root = path.join(home, "skills", id, "assets", "atms");
  fs.mkdirSync(root, { recursive: true });
...
describe("Manager Agent native Skill visibility", () => {
  ...
  beforeEach(() => {
    ...
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-manager-routing-"));
  ...
  it("delivers the same complete bundled Skill/Tool snapshot to host and host-shell", async () => {
    ...
    expect(hostContext.tools.map((tool) => tool.qualified_id)).toEqual([
      ...
      "com.atms.topic-outline:upsert_topic_outline",
    ]);
    ...
    const coreSkill = deliveredSkills.find((skill) => skill.id === "com.atms.core:voice-generative-ui");
  ...
  it("injects leased confirmed Tool results once and releases the lease on Agent failure", async () => {
    captured.continuation_records.push({
      ...
      payload: {
        ...
        tool: {
          ...
          qualified_id: "com.atms.topic-outline:compose_outline",
    ...
    expect(captured.host.at(-1)?.message).toContain("atms_plugin_tool_continuations");
  ...
  it("uses the same selected context for the host streaming path", async () => {
    const canvasContext: GenerativeUiCanvasContextV1 = {
      ...
      document_revision: 3,
      selected_node_id: "com.atms.core:selected-card",
      nodes: [{
        id: "com.atms.core:selected-card",
        revision: 2,
  ...
  it("keeps trusted Registry Agent assets out of the host Manager", () => {
    ...
    configureRemotePluginRegistry({
      registry_id: "agent-trust.atms",
      source_url: "https://registry.atms.example/agent-trust.json",
