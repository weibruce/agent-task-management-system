...
describe("immutable DAG run inputs", () => {
  ...
  beforeEach(() => {
    ...
    previousAllowlist = process.env.ATMS_DAG_COMMAND_ALLOWLIST;
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-run-inputs-"));
  ...
  afterEach(() => {
    ...
    if (previousAllowlist === undefined) delete process.env.ATMS_DAG_COMMAND_ALLOWLIST;
    else process.env.ATMS_DAG_COMMAND_ALLOWLIST = previousAllowlist;
