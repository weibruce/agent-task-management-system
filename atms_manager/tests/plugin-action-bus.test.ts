...
`, "utf8");
  writeJson(path.join(root, "atms.plugin.json"), {
    manifest_version: 1,
    id: pluginId,
    ...
    permissions: {
      required: [{ permission: AtmsPluginPermission.WORKSPACE_READ }],
      optional: [{ permission: AtmsPluginPermission.ARTIFACT_WRITE }],
    },
...
describe("plugin Action Bus", () => {
  ...
  beforeEach(() => {
    ...
    home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-action-bus-home-"));
    fixture = fs.mkdtempSync(path.join(os.tmpdir(), "atms-action-bus-plugin-"));
  ...
  function createDeferredCommitRaceFixture(
    ...
    const root = fs.mkdtempSync(path.join(os.tmpdir(), `atms-commit-race-${race}-`));
    writeFixture(root);
    ...
    ): AtmsPluginRuntimeExecutionOutputV1 => {
    ...
    expect(pending).toMatchObject({
      ...
      challenge: {
        ...
        effective_grants: [
          ...
          { permission: AtmsPluginPermission.WORKSPACE_READ },
        ],
  ...
  it("allows only one executor when an approved confirmation is retried concurrently", async () => {
    ...
    const concurrentRoot = fs.mkdtempSync(path.join(os.tmpdir(), "atms-concurrent-action-plugin-"));
    try {
      ...
      const manifestFile = path.join(concurrentRoot, "atms.plugin.json");
      const manifest = JSON.parse(fs.readFileSync(manifestFile, "utf8")) as {
        tools: Array<{ id: string } & Record<string, unknown>>;
        runtime: Record<string, unknown>;
  ...
  it("contains Runtime crashes and timeouts without changing the canonical document", async () => {
    ...
    const runtimeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "atms-runtime-action-plugin-"));
    try {
      ...
      const manifestFile = path.join(runtimeRoot, "atms.plugin.json");
      const manifest = JSON.parse(fs.readFileSync(manifestFile, "utf8")) as {
        tools: Array<{ id: string } & Record<string, unknown>>;
        runtime: Record<string, unknown>;
        permissions: { required: unknown[]; optional: unknown[] };
