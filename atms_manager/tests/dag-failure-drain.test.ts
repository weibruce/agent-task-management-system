...
beforeEach(()=>{
 prior=Object.fromEntries(['ATMS_HOME','ATMS_DAG_COMMAND_ALLOWLIST','ATMS_DAG_ALLOW_DYNAMIC_COMMANDS'].map(k=>[k,process.env[k]]));
 home=fs.mkdtempSync(path.join(os.tmpdir(),'atms-failure-drain-'));process.env.ATMS_HOME=home;process.env.ATMS_DAG_COMMAND_ALLOWLIST='node';process.env.ATMS_DAG_ALLOW_DYNAMIC_COMMANDS='true';closeDb();_clearActiveRuns();
});
