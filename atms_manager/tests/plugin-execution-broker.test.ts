...
describe("plugin Tool execution broker", () => {
  ...
  beforeEach(() => {
    ...
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-plugin-execution-"));
  ...
  it("replays a trusted envelope into one semantic node and preserves readable disabled history", () => {
    ...
    const descriptor = context.tools.find((tool) => tool.plugin_id === "com.atms.topic-outline")!;
    const envelope = executeAtmsPluginTool(descriptor, {
      id: "com.atms.topic-outline:topic-broker",
    ...
    expect(accepted.node).toMatchObject({
      ...
      owner: { id: "com.atms.topic-outline", version: "1.0.0" },
  ...
  it("accepts an evidence-grounded PR closeout projection without a legacy widget bridge", () => {
    ...
    const descriptor = context.tools.find((tool) => tool.plugin_id === "com.atms.pr-closeout")!;
    const envelope = executeAtmsPluginTool(descriptor, {
      id: "com.atms.pr-closeout:xiaotianfotos-atms-21",
    ...
    expect(accepted.node).toMatchObject({
      id: "com.atms.pr-closeout:xiaotianfotos-atms-21",
      kind: "com.atms.pr-closeout/report",
      owner: { id: "com.atms.pr-closeout", version: "1.0.0" },
      content: {
        recommendation: "blocked",
  ...
  it("accepts runtime-authored A2UI with dynamic surface and density outside semantic content", () => {
    const context = assemblePluginTurnContext(undefined, { modality: "text" });
    const descriptor = context.tools.find((tool) => tool.plugin_id === "com.atms.core")!;
    const envelope = executeAtmsPluginTool(descriptor, {
      id: "com.atms.core:ci-overview",
      title: "CI overview",
      ...
      a2ui: {
        ...
        surfaceProperties: {
          iconUrl: "https://atms.dev/icon.png",
          agentDisplayName: "Atms CI",
        },
        components: [
  ...
  it("rejects executable fields, unregistered Action events, and unsafe Artifact URIs in runtime-authored A2UI", () => {
    const context = assemblePluginTurnContext(undefined, { modality: "text" });
    const descriptor = context.tools.find((tool) => tool.qualified_id === "com.atms.core:upsert_generated_view")!;
    const base = {
      id: "com.atms.core:unsafe-view",
      title: "Unsafe view",
      surface: "result",
      importance: "primary",
      density: "detail",
  ...
  it("rejects A2UI Action context instead of treating it as execution arguments", () => {
    ...
    expect(validation).toMatchObject({
      ...
      errors: [{
        ...
        message: "non-empty event.context is not supported by the Atms host",
  ...
  it("rejects a projection that no longer matches its validated Tool arguments", () => {
    ...
    const descriptor = context.tools.find((tool) => tool.plugin_id === "com.atms.topic-outline")!;
    expect(() => executeAtmsPluginTool(descriptor, {
      id: "com.atms.topic-outline:topic-blank-title",
      title: "   ",
    })).toThrow(/input is invalid|fallback title did not resolve/i);
    expect(() => executeAtmsPluginTool(descriptor, {
      id: "com.atms.topic-outline:topic-source-without-url",
      title: "Source policy",
      sources: [{ title: "Missing URL" }],
    })).toThrow(/input is invalid/);
    const envelope = executeAtmsPluginTool(descriptor, {
      id: "com.atms.topic-outline:topic-tampered",
