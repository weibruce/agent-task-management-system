...
import { atmsPluginTurnContextDigestInput } from "atms-protocol";
...
const TOPIC_CAPABILITY = "com.atms.topic-outline:compose-outline";
...
const ROUTING_FIXTURES = {
  ...
  explicit: {
    ...
    explicit_plugin_id: "com.atms.topic-outline",
...
function topicPlugin(state: PluginRegistryState): PluginRegistryState["plugins"][number] {
  return state.plugins.find((plugin) => plugin.plugin_id === "com.atms.topic-outline")!;
}
...
describe("Plugin Capability Index and deterministic Top-K Router", () => {
  ...
  beforeEach(() => {
    ...
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-capability-router-"));
  ...
  it("compiles a compact, ordered, replayable index and excludes disabled or incompatible plugins", () => {
    ...
    expect(first.entries.map((entry) => entry.qualified_id)).toEqual([
      ...
      "com.atms.pr-closeout:summarize-pr-closeout",
      TOPIC_CAPABILITY,
    ]);
    expect(first.entries[2]).toMatchObject({
      ...
      skill: {
        qualified_id: "com.atms.topic-outline:topic-outline",
    ...
    topicPlugin(incompatible).descriptor.manifest.compatibility.atms.max_exclusive = "0.1.0";
  ...
  it("signals ambiguous side-effect conflicts until a capability is explicit", () => {
    ...
    expect(ambiguous.signals.ambiguity_capability_ids).toEqual([
      TOPIC_CAPABILITY,
      "com.atms.topic-outline:compose-outline-alternate",
    ]);
  ...
  it("enforces Top-K and prompt byte budgets without partial Skill/Tool exposure", () => {
    ...
    const core = coreState.plugins.find((plugin) => plugin.plugin_id === "com.atms.core")!;
  ...
  it("rejects a self-digested source context that is not owned by the live registry", () => {
    ...
    const tool = forged.tools.find((entry) => entry.plugin_id === "com.atms.topic-outline")!;
    tool.description = `${tool.description} forged prompt instructions`;
    forged.context_digest = pluginJsonDigest(atmsPluginTurnContextDigestInput(forged));
