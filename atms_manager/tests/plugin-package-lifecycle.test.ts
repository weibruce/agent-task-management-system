...
describe("external plugin package lifecycle", () => {
  ...
  beforeEach(() => {
    ...
    home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-plugin-lifecycle-home-"));
    sourceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "atms-plugin-lifecycle-source-"));
  ...
  it("requires trusted registry signatures and disables an active package after revocation", () => {
    ...
    expect(fs.existsSync(path.join(installed.installation.package_path, "atms.signature.json"))).toBe(true);
  ...
  it("installs and enables data-only custom Renderer packages through the M6 isolated tier", () => {
    const manifestFile = path.join(sourceRoot, "atms.plugin.json");
  ...
  function setVersion(version: string): void {
    const file = path.join(sourceRoot, "atms.plugin.json");
  ...
  it("treats an identical reinstall as a no-op and rejects a modified immutable package", () => {
    ...
    fs.appendFileSync(path.join(first.installation.package_path, "atms.plugin.json"), " \n");
  ...
  it("rejects invalid plugin ids before touching uninstall storage and quarantines bad journals", () => {
    expect(() => uninstallInstalledPlugin("..", "0".repeat(64))).toThrow(/Invalid Atms plugin id/);
    expect(fs.existsSync(path.join(home, "plugins", ".trash"))).toBe(false);
