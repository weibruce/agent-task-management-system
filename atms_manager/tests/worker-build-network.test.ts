...
describe("resolveWorkerBuildNetwork", () => {
  ...
  it("turns valid public source URLs into valued build arguments", () => {
    const config = resolveWorkerBuildNetwork({
      ...
      ATMS_WORKER_BUILD_APT_SECURITY_MIRROR: "https://mirrors.example.com/debian-security",
    ...
    expect(workerBuildNetworkDockerArgs(config)).toEqual([
      ...
      "--build-arg", "ATMS_WORKER_BUILD_APT_SECURITY_MIRROR=https://mirrors.example.com/debian-security",
  ...
  it("keeps main and security APT overrides independent", () => {
    const mainOnly = resolveWorkerBuildNetwork({
      ATMS_WORKER_BUILD_APT_MIRROR: "https://main.example.com/debian",
    });
    expect(workerBuildNetworkDockerArgs(mainOnly)).toEqual([
      "--build-arg", "ATMS_WORKER_BUILD_APT_MIRROR=https://main.example.com/debian",
    ]);
    ...
    const securityOnly = resolveWorkerBuildNetwork({
      ATMS_WORKER_BUILD_APT_SECURITY_MIRROR: "https://security.example.com/debian-security",
    });
    expect(workerBuildNetworkDockerArgs(securityOnly)).toEqual([
      "--build-arg", "ATMS_WORKER_BUILD_APT_SECURITY_MIRROR=https://security.example.com/debian-security",
    ]);
  ...
  it("normalizes default ports, scheme/host case, and bracketed IPv6 consistently", () => {
    const defaultPorts = resolveWorkerBuildNetwork({
      ATMS_WORKER_BUILD_APT_MIRROR: "http://mirrors.example.com:80/debian",
      ATMS_WORKER_BUILD_APT_SECURITY_MIRROR: "https://mirrors.example.com:443/debian-security",
      ATMS_WORKER_BUILD_NPM_REGISTRY: "HTTPS://REGISTRY.EXAMPLE.COM/",
    });
    expect(workerBuildNetworkDockerArgs(defaultPorts)).toEqual([
      "--build-arg", "ATMS_WORKER_BUILD_APT_MIRROR=http://mirrors.example.com/debian",
      "--build-arg", "ATMS_WORKER_BUILD_APT_SECURITY_MIRROR=https://mirrors.example.com/debian-security",
      "--build-arg", "NPM_CONFIG_REGISTRY=https://registry.example.com",
    ]);

    const ipv6 = resolveWorkerBuildNetwork({
      ATMS_WORKER_BUILD_APT_MIRROR: "http://[2001:DB8::1]:8080/debian",
    });
    expect(workerBuildNetworkDockerArgs(ipv6)).toEqual([
      "--build-arg", "ATMS_WORKER_BUILD_APT_MIRROR=http://[2001:db8::1]:8080/debian",
    ]);
  });
  ...
  it("allows an at-sign in the path when the authority has no userinfo", () => {
    const config = resolveWorkerBuildNetwork({
      ATMS_WORKER_BUILD_NPM_REGISTRY: "https://registry.example.com/scope/@package",
    });
