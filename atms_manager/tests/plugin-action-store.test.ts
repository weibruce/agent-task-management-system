...
describe("plugin Tool persistence", () => {
  ...
  beforeEach(() => {
    ...
    home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-action-home-"));
    source = fs.mkdtempSync(path.join(os.tmpdir(), "atms-action-source-"));
  ...
  function invocation(input: { requestId?: string; idempotencyKey?: string } = {}): AtmsPluginToolInvocationV1 {
  ...
  ): AtmsPluginToolConfirmationChallengeV1 {
    return {
  ...
  it("persists expiry and makes the waiting Action terminal", () => {
    ...
    const late: AtmsPluginToolConfirmationDecisionV1 = {
      confirmation_version: 1,
      challenge_id: challenge.challenge_id,
      request_id: request.request_id,
      request_digest: request.request_digest,
