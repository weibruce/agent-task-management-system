...
describe("Manager Git isolation", () => {
  ...
  beforeEach(() => {
    ...
    root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-manager-git-"));
    repository = path.join(root, "repo");
    fs.mkdirSync(repository);
    process.env.ATMS_HOME = path.join(root, "atms-home");
    ...
    expect(spawnSync("git", ["-C", repository, "config", "user.name", "Atms Test"], { encoding: "utf8" }).status).toBe(0);
    expect(spawnSync("git", ["-C", repository, "config", "user.email", "atms@example.invalid"], { encoding: "utf8" }).status).toBe(0);
  });
  ...
  it("overrides repository command hooks with Manager-owned safe settings", () => {
    ...
    expect(String(hooks.stdout).trim()).toBe(path.join(process.env.ATMS_HOME!, "runtime", "disabled-git-hooks"));
