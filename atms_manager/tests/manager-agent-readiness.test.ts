...
describe("/api/manager-agent/readiness", () => {
  ...
  beforeEach(() => {
    ...
    oldRepoRoot = process.env.ATMS_REPO_ROOT;
    oldAnthropicKey = process.env.ANTHROPIC_API_KEY;
    oldCodexHome = process.env.CODEX_HOME;
    oldCodexBin = process.env.ATMS_CODEX_BIN;
    oldOpenAiKey = process.env.OPENAI_API_KEY;
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-manager-agent-readiness-"));
    ...
    delete process.env.ATMS_CODEX_BIN;
  ...
  afterEach(async () => {
    ...
    if (oldRepoRoot === undefined) delete process.env.ATMS_REPO_ROOT;
    else process.env.ATMS_REPO_ROOT = oldRepoRoot;
    ...
    if (oldCodexBin === undefined) delete process.env.ATMS_CODEX_BIN;
    else process.env.ATMS_CODEX_BIN = oldCodexBin;
  ...
  it("reports missing Codex binary and authentication as independent blockers", () => {
    process.env.ATMS_CODEX_BIN = path.join(tmpHome, "missing-codex");
  ...
  it("reports missing host prerequisites without requiring a Docker node", async () => {
    process.env.ATMS_REPO_ROOT = tmpHome;
  ...
  it("reports a host-shell harness ready without a Docker node", async () => {
    ...
    process.env.ATMS_MANAGER_AGENT_HOST_ENTRY = workerEntry;
