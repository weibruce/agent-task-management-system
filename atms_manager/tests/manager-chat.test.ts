...
import { managerAgentRuntimePlacementForHarness } from "atms-protocol";
...
describe("/api/manager/chat", () => {
  ...
  beforeEach(async () => {
    ...
    oldDagMutationToken = process.env.ATMS_DAG_MUTATION_TOKEN;
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-manager-chat-"));
  ...
  afterEach(async () => {
    ...
    if (oldDagMutationToken === undefined) delete process.env.ATMS_DAG_MUTATION_TOKEN;
    else process.env.ATMS_DAG_MUTATION_TOKEN = oldDagMutationToken;
    closeDb();
  ...
  it("routes every non-Codex Manager Agent chat through a host process", async () => {
    ...
    fs.writeFileSync(hostEntry, `
      ...
      server.listen(port, '127.0.0.1');
    `, "utf-8");
    process.env.ATMS_MANAGER_AGENT_HOST_ENTRY = hostEntry;
  ...
  it("uses the Atms default workspace for host Codex when no project workspace is configured", () => {
  ...
  it("passes Responses-capable Atms settings into host Codex app-server", async () => {
    upsertProvider({
      id: "qwen36",
      name: "Qwen3.6 Local",
      default_model: "qwen3.6",
  ...
  it("routes codex_appserver Manager Agent chat through host Codex without a container", async () => {
    ...
    expect(seenPluginContext).toMatchObject({
      enabled_plugins: [
        { id: "com.atms.core" },
        { id: "com.atms.pr-closeout" },
        { id: "com.atms.topic-outline" },
      ],
      skills: [
        { qualified_id: "com.atms.core:voice-generative-ui" },
        { qualified_id: "com.atms.pr-closeout:pr-closeout" },
      ],
      tools: [],
      actions: [],
    });
  ...
  it("accepts successful MCP-prefixed read tools as required host Codex calls", async () => {
    _setHostCodexAgentEventRunnerForTest(async function* (_prompt, tools) {
      ...
      yield { type: "tool_use", id, name: "mcp__atms-tools__list_orchestrations", input: {} };
    ...
    expect(body.data.tool_calls).toContainEqual(expect.objectContaining({
      ...
      runtime_name: "mcp__atms-tools__list_orchestrations",
    }));
  });
  ...
  it("rejects failed MCP-prefixed host Codex calls required by the objective", async () => {
    _setHostCodexAgentEventRunnerForTest(async function* () {
      yield {
        ...
        name: "mcp__atms-tools__list_dag_patterns",
        input: {},
      };
      yield {
        type: "tool_result",
    ...
    expect(body.data).toMatchObject({
      ...
      observed_tool_calls: ["mcp__atms-tools__list_dag_patterns"],
    });
  });
  ...
  it("exposes skill and DAG pattern operations through host Codex tools", async () => {
    process.env.ATMS_DAG_MUTATION_TOKEN = "host-tool-mutation-secret";
    const port = await listen(server);
    ...
    expect(listedSkills.result.content[0].text).toContain("atms-dag-patterns");

    const skill = await _invokeHostCodexVoiceToolForTest(
      "read_skill",
      { skill_id: "atms-dag-patterns" },
      { managerRestUrl },
    );
    ...
    expect(schema.result.content[0].text).toContain("atms.ai/v1");

    const validation = await _invokeHostCodexVoiceToolForTest(
      "validate_dag_workflow",
      { source: "api_version: atms.ai/v1\nkind: Workflow\n" },
      { managerRestUrl },
    );
