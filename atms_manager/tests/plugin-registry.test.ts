...
function writeAccidentalA2uiV1Core(root: string): string {
  ...
  const manifest = JSON.parse(fs.readFileSync(manifestFile, "utf8")) as AtmsPluginManifestV1;
  manifest.version = "0.1.7";

  const generatedView = manifest.kinds.find((kind) => kind.kind === "com.atms.core/generated_view");
...
describe("Atms plugin archive and activation registry", () => {
  ...
  beforeEach(() => {
    ...
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-plugin-registry-"));
  ...
  it("loads the bundled Core package into a deterministic immutable descriptor", () => {
    ...
    expect(first.manifest.skills[0]).toMatchObject({
      id: "voice-generative-ui",
      visual_profile: "skills/voice-generative-ui/assets/atms/worker-visual-profile.json",
    });
    ...
    expect(first.referenced_files.map((entry) => entry.path)).toContain(
      "skills/voice-generative-ui/assets/atms/worker-visual-profile.json",
    );
  ...
  it("keeps the accidental builtin Core 0.1.7 generated_view v1 package readable", () => {
    ...
    expect(registry.compositionMetadata().filter((kind) => kind.kind === "com.atms.core/generated_view"))
      .toEqual([expect.objectContaining({ kind_version: 2 })]);
    expect(registry.uiProjection().kinds
      .filter((kind) => kind.kind === "com.atms.core/generated_view")
  ...
  it("allows legacy UI bridges only for explicitly approved builtin migrations", () => {
    ...
    }).manifest.id).toBe("com.atms.topic-outline");
  });
  ...
  it("keeps Core locked and preserves an optional plugin disable across resync", () => {
    ...
    expect(first.plugins.find((plugin) => plugin.plugin_id === "com.atms.topic-outline")?.activation)
    ...
    expect(setPluginEnabled("com.atms.topic-outline", false, { timestamp: "2026-07-11T12:01:00.000Z" }))
      .toMatchObject({ enabled: false, revision: 2 });
    registry.syncBuiltins();
    expect(getPluginRegistryState().plugins.find((plugin) => plugin.plugin_id === "com.atms.topic-outline")?.activation)
      .toMatchObject({ enabled: false, revision: 2 });
  });
