...
const CAPABILITY_ID = "com.atms.topic-outline:compose-outline";
...
describe("Manager-issued Plugin Tool turn tokens", () => {
  ...
  beforeEach(() => {
    ...
    closeDb();
    home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-tool-turn-home-"));
    permissionSource = fs.mkdtempSync(path.join(os.tmpdir(), "atms-tool-turn-plugin-"));
    process.env.ATMS_HOME = home;
    syncBuiltinPlugins();

    scaffoldPluginProject(permissionSource, "com.example.turn-permission");
    const manifestFile = path.join(permissionSource, "atms.plugin.json");
  ...
  function selectedContext(): AtmsPluginTurnContextV1 {
  ...
  it("invalidates tokens after registry or permission revision changes", () => {
    ...
    setPluginEnabled("com.atms.topic-outline", true);
  ...
  it("refuses a caller-shaped context that omits an asset from the selected capability", () => {
    ...
    incomplete.tools = [];
    incomplete.context_digest = pluginJsonDigest(atmsPluginTurnContextDigestInput(incomplete));
    expect(() => authority.issue({
      context: incomplete,
      modality: "voice",
      scope: { type: "voice_session", id: "voice-turn-session" },
