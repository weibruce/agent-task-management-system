...
`);
    ...
    expect(parsed.graph.nodes[0]?.image).toBe("atms-worker:custom");
  });
...
`);
    ...
    expect(nodes.actor).toMatchObject({ node_type: "agent", agent: "worker", image: "atms-worker:custom" });
