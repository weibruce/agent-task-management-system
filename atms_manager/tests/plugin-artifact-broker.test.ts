...
function temporaryRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "atms-artifact-broker-"));
  roots.push(root);
  return root;
}
...
export function artifactAuthorization(requestId = "artifact-request-0001"): AtmsPluginAuthorizedToolInvocationV1 {
...
describe("Plugin Artifact Broker", () => {
  it("atomically publishes and verifies a content-addressed, request-scoped artifact", () => {
    ...
    expect(issued.claims).toMatchObject({
      binding: { plugin_id: "com.atms.video-cover" },
    ...
    const metadataFile = path.join(
      root,
      "metadata",
      createHash("sha256").update("com.atms.video-cover").digest("hex"),
