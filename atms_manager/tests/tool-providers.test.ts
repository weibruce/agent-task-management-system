...
describe("read-only tool provider catalog", () => {
  it("derives built-in declarations and browser runtime presence independently", () => {
    ...
    const builtIn = disconnected.providers.find((provider) => provider.id === "atms.dag-control")!;
    const browserOff = disconnected.providers.find((provider) => provider.id === "atms.browser-ui")!;
    const browserOn = connected.providers.find((provider) => provider.id === "atms.browser-ui")!;
