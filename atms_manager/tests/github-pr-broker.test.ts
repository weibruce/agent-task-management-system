...
describe("bounded GitHub Draft PR credential broker", () => {
  ...
  beforeEach(() => {
    ...
    home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-github-broker-"));
  ...
  function initializeRepository(runId: string): {
    ...
    git(["config", "user.name", "Atms Test"]);
    git(["config", "user.email", "atms@example.invalid"]);
...
`);
  ...
  it("fails closed when the Draft PR head changes outside the broker", async () => {
    ...
    expect(drifted).toMatchObject({ ok: false, error: expect.stringContaining("outside the Atms broker") });
  });
