...
describe("voice artifact publishing", () => {
  ...
  beforeEach(() => {
    ...
    home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-artifact-"));
  ...
  it("injects one appearance bridge into the served HTML copy", () => {
    ...
    expect(injected).toContain("data-atms-artifact-appearance-bridge");
    expect(injected).toContain("atms:artifact-appearance");
    expect(injected).toContain("visibleCanvasColor() || colors[0]");
    expect(injected).toContain("background-color: var(--atms-artifact-scrollbar-track) !important");
    expect(injected).toContain("<h1>Story</h1>");
    expect(injected.indexOf("data-atms-artifact-appearance-bridge"))
