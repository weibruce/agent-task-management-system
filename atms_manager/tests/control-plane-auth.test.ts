...
import { WsClient } from "../../atms_worker/src/ws-client.js";
...
describe("control-plane websocket authentication", () => {
  ...
  beforeEach(() => {
    ...
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "atms-control-plane-auth-"));
  ...
  it("uses configured tokens or generates a provision token", () => {
    expect(resolveWorkerControlPlaneAuth({
      ATMS_WORKER_TOKEN: " configured-token ",
    }, () => "unused")).toEqual({
  ...
  it("preserves control-plane defaults when provisioner options are customized", () => {
    ...
    const merged = mergeProvisionerOptions({
      provisioner: {
        ...
        env: { CLAUDE_MAX_TURNS: "8", ATMS_WORKER_TOKEN: "default-token" },
      },
      ...
      provisioner: {
        image: "custom-worker",
        env: { CUSTOM_RUNTIME_FLAG: "1", ATMS_WORKER_TOKEN: "stale-token" },
      },
    }, "effective-token");
    ...
    expect(merged.provisioner).toMatchObject({
      ...
      env: {
        ...
        ATMS_WORKER_TOKEN: "effective-token",
      },
    });
  });
