...
describe("plugin Tool capability tokens", () => {
  ...
  beforeEach(() => {
    ...
    closeDb();
    home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-capability-home-"));
    process.env.ATMS_HOME = home;
    delete process.env.ATMS_PLUGIN_CAPABILITY_SECRET;
  ...
  function project(id: string): string {
    const source = fs.mkdtempSync(path.join(os.tmpdir(), "atms-capability-source-"));
    sources.push(source);
  ...
  function invocation(suffix: string, permissionRevision = 0): AtmsPluginToolInvocationV1 {
