...
describe("Host Codex app-server protocol params", () => {
  it("passes Atms reasoning effort through thread config instead of the removed legacy field", () => {
    const params = _buildCodexThreadStartParamsForTest({
      systemPrompt: "You are Atms.",
      cwd: "/workspace",
      model: "gpt-5.5",
    ...
    expect(params).toMatchObject({
      ...
      developerInstructions: "You are Atms.",
      model: "gpt-5.5",
  ...
  it("uses a persisted Codex thread for a stable Manager Agent session", () => {
    ...
    const resumed = _buildCodexThreadResumeParamsForTest({
      ...
      systemPrompt: "Updated Atms contract",
      cwd: "/workspace",
      model: "gpt-5.6",
      sandbox: "workspace-write",
    ...
    expect(resumed).toMatchObject({
      threadId: "019f0000-0000-7000-8000-000000000000",
      baseInstructions: null,
      developerInstructions: "Updated Atms contract",
      cwd: "/workspace",
      model: "gpt-5.6",
  ...
  it("materializes immutable Plugin Skills for Codex native discovery", () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "atms-codex-plugin-skills-"));
    ...
    try {
      const skills = [{
        ...
        projection_content: [
          ...
          "---",
          "",
          "Follow the Atms visual contract.",
        ].join("\n"),
      }, {
        id: "home-only",
        source: "home",
      ...
      expect(body).toContain("Atms Skill id: com.atms.core:voice-generative-ui");
      expect(body).toContain("Follow the Atms visual contract.");
  ...
  it("passes Atms reasoning effort through turn/start effort", () => {
  ...
  it("injects a process-local Responses provider for provider-backed Codex", () => {
    ...
    expect(args.join(" ")).toContain("model_providers.atms_responses.wire_api=\"responses\"");
