...
import { invokeAtmsBrowserUiTool } from "../src/server/browser-ui-tools.js";
...
describe("Browser Tools WebSocket", () => {
  ...
  it("publishes the Desktop catalog atomically and fails closed after it degrades", async () => {
    ...
    client.send(JSON.stringify({
      ...
      tools: ATMS_UI_TOOL_NAMES.map((name) => catalogEntryFor(name)),
    }));
    await waitFor(() => broker.ready());
    expect(broker.status().tools).toEqual([...ATMS_UI_TOOL_NAMES].sort());
    ...
    await expect(invokeAtmsBrowserUiTool("ui_get_state", {}, {
