...
function decodeRequest(row: PluginToolRequestRow): PluginToolRequestRecord {
  ...
  const validation = validateAtmsPluginToolInvocation(invocation);
...
export function createPluginToolRequest(input: {
  ...
  confirmation_challenge?: AtmsPluginToolConfirmationChallengeV1;
}): { record: PluginToolRequestRecord; idempotent: boolean } {
  const validation = validateAtmsPluginToolInvocation(input.invocation);
  ...
  let confirmationChallenge: AtmsPluginToolConfirmationChallengeV1 | undefined;
  if (input.confirmation_challenge) {
    const confirmationValidation = validateAtmsPluginToolConfirmationChallenge(
...
function decodeConfirmation(row: ConfirmationRow): PluginConfirmationRecord {
  const challengeValidation = validateAtmsPluginToolConfirmationChallenge(
  ...
  let decision: AtmsPluginToolConfirmationDecisionV1 | undefined;
  if (row.decision_json !== null) {
    const decisionValidation = validateAtmsPluginToolConfirmationDecision(
...
export function createPluginToolConfirmation(input: {
  ...
  const validation = validateAtmsPluginToolConfirmationChallenge(
    input.challenge,
    request.invocation,
...
export function decidePluginToolConfirmation(input: {
  ...
  return getDb().transaction(() => {
    ...
    const validation = validateAtmsPluginToolConfirmationDecision(
      input.decision,
      request.invocation,
      challenge,
    );
