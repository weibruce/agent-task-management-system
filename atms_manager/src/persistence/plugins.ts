...
function syncDeclaredPermissionGrants(
  manifest: AtmsResolvedPluginDescriptorV1["manifest"],
  timestamp: string,
): void {
  const grants = [
...
function decodePackage(row: PluginPackageRow): PluginPackageRecord {
  ...
  const descriptor = parseJsonRow<AtmsResolvedPluginDescriptorV1>(row.resolved_descriptor_json);
  const manifestValidation = validateAtmsPluginManifest(manifest);
...
export function promoteAttestedPluginRuntimeInstallation(input: {
  ...
  return getDb().transaction(() => {
    ...
    const manifestValidation = validateAtmsPluginManifest(parseJsonRow(row.manifest_json));
