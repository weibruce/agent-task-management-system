...
const CORE_PLUGIN_ID = "com.atms.core";
const GENERATED_VIEW_KIND = "com.atms.core/generated_view";
...
const LIVE_SURFACE_A2UI: AtmsA2uiSurfaceV1 = {
...
function composedLiveSurfaceA2ui(body?: DagActorSurfaceBodyV1): AtmsA2uiSurfaceV1 {
  if (!body) return structuredClone(LIVE_SURFACE_A2UI);
  const composed: AtmsA2uiSurfaceV1 = {
    version: ATMS_A2UI_VERSION,
    catalogId: ATMS_A2UI_CATALOG_ID,
    components: [
  ...
  if (composed.components.length > ATMS_A2UI_MAX_COMPONENTS) {
...
function visibilityChanges(
  ...
  nextRevision: number,
  focusedUntil?: number,
): { content: Record<string, unknown>; a2ui: AtmsA2uiSurfaceV1; fallback: ProjectedFallback } {
  assertOwnedNode(node, projection);
  const data = projectedDataFromNode(node)!;
