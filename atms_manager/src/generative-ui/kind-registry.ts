...
import {
  ...
  type AtmsPluginActionDescriptorV1,
  type AtmsDeclarativeRendererV1,
  type AtmsPluginKindRegistrationV1,
  type AtmsPluginRendererRegistrationV1,
  type AtmsPluginResolvedRendererSourceV1,
  type AtmsPluginRendererV1,
...
interface KindDefinition {
  ...
  allowed_surfaces: AtmsPluginKindRegistrationV1["allowed_surfaces"];
  default_surface: AtmsPluginKindRegistrationV1["allowed_surfaces"][number];
...
function isKnownLegacyGeneratedViewVersionCollision(input: {
  ...
  return input.plugin_id === "com.atms.core"
    && input.kind === "com.atms.core/generated_view"
...
function resolvedRendererSource(
  ...
  source: AtmsPluginRendererV1["source"],
): AtmsPluginResolvedRendererSourceV1 {
  ...
  return {
    ...
    file: source.file,
    digest: archived.digest,
    document: document as AtmsDeclarativeRendererV1,
  };
}
...
export class GenerativeUiKindRegistry {
  ...
  readonly #renderers: AtmsPluginRendererRegistrationV1[] = [];
  readonly #actions: AtmsPluginActionDescriptorV1[] = [];
  ...
  authorizeNewTransaction(
    ...
    transaction.operations.forEach((operation, index) => {
      ...
      } else if (node.owner.id !== "com.atms.core") {
        errors.push({
          path: `/operations/${index}`,
  ...
  compositionMetadata(): GenerativeUiKindCompositionMetadataV1[] {
    return this.#activeDefinitions
      ...
      .map((definition) => ({
        ...
        ...(definition.plugin_id === "com.atms.core" ? { allow_critical: true } : {}),
      }));
  }

  uiProjection(): AtmsPluginUiProjectionV1 {
    const kinds: AtmsPluginKindRegistrationV1[] = this.#activeDefinitions.map((definition) => ({
    ...
    const projection: AtmsPluginUiProjectionV1 = {
    ...
    const validation = validateAtmsPluginUiProjection(projection);
...
export function getGenerativeUiKindRegistry(): GenerativeUiKindRegistry {
  ...
  if (!migratedRegistryStates.has(migrationKey)) {
    const core = state.plugins.find((plugin) => plugin.plugin_id === "com.atms.core");
    if (core) {
