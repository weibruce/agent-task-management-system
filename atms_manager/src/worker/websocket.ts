...
import { dagCredentialBrokerCallIdentity } from "atms-protocol";
...
export function setupWorkerWebSocket(
  ...
  const authToken = options.authToken
    ?? process.env.ATMS_WORKER_TOKEN
    ?? process.env.ATMS_CONTROL_PLANE_TOKEN;
      ...
      ws.on("message", async (raw: Buffer) => {
          ...
          if (!registered) {
            ...
            if (!firstWorkerFired) {
              ...
              try {
                ...
                console.error(
                  `[atms_manager] first worker recovery hook failed for worker ${worker_id}: ${err instanceof Error ? err.message : String(err)}`,
                );
              }
            }
        ...
        if (msg.type === "dag_actor_command_status") {
          ...
          if (result.status === "malformed_payload") {
            console.warn(
              `[atms_manager] rejected live command status from worker ${worker_id}: ${result.reason}`,
            );
          }
          return;
        }
        ...
        if (msg.type === "stream") {
          ...
          if (msg.data.event === "dag_activity") {
            try {
              ...
              console.warn(
                `[atms_manager] rejected DAG activity from worker ${worker_id}: ${error instanceof Error ? error.message : String(error)}`,
              );
            }
            return;
          }
          if (msg.data.event === "dag_actor_surface_media") {
            try {
              ...
              console.warn(
                `[atms_manager] rejected Actor surface media from worker ${worker_id}: ${error instanceof Error ? error.message : String(error)}`,
              );
            }
            return;
          }
          if (msg.data.event === "dag_actor_surface_patch") {
            try {
              ...
              console.warn(
                `[atms_manager] rejected Actor surface patch from worker ${worker_id}: ${error instanceof Error ? error.message : String(error)}`,
              );
            }
            return;
          }
