      ...
      ws.on("message", (raw: Buffer) => {
        ...
        if (msg.type === "dag_actor_command_status") {
          ...
          if (result.status === "malformed_payload") {
            console.warn(
              `[atms_manager] rejected live command status from node ${node_id}: ${result.reason}`,
            );
          }
          return;
        }
        ...
        if (msg.type === "stream") {
          ...
          if (msg.data.event === "dag_activity") {
            try {
              ...
              console.warn(
                `[atms_manager] rejected DAG activity from node ${node_id}: ${error instanceof Error ? error.message : String(error)}`,
              );
            }
            return;
          }
          if (msg.data.event === "dag_actor_surface_media") {
            try {
              ...
              console.warn(
                `[atms_manager] rejected Actor surface media from node ${node_id}: ${error instanceof Error ? error.message : String(error)}`,
              );
            }
            return;
          }
          if (msg.data.event === "dag_actor_surface_patch") {
            try {
              ...
              console.warn(
                `[atms_manager] rejected Actor surface patch from node ${node_id}: ${error instanceof Error ? error.message : String(error)}`,
              );
            }
            return;
          }
