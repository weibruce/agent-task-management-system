      ...
      if (
        ...
        if (activeNodeDisconnected) {
          try {
            ...
            console.error(
              `[atms_manager] active DAG Worker disconnect handler failed for ${worker.run_id}/${worker.node_id}: ${error instanceof Error ? error.message : String(error)}`,
            );
          }
        }
      }
...
function resolveIntervalMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ATMS_DAG_ACTOR_LEASE_REAPER_INTERVAL_MS;
...
export function startDagActorLeaseReaper(
  ...
  const tick = () => {
    ...
    void reapDagActorLeases(options)
      .catch((error) => {
        console.error(
          `[atms_manager] DAG actor lease reaper failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      })
