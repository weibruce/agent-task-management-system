...
function resolveLocalSkill(
  ...
  const homeSkillsRoot = normalizeRoot(path.join(homeRoot, "skills"), "ATMS_HOME Skill root");
...
function resolvePluginSkill(
  ...
  if (
    pluginId.length > 160
    || !ATMS_PLUGIN_ID_PATTERN.test(pluginId)
...
function resolveSkill(
  ...
  const homeRoot = path.resolve(options.atms_home ?? getAtmsHome());
