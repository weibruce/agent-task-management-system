...
export function recoverAllActiveRuns(): ColdRecoverySummary {
  ...
  for (const runId of listPersistedRunIdsByStatus(["active", "waiting"])) {
    try {
      ...
      store.delete(runId);
      console.error(
        `[atms_manager] cold recovery skipped run ${runId}: ${err instanceof Error ? err.message : String(err)}`,
      );
      summary.skipped.push(runId);
    }
  }
...
function _correctionPrompt(
  ...
  const workspaceEvidenceGuidance = Object.keys(workspaceFileContracts).length > 0
    ? [
        ...
        "If the previous error is a DAG_HANDOFF_WORKSPACE_FILE_REQUIREMENT, inspect and repair only the declared evidence JSON file under .atms; do not edit source files, rerun completed tests, or repeat external side effects. Recompute the file SHA-256 after the repair.",
      ]
    : [];
...
function _singleWritableWorkspace(run: ActiveRun, nodeId: string): { relative: string; absolute: string } {
  ...
  const runRoot = realpathSync(path.resolve(getAtmsHome(), "workspace", ...run.runId.split("/")));
...
export function expireWaitingActiveRuns(now = Date.now()): string[] {
  ...
  for (const run of store.values()) {
    ...
    try {
      ...
      _restoreMutableRun(run, mutableBefore);
      console.error(
        `[atms_manager] failed to expire waiting run ${run.runId}: ${error instanceof Error ? error.message : String(error)}`,
      );
      continue;
    }
    _emitNodeStateChanges(run, before);
...
function _sendInterventionInterrupt(
  ...
  try {
    ...
    console.warn(
      `[atms_manager] failed to interrupt previous DAG actor attempt: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
function _commandAllowlist(): Set<string> {
  const configured = process.env.ATMS_DAG_COMMAND_ALLOWLIST ?? "";
...
function _commandGatewayCwd(
  ...
  const workspaceRoot = path.resolve(getAtmsHome(), "workspace");
...
function _commandGatewayResult(run: ActiveRun, node: DAGGraphNode): { port: string; payload: unknown } {
  ...
  if (config?.command_field && process.env.ATMS_DAG_ALLOW_DYNAMIC_COMMANDS !== "true") {
    return {
      port: config?.failure_port || "failed",
      payload: { ok: false, error: "dynamic command_field requires ATMS_DAG_ALLOW_DYNAMIC_COMMANDS=true" },
    };
  }
  ...
  if (!allowed) {
    return {
      port: config?.failure_port || "failed",
      payload: { ok: false, error: `executable '${command[0]}' is not in ATMS_DAG_COMMAND_ALLOWLIST` },
    };
  }
...
function _makeFanoutGitWorktreeRelocatable(repository: string, target: string): void {
  ...
  // under Atms's run-workspace spelling. Finding the same filesystem object
...
function _materializeManagerOwnedFanoutCommit(
  ...
  const message = `atms fanout ${parent.node_id}/${child.node_id}`;
  const status = git(["status", "--porcelain"]);
  ...
  if (String(status.stdout).trim()) {
    ...
    const committed = git([
      "-c", "user.name=Atms Manager",
      "-c", "user.email=atms-manager@localhost",
...
function _executeGatewayNode(
  ...
  if (node.node_type === "await_command_gateway") {
    ...
    try {
      ...
      console.warn(
        `[atms_manager] live command fallback remained queued for ${runId}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    return true;
  }
...
function _credentialProjections(run: ActiveRun, node: DAGGraphNode): DagCredentialProjection[] {
  ...
  const forbiddenEnv = /^(?:ATMS_|MANAGER_|AGENT_BACKEND$|PATH$|HOME$|NODE_OPTIONS$|LD_|DYLD_|ANTHROPIC_|OPENAI_|KIMI_|LLM_)/;
