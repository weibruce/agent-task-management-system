...
import { getDefaultWorkspacePath, getAtmsHome } from "../config/env.js";
...
export function resolveWorkspaceRetentionPolicy(
  ...
  return {
    ...
    intervalMs: Math.max(
      60_000,
      nonNegativeNumber(env.ATMS_WORKSPACE_CLEANUP_INTERVAL_MS, 6 * 60 * 60 * 1_000),
    ),
  };
}

export function runWorkspacePath(runId: string): string {
  const root = path.resolve(getAtmsHome(), "workspace");
...
async function inspectWorkspace(target: string): Promise<WorkspaceEntry | undefined> {
  ...
  const root = await fs.realpath(path.resolve(getAtmsHome(), "workspace"));
