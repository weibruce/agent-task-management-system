...
import { isCanonicalAtmsPluginSemver, isAtmsPluginId } from "atms-protocol";
...
import { AtmsPluginRegistry } from "../plugins/registry.js";
...
function decodePluginId(raw: string, res: http.ServerResponse): string | undefined {
  try {
    ...
    if (!isAtmsPluginId(pluginId)) throw new Error("invalid plugin id");
    return pluginId;
  } catch {
...
function emitRegistryChange(
  pluginId: string,
  before: ReturnType<AtmsPluginRegistry["snapshot"]>,
  after: ReturnType<AtmsPluginRegistry["snapshot"]>,
): void {
...
function listResponse(registry: AtmsPluginRegistry): Record<string, unknown> {
...
function pluginRoutesHandlerUnsafe(req: http.IncomingMessage, res: http.ServerResponse): boolean {
  ...
  if (registryReleaseMatch) {
    ...
    if (!isCanonicalAtmsPluginSemver(pluginVersion)) {
      json(res, 400, { success: false, error: "Invalid plugin version" });
      return true;
    }
    ...
    if (contentType !== "application/vnd.atms.plugin+zip" && contentType !== "application/zip") {
      json(res, 415, { success: false, error: "Plugin registry install requires application/vnd.atms.plugin+zip" });
      return true;
    }
    const before = registry.snapshot();
  ...
  if (registryRollbackMatch && req.method === "POST") {
    ...
    readBody(req).then((body) => {
      if (
        body.expected_revision === undefined
        || (body.version !== undefined && !isCanonicalAtmsPluginSemver(body.version))
  ...
  if (registryEnableMatch && req.method === "PUT") {
    ...
    readBody(req).then((body) => {
      if (
        body.enabled !== true
        || body.expected_revision === undefined
        || !isCanonicalAtmsPluginSemver(body.expected_active_version)
        || Object.keys(body).some((key) => ![
  ...
  if (rendererSourceMatch && req.method === "GET") {
    ...
    if (
      ...
      || !isCanonicalAtmsPluginSemver(pluginVersion)
  ...
  if (url.pathname === "/api/plugins/install" && req.method === "POST") {
    const contentType = String(req.headers["content-type"] ?? "").split(";", 1)[0].trim().toLowerCase();
    if (contentType !== "application/vnd.atms.plugin+zip" && contentType !== "application/zip") {
      json(res, 415, { success: false, error: "Plugin install requires application/vnd.atms.plugin+zip" });
      return true;
    }
