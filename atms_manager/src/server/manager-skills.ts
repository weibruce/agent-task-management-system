...
const SKILL_VIEW_MANIFEST = path.join("assets", "atms", "view-templates.json");
...
function validTemplateDataSchema(value: unknown): value is Record<string, unknown> {
  ...
  return analyzeAtmsPluginSchemaPolicy(schema).length === 0;
}
...
function parseSkillViewTemplate(value: unknown): ManagerAgentSkillViewTemplateV1 | undefined {
  ...
  const a2ui = validateAtmsA2uiSurface(template.a2ui);
...
function skillViewPresenterEnv(): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ATMS_HOME: getAtmsHome() };
  for (const key of [
    "PATH",
    "HOME",
    "USERPROFILE",
  ...
  return entries
    ...
    .flatMap((entry) => {
      ...
      return [{
        ...
        description: metadata.description || "Atms Manager Agent skill",
...
export function getManagerSkillsRoot(): string {
  return path.join(getAtmsHome(), "skills");
}
...
export function ensureManagerSkillsInstalled(): ManagerSkillInstallResult {
  ...
  for (const entry of fs.readdirSync(sourceRoot, { withFileTypes: true })) {
    if (!entry.isDirectory() || !entry.name.startsWith("atms-")) continue;
...
export function listManagerSkills(
  pluginContext: AtmsPluginTurnContextV1 | null = assemblePluginTurnContext(),
): ManagerSkillSummary[] {
  ensureManagerSkillsInstalled();
