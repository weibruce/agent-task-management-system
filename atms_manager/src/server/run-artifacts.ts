...
async function receiveArtifactUpload(
  ...
  try {
    ...
    const expectedSha = Array.isArray(req.headers["x-atms-artifact-sha256"])
      ? req.headers["x-atms-artifact-sha256"][0]
      : req.headers["x-atms-artifact-sha256"];
    ...
    const record = completeRunArtifactUpload(runId, name, {
      sizeBytes,
      sha256,
      uncompressedBytes: numericHeader(req, "x-atms-artifact-uncompressed-bytes"),
      fileCount: numericHeader(req, "x-atms-artifact-file-count"),
    });
