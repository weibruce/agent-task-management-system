...
export function mutationRoutesHandler(
  ...
  if (approvalMatch && req.method === "POST") {
    ...
    void _readJsonBody(req).then((raw) => {
      const body = raw as Record<string, unknown>;
      const headerToken = Array.isArray(req.headers["x-atms-approval-token"])
        ? req.headers["x-atms-approval-token"]?.[0]
        : req.headers["x-atms-approval-token"];
      if (!isDagApprovalRequestAuthorized({
        remoteAddress: req.socket.remoteAddress,
        headerToken,
        bodyToken: body.authorization_token,
        configuredToken: process.env.ATMS_DAG_APPROVAL_TOKEN,
      })) {
        _forbidden(res, "approval decisions require a local request or valid ATMS_DAG_APPROVAL_TOKEN");
        return;
      }
  ...
  if (triggerEventMatch && req.method === "POST") {
    void _readJsonBody(req).then((raw) => {
      const body = raw as Record<string, unknown>;
      const headerToken = Array.isArray(req.headers["x-atms-dag-token"])
        ? req.headers["x-atms-dag-token"]?.[0]
        : req.headers["x-atms-dag-token"];
      if (!isDagMutationRequestAuthorized({
        remoteAddress: req.socket.remoteAddress,
        headerToken,
        bodyToken: body.authorization_token,
        configuredToken: process.env.ATMS_DAG_MUTATION_TOKEN ?? process.env.ATMS_DAG_APPROVAL_TOKEN,
      })) {
        _forbidden(res, "DAG event delivery requires a local request or valid ATMS_DAG_MUTATION_TOKEN");
        return;
      }
      const idempotencyKey = typeof body.idempotency_key === "string" ? body.idempotency_key.trim() : "";
      if (!idempotencyKey) throw new Error("idempotency_key is required");
  ...
  if (stateMatch && req.method === "POST") {
    ...
    void _readJsonBody(req).then((raw) => {
      ...
      if (!isDagMutationRequestAuthorized({
        ...
        _forbidden(res, "DAG state mutation requires a local request or valid ATMS_DAG_MUTATION_TOKEN");
        return;
      }
