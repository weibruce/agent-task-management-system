...
function requestedCodexBinary(options: Required<CodexBinaryResolveOptions>, requested?: string): string {
  const values = [requested, options.env.ATMS_CODEX_BIN, options.env.CODEX_BIN_PATH];
...
function hasExplicitCodexBinaryRequest(
  ...
  return [env.ATMS_CODEX_BIN, env.CODEX_BIN_PATH]
...
export function codexBinaryNotFoundMessage(
  ...
  if (!windowsShellCommandIsSafe(trimmed, options.platform)) {
    return "Codex Windows shim path contains unsupported shell metacharacters. Choose a different install path or set ATMS_CODEX_BIN.";
  }
  if (isPathLike(trimmed, options.platform)) {
    return `Codex binary not found at: ${codexBinaryDisplayPath(trimmed, options)}. Install codex or set ATMS_CODEX_BIN.`;
  }
  return "Codex binary not found. Install codex or set ATMS_CODEX_BIN.";
}
