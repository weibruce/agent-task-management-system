...
export function pinAtmsBrowserUiTurnBinding(
  ...
  if (binding.browser_tools_transport === "renderer") {
    ...
    if (!broker) throw new Error("Atms browser renderer is unavailable");
    return {
  ...
  if (binding.browser_tools_transport === "desktop" && !getBrowserToolsBroker()?.ready()) {
    throw new Error("Atms Desktop Browser Tools is unavailable");
  }
  return binding;
}

export function atmsBrowserUiToolsAvailable(
...
function inputRecord(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("Atms UI tool input must be an object");
  }
...
function normalizeWebMcpOutput(value: unknown): unknown {
  ...
  if (Buffer.byteLength(value, "utf8") > 64 * 1024) {
    throw new Error("Atms UI tool returned oversized text");
  }
  try {
...
export async function invokeAtmsBrowserUiTool(
  name: AtmsUiToolName,
  rawInput: unknown,
  options: {
  ...
  if (binding.browser_tools_transport === "none") {
    throw new Error("Atms Browser Tools is unavailable for this turn");
  }
  ...
  if (desktopBroker && !desktopBroker.ready()) {
    throw new Error("Atms Desktop Browser Tools catalog is unavailable");
  }
  if (!rendererBroker && !desktopBroker) throw new Error("Atms Browser Tools is unavailable");
  const invoke = (toolName: AtmsUiToolName, toolInput: unknown): Promise<unknown> => (
  ...
  if (name === "ui_open_surface") {
    ...
    if (!surface || !ATMS_UI_SURFACES.includes(surface as "dag_status")) {
...
function defaultTurnTarget(req: http.IncomingMessage): {
  ...
  const scope = getManagerAgentTurnEnvelopeAuthority().authorizeApiRequestScope({
    credential: singleHeader(req.headers[ATMS_MANAGER_TURN_HEADER]),
    method: req.method ?? "",
...
export function browserUiToolRoutesHandler(
  ...
  void readBrowserUiRequest(req)
    .then(async (body) => {
      ...
      if (!ATMS_UI_TOOL_NAMES.includes(name as AtmsUiToolName)) {
        throw new Error(`Unsupported Atms UI tool: ${name || "(missing)"}`);
      }
      const input = validateAtmsUiToolInput(name as AtmsUiToolName, body.input);
      ...
      let result: unknown;
      try {
        result = await (options.invoke ?? invokeAtmsBrowserUiTool)(
          name as AtmsUiToolName,
          input,
          {
