...
import {
  ATMS_WORKER_CREATED_LABEL,
  ATMS_WORKER_IMAGE,
  ATMS_WORKER_PROTOCOL_LABEL,
  ATMS_WORKER_REVISION_LABEL,
...
export function resolveDagEnvironmentRepoRoot(env: NodeJS.ProcessEnv = process.env): string {
  const configured = env.ATMS_REPO_ROOT?.trim();
...
function fingerprintContent(relativePath: string, content: Buffer): Buffer | string {
  ...
  try {
    ...
    if (isPackageLock && typeof parsed.packages === "object" && parsed.packages !== null) {
      ...
      for (const workspacePath of ["", "../atms_protocol", "../atms_plugin_sdk"]) {
...
function readPackageVersion(repoRoot: string): string | undefined {
  try {
    const raw = JSON.parse(fs.readFileSync(path.join(repoRoot, "atms_worker", "package.json"), "utf8")) as {
      version?: unknown;
    };
...
function dockerReason(error: unknown, platform: NodeJS.Platform): {
  ...
  if (
    ...
    return { code: "docker_permission_denied", message: "Atms does not have permission to access the Docker engine." };
  }
  if (
...
function defaultStatus(
  ...
  return {
    ...
    source: {
      ...
      image_revision: nonEmpty(env.ATMS_BUILD_REVISION) ?? nonEmpty(env.GITHUB_SHA),
    },
    worker_image: {
      status: "unknown",
      image: workerImage,
...
export class DagEnvironmentController {
  ...
  constructor(options: DagEnvironmentControllerOptions = {}) {
    ...
    this.workerImage = nonEmpty(options.workerImage)
      ?? nonEmpty(this.env.ATMS_WORKER_IMAGE)
      ?? ATMS_WORKER_IMAGE;
  ...
  private async runCheck(epoch: number): Promise<DagEnvironmentStatus> {
    ...
    if (osType === "windows") {
      const message = "Atms Worker images require Docker's Linux container engine.";
      this.status.docker = {
        status: "error",
    ...
    try {
      ...
      if (!this.hasActiveBuild()) {
        this.status.worker_image = {
          ...
          reason: reason.code,
          reason_code: reason.code,
          message: "Atms could not inspect the available Worker images.",
          updated_at: this.now(),
          error: reason.message,
    ...
    if (!this.hasActiveBuild()) {
      ...
      if (!selected) {
        this.status.worker_image = {
          ...
          updated_at: this.now(),
          error: "The configured Atms Worker image has not been built.",
          compatibility: "unknown",
        };
  ...
  private async inspectImages(): Promise<DagEnvironmentImage[]> {
    ...
    for (const row of parseJsonLines(inventory.stdout)) {
      ...
      if (
        ...
        && (
          ...
          // they do not identify an image as a Atms Worker.
          repository === "atms-worker"
          || repository.endsWith("/atms-worker")
          || hasDockerLabel(labels, ATMS_WORKER_SOURCE_LABEL)
          || hasDockerLabel(labels, ATMS_WORKER_PROTOCOL_LABEL)
        )
      ) {
    ...
    for (const inspection of inspections) {
      ...
      const tags = Array.isArray(inspection.RepoTags)
        ...
        : [];
      const source = nonEmpty(labels[ATMS_WORKER_SOURCE_LABEL]);
      const protocol = nonEmpty(labels[ATMS_WORKER_PROTOCOL_LABEL]);
      const workerVersion = nonEmpty(labels[ATMS_WORKER_VERSION_LABEL]);
      const image: DagEnvironmentImage = {
        ...
        image_revision: nonEmpty(labels[ATMS_WORKER_REVISION_LABEL]),
  ...
  private async runBuild(): Promise<void> {
    ...
    if (!fingerprint || !workerVersion) {
      this.failBuild(
        "Atms Worker source files are unavailable in this installation.",
        "worker_source_unavailable",
        operationId,
      );
      return;
    }
    const revision = nonEmpty(this.env.ATMS_BUILD_REVISION)
    ...
    const args = [
      ...
      "-f",
      "atms_worker/Dockerfile",
      "--label", `${ATMS_WORKER_SOURCE_LABEL}=${fingerprint}`,
      "--label", `${ATMS_WORKER_PROTOCOL_LABEL}=${WORKER_CONTRACT_VERSION}`,
      "--label", `${ATMS_WORKER_VERSION_LABEL}=${workerVersion}`,
      "--label", `${ATMS_WORKER_REVISION_LABEL}=${revision}`,
      "--label", `${ATMS_WORKER_CREATED_LABEL}=${created}`,
      "--build-arg", `ATMS_WORKER_SOURCE_FINGERPRINT=${fingerprint}`,
      "--build-arg", `ATMS_WORKER_PROTOCOL_VERSION=${WORKER_CONTRACT_VERSION}`,
      "--build-arg", `ATMS_WORKER_VERSION=${workerVersion}`,
