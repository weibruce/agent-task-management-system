...
import { ATMS_ARTIFACT_BROKER_MAX_BYTES, isAtmsPluginId } from "atms-protocol";
...
function artifactAuthorization(req: http.IncomingMessage): string {
  ...
  const match = /^AtmsArtifact ([^\s,]+)$/.exec(raw);
...
function readArtifactBody(req: http.IncomingMessage, exactBytes: number): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    if (!Number.isSafeInteger(exactBytes) || exactBytes < 1 || exactBytes > ATMS_ARTIFACT_BROKER_MAX_BYTES) {
...
export function pluginArtifactRoutesHandler(req: http.IncomingMessage, res: http.ServerResponse): boolean {
  ...
  try {
    ...
    res.setHeader("X-Atms-Artifact-Request", result.metadata.request_id);
