...
// Keep this explicit until Atms's minimum Codex version includes that fix.
...
function liveThreadName(sessionId: string, tools: ToolDefinition[]): string {
  ...
  return `atms-live-${sessionDigest}-${digest}`;
}
      ...
      case "thread/realtime/itemAdded": {
        ...
        if (type.includes("handoff") || type.includes("delegation")) {
          if (
            ...
            this.options.onEvent({
              type: "session.error",
              message: "Atms Manager tools or permissions changed. Reconnect Live Voice to continue.",
              recoverable: false,
            });
            await this.stop();
            break;
  ...
  private async handleToolCall(
    ...
    if (!tool) {
      this.client?.respond(requestId, {
        contentItems: [{ type: "inputText", text: `Unknown Atms tool: ${name}` }],
        success: false,
      });
      return;
    }
