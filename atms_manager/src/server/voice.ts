...
function _defaultVoiceSettings() {
  ...
  return {
    ...
    tts_speed: null,
    tts_token_set: Boolean(ttsSetting?.api_key || process.env.ATMS_TTS_API_KEY),
    tts_stream: false,
...
function _voiceSettingsWithCapabilityDefaults(settings?: Record<string, unknown>): Record<string, unknown> {
  ...
  data.asr_token_set = Boolean(asrSetting?.api_key || process.env.ATMS_MIMO_API_KEY);
  ...
  data.tts_token_set = _stringField(data.tts_model) === BUILTIN_EDGE_TTS_MODEL
    ? false
    : Boolean(ttsSetting?.api_key || process.env.ATMS_TTS_API_KEY);
...
function _resolveTtsRuntime(settings?: Record<string, unknown>): {
  ...
  const apiKey = setting?.api_key ?? process.env.ATMS_TTS_API_KEY ?? "";
...
function _resolveAsrRealtimeRuntime(): {
  ...
  const apiKey = setting?.api_key ?? process.env.ATMS_MIMO_API_KEY ?? "";
