...
const ARTIFACT_APPEARANCE_BRIDGE = `<style data-atms-artifact-appearance-bridge>
:root {
  color-scheme: dark;
  --atms-artifact-scrollbar-track: #0a0f18;
  --atms-artifact-scrollbar: #2a3541;
  --atms-artifact-scrollbar-hover: #435565;
}
* {
  scrollbar-width: thin;
  scrollbar-color: var(--atms-artifact-scrollbar) var(--atms-artifact-scrollbar-track) !important;
}
*::-webkit-scrollbar {
  width: 8px;
  height: 8px;
  background-color: var(--atms-artifact-scrollbar-track) !important;
}
...
*::-webkit-scrollbar-thumb {
  background-color: var(--atms-artifact-scrollbar) !important;
  border: 2px solid var(--atms-artifact-scrollbar-track) !important;
  background-clip: border-box;
  border-radius: 999px;
}
*::-webkit-scrollbar-thumb:hover,
*::-webkit-scrollbar-thumb:active { background-color: var(--atms-artifact-scrollbar-hover) !important; }
*::-webkit-scrollbar-corner { background-color: var(--atms-artifact-scrollbar-track) !important; }
</style><script data-atms-artifact-appearance-bridge>
(() => {
  const messageType = "atms:artifact-appearance";
  ...
  window.addEventListener("message", (event) => {
    ...
    root.style.setProperty("--atms-artifact-scrollbar-track", visibleCanvasColor() || colors[0]);
    root.style.setProperty("--atms-artifact-scrollbar", colors[1]);
    root.style.setProperty("--atms-artifact-scrollbar-hover", colors[2]);
  });
})();
</script>`;
...
export function injectVoiceArtifactAppearanceBridge(html: string): string {
  if (html.includes("data-atms-artifact-appearance-bridge")) return html;
