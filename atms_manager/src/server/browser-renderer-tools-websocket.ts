...
function boundedJsonBytes(value: unknown): number {
  ...
  const visit = (current: unknown, depth: number): void => {
    nodes += 1;
    if (nodes > 10_000 || depth > 32) throw new Error("Atms UI tool result is too complex");
  ...
  try {
    ...
    throw new Error("Atms UI tool result is not valid JSON");
  }
...
function expectedContracts(): Map<AtmsUiToolName, string> {
  return new Map(ATMS_UI_TOOL_CONTRACTS.map((contract) => [
    contract.name,
...
function validateContracts(value: unknown): void {
  if (!Array.isArray(value) || value.length !== ATMS_UI_TOOL_CONTRACTS.length) {
    throw new Error("Renderer contract catalog must contain the exact Atms UI tool set");
  }
  ...
  for (const raw of value) {
    ...
    const name = safeId(binding.name, "contract.name", 128) as AtmsUiToolName;
...
export class BrowserRendererToolsBroker {
  ...
  requireConnection(rawTarget: BrowserRendererConnectionRefV1): BrowserRendererConnectionRefV1 {
    ...
    if (!current || targetKey(current) !== targetKey(target)) {
      throw new Error("The requested Atms browser renderer target is stale or unavailable");
    }
    return current;
  }
  ...
  async invoke(
    ...
    if (!client || client.socket.readyState !== WebSocket.OPEN) {
      throw new Error("The requested Atms browser renderer connection is unavailable");
    }
    ...
    if (this.#pending.size >= MAX_PENDING_CALLS || this.#pendingCount(connectionId) >= MAX_PENDING_CALLS_PER_CONNECTION) {
      throw new Error("Atms browser renderer has too many pending calls");
    }
    const input = validateAtmsUiToolInput(toolName, rawInput);
    const contract = ATMS_UI_TOOL_CONTRACTS.find((candidate) => candidate.name === toolName)!;
      ...
      const requestCancellation = (
        ...
        pending.timer = setTimeout(() => {
          ...
          finished?.reject(new Error(
            `Atms UI tool outcome is indeterminate: renderer did not acknowledge ${reason}`,
          ));
        }, CANCEL_ACK_GRACE_MS);
        pending.timer.unref?.();
      };
  ...
  #rejectPending(connectionId: string, reason: string): void {
    for (const [callId, pending] of [...this.#pending]) {
      ...
      const finished = this.#takePending(callId);
      finished?.reject(new Error(`Atms UI tool outcome is indeterminate: ${reason}`));
    }
  }
