...
import {
  ATMS_ANDROID_LIVE_VOICE_ENABLED,
  ATMS_UNSAFE_ALLOW_PUBLIC_MANAGER_WITHOUT_AUTH,
...
function setCorsHeaders(res: http.ServerResponse): void {
  ...
  res.setHeader(
    ...
    "Authorization, Content-Type, If-None-Match, Range, X-Atms-Approval-Token, X-Atms-Dag-Token, X-Atms-Artifact-Sha256, X-Atms-Artifact-Uncompressed-Bytes, X-Atms-Artifact-File-Count",
  );
...
export function resolveManagerWorkerWsBaseUrl(actualPort: number): string {
  const explicit = process.env.ATMS_MANAGER_WORKER_WS_BASE_URL;
  if (explicit) return explicit.replace(/\/$/, "");

  const explicitHost = process.env.ATMS_MANAGER_WORKER_WS_HOST;
...
export function resolveManagerLocalHttpBaseUrl(
  ...
  const configuredHost = env.ATMS_MANAGER_HOST?.trim();
...
export function resolveManagerWorkerExtraHosts(): string[] {
  const explicit = process.env.ATMS_MANAGER_WORKER_EXTRA_HOSTS;
...
export function resolveWorkerControlPlaneAuth(
  ...
  const configured = env.ATMS_WORKER_TOKEN?.trim()
    || env.ATMS_CONTROL_PLANE_TOKEN?.trim();
  return configured
...
const MANAGER_AGENT_ENV_PASSTHROUGH = [
  "ATMS_MANAGER_AGENT_BACKEND",
  "ATMS_MANAGER_AGENT_SMOKE",
  "ATMS_MANAGER_AGENT_SMOKE_YAML",
  "ATMS_MANAGER_AGENT_SMOKE_PROFILE",
  "ATMS_MANAGER_AGENT_SMOKE_PROMPT",
  "MANAGER_AGENT_TURN_TIMEOUT_MS",
  "ATMS_DAG_MUTATION_TOKEN",
] as const;
...
export function resolveManagerAgentRuntimeEnv(): Record<string, string> | undefined {
  ...
  const managerAgentBackend = process.env.ATMS_MANAGER_AGENT_BACKEND ?? process.env.AGENT_BACKEND;
...
export function createServer(
  ...
  const pluginHttpTrust = createPluginHttpTrustPolicy({
    bindHost: process.env.ATMS_MANAGER_HOST,
    publicUrl: process.env.ATMS_MANAGER_PUBLIC_URL,
    adminToken: process.env.ATMS_MANAGER_ADMIN_TOKEN,
    allowedOrigins: process.env.ATMS_MANAGER_ADMIN_ORIGINS,
    androidLiveVoiceEnabled: process.env[ATMS_ANDROID_LIVE_VOICE_ENABLED] === "1",
    unsafeAllowUnauthenticatedPublic:
      process.env[ATMS_UNSAFE_ALLOW_PUBLIC_MANAGER_WITHOUT_AUTH] === "1",
    ...
    scopedMutationAuthorizer: ({ method, pathname, headers, remoteAddress }) => {
      ...
      const rawHeader = headers["x-atms-dag-token"];
      const headerToken = Array.isArray(rawHeader) ? rawHeader[0] : rawHeader;
      return isDagMutationRequestAuthorized({
        remoteAddress,
        headerToken,
        configuredToken: process.env.ATMS_DAG_MUTATION_TOKEN,
      });
    },
  });
  ...
  const configuredNodeToken = process.env.ATMS_NODE_TOKEN?.trim()
    || process.env.ATMS_CONTROL_PLANE_TOKEN?.trim();
  const workerImage = process.env.ATMS_WORKER_IMAGE || "atms-worker:latest";
  ...
  const defaultProvisionerOptions: WsDispatchAdapterOptions = {
    provisioner: {
      ...
      network: process.env.ATMS_WORKER_NETWORK || undefined,
    ...
    projectId: process.env.ATMS_PROJECT_ID ?? "p1",
  };
  ...
  server = http.createServer((req, res) => {
    ...
    if (requiresDagMutationAuthorization(pathname, req.method)) {
      const rawHeader = req.headers["x-atms-dag-token"];
      const headerToken = Array.isArray(rawHeader) ? rawHeader[0] : rawHeader;
      if (!isDagMutationRequestAuthorized({
        ...
        json(res, 403, {
          success: false,
          message: "DAG mutations require a local request or valid ATMS_DAG_MUTATION_TOKEN",
          error: "DAG mutations require a local request or valid ATMS_DAG_MUTATION_TOKEN",
        });
        return;
      }
    }
