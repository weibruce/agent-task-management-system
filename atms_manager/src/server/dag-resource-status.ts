...
export function dagResourceStatusPath(): string {
  return path.join(getAtmsHome(), "runtime", "dag-resources.json");
}
...
export function defaultDagResourceStatus(): DagResourceStatus {
  return {
    worker_image: {
      status: "unknown",
      image: process.env.ATMS_WORKER_IMAGE || "atms-worker:latest",
...
export function readDagResourceStatus(): DagResourceStatus {
  try {
    ...
    return {
      ...
      worker_image: {
        status,
        image: typeof worker.image === "string" && worker.image ? worker.image : process.env.ATMS_WORKER_IMAGE || "atms-worker:latest",
