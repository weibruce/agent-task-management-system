...
interface AuthenticatedClient {
  ...
  pageId: string | null;
  tools: Map<AtmsUiToolName, BrowserToolsCatalogEntry>;
}
...
function isTrustedAtmsOrigin(raw: string): boolean {
  try {
...
export class BrowserToolsBroker {
  ...
  ready(): boolean {
    ...
    return Boolean(
      ...
      && client.pageId
      && client.tools.size === ATMS_UI_TOOL_NAMES.length
      && ATMS_UI_TOOL_NAMES.every((name) => client.tools.has(name)),
    );
  }
  ...
  async invoke(
    ...
    if (!client || client.socket.readyState !== WebSocket.OPEN) {
      throw new Error("Atms Browser Tools is disabled or Desktop is not connected");
    }
    if (signal?.aborted) throw new Error(`Atms UI tool was cancelled: ${toolName}`);
    if (this.pending.size >= MAX_PENDING_CALLS) {
      throw new Error("Atms Browser Tools has too many pending calls");
    }
    const catalog = client.tools.get(toolName);
    if (!catalog || !client.pageId) {
      throw new Error(`Atms UI tool is unavailable in the current page: ${toolName}`);
    }
    const input = validateAtmsUiToolInput(toolName, rawInput);
    const boundedTimeout = Math.min(30_000, Math.max(250, Math.floor(timeoutMs)));
    const callId = randomUUID();
    const contract = atmsUiToolContract(toolName)!;
    ...
    return new Promise((resolve, reject) => {
      const requestCancellation = (
        ...
        if (!pending.invocationSent) {
          ...
          finished?.reject(new Error(`Atms UI tool was cancelled before execution: ${reason}`));
          return;
        }
        ...
        pending.timer = setTimeout(() => {
          ...
          finished?.reject(new Error(
            `Atms UI tool outcome is indeterminate: Desktop did not acknowledge ${reason}`,
          ));
        }, CANCEL_ACK_GRACE_MS);
        pending.timer.unref?.();
      };
  ...
  private handleAuthenticatedMessage(
    ...
    if (message.type === "page.catalog") {
      ...
      const accepted = new Map<AtmsUiToolName, BrowserToolsCatalogEntry>();
      for (const raw of message.tools) {
        const tool = record(raw, "page.catalog tool");
        const name = safeString(tool.name, "tool.name");
        if (!ATMS_UI_TOOL_NAMES.includes(name as AtmsUiToolName)) continue;
        const contract = atmsUiToolContract(name)!;
        ...
        if (!isTrustedAtmsOrigin(origin)) continue;
        ...
        if (accepted.has(name as AtmsUiToolName)) continue;
        accepted.set(name as AtmsUiToolName, {
          name,
          description,
          input_schema: inputSchema,
          frame_id: frameId,
      ...
      for (const [callId, pending] of this.pending) {
        ...
        const finished = this.takePending(callId);
        finished?.reject(new Error("Atms page catalog changed during the UI tool call"));
      }
      return;
    }
    ...
    if (message.type === "page.invalidated") {
      ...
      if (pageId === client.pageId) {
        client.pageId = null;
        client.tools.clear();
        this.rejectPending(client.connectionId, "Atms page navigation invalidated the UI tool call");
      }
      return;
    }
    ...
    if (message.type === "tool.result") {
      ...
      if (message.ok === true) {
        ...
        if (outputBytes > BROWSER_TOOLS_MAX_RESULT_BYTES) {
          finished.reject(new Error("Atms UI tool result exceeded the allowed size"));
        } else {
        ...
        const error = typeof message.error === "string" ? message.error.slice(0, 2_000) : "Atms UI tool failed";
        const finished = this.takePending(callId)!;
        finished.reject(new Error(
          terminalState === "indeterminate"
            ? `Atms UI tool outcome is indeterminate: ${error}`
            : terminalState === "cancelled"
              ? `Atms UI tool was cancelled: ${error}`
              : error,
        ));
      }
      return;
