...
function localNodeCliPath(): string | undefined {
  const explicit = process.env.ATMS_NODE_CLI;
  if (explicit && fs.existsSync(explicit)) return path.resolve(explicit);
  const candidate = path.join(runtimeRoot(), "atms_node", "dist", "cli.js");
...
function localNodeAutostartEnabled(): boolean {
  const raw = process.env.ATMS_LOCAL_NODE_AUTOSTART;
...
export async function ensureLocalDockerNode(
  ...
  localNodeStartPromise = (async () => {
    ...
    const nodeId = process.env.ATMS_NODE_ID || LOCAL_NODE_ID;
    const child = spawn(process.execPath, [cliPath], {
      ...
      env: {
        ...
        ATMS_MANAGER_WS_URL: resolveLocalNodeManagerWsUrl(options),
        ATMS_PROJECT_ID: projectId || process.env.ATMS_PROJECT_ID || "p1",
        ATMS_NODE_ID: nodeId,
        ATMS_NODE_PROVIDER: process.env.ATMS_NODE_PROVIDER || "docker-cli",
      },
    });
    fs.closeSync(out);
    fs.closeSync(err);
