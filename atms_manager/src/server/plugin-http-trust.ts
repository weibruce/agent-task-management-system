...
import { ATMS_MANAGER_TURN_HEADER } from "atms-protocol";

export const ATMS_MANAGER_ADMIN_TOKEN = "ATMS_MANAGER_ADMIN_TOKEN";
export const ATMS_MANAGER_ADMIN_ORIGINS = "ATMS_MANAGER_ADMIN_ORIGINS";
export const ATMS_UNSAFE_ALLOW_PUBLIC_MANAGER_WITHOUT_AUTH = "ATMS_UNSAFE_ALLOW_PUBLIC_MANAGER_WITHOUT_AUTH";
export const ATMS_ANDROID_APPASSETS_ORIGIN = "https://appassets.androidplatform.net";
export const ATMS_ANDROID_LIVE_VOICE_ENABLED = "ATMS_ANDROID_LIVE_VOICE_ENABLED";
...
const DAG_MUTATION_TOKEN_HEADER = "x-atms-dag-token";
const PREFLIGHT_HEADERS = new Set(["authorization", "content-type", "if-none-match", DAG_MUTATION_TOKEN_HEADER, ATMS_MANAGER_TURN_HEADER]);
...
export function createPluginHttpTrustPolicy(
  ...
  if (
    ...
    throw new Error(
      `${ATMS_MANAGER_ADMIN_TOKEN} must contain at least ${MIN_ADMIN_TOKEN_BYTES} bytes `
...
export function pluginHttpTrustHandler(
  ...
  if (req.method === "OPTIONS") {
    ...
    res.setHeader("Access-Control-Allow-Headers", `Authorization, Content-Type, If-None-Match, ${ATMS_MANAGER_TURN_HEADER}`);
  ...
  if (MANAGER_ADMIN_TOKEN_AUTH_ENABLED && policy.adminToken) {
    const turnCredential = singleHeader(req.headers[ATMS_MANAGER_TURN_HEADER]);
 ...
 * does not identify a Atms installation and is never a global UI origin.
...
export function isAndroidLiveVoiceOriginAllowed(
  ...
  return policy.androidLiveVoiceEnabled && origin === ATMS_ANDROID_APPASSETS_ORIGIN;
}
...
function validateAdminToken(raw: string | undefined): string | undefined {
  ...
  if (raw !== raw.trim() || /[\u0000-\u001f\u007f]/.test(raw)) {
    throw new Error(`${ATMS_MANAGER_ADMIN_TOKEN} must not contain surrounding whitespace or control bytes`);
  }
  const bytes = Buffer.byteLength(raw, "utf8");
  if (bytes < MIN_ADMIN_TOKEN_BYTES || bytes > MAX_ADMIN_TOKEN_BYTES) {
    throw new Error(
      `${ATMS_MANAGER_ADMIN_TOKEN} must contain ${MIN_ADMIN_TOKEN_BYTES}-${MAX_ADMIN_TOKEN_BYTES} UTF-8 bytes`,
    );
  }
  if (!/^[A-Za-z0-9._~+/=-]+$/.test(raw)) {
    throw new Error(`${ATMS_MANAGER_ADMIN_TOKEN} must use a printable base64url, base64, or hexadecimal token`);
  }
  return raw;
}
...
function parseAllowedOrigins(raw: string | undefined): string[] {
  ...
  for (const value of values) {
    if (value === "*" || value === "null") {
      throw new Error(
        `${ATMS_MANAGER_ADMIN_ORIGINS} must contain exact comma-separated http(s) origins without wildcards`,
      );
    }
    let url: URL;
    try {
      ...
      throw new Error(`${ATMS_MANAGER_ADMIN_ORIGINS} contains an invalid Origin`);
    }
    if (
      ...
      throw new Error(
        `${ATMS_MANAGER_ADMIN_ORIGINS} must contain exact comma-separated http(s) origins without paths`,
      );
    }
    origins.add(value);
  }
