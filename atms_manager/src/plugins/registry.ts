...
export const CORE_PLUGIN_ID = "com.atms.core" as const;
...
export function syncBuiltinPlugins(root = getBuiltinPluginRoot()): SyncBuiltinPluginsResult {
  repairLegacyBuiltinPluginPackages();
  const descriptors: AtmsResolvedPluginDescriptorV1[] = listBuiltinPluginPackageRoots(root)
  ...
  const plugins = descriptors.map((descriptor) => {
    ...
    return syncPluginPackage({
      ...
      // Bundled packages ship with Atms itself. Refresh their persisted
...
export class AtmsPluginRegistry {
