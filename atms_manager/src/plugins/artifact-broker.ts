...
import {
  ATMS_ARTIFACT_BROKER_MAX_TTL_MS,
  AtmsPluginPermission,
  atmsPluginArtifactCapabilitySigningInput,
  atmsPluginToolInvocationDigestInput,
  isAtmsPluginId,
  stableStringify,
  validateAtmsPluginArtifactWriteCapabilityClaims,
  validateAtmsPluginAuthorizedToolInvocation,
  type AtmsPluginArtifactMediaTypeV1,
  type AtmsPluginArtifactWriteCapabilityClaimsV1,
...
export interface PluginArtifactMetadataV1 {
  ...
  binding: AtmsPluginArtifactWriteCapabilityClaimsV1["binding"];
  request_id: string;
  request_digest: string;
  document_scope: AtmsPluginArtifactWriteCapabilityClaimsV1["document_scope"];
  label: string;
  media_type: AtmsPluginArtifactMediaTypeV1;
  digest: string;
  size_bytes: number;
...
function metadataMac(secret: Buffer, value: Omit<PluginArtifactMetadataV1, "integrity">): string {
  return createHmac("sha256", secret)
    .update(`atms-artifact-metadata-v1\0${stableStringify(value)}`, "utf8")
    .digest("hex");
}
...
function assertContentMatchesMediaType(content: Buffer, mediaType: AtmsPluginArtifactMediaTypeV1): void {
...
export class PluginArtifactBroker {
  ...
  #metadataFile(pluginId: string, requestId: string, contentDigest: string): string {
    if (!isAtmsPluginId(pluginId)) throw new Error("Artifact plugin id is invalid");
  ...
  issueWriteCapability(input: {
    ...
    const authorization = validateAtmsPluginAuthorizedToolInvocation(input.authorization, {
      now_ms: now.getTime(),
    });
    ...
    if (pluginJsonDigest(atmsPluginToolInvocationDigestInput(invocation)) !== invocation.request_digest) {
      throw new Error("Artifact capability Tool request digest is invalid");
    }
    if (!invocation.policy.permissions.includes(AtmsPluginPermission.ARTIFACT_WRITE)
    ...
    if (!Number.isSafeInteger(ttl) || ttl < 1 || ttl > ATMS_ARTIFACT_BROKER_MAX_TTL_MS) {
    ...
    const claims: AtmsPluginArtifactWriteCapabilityClaimsV1 = {
    ...
    const validation = validateAtmsPluginArtifactWriteCapabilityClaims(claims, { now_ms: now.getTime() });
    if (!validation.valid || !validation.value) {
      throw new Error(`Manager produced an invalid Artifact capability: ${JSON.stringify(validation.errors)}`);
    }
    const payload = Buffer.from(atmsPluginArtifactCapabilitySigningInput(validation.value), "utf8").toString("base64url");
  ...
  inspectWriteCapability(input: {
    ...
    capability_id?: string;
    now?: Date;
  }): AtmsPluginArtifactWriteCapabilityClaimsV1 {
    ...
    const validation = validateAtmsPluginArtifactWriteCapabilityClaims(raw, {
    ...
    if (atmsPluginArtifactCapabilitySigningInput(validation.value)
