...
import {
  ...
  atmsPluginRuntimeSandboxAttestationSigningInput,
  stableStringify,
  validateAtmsPluginRuntimeSandboxAttestation,
  ...
  type AtmsPluginRuntimeRpcCancelResultV1,
  type AtmsPluginRuntimeRpcExecuteResultV1,
  type AtmsPluginRuntimeRpcHealthResultV1,
  type AtmsPluginRuntimeRpcPrepareResultV1,
  type AtmsPluginRuntimeRpcReconcileResultV1,
  type AtmsPluginRuntimeRpcRequestV1,
  type AtmsPluginRuntimeRpcResponseV1,
  type AtmsPluginRuntimeTrust as AtmsPluginRuntimeTrustValue,
  type AtmsPluginRuntimeSandboxAttestationV1,
  type AtmsPluginRuntimeArtifactUploadV1,
  type AtmsPluginToolPolicyV1,
} from "atms-protocol";
import {
...
export interface PluginRuntimeTransport {
  request(request: AtmsPluginRuntimeRpcRequestV1, signal: AbortSignal): Promise<unknown>;
}
...
export interface ResolvedPluginRuntimeV1 {
  ...
  trust: AtmsPluginRuntimeTrustValue;
...
export class VerifiedPluginRuntimeSandboxGate implements PluginRuntimeSandboxGate {
  ...
  assertAllowed(input: PluginRuntimeSandboxGateInput): void {
    const validation = validateAtmsPluginRuntimeSandboxAttestation(input.attestation, {
    ...
    if (!verify(
      null,
      Buffer.from(atmsPluginRuntimeSandboxAttestationSigningInput(attestation.claims), "utf8"),
      trusted.public_key,
...
function assertIsolationDoesNotWiden(
  grants: AtmsPluginToolPolicyV1["effective_grants"],
  isolation: AtmsPluginRuntimeSandboxAttestationV1["claims"]["isolation"],
): void {
  const permissions = new Set(grants.map((grant) => grant.permission));
  const networkGrant = grants.find((grant) => grant.permission === AtmsPluginPermission.NETWORK_CONNECT);
  ...
  if (!permissions.has(AtmsPluginPermission.GPU_USE)
  ...
  const writePermissions = [
    ...
    AtmsPluginPermission.ARTIFACT_WRITE,
...
interface PluginRuntimeTransportRegistration {
  ...
  sandbox_attestation?: AtmsPluginRuntimeSandboxAttestationV1;
...
function runtimeTransportKey(
  binding: AtmsPluginToolBindingV1,
  effectiveGrants: AtmsPluginToolPolicyV1["effective_grants"],
): string {
...
export class PluginRuntimeTransportRegistry {
  ...
  unregister(binding: AtmsPluginToolBindingV1, policy?: AtmsPluginToolPolicyV1): boolean {
...
export class PluginRuntimeRpcFailure extends Error {
  ...
  readonly response: AtmsPluginRuntimeRpcResponseV1;

  constructor(response: Extract<AtmsPluginRuntimeRpcResponseV1, { message_type: "error" }>) {
...
function postDispatchFailure(
  request: AtmsPluginRuntimeRpcRequestV1,
  message: string,
  cause?: unknown,
): Error {
...
export type PluginRuntimeResolver = (binding: AtmsPluginToolBindingV1) => ResolvedPluginRuntimeV1;
...
function defaultRuntimeResolver(binding: AtmsPluginToolBindingV1): ResolvedPluginRuntimeV1 {
...
function assertRuntimeTrust(input: PluginRuntimeSandboxGateInput, sandbox: PluginRuntimeSandboxGate): void {
  const { runtime } = input;
  if (runtime.trust === AtmsPluginRuntimeTrust.TRUSTED_BUILTIN) {
    if (runtime.source !== "builtin") throw new Error("Only bundled plugins may use trusted_builtin runtime");
    return;
  }
  if (runtime.trust === AtmsPluginRuntimeTrust.SANDBOXED_RUNTIME) {
...
export class PluginRuntimeBroker {
  ...
  async #transport(
    binding: AtmsPluginToolBindingV1,
    policy: AtmsPluginToolPolicyV1 | undefined,
    now: Date,
  ...
  async #request(
    ...
    transport: PluginRuntimeTransport,
    timeoutMs: number,
    expected: Parameters<typeof validateAtmsPluginRuntimeRpcResponse>[1],
  ): Promise<AtmsPluginRuntimeRpcResponseV1> {
    const requestValidation = validateAtmsPluginRuntimeRpcRequest(request, expected);
    ...
    const response = validateAtmsPluginRuntimeRpcResponse(raw, expected);
  ...
  async execute(input: {
    ...
    now?: Date;
  }): Promise<AtmsPluginRuntimeRpcExecuteResultV1> {
    const now = input.now ?? new Date();
    const validation = validateAtmsPluginAuthorizedToolInvocation(input.authorization, { now_ms: now.getTime() });
    if (!validation.valid || !validation.value) {
    ...
    const request: AtmsPluginRuntimeRpcRequestV1 = {
    ...
    return response as AtmsPluginRuntimeRpcExecuteResultV1;
  }
  ...
  async prepare(input: {
    authorization: AtmsPluginAuthorizedToolInvocationV1;
    now?: Date;
  }): Promise<AtmsPluginRuntimeRpcPrepareResultV1> {
    const now = input.now ?? new Date();
    const validation = validateAtmsPluginAuthorizedToolInvocation(input.authorization, { now_ms: now.getTime() });
    if (!validation.valid || !validation.value) {
      throw new Error(`Runtime authorization is invalid: ${JSON.stringify(validation.errors)}`);
    ...
    return response as AtmsPluginRuntimeRpcPrepareResultV1;
  }
  ...
  async cancel(input: {
    ...
    now?: Date;
  }): Promise<AtmsPluginRuntimeRpcCancelResultV1> {
    ...
    return response as AtmsPluginRuntimeRpcCancelResultV1;
  }
  ...
  async reconcile(input: {
    ...
    request_digest: string;
    now?: Date;
  }): Promise<AtmsPluginRuntimeRpcReconcileResultV1> {
    const record = getPluginToolRequest(input.request_id);
    ...
    const result = response as AtmsPluginRuntimeRpcReconcileResultV1;
  ...
  async health(input: {
    binding: AtmsPluginToolBindingV1;
    now?: Date;
  }): Promise<AtmsPluginRuntimeRpcHealthResultV1> {
    const now = input.now ?? new Date();
    ...
    return response as AtmsPluginRuntimeRpcHealthResultV1;
  }
}
