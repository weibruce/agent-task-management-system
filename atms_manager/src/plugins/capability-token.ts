...
const SECRET_ENV = "ATMS_PLUGIN_CAPABILITY_SECRET";
...
export class PluginToolCapabilityTokenAuthority {
  ...
  issue(input: {
    ...
    now?: Date;
    ttl_ms?: number;
  }): { token: string; claims: AtmsPluginToolCapabilityClaimsV1; token_digest: string } {
    const now = input.now ?? new Date();
    const nowMs = now.getTime();
    const invocationValidation = validateAtmsPluginToolInvocation(input.invocation, { now_ms: nowMs });
    ...
    if (!Number.isSafeInteger(requestedTtl) || requestedTtl < 1 || requestedTtl > ATMS_ACTION_CAPABILITY_MAX_TTL_MS) {
    ...
    const claims: AtmsPluginToolCapabilityClaimsV1 = {
    ...
    const validation = validateAtmsPluginToolCapabilityClaims(claims, invocation, { now_ms: nowMs });
  ...
  verifyAndConsume(input: {
    ...
    invocation: AtmsPluginToolInvocationV1;
    now?: Date;
  }): AtmsPluginToolCapabilityClaimsV1 {
    ...
    if (pluginJsonDigest(atmsPluginToolInvocationDigestInput(input.invocation)) !== input.invocation.request_digest) {
      throw new Error("Plugin capability Tool request digest is invalid");
    }
    const now = input.now ?? new Date();
    const validation = validateAtmsPluginToolCapabilityClaims(
      claims,
      input.invocation,
      { now_ms: now.getTime() },
    );
