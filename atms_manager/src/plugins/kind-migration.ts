...
import {
  ...
  isAtmsDeclarativeKindMigrationManifest,
  parseAtmsKindMigrationV1,
  type AtmsKindMigrationV1,
...
function assertDeclarativeCandidatePolicy(
  ...
  if (!isAtmsDeclarativeKindMigrationManifest(manifest)) {
...
export function loadVerifiedKindMigrationPlan(
  target: PluginVersionRecord,
): Map<string, Map<number, AtmsKindMigrationV1>> {
  const byPath = new Map(target.descriptor.referenced_files.map((file) => [file.path, file]));
  const plan = new Map<string, Map<number, AtmsKindMigrationV1>>();
  for (const kind of target.descriptor.manifest.kinds) {
    const steps = new Map<number, AtmsKindMigrationV1>();
...
function migratedNode(
  ...
  target: PluginVersionRecord,
  plan: Map<string, Map<number, AtmsKindMigrationV1>>,
): GenerativeUiNodeV1 {
  ...
  while (version < kind.current_version) {
    ...
    content = applyAtmsKindMigrationV1(content, migration);
