...
import {
  ...
  type AtmsPluginSkillDescriptorV1,
  type AtmsPluginModality,
...
export function assembleLegacyWidgetReservations(): Array<{ plugin_id: string; legacy_types: string[] }> {
  ...
  for (const plugin of listPluginPackages()) {
    for (const tool of plugin.descriptor.manifest.tools) {
      ...
      const validation = validateAtmsDirectUiProjection(handler.document);
...
export function assemblePluginTurnContext(
  ...
  const skills: AtmsPluginSkillDescriptorV1[] = [];
  const tools: AtmsPluginToolDescriptorV1[] = [];
  const actions: AtmsPluginTurnContextV1["actions"] = [];
    ...
    for (const skill of plugin.descriptor.skills) {
      ...
      skills.push({
        ...
        capability_ids: skillCapabilities
          ...
          .sort(),
        description: skill.description ?? declaration.description ?? `Atms Skill ${skill.id}`,
        digest: skill.digest,
      });
    }
  ...
  const context: AtmsPluginTurnContextV1 = {
    ...unsigned,
    context_digest: pluginJsonDigest(unsigned),
  };
  const validation = validateAtmsPluginTurnContext(context);
...
export function selectPluginTurnContext(
  ...
  const selectedContext: AtmsPluginTurnContextV1 = {
    ...unsigned,
    context_digest: pluginJsonDigest(unsigned),
  };
  const validation = validateAtmsPluginTurnContext(selectedContext);
...
export function assertCurrentPluginTurnContextSubset(
  ...
  state?: PluginRegistryState,
  options: { modality?: AtmsPluginModality } = {},
): AtmsPluginTurnContextV1 {
  if (!state) ensureBuiltinPluginsSynced();
  const registry = state ?? getPluginRegistryState();
  const validation = validateAtmsPluginTurnContext(value);
  if (
    !validation.valid
    || !validation.value
    || pluginJsonDigest(atmsPluginTurnContextDigestInput(value)) !== value.context_digest
...
export interface ArchivedPluginSkill {
  descriptor: AtmsPluginSkillDescriptorV1;
  content: string;
...
function archivedSkillVisualProfile(
  ...
  return parseDagWorkerSkillVisualProfileV1(JSON.parse(decodeAtmsPluginUtf8(bytes, pathValue)));
}
  ...
  return {
    descriptor: {
      ...
      capability_ids: plugin.descriptor.manifest.capabilities
        ...
        .sort(),
      description: skill.description ?? declaration.description ?? `Atms Skill ${reference.local_id}`,
      digest: skill.digest,
    },
  ...
  return {
    descriptor: {
      ...
      capability_ids: plugin.descriptor.manifest.capabilities
        ...
        .map((capability) => qualified(plugin.plugin_id, capability.id))
        .sort(),
      description: skill.description ?? declaration.description ?? `Atms Skill ${localId}`,
      digest: skill.digest,
    },
    content: skill.content,
    ...(declaration.visual_profile
