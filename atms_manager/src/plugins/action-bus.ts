...
import {
  ...
  validateAtmsDirectUiProjection,
  validateAtmsPluginAuthorizedToolInvocation,
  validateAtmsPluginToolInput,
  ...
  type AtmsPluginResolvedHandlerV1,
  type AtmsPluginRuntimeExecutionOutputV1,
  ...
  type AtmsPluginToolDescriptorV1,
...
const REQUEST_TTL_MS = Math.min(10 * 60_000, ATMS_ACTION_REQUEST_MAX_TTL_MS);
const CONFIRMATION_TTL_MS = Math.min(5 * 60_000, ATMS_ACTION_CONFIRMATION_MAX_TTL_MS);
...
}) => Promise<AtmsPluginRuntimeExecutionOutputV1> | AtmsPluginRuntimeExecutionOutputV1;
...
interface ResolvedTool {
  ...
  descriptor: AtmsPluginToolDescriptorV1;
...
function archivedProjectionHandler(
  plugin: ActivePluginRecord,
  file: string,
): Extract<AtmsPluginResolvedHandlerV1, { type: "projection" }> {
  ...
  const validation = validateAtmsDirectUiProjection(document);
...
function toolDescriptor(
  ...
  const handler: AtmsPluginResolvedHandlerV1 = tool.handler.type === "projection"
...
function handlerIdentity(handler: AtmsPluginResolvedHandlerV1): AtmsPluginToolInvocationV1["tool"]["handler"] {
...
function toolPermissions(plugin: ActivePluginRecord, descriptor: AtmsPluginToolDescriptorV1): string[] {
  return [...new Set([
...
function policyFor(plugin: ActivePluginRecord, descriptor: AtmsPluginToolDescriptorV1): PluginPermissionPolicySnapshotV1 {
...
function buildInvocation(input: {
  ...
  plugin: ActivePluginRecord;
  descriptor: AtmsPluginToolDescriptorV1;
  context: AtmsPluginTurnContextV1;
  policy: PluginPermissionPolicySnapshotV1;
  arguments: Record<string, unknown>;
  invoked_at?: string;
}): AtmsPluginToolInvocationV1 {
  const inputValidation = validateAtmsPluginToolInput(input.descriptor.input_schema, input.arguments);
  ...
  const unsigned: Omit<AtmsPluginToolInvocationV1, "request_digest"> = {
    tool_bus_version: 1,
    ...
    policy: {
      ...
      permissions: toolPermissions(input.plugin, input.descriptor) as AtmsPluginToolInvocationV1["policy"]["permissions"],
  ...
  const invocation: AtmsPluginToolInvocationV1 = {
    ...unsigned,
    request_digest: pluginJsonDigest(unsigned, 256 * 1024),
  };
  if (pluginJsonDigest(atmsPluginToolInvocationDigestInput(invocation), 256 * 1024) !== invocation.request_digest) {
...
function response(
  ...
  options: { idempotent: boolean; policy?: PluginPermissionPolicySnapshotV1; challenge?: AtmsPluginToolConfirmationChallengeV1 },
...
function targetFor(invocation: AtmsPluginToolInvocationV1): {
  document_id: string;
...
function confirmationFor(record: PluginToolRequestRecord): AtmsPluginAuthorizedToolInvocationV1["confirmation"] {
...
export class PluginToolInvocationService {
  ...
  constructor(input: {
    ...
    this.#artifactUploadBaseUrl = normalizeArtifactBrokerBaseUrl(
      input.artifact_upload_base_url ?? process.env.ATMS_PLUGIN_ARTIFACT_BROKER_URL,
    );
  ...
  #resolveLive(record: PluginToolRequestRecord): ResolvedTool {
    ...
    const inputValidation = validateAtmsPluginToolInput(descriptor.input_schema, invocation.arguments);
  ...
  #consumeInternalAuthorization(
    ...
    const validation = validateAtmsPluginAuthorizedToolInvocation(authorization, { now_ms: now.getTime() });
    if (!validation.valid || !validation.value) {
  ...
  #projectionOutput(
    ...
    record: PluginToolRequestRecord,
  ): { output: AtmsPluginRuntimeExecutionOutputV1; envelope: AtmsPluginToolExecutionEnvelopeV1 } {
    ...
    const projection = applyAtmsDirectUiProjection({
    ...
    const projectionSpec = validateAtmsDirectUiProjection(resolved.descriptor.handler.document);
    ...
    const outputValidation = validateAtmsPluginToolInput(resolved.descriptor.output_schema, projection.node.content);
    ...
    const envelope: AtmsPluginToolExecutionEnvelopeV1 = {
      execution_version: 1,
      status: "projected",
      committed: false,
      plugin: { id: resolved.plugin.plugin_id, version: resolved.plugin.plugin_version },
  ...
  #commitOutput(
    ...
    now: Date,
    envelope?: AtmsPluginToolExecutionEnvelopeV1,
    ...
    if (output.type === "domain_output") {
      return getDb().transaction(() => {
        ...
        if (live.descriptor.output_schema) {
          const validation = validateAtmsPluginToolInput(live.descriptor.output_schema, output.output);
  ...
  async #execute(record: PluginToolRequestRecord): Promise<PluginToolRequestRecord> {
    ...
    try {
      ...
      const authorization: AtmsPluginAuthorizedToolInvocationV1 = {
        authorization_version: 1,
        invocation: structuredClone(running.invocation),
        capability: issued.claims,
        ...(confirmation ? { confirmation } : {}),
      };
      let output: AtmsPluginRuntimeExecutionOutputV1;
      let envelope: AtmsPluginToolExecutionEnvelopeV1 | undefined;
      if (resolved.descriptor.handler.type === "runtime") {
        let artifactUploads: AtmsPluginRuntimeArtifactUploadV1[] | undefined;
  ...
  async invokeAgent(value: unknown): Promise<PluginActionBusResponse> {
    ...
    const inputValidation = validateAtmsPluginToolInput(exactDescriptor.input_schema, input.arguments);
  ...
  async confirm(requestId: string, value: unknown): Promise<PluginActionBusResponse> {
    ...
    const decision: AtmsPluginToolConfirmationDecisionV1 = {
...
export class PluginActionBus {
  ...
  #resolve(input: PluginActionInteractionInput, now: Date): ResolvedAction {
    ...
    const inputValidation = validateAtmsPluginToolInput(descriptor.input_schema, effective);
    if (!inputValidation.valid || !inputValidation.value) {
