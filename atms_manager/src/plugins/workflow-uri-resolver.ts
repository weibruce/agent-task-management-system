...
import {
  ...
  type AtmsPluginConfirmation,
  type AtmsPluginEffect,
...
export interface ResolvePluginWorkflowUriRequestV1 {
  ...
  selected_context: AtmsPluginTurnContextV1;
}
...
export interface ResolvedPluginWorkflowUriV1 {
  ...
  permissions: AtmsPluginPermissionValue[];
...
function parseQualifiedCapability(value: unknown): {
  ...
  if (
    ...
    || !isAtmsPluginId(value.slice(0, separator))
...
function parsePluginUri(value: unknown): { uri: string; plugin_id: string } {
  ...
  if (
    !match
    || !isAtmsPluginId(match[1])
    || !isSafeAtmsPluginPackagePath(match[2])
...
function normalizeRequest(value: unknown): NormalizedRequest {
  ...
  return {
    ...
    selected_context: input.selected_context as AtmsPluginTurnContextV1,
  };
}
...
function selectedCapabilityIds(context: AtmsPluginTurnContextV1): string[] {
  return uniqueSorted([
...
function effectiveGrants(
  ...
  workflowPermissions: readonly AtmsPluginPermissionValue[],
): AtmsPluginEffectivePermissionGrantV1[] {
  ...
  const declarationByPermission = new Map<AtmsPluginPermissionValue, typeof declarations[number]>();
  ...
  return permissions.map((permission) => {
    ...
    if (permission === AtmsPluginPermission.NETWORK_CONNECT && !hosts?.length) {
      throw new Error("Plugin workflow network.connect permission requires an effective host allowlist");
    }
    if (permission !== AtmsPluginPermission.NETWORK_CONNECT && hosts !== undefined) {
...
function archivedWorkflowContent(
  ...
  if (
    bytes.byteLength > ATMS_PLUGIN_DESCRIPTOR_MAX_BYTES
  ...
  const content = decodeAtmsPluginUtf8(bytes, workflowFile);
