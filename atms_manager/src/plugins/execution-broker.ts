...
export function acceptPluginToolExecution(
  value: unknown,
  turnContext: AtmsPluginTurnContextV1,
): { envelope: AtmsPluginToolExecutionEnvelopeV1; node: GenerativeUiNodeV1 } {
  const validation = validateAtmsPluginToolExecutionEnvelope(value);
  if (!validation.valid || !validation.value) {
    throw new Error(`Invalid plugin Tool execution envelope: ${JSON.stringify(validation.errors)}`);
  }
  const contextValidation = validateAtmsPluginTurnContext(turnContext);
  if (
    !contextValidation.valid
    || !contextValidation.value
    || pluginJsonDigest(atmsPluginTurnContextDigestInput(contextValidation.value))
  ...
  const matchesEnvelope = (candidate: AtmsPluginTurnContextV1["tools"][number]) => (
  ...
  const replayed = executeAtmsPluginTool(frozenTool, envelope.arguments);
