...
import {
  ...
  validatePluginSkill,
  ATMS_PLUGIN_SDK_ABI_VERSION,
} from "atms-plugin-sdk";
import {
  ...
  analyzeAtmsPluginSchemaPolicy,
  decodeAtmsPluginUtf8,
...
function parseJsonObject(buffer: Buffer, label: string): Record<string, unknown> {
  let value: unknown;
  try {
    value = JSON.parse(decodeAtmsPluginUtf8(buffer, label));
  } catch (cause) {
...
function validateCompiledSchema(schemaId: string, schema: Record<string, unknown>): void {
  ...
  const policyIssues = analyzeAtmsPluginSchemaPolicy(schema);
...
function enforceM3TrustPolicy(
  ...
  if (
    manifest.runtime.trust === AtmsPluginRuntimeTrust.SANDBOXED_RUNTIME
  ...
  if (
    manifest.runtime.trust === AtmsPluginRuntimeTrust.TRUSTED_BUILTIN
  ...
  for (const renderer of manifest.renderers) {
    ...
    if (renderer.fallback.type === "core_projection" && manifest.id !== "com.atms.core") {
...
export function loadPluginPackage(
  packageRoot: string,
  options: LoadPluginPackageOptions,
): AtmsResolvedPluginDescriptorV1 {
  const rootStat = fs.lstatSync(packageRoot);
  if (rootStat.isSymbolicLink() || !rootStat.isDirectory()) {
    throw new Error(`Plugin package root must be a real directory: ${packageRoot}`);
  }
  const manifestFile = path.join(packageRoot, "atms.plugin.json");
  const manifestBuffer = readBoundedFile(manifestFile, MAX_MANIFEST_BYTES);
  const parsedManifest = parseJsonObject(manifestBuffer, "atms.plugin.json");
  const validation = validateAtmsPluginManifest(parsedManifest);
  if (!validation.valid || !validation.value) {
    throw new Error(`Invalid Atms plugin manifest: ${JSON.stringify(validation.errors)}`);
  }
  const manifest = validation.value;
  const compatibilityErrors = validateAtmsPluginCompatibility(manifest, {
    atms: ATMS_VERSION,
  ...
  if (compatibilityErrors.length) {
    throw new Error(`Incompatible Atms plugin: ${JSON.stringify(compatibilityErrors)}`);
  }
  enforceM3TrustPolicy(manifest, options);

  const referencedPaths = collectAtmsPluginFileReferences(manifest);
  ...
  const schemas = manifest.schemas.map((declaration) => {
    ...
    if (buffer.byteLength > ATMS_PLUGIN_SCHEMA_MAX_BYTES) {
      throw new Error(`Plugin schema ${declaration.id} exceeds ${ATMS_PLUGIN_SCHEMA_MAX_BYTES} bytes`);
    }
  ...
  for (const renderer of manifest.renderers) {
    if (renderer.source.type === "declarative") {
      ...
      const rendererValidation = validateAtmsDeclarativeRenderer(document);
  ...
  for (const tool of manifest.tools) {
    ...
    const projectionValidation = validateAtmsDirectUiProjection(projection);
  ...
  for (const action of manifest.actions) {
    ...
    if (delegatedTool.handler.type === "projection") {
      const projection = validateAtmsDirectUiProjection(
  ...
  const skills = manifest.skills.map((declaration) => {
    ...
    if (buffer.byteLength > ATMS_PLUGIN_SKILL_MAX_BYTES) {
      throw new Error(`Plugin Skill ${declaration.id} exceeds ${ATMS_PLUGIN_SKILL_MAX_BYTES} bytes`);
    }
    const content = decodeAtmsPluginUtf8(buffer, declaration.path);
    ...
    if (!metadata || typeof metadata.description !== "string") {
      throw new Error(
        `atms-plugin-sdk ABI mismatch (Manager expects ABI version 1, ` +
        `runtime SDK reports ${ATMS_PLUGIN_SDK_ABI_VERSION}): ` +
        `validatePluginSkill() did not return { name, description } for skill "${declaration.id}". ` +
        `The runtime SDK dist is likely stale — rebuild atms_plugin_sdk and ensure the Manager loads the updated artifact.`,
      );
    }
  ...
  const unsigned: Omit<AtmsResolvedPluginDescriptorV1, "package_digest"> = {
  ...
  const descriptor: AtmsResolvedPluginDescriptorV1 = {
    ...unsigned,
...
export function listBuiltinPluginPackageRoots(root = getBuiltinPluginRoot()): string[] {
  ...
  return fs.readdirSync(root, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && fs.existsSync(path.join(root, entry.name, "atms.plugin.json")))
