...
export interface PluginCapabilityPromptContext {
  ...
  tools: AtmsPluginTurnContextV1["tools"];
}
...
export interface RoutePluginCapabilitiesOptions extends CompilePluginCapabilityIndexOptions {
   ...
   */
  source_context?: AtmsPluginTurnContextV1;
}
...
function normalizedRequest(value: unknown): NormalizedPluginCapabilityRouteRequest {
  ...
  const modality = input.modality ?? AtmsPluginModality.TEXT;
  if (!Object.values(AtmsPluginModality).includes(modality as AtmsPluginModality)) {
  ...
  if (input.explicit_plugin_id !== undefined && (
    typeof input.explicit_plugin_id !== "string" || !isAtmsPluginId(input.explicit_plugin_id)
  ...
  const normalized: NormalizedPluginCapabilityRouteRequest = {
    ...
    modality: modality as AtmsPluginModality,
