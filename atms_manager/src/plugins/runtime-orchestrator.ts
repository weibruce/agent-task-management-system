...
export const PLUGIN_RUNTIME_NODE_ENV = "ATMS_PLUGIN_RUNTIME_NODE_ID";
export const PLUGIN_RUNTIME_IMAGE_ENV = "ATMS_PLUGIN_RUNTIME_IMAGE";
export const PLUGIN_RUNTIME_IMAGE_DIGEST_ENV = "ATMS_PLUGIN_RUNTIME_IMAGE_DIGEST";
...
export async function ensurePluginRuntimeTransport(input: {
  ...
  if (input.runtime.trust !== AtmsPluginRuntimeTrust.SANDBOXED_RUNTIME
...
export async function preflightInstalledPluginRuntime(input: {
  ...
  if (!version || !installation || version.source !== "installed"
    ...
    || version.descriptor.manifest.runtime.trust !== AtmsPluginRuntimeTrust.SANDBOXED_RUNTIME
  ...
  const policy: AtmsPluginToolPolicyV1 = {
  ...
  const binding: AtmsPluginToolBindingV1 = {
    plugin_id: input.plugin_id,
    plugin_version: input.plugin_version,
