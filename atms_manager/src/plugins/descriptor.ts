...
export function pluginJsonDigest(value: unknown, maxBytes = ATMS_PLUGIN_DESCRIPTOR_MAX_BYTES): string {
...
export function pluginDescriptorPackageDigest(
  descriptor: Omit<AtmsResolvedPluginDescriptorV1, "package_digest">,
): string {
...
export function validateResolvedPluginDescriptor(
  descriptor: AtmsResolvedPluginDescriptorV1,
): string[] {
  const errors: string[] = [];
  const wire = validateAtmsResolvedPluginDescriptorWire(descriptor);
  if (!wire.valid) {
  ...
  const manifestValidation = validateAtmsPluginManifest(descriptor.manifest);
  ...
  for (const schema of descriptor.schemas) {
    ...
    for (const issue of analyzeAtmsPluginSchemaPolicy(schema.schema)) {
      errors.push(`resolved schema violates policy at ${schema.id}${issue.path}: ${issue.message}`);
    }
    try { pluginJsonDigest(schema.schema, ATMS_PLUGIN_SCHEMA_MAX_BYTES); } catch (cause) {
  ...
  for (const skill of descriptor.skills) {
    ...
    if (Buffer.byteLength(skill.content, "utf8") > ATMS_PLUGIN_SKILL_MAX_BYTES) {
      errors.push(`resolved skill exceeds ${ATMS_PLUGIN_SKILL_MAX_BYTES} bytes: ${skill.id}`);
    }
  ...
  for (const entry of actualFiles) {
    ...
    if (totalFileBytes > ATMS_PLUGIN_DESCRIPTOR_MAX_BYTES) errors.push("referenced files exceed descriptor byte budget");
  ...
  for (const schema of descriptor.schemas) {
    ...
    if (bytes) {
      try {
        const parsed = JSON.parse(decodeAtmsPluginUtf8(bytes, schema.file)) as unknown;
        if (
          pluginJsonDigest(parsed, ATMS_PLUGIN_SCHEMA_MAX_BYTES)
          !== pluginJsonDigest(schema.schema, ATMS_PLUGIN_SCHEMA_MAX_BYTES)
        ) {
  ...
  for (const skill of descriptor.skills) {
    ...
    if (bytes && decodeAtmsPluginUtf8(bytes, skill.path) !== skill.content) {
