...
export const DEFAULT_CAPABILITY_COMPATIBILITY_TARGET: Readonly<AtmsPluginCompatibilityTargetV1> = {
  atms: ATMS_VERSION,
...
export interface CompilePluginCapabilityIndexOptions {
  ...
  permission_revision?: number;
  compatibility_target?: AtmsPluginCompatibilityTargetV1;
}
...
function effectRank(effect: AtmsPluginEffect): number {
...
export function compilePluginCapabilityIndex(
  ...
  for (const plugin of [...registry.plugins].sort((left, right) => compareText(left.plugin_id, right.plugin_id))) {
    ...
    if (validateAtmsPluginCompatibility(manifest, target).length) continue;
