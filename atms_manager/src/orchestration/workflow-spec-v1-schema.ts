...
import { AGENT_BUILTIN_TOOL_NAMES, DAG_AGENT_TOOL_NAMES } from "atms-protocol";

export const WORKFLOW_API_VERSION = "atms.ai/v1" as const;
...
const AgentRuntimeFields = {
  ...
  max_builtin_tool_calls: Type.Optional(Type.Integer({
    minimum: 1,
    maximum: 100_000,
    description: "Per-node hard budget for backend-provided tools. Atms DAG tools remain available for final handoff.",
  })),
  ...
  allowed_dag_tools: Type.Optional(Type.Array(DagAgentToolName, {
    ...
    description: "Exact allowlist for Atms DAG tools.",
  })),
...
export const WorkflowSpecV1Schema = Type.Object({
  ...
  $id: "https://atms.ai/schemas/workflow/atms.ai-v1.json",
  additionalProperties: false,
});
