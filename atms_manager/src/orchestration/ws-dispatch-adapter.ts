...
import { summarizeDagCredentialProjection } from "atms-protocol";
import {
...
export class WsDispatchAdapter implements DAGDispatcher {
    ...
    WsDispatchAdapter.offlineRetryTimer = setTimeout(() => {
      ...
      for (const adapter of Array.from(WsDispatchAdapter.offlineRetryAdapters)) {
        try {
          ...
          console.error(
            `[atms_manager] deferred DAG dispatch retry failed: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
  ...
  private _retryDeferredOfflineDispatches(): boolean {
    ...
    for (const [runId, pendingEntries] of pendingByRun) {
      try {
        ...
        console.error(
          `[atms_manager] deferred DAG dispatch failed for ${runId}: ${error instanceof Error ? error.message : String(error)}`,
        );
  ...
  private _warnPostSendFailure(envelope: DispatchEnvelope, operation: string, error: unknown): void {
    console.warn(
      `[atms_manager] DAG prompt was sent but ${operation} failed for ${envelope.runId}/${envelope.nodeId}: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  private _releaseFailedDispatchLease(lease: DagActorLeaseRecord): void {
    ...
    try {
      ...
      console.warn(
        `[atms_manager] failed to release DAG actor lease after dispatch send failure: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private _startProvisioning(
    ...
    const provisionerOpts: ProvisionerOptions = {
      ...
      env: {
        ...
        ATMS_AUDIT_DIR: "/workspace/.atms-runtime/audit",
        MANAGER_WORKER_WS_URL: this.getManagerWorkerWsUrl(workerId),
        ATMS_WORKER_ID: workerId,
      },
    };
