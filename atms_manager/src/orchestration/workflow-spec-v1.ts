...
function semanticDiagnostics(context: SourceContext, workflow: WorkflowSpecV1): WorkflowDiagnostic[] {
  ...
  const validateWorkspacePolicyPaths = (
    ...
    if (!access) return;
    const reservedSegments = new Set([".git", ".atms-runtime", "node_modules"]);
    for (const [field, paths] of [
      ...
      paths.forEach((declared, index) => {
        ...
        add(
          ...
          reserved === ".atms-runtime"
            ? "Remove this path: Atms mounts .atms-runtime separately for trusted runtime telemetry."
            : `Declare the project root instead; Atms handles '${reserved}' as protected metadata.`,
        );
      });
    }
  };
  ...
  return {
    meta: {
      ...
      limits: {
        ...
        // Manager-owned review evidence (see atms_protocol pr-review).
        ...(canonical.capabilities.length > 0
          ? { capabilities: canonical.capabilities }
          : {}),
      },
