...
const orchestratorWorkers: DAGPatternDefinition = {
  ...
  workflow_template: {
    ...
    spec: {
      ...
      agents: {
        orchestrator: { system: "Plan only: never inspect repositories, read task files, call shell tools, or execute any work item. The user's objective is provided in the 'objective' input field — that is the ONLY text you must copy verbatim into the top-level context field. Do NOT use these system prompt instructions as the context. After copying the objective, decompose it into 1..N non-overlapping work_items. Every item must contain exactly id as a non-empty string, task as a non-empty string, and acceptance_criteria as a non-empty JSON array of strings; acceptance_criteria must never be a single string. Workers must report status using exactly success or failed; never ask for pass/fail or any alternative status vocabulary. Never precompute findings, evidence, status, or result values for a worker. The DAG already has a separate verifier after fan-out: never add verifier, reviewer, aggregator, coordinator, or summary items to work_items. Your first and only tool call must hand off on the exact port planned, never plan or plan.planned, with only top-level context and work_items." },
        worker: { system: "Execute only the supplied fan-out item. Its envelope contains item plus the original immutable context; use that context as source evidence and inspect additional real inputs only when the task requires them. Use the minimum tools needed to produce grounded evidence and do not wait for other workers. CRITICAL: Writing the JSON in your reply text is NOT sufficient and does NOT complete the task. You MUST call the handoff tool as your final action; if you do not call the handoff tool, your entire work will be discarded and the task will fail. After the work is complete, call the handoff tool with port result and content set to exactly one JSON object with top-level status (success or failed) and top-level evidence grounded in the work performed. The content must be a non-empty JSON object; never call handoff with empty or missing content. If you cannot produce valid JSON, call handoff with status failed and evidence explaining why. Use status failed when an acceptance criterion cannot be checked. Never use done, completed, pass, or any other port or status vocabulary; never copy planner claims as evidence; never omit, nest, or rename status or evidence." },
...
function buildPatternWorkflow(
  ...
  if (workflow.api_version === "atms.ai/v1") {
