...
if (ATMS_PLUGIN_SDK_ABI_VERSION !== EXPECTED_PLUGIN_SDK_ABI_VERSION) {
  console.error(
    `[atms_manager] FATAL: atms-plugin-sdk ABI version mismatch — ` +
    `Manager expects ${EXPECTED_PLUGIN_SDK_ABI_VERSION}, runtime SDK reports ${ATMS_PLUGIN_SDK_ABI_VERSION}. ` +
    `Rebuild atms_plugin_sdk (npm run build) and restart.`,
  );
  process.exit(1);
}
...
server.listen(port, host, () => {
  ...
  console.error(`atms_manager listening on ${host}:${port}`);
  console.error(
