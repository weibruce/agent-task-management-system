...
import {
  ...
  type AtmsDeclarativeRendererModelV1,
...
export interface PluginFixtureResult {
  ...
  renderer_models?: Array<{ renderer: string; model: AtmsDeclarativeRendererModelV1 }>;
  a2ui_surface?: AtmsA2uiSurfaceV1;
}
...
export function runPluginFixtureMatrix(
  ...
  const fixtures: PluginFixtureResult[] = fixtureFiles.map((file) => {
    try {
      ...
      const input = validateAtmsPluginToolInput(schemas.get(tool.input_schema)!, fixture.arguments);
      ...
      const projection = validateAtmsDirectUiProjection(rawProjection);
      ...
      if (tool.output_schema) {
        const output = validateAtmsPluginToolInput(schemas.get(tool.output_schema)!, result.node.content);
      ...
      const rendererModels = snapshot.manifest.renderers.flatMap((renderer) => {
        ...
        return [{
          ...
          model: buildAtmsDeclarativeRendererModel(
            JSON.parse(snapshot.files.get(renderer.source.file)!.toString("utf8")) as AtmsDeclarativeRendererV1,
            result.node.content,
            options,
          ),
        }];
