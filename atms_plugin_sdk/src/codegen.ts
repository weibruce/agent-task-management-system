...
function prepareOutputDirectory(root: string, create: boolean): boolean {
  ...
  let cursor = root;
  for (const component of [".atms", "generated"]) {
...
export function generatedPluginTypes(snapshot: PluginSourceSnapshot): string {
  ...
  return [
    "// Generated by `atms plugin codegen`. Do not edit.",
...
export function generatePluginTypes(root: string, options: { check?: boolean } = {}): {
  ...
  const output = path.join(sourceRoot, ".atms", "generated", "plugin-types.d.ts");
  ...
  if (options.check) {
    if (changed) throw new Error("Generated plugin types are stale; run `atms plugin codegen`");
  } else if (changed) {
