...
export const ATMS_CUSTOM_RENDERER_SOURCE_MAX_BYTES = 512 * 1024;
...
export function validatePluginCustomRendererSource(
  ...
  if (content.byteLength > ATMS_CUSTOM_RENDERER_SOURCE_MAX_BYTES) {
    throw new Error(`${label} exceeds ${ATMS_CUSTOM_RENDERER_SOURCE_MAX_BYTES} bytes`);
  }
  const source = decodeAtmsPluginUtf8(content, label);
