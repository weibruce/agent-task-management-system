...
import { decodeAtmsPluginUtf8 } from "atms-protocol";

export const HRP_LOCK_FILE = "atms.lock.json" as const;
export const HRP_MANIFEST_FILE = "atms.plugin.json" as const;
...
export const HRP_SIGNATURE_FILE = "atms.signature.json" as const;
...
function parseHrpSignature(content: Buffer, lock: HrpLockFileV1): {
  ...
  let value: unknown;
  try {
    value = JSON.parse(decodeAtmsPluginUtf8(content, HRP_SIGNATURE_FILE));
  } catch (cause) {
...
function pluginIdentity(manifest: Buffer): { id: string; version: string } {
  let value: unknown;
  try {
    value = JSON.parse(decodeAtmsPluginUtf8(manifest, HRP_MANIFEST_FILE));
  } catch (cause) {
...
function parseLock(content: Buffer): HrpLockFileV1 {
  let value: unknown;
  try {
    value = JSON.parse(decodeAtmsPluginUtf8(content, HRP_LOCK_FILE));
  } catch (cause) {
