...
export const ATMS_KIND_MIGRATION_VERSION = 1 as const;
export const MAX_ATMS_KIND_MIGRATION_BYTES = 32 * 1024;
export const MAX_ATMS_KIND_MIGRATION_OPERATIONS = 64;

export type AtmsKindMigrationOperationV1 =
  | { op: "rename"; from: string; path: string }
  | { op: "set_default"; path: string; value: unknown }
  | { op: "remove"; path: string };

export interface AtmsKindMigrationV1 {
  migration_version: 1;
  type: "declarative_kind_content";
  from: number;
  to: number;
  operations: AtmsKindMigrationOperationV1[];
}
...
export function isAtmsDeclarativeKindMigrationManifest(
...
function normalizeOperation(value: unknown, index: number): AtmsKindMigrationOperationV1 {
...
export function parseAtmsKindMigrationV1(
  contentValue: Uint8Array,
  expected?: { from: number; to: number },
): AtmsKindMigrationV1 {
  const content = Buffer.from(contentValue);
  if (!content.byteLength || content.byteLength > MAX_ATMS_KIND_MIGRATION_BYTES) {
    throw new Error("Kind migration file size is outside limits");
  }
  let value: unknown;
  try {
    value = JSON.parse(decodeAtmsPluginUtf8(content, "kind migration"));
  } catch (cause) {
  ...
  if (
    ...
    || raw.operations.length > MAX_ATMS_KIND_MIGRATION_OPERATIONS
  ) throw new Error("Kind migration header or operation count is invalid");
  const migration: AtmsKindMigrationV1 = {
    migration_version: 1,
    type: "declarative_kind_content",
...
export function applyAtmsKindMigrationV1(
  content: Record<string, unknown>,
  migration: AtmsKindMigrationV1,
