...
export function scaffoldPluginProject(
  ...
  const files = new Map<string, string>([
    ...
    ["fixtures/basic.json", json({
      ...
      arguments: {
        id: `${pluginId}:current`,
        title: `${name} example`,
        summary: "Generated by the Atms plugin scaffold.",
        items: ["Validate", "Pack", "Install"],
      },
      expect: { title: `${name} example` },
    })],
    ["README.md", `# ${name}\n\nGenerated with \`atms plugin init ${pluginId}\`.\n`],
  ]);
  ...
  const staging = fs.mkdtempSync(path.join(parent, `.${path.basename(root)}.atms-scaffold-`));
