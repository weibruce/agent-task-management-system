...
import {
  ...
  ATMS_PLUGIN_DESCRIPTOR_MAX_BYTES,
...
function parseJsonObject(content: Buffer, label: string): Record<string, unknown> {
  let value: unknown;
  try {
    value = JSON.parse(decodeAtmsPluginUtf8(content, label));
  } catch (cause) {
...
function expectedPayloadPaths(manifest: AtmsPluginManifestV1): string[] {
  return [HRP_MANIFEST_FILE, ...collectAtmsPluginFileReferences(manifest)]
 ...
 */
export const ATMS_PLUGIN_SDK_ABI_VERSION = 1 as const;
...
export function validatePluginSkill(
  ...
  if (byteLength > ATMS_PLUGIN_SKILL_MAX_BYTES) {
    throw new Error(`Plugin Skill ${skillId} exceeds ${ATMS_PLUGIN_SKILL_MAX_BYTES} bytes`);
  }
  const content = Buffer.isBuffer(contentValue)
    ? decodeAtmsPluginUtf8(contentValue, `Plugin Skill ${skillId}`)
    : contentValue;
...
function validateSchema(content: Buffer, label: string): Record<string, unknown> {
  if (content.byteLength > ATMS_PLUGIN_SCHEMA_MAX_BYTES) {
    throw new Error(`${label} exceeds ${ATMS_PLUGIN_SCHEMA_MAX_BYTES} bytes`);
  }
...
function policyEligible(manifest: AtmsPluginManifestV1): boolean {
  return manifest.runtime.trust === AtmsPluginRuntimeTrust.DATA_ONLY
...
function m5ProjectionActionEligibility(
  ...
  if (manifest.runtime.trust !== AtmsPluginRuntimeTrust.DATA_ONLY) {
...
export function validatePluginFiles(inputFiles: ReadonlyMap<string, Buffer>, root?: string): PluginSourceSnapshot {
  ...
  const manifestValidation = validateAtmsPluginManifest(parsed);
  if (!manifestValidation.valid || !manifestValidation.value) {
    return {
      ...(root ? { root } : {}),
      manifest: parsed as unknown as AtmsPluginManifestV1,
      files,
  ...
  for (const entry of validateAtmsPluginCompatibility(manifest, {
    atms: ATMS_VERSION,
  ...
  for (const skill of manifest.skills) {
    ...
    try {
      ...
      if (skill.visual_profile) {
        ...
        parseDagWorkerSkillVisualProfileV1(JSON.parse(decodeAtmsPluginUtf8(
          profile,
  ...
  for (const entry of handlers) {
    ...
    try {
      ...
      const projection = validateAtmsDirectUiProjection(document);
  ...
  for (const renderer of manifest.renderers) {
    ...
    try {
      if (renderer.source.type === "declarative") {
        ...
        const validation = validateAtmsDeclarativeRenderer(document);
  ...
  for (const workflow of manifest.workflows) {
    ...
    if (content && !decodeAtmsPluginUtf8(content, workflow.file).trim()) {
  ...
  if (issues.every((issue) => issue.severity !== "error")) {
    try {
      const referencedFiles = collectAtmsPluginFileReferences(manifest).map((filePath) => ({
        path: filePath,
      ...
      const descriptor = {
        ...
        schemas: manifest.schemas.map((declaration) => ({
          ...
          schema: JSON.parse(decodeAtmsPluginUtf8(files.get(declaration.file)!, declaration.file)) as unknown,
        })),
        skills: manifest.skills.map((skill) => ({
          id: skill.id,
          path: skill.path,
          digest: fileDigests.get(skill.path)!,
          content: decodeAtmsPluginUtf8(files.get(skill.path)!, skill.path),
        })),
        referenced_files: referencedFiles,
      };
      const descriptorBytes = Buffer.byteLength(JSON.stringify(descriptor), "utf8");
      if (descriptorBytes > ATMS_PLUGIN_DESCRIPTOR_MAX_BYTES) {
        issues.push({
          path: "/",
          message: `Resolved plugin descriptor exceeds ${ATMS_PLUGIN_DESCRIPTOR_MAX_BYTES} bytes`,
          severity: "error",
        });
      }
    } catch (cause) {
...
export function scanPluginSource(rootValue: string): PluginSourceSnapshot {
  ...
  const validation = validateAtmsPluginManifest(parsed);
  ...
  for (const relativePath of collectAtmsPluginFileReferences(validation.value)) {
