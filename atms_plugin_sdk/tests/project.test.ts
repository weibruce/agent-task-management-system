...
import {
  ATMS_PLUGIN_SCHEMA_MAX_BYTES,
  ATMS_PLUGIN_SKILL_MAX_BYTES,
} from "atms-protocol";
import {
  buildHrpArchive,
  generatePluginTypes,
  ATMS_CUSTOM_RENDERER_SOURCE_MAX_BYTES,
...
function addProjectionAction(root: string): void {
  const manifestFile = path.join(root, "atms.plugin.json");
...
describe("Atms plugin project SDK", () => {
  it("runs empty directory -> scaffold -> codegen -> fixture matrix -> pack -> verify", () => {
    ...
    expect(fs.existsSync(path.join(root, ".atms"))).toBe(false);
    ...
    fs.utimesSync(path.join(root, "atms.plugin.json"), new Date(2030, 1, 1), new Date(2030, 1, 1));
  ...
  it("snapshots, packs, and verifies an M5 data-only projection Action without changing M4 eligibility", () => {
    const root = temp("atms-plugin-action");
  ...
  it("classifies immutable data-only Workflows for the M5 resolution-only tier", () => {
    const root = temp("atms-plugin-workflow");
    ...
    const unreachable = temp("atms-plugin-unreachable-workflow");
    ...
    const executable = temp("atms-plugin-runtime-workflow");
  ...
  it("rejects unsafe Action projections and reports runtime Actions as M6-only", () => {
    const invalidProjection = temp("atms-plugin-invalid-action-projection");
    ...
    const schemaMismatch = temp("atms-plugin-action-schema-mismatch");
    ...
    const runtimeAction = temp("atms-plugin-runtime-action");
    scaffoldPluginProject(runtimeAction, "com.example.runtime-action");
    addProjectionAction(runtimeAction);
    const manifestFile = path.join(runtimeAction, "atms.plugin.json");
    const manifest = JSON.parse(fs.readFileSync(manifestFile, "utf8")) as {
      runtime: Record<string, unknown>;
    ...
    const customRenderer = temp("atms-plugin-custom-action-renderer");
    ...
    addCustomRenderer(customRenderer, [
      ...
      "    : 'Custom card';",
      "  return { version: 'v1.0', catalogId: 'https://atms.dev/a2ui/catalogs/core/v1', components: [{ id: 'root', component: 'Text', text: title }] };",
      "}",
      "",
    ].join("\n"));
    ...
    const builtinAction = temp("atms-plugin-builtin-action");
    scaffoldPluginProject(builtinAction, "com.example.builtin-action");
    addProjectionAction(builtinAction);
    const builtinManifestFile = path.join(builtinAction, "atms.plugin.json");
  ...
  it("enforces the single-file Worker Renderer contract in validate, pack, and verify", () => {
    const invalidSources = [
      ...
      },
      {
        ...
        source: [
          ...
          "export function render(payload) {",
          "  return { version: 'v1.0', catalogId: 'https://atms.dev/a2ui/catalogs/core/v1', components: [{ id: 'root', component: 'Text', text: helper(payload) }] };",
          "}",
        ].join("\n"),
        message: "exactly one export",
      },
    ...
    for (const invalid of invalidSources) {
      const root = temp(`atms-plugin-custom-${invalid.name.replaceAll(" ", "-")}`);
  ...
  it("bounds Worker Renderer source bytes and requires fatal UTF-8 decoding", () => {
    const validPrefix = Buffer.from([
      ...
      "  return { version: 'v1.0', catalogId: 'https://atms.dev/a2ui/catalogs/core/v1', components: [{ id: 'root', component: 'Text', text: 'bounded' }] };",
      "}",
      "",
    ].join("\n"));
    const atLimit = Buffer.concat([
      validPrefix,
      Buffer.alloc(ATMS_CUSTOM_RENDERER_SOURCE_MAX_BYTES - validPrefix.byteLength, 0x20),
    ]);
    ...
    expect(() => validatePluginCustomRendererSource(Buffer.concat([atLimit, Buffer.from(" ")])))
      .toThrow(`exceeds ${ATMS_CUSTOM_RENDERER_SOURCE_MAX_BYTES} bytes`);
  ...
  it("rejects symbolic-link and non-directory codegen parents component by component", () => {
    const root = temp("atms-plugin-codegen-parent");
    const outside = temp("atms-plugin-codegen-outside");
    scaffoldPluginProject(root, "com.example.codegen-parent");

    fs.symlinkSync(outside, path.join(root, ".atms"), "dir");
    ...
    fs.unlinkSync(path.join(root, ".atms"));
    fs.mkdirSync(path.join(root, ".atms"));
    fs.symlinkSync(outside, path.join(root, ".atms", "generated"), "dir");
    expect(() => generatePluginTypes(root)).toThrow(/output parent must not be a symbolic link/);
    expect(fs.readdirSync(outside)).toEqual([]);

    fs.unlinkSync(path.join(root, ".atms", "generated"));
    fs.writeFileSync(path.join(root, ".atms", "generated"), "not a directory");
  ...
  it("never follows an existing codegen target symlink for check or write", () => {
    const root = temp("atms-plugin-codegen-target");
    const outside = temp("atms-plugin-codegen-victim");
    scaffoldPluginProject(root, "com.example.codegen-target");
    const generatedDirectory = path.join(root, ".atms", "generated");
  ...
  it("refuses to scaffold into non-empty or aliased roots", () => {
    const root = temp("atms-plugin-nonempty");
  ...
  it("rejects manifest and parent-directory symlinks before reading source bytes", () => {
    const root = temp("atms-plugin-symlink-source");
    const outside = temp("atms-plugin-symlink-outside");
    ...
    fs.renameSync(path.join(root, "atms.plugin.json"), path.join(outside, "manifest.json"));
    fs.symlinkSync(path.join(outside, "manifest.json"), path.join(root, "atms.plugin.json"), "file");
    expect(() => scanPluginSource(root)).toThrow(/traverses a symlink/);
  });

  it("rejects undeclared payloads and malformed declarative renderer documents", () => {
    const root = temp("atms-plugin-invalid-renderer");
  ...
  it("applies the same strict Skill and local-schema policy used by installation", () => {
    const root = temp("atms-plugin-static-policy");
    ...
    const manifest = JSON.parse(snapshot.files.get("atms.plugin.json")!.toString("utf8")) as {
      kinds: Array<{
    ...
    migrationFiles.set("atms.plugin.json", Buffer.from(JSON.stringify(manifest)));
  ...
  it("uses Skill YAML frontmatter as discovery metadata without duplicating it in the manifest", () => {
    const root = temp("atms-plugin-frontmatter-metadata");
  ...
  it("rejects invalid UTF-8 consistently in source validation and packed archives", () => {
    const root = temp("atms-plugin-invalid-utf8");
  ...
  it("aligns per-file and resolved-descriptor budgets with Manager installation", () => {
    const root = temp("atms-plugin-static-budgets");
    ...
    skillFiles.set("skills/compose-card/SKILL.md", Buffer.concat([
      skillPrefix,
      Buffer.alloc(ATMS_PLUGIN_SKILL_MAX_BYTES - skillPrefix.byteLength, 0x61),
    ]));
    expect(validatePluginFiles(skillFiles).valid).toBe(true);
    skillFiles.set("skills/compose-card/SKILL.md", Buffer.concat([
      skillPrefix,
      Buffer.alloc(ATMS_PLUGIN_SKILL_MAX_BYTES - skillPrefix.byteLength + 1, 0x61),
    ]));
    expect(validatePluginFiles(skillFiles).issues).toContainEqual(expect.objectContaining({
      message: expect.stringContaining(`exceeds ${ATMS_PLUGIN_SKILL_MAX_BYTES} bytes`),
    }));
    ...
    schemaFiles.set("schemas/card-input.v1.schema.json", Buffer.concat([
      schemaBase,
      Buffer.alloc(ATMS_PLUGIN_SCHEMA_MAX_BYTES - schemaBase.byteLength, 0x20),
    ]));
    expect(validatePluginFiles(schemaFiles).valid).toBe(true);
    schemaFiles.set("schemas/card-input.v1.schema.json", Buffer.concat([
      schemaBase,
      Buffer.alloc(ATMS_PLUGIN_SCHEMA_MAX_BYTES - schemaBase.byteLength + 1, 0x20),
    ]));
    expect(validatePluginFiles(schemaFiles).issues).toContainEqual(expect.objectContaining({
      message: expect.stringContaining(`exceeds ${ATMS_PLUGIN_SCHEMA_MAX_BYTES} bytes`),
    }));
    ...
    descriptorFiles.set("atms.plugin.json", Buffer.from(JSON.stringify(manifest)));
