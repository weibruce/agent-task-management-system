...
describe("Core generated A2UI capability", () => {
  it("validates, projects, previews, and packages the runtime-authored surface", () => {
    ...
    expect(snapshot.manifest.id).toBe("com.atms.core");
    ...
    const catalogComponents = atmsA2uiComponentSchema.oneOf.map(schema => (
    ...
    expect(snapshot.manifest.kinds.find(kind => kind.kind === "com.atms.core/generated_view"))
      .toMatchObject({
        current_version: 2,
        versions: [
    ...
    expect(parseAtmsKindMigrationV1(
    ...
    expect(validateAtmsA2uiSurface(projected.node.a2ui).valid).toBe(true);

    const localIdProjection = applyAtmsDirectUiProjection({
      projection: projector,
      plugin: { id: snapshot.manifest.id, version: snapshot.manifest.version },
      arguments: { ...fixture.arguments, id: "generated-local-overview" },
    });
    expect(localIdProjection.node.id).toBe("com.atms.core:generated-local-overview");
