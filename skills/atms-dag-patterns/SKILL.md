---
name: atms-dag-patterns
description: |
  Select, inspect, instantiate, adapt, and validate Atms's built-in DAG design patterns.
  ...
  for stable live Actor panels, Surface updates, or follow-up commands, use atms-dag-ops and a concrete presentation-aware Workflow.
---
...
When running as the Atms Manager Agent, dedicated Manager Tools are the
shortest path for the operations below. The harness-native shell and `atms --json`
...
2. Emit `api_version: atms.ai/v1` with explicit top-level edges and terminal
   nodes.
...
per-panel correction, or later follow-up, load `atms-dag-ops` and select a
...
```bash
atms doctor
atms --json patterns list
atms --json patterns show <pattern-id>
```
...
```bash
atms patterns instantiate quorum \
...
- Keep `issue-diagnosis` revision-pinned, credential-free, remote-read-only, and
  ...
  fixed `diagnosis.json` and `verification.json` outputs through generic `atms dag
  artifacts <run-id>` / `atms dag artifact <run-id> <name>` commands.

Do not put provider, model, API key, or base URL fields in generated workflow
YAML. Bind models through Atms database settings and runtime profiles.
...
- `command` runs an allowlisted argument vector with explicit timeout and typed
  ...
  require `ATMS_DAG_ALLOW_DYNAMIC_COMMANDS=true`.
- `approval` persists a proposal hash and waits for an authorized human actor.
  ...
  list pending approvals, but never decide one; direct the human to `atms dag
...
```bash
atms dag schema
atms --json dag validate /tmp/release-decision.yaml
atms dag sync /tmp/release-decision.yaml
atms profile sync <profile.yaml> --workflow release-decision
atms run --workflow release-decision --profile <profile-id> --prompt "<task>"
atms dag watch <run-id> --timeout 600
atms dag handoffs <run-id> --content-limit 0

# Durable governance and state operations
atms dag approvals
atms dag decide <run-id> <node-id> --decision approved --actor <actor> --proposal-hash <hash>
atms dag triggers
atms dag trigger-event <event> --idempotency-key <stable-key>
atms dag state-get <namespace> <key>
atms dag state-set <namespace> <key> '<json>' --expected-version <version>
```

For a non-loopback Manager, set `ATMS_DAG_MUTATION_TOKEN` in both the
...
- `atms patterns instantiate` reports a valid graph.
