...
name: atms-dag-ops
description: |
  Run, monitor, inspect, debug, and author Atms DAG workflows through the CLI or Manager Tools, including supervised multi-Actor runs with stable live Surfaces and later per-Actor follow-ups.
  ...
  For installing or starting Manager/Node/Worker services, use atms-install-ops first.
---

# Atms DAG Operations (TypeScript Backend)

Before acting, apply `atms-shared` rules. This skill assumes the local runtime
is already ready. If not, run `atms start` or use `atms-install-ops`.

When this Skill is loaded inside the Atms Manager Agent, use the harness's
native shell and the Atms CLI whenever they provide the clearest or most
...
`atms-dag-patterns` and use either its Manager Tools or the corresponding
`atms patterns` commands before running the instantiated workflow.
...
```bash
atms doctor                                              # sole readiness check
atms templates list                                      # pick a template
atms run <template> --prompt "<prompt per contract below>"
atms dag watch <run_id> --interval 10 --timeout 600      # wait for terminal state
atms dag handoffs <run_id> --content-limit 0             # authoritative output
```

Use `atms run <template> --profile <profile>` only for profiles embedded in that
...
## Template Prompt Contracts (read before `atms run`)
...
- **Repo-access templates** (any template whose agent system prompt says
  ...
  A host-local path like `/Volumes/.../Atms` will fail inside the container —
...
```bash
atms dag quick <run_id> --events 30
atms dag chats <run_id> --tools 10
atms dag chats <run_id> --tools 10 --raw-tools
atms dag handoffs <run_id>
...
Atms CLI, avoided Write/Edit/MultiEdit, or bypassed the CLI with direct API
...
audit files are run-scoped at `${ATMS_HOME}/audit/tool-events/<run_id>.jsonl`;
legacy installs may still contain a global `${ATMS_HOME}/audit/tool-events.jsonl`.
...
```bash
curl -s "$ATMS_MANAGER_URL/api/dag-status/<run_id>/node/<node_id>/result"
```
...
handoff content (`atms dag handoffs <run_id> --content-limit 0`) and check for
...
```bash
atms inject <run_id> <node_id> "Correction or additional instruction" --mode inbox
```
...
```
skills/
  atms-shared/       # Common service, security, provider, and local-source rules
  atms-install-ops/  # Local-source install, service startup, maintenance, smoke
  atms-cli/          # CLI command reference
  atms-dag-ops/      # DAG run, monitor, inspect, and author operations
  atms-dag-patterns/ # DAG pattern selection, composition, and instantiation
...
- Use only public component names: `atms-manager`, `atms-node`, `atms_worker`.
...
- Use the CLI config flow for Manager address and credentials. `--base-url` and
  `ATMS_MANAGER_URL` remain per-command overrides; `ATMS_MANAGER_PORT`
...
When a DAG is used to change Atms itself, the first implementation handoff
...
For intentionally broad Manager changes, set `ATMS_BOUNDARY_SCAN_STRICT=1` and
provide `ATMS_MANAGER_CHANGE_JUSTIFICATION` during local review so the
...
```bash
atms run assets/orchestrations/my-linear-pipeline.yaml --prompt "Research TypeScript best practices"
atms dag supervise <run_id>
```
