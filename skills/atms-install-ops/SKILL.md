---
name: atms-install-ops
description: Install, configure, start, verify, update, and troubleshoot a local-source Atms deployment for AI-agent use. Use when a user wants to deploy Atms from a local checkout or local Git address, install Atms skills, set up Manager/Node/Worker services, configure provider credentials, run doctor/smoke, or repair a broken local install before npm publication exists.
---

# Atms Install Ops

Before acting, apply `atms-shared` rules. This pre-publication flow uses a local source checkout or local Git clone, not npm publication.

## Local Skill Install

Install Atms skills by linking the checkout's `skills/atms-*`
...
```bash
git clone <local-atms-git-url> Atms
cd Atms
```
...
as the local fallback; repeat the junction for every `skills/atms-*`
...
- Use `atms-install-ops` for first install, update, service startup,
  provider configuration, and smoke verification. Codex invocation:
  `$atms-install-ops`; Claude Code invocation: `/atms-install-ops`.
- Use `atms-dag-ops` for running DAGs, monitoring runs, checking handoffs,
  and writing DAG templates.
- Use `atms-dag-patterns` for selecting, composing, instantiating, and
  validating built-in DAG design patterns.
- Use `atms-cli` for command syntax, flags, and evidence commands.
- Use `atms-shared` as background rules for all Atms operations.
...
```bash
export ATMS_HOME="${ATMS_HOME:-$HOME/.atms}"
```

Configure local runtime settings and credentials through the CLI:

```bash
node atms_cli/dist/cli.js config
```

Start Manager and Node together:

```bash
node atms_cli/dist/cli.js start
node atms_cli/dist/cli.js start --rebuild-worker-image
node atms_cli/dist/cli.js start --ui
node atms_cli/dist/cli.js doctor
```

`atms start` uses the shared `ATMS_HOME`, starts Manager if needed, applies
stored model config, builds `atms-worker:latest` when missing, and starts a
...
Use `--ui` or `node atms_cli/dist/cli.js ui start` to start the decoupled
Agent UI. HTTPS is the default endpoint, `https://localhost:19192`, and HTTP is
kept as a fallback on `http://localhost:19193`.
If provider credentials are not configured yet, `atms start` should still bring
up Manager and Node; `atms doctor` reports the missing model or Manager Agent
runtime configuration and the agent should run `atms config` /
`atms model configure`.

Use `node atms_cli/dist/cli.js runtime status|logs|stop` for service
inspection and cleanup. Use `node atms_cli/dist/cli.js ui status|logs|stop`
for UI-only inspection and cleanup.

`atms doctor` is the first environment probe an agent needs for normal operation.
...
A `Atms is ready.` trailer is the readiness gate; any `FAIL`/`WARN` line names
the exact next command to run (e.g. `atms model configure`).
...
```bash
node atms_cli/dist/cli.js config
node atms_cli/dist/cli.js config apply
node atms_cli/dist/cli.js doctor
```

Do not put the key into repo files. The config flow stores secrets under
Manager's encrypted secret store. `${ATMS_HOME}/secrets/env` is a legacy
...
```bash
node atms_cli/dist/cli.js smoke dag \
...
```bash
node atms_cli/dist/cli.js smoke manager-agent
```
...
```bash
node atms_cli/dist/cli.js dag quick <run_id> --events 30
node atms_cli/dist/cli.js dag chats <run_id> --tools 10
node atms_cli/dist/cli.js dag handoffs <run_id>
node atms_cli/dist/cli.js scorecard <run_id>
node atms_cli/dist/cli.js eval-run <run_id>
```
...
- Keep installed skills as symlinks to the checkout. After skill edits, verify
  the target agent skill directory still points at `skills/atms-*`; do not
  refresh by copying stale directories.
- Re-run `npm run build` after code changes.
- Run `node atms_cli/dist/cli.js start`; add `--rebuild-worker-image` after Worker runtime changes.
- Keep Manager and Node on the same `ATMS_HOME`.
