...
- Never represent Atms approval as a GitHub approval or merge.
...
- Run `atms dag run-template pr-closeout` with explicit operator-attested local
...
especially the matching Atms PR Review run. Do not use generic
...
with `atms dag artifact <run-id> pr-closeout.json --output pr-closeout.json`.
